...
In the antex-rs folder where the rust code lives:

- Crate names are prefixed with `antex-`. For example, the `core` folder's crate is named `antex-core`
...
- Do not add general product or user-facing documentation to the `docs/` folder. The official Antex documentation lives elsewhere. The exception is app-server API documentation, which is covered by the app-server guidance below.
- Prefer private modules and explicitly exported public crate API.
- If you change `ConfigToml` or nested config types, run `just write-config-schema` to update `antex-rs/core/config.schema.json`.
- When working with MCP tool calls, prefer using `antex-rs/antex-mcp/src/mcp_connection_manager.rs` to handle mutation of tools and tool calls. Aim to minimize the footprint of changes and leverage existing abstractions rather than plumbing code through multiple levels of function calls.
...
- Avoid large modules:
  ...
  - This rule applies especially to high-touch files that already attract unrelated changes, such
    as `antex-rs/tui/src/app.rs`, `antex-rs/tui/src/bottom_pane/chat_composer.rs`,
    `antex-rs/tui/src/bottom_pane/footer.rs`, `antex-rs/tui/src/chatwidget.rs`,
    `antex-rs/tui/src/bottom_pane/mod.rs`, and similarly central orchestration modules.
  - When extracting code from a large module, move the related tests and module/type docs toward
    the new implementation so the invariants stay close to the code that owns them.
  - Avoid adding new standalone methods to `antex-rs/tui/src/chatwidget.rs` unless the change is
    trivial; prefer new modules/files and keep `chatwidget.rs` focused on orchestration.
- When running Rust commands (e.g. `just fix` or `just test`) be patient with the command and never try to kill them using the PID. Rust lock can make the execution slow, this is expected.

Run `just fmt` (in the `antex-rs` directory) automatically after you have finished making code changes anywhere in this repository; do not ask for approval to run it. Additionally, run the tests:

1. Do not run `cargo test` directly. Use `just test` so test execution follows the repo defaults.
2. Run the test for the specific project that was changed. For example, if changes were made in `antex-rs/tui`, run `just test -p antex-tui`.
...
Before finalizing a large change to `antex-rs`, run `just fix -p <project>` (in `antex-rs` directory) to fix any linter issues in the code. Prefer scoping with `-p` to avoid slow workspace‑wide Clippy builds; only run `just fix` without `-p` if you changed shared crates. Do not re-run tests after running `fix` or `fmt`.

## The `antex-core` crate

Over time, the `antex-core` crate (defined in `antex-rs/core/`) has become bloated because it is the largest crate, so it is often easier to add something new to `antex-core` rather than refactor out the library code you need so your new code neither takes a dependency on, nor contributes to the size of, `antex-core`.

To that end: **resist adding code to antex-core**!

Particularly when introducing a new concept/feature/API, before adding to `antex-core`, consider whether:

- There is an existing crate other than `antex-core` that is an appropriate place for your new code to live.
- It is time to introduce a new crate to the Cargo workspace for your new functionality. Refactor existing code as necessary to make this happen.

Likewise, when reviewing code, do not hesitate to push back on PRs that would unnecessarily add code to `antex-core`.
...
Antex maintains a context (history of messages) that is sent to the model in inference requests.
...
For agent changes prefer integration tests over unit tests. Integration tests are under `core/suite` and use `test_antex` to set up a test instance of codex.
...
This repo uses snapshot tests (via `insta`), especially in `antex-rs/tui`, to validate rendered output.
...
- Run tests to generate any updated snapshots:
  - `just test -p antex-tui`
- Check what’s pending:
  - `cargo insta pending-snapshots -p antex-tui`
- Review changes by reading the generated `*.snap.new` files directly in the repo, or preview a specific file:
  - `cargo insta show -p antex-tui path/to/file.snap.new`
- Only if you intend to accept all new snapshots in this crate, run:
  - `cargo insta accept -p antex-tui`
...
- Prefer the utilities in `core_test_support::responses` when writing end-to-end Antex tests.
- Use `TestAntexBuilder::build_with_auto_env()` by default to ensure that new tests work with
...
These guidelines apply to app-server protocol work in `antex-rs`, especially:
...
- Validate with `just test -p antex-app-server-protocol`.
...
Antex supports running connected app-server and exec-server on different operating systems. See the
