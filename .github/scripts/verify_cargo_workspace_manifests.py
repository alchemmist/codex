...
"""Verify that antex-rs Cargo manifests follow workspace manifest policy.

Checks:
- Crates inherit `[workspace.package]` metadata.
- Crates opt into `[lints] workspace = true`.
- Crate names follow the antex-rs directory naming conventions.
...
CARGO_RS_ROOT = ROOT / "antex-rs"
WORKSPACE_PACKAGE_FIELDS = ("version", "edition", "license")
TOP_LEVEL_NAME_EXCEPTIONS = {
    "windows-sandbox-rs": "antex-windows-sandbox",
}
UTILITY_NAME_EXCEPTIONS = {
    "path-utils": "antex-utils-path",
}
MANIFEST_FEATURE_EXCEPTIONS = {
    "antex-rs/code-mode/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
    "antex-rs/v8-poc/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
}
...
def main() -> int:
    ...
    print(
        "Cargo manifests under antex-rs must inherit workspace package metadata, "
    ...
    print(
        "Cargo only applies `antex-rs/Cargo.toml` `[workspace.lints.clippy]` "
    ...
    print()
    print(
        "Package-name checks apply to `antex-rs/<crate>/Cargo.toml` and "
        "`antex-rs/utils/<crate>/Cargo.toml`."
    )
    print(
