...
publishes the result via `GITHUB_ENV` as `ANTEX_BAZEL_WINDOWS_PATH` so later
...
Write-Host 'Derived ANTEX_BAZEL_WINDOWS_PATH entries:'
foreach ($pathEntry in $stablePathEntries) {
  Write-Host "  $pathEntry"
}
"ANTEX_BAZEL_WINDOWS_PATH=$stablePath" | Out-File -FilePath $env:GITHUB_ENV -Encoding utf8 -Append
