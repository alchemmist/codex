...
./scripts/stage_npm_packages.py \
  ...
  --package antex \
  --package antex-responses-api-proxy \
  --package antex-sdk
```
...
When `--package antex` is provided, the staging helper builds the lightweight
`@alchemmist/antex` meta package plus all platform-native `@alchemmist/antex` variants
