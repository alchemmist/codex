...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> antex_login::ExternalAuthFuture<'_, AntexAuth> {
        Box::pin(async { Ok(AntexAuth::from_api_key("test-external-api-key")) })
    }
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_antex_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_antex_backend })
    }
...
async fn chatgpt_auth_tokens_for_tests(codex_home: &Path) -> AntexAuth {
    let auth_dot_json = antex_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: antex_login::token_data::parse_chatgpt_jwt_claims(
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_antex_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_antex_backend(self))
    }
