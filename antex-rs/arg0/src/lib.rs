...
use antex_apply_patch::ANTEX_CORE_APPLY_PATCH_ARG1;
use antex_async_utils::THREAD_STACK_SIZE_BYTES;
#[cfg(unix)]
...
use antex_windows_sandbox::ANTEX_WINDOWS_SANDBOX_ARG1;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "antex-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Antex executable for child re-execs.
    ///
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            antex_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == ANTEX_LINUX_SANDBOX_ARG0 {
        ...
        antex_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        antex_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    #[cfg(unix)]
    if argv1 == ANTEX_ARG0_EXEC_HELPER_ARG1 {
        antex_exec_server::run_arg0_exec_helper_main();
    }
    if argv1 == ANTEX_FS_HELPER_ARG1 {
        antex_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == ANTEX_WINDOWS_SANDBOX_ARG1 {
        antex_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == ANTEX_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match antex_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let cwd = cwd.into();
                let update_file_mode = antex_apply_patch::apply_patch_file_update_mode_from_env();
                match runtime.block_on(antex_apply_patch::apply_patch_with_options(
                    &patch_arg,
                    antex_apply_patch::ApplyPatchOptions {
                        update_file_mode,
                        ..Default::default()
                    },
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    antex_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            None => {
                eprintln!("Error: {ANTEX_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
    ...
    let (path_entry_guard, updated_path_env_var) = prepare_path_env_var_with_aliases(
        ...
        std::env::var_os("PATH"),
        prepare_path_entry_for_antex_aliases,
    );
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that Antex will proceed successfully even if
...
/// While we want to deploy the Antex CLI as a single executable for simplicity,
...
/// `antex-linux-sandbox` we *directly* execute
/// [`antex_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.antex/.env` before creating any threads.
...
///     `antex-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        antex_self_exe: current_exe.clone(),
        antex_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `antex-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().antex_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
/// Load env vars from ~/.antex/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `CODEX_`.
fn load_dotenv() {
    if let Ok(codex_home) = find_antex_home() {
        if let Ok(iter) = dotenvy::from_path_iter(codex_home.join(".env")) {
            set_filtered(iter);
        }
        unsafe { std::env::set_var("CODEX_HOME", codex_home.as_path()) };
    }
}
    ...
    I: IntoIterator<Item = Result<(String, String), dotenvy::Error>>,
{
    for (key, value) in iter.into_iter().flatten() {
        ...
        if !uppercase.starts_with(ILLEGAL_ENV_VAR_PREFIX) && !uppercase.starts_with("ANTEX_") {
...
/// user to install a separate executable, simplifying the deployment of Antex
/// CLI.
...
fn prepare_path_entry_for_antex_aliases(
    existing_path: Option<OsString>,
    ...
    // Use a ANTEX_HOME-scoped temp root to avoid cluttering the top-level directory.
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {ANTEX_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        antex_self_exe: std::env::current_exe().ok(),
        antex_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(ANTEX_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("antex-path");
        ...
        let install_context = InstallContext {
            ...
            package_layout: Some(AntexPackageLayout {
    ...
    fn windows_batch_alias_preserves_unicode_executable_paths() -> anyhow::Result<()> {
        ...
        let alias_directory = profile.join(".antex").join("tmp").join("arg0");
    ...
    fn windows_batch_alias_preserves_cross_volume_executable_paths() {
        assert_eq!(
            super::windows_batch_executable_path(
                ...
                Path::new(r"C:\Users\person\.antex\tmp\arg0"),
            ),
            r"D:\Tools\codex.exe",
        );
    }
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_antex_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("antex-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                antex_self_exe: Some(PathBuf::from("/usr/bin/codex")),
                antex_linux_sandbox_exe: Some(alias_path.clone()),
