...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Antex TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Antex TUI):
    /// - Build this binary: `cargo build -p antex-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `antex mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Antex TUI, ask it to call:
...
/// Scenarios for `image_scenario`, intended to exercise Antex TUI handling of MCP image outputs.
///
