...
const LOCK_CONTENTION_EVENT_TARGET: &str = "antex_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    ...
    fn acquire_with_timeout(
        ...
        // authority. Those stores are ANTEX_HOME-backed today: if ANTEX_HOME is unset they use
        // the default home (`~/.antex`), and if an embedder has no local home/filesystem authority
        ...
        let codex_home = find_antex_home()
            .map_err(|source| OAuthStoreLockFailure::AntexHome { store, source })?;
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve ANTEX_HOME for MCP OAuth {store} aggregate-store lock")]
    AntexHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
