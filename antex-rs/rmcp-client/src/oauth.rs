...
//!
//! If the keyring is not available or fails, we fall back to ANTEX_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Antex MCP Credentials";
...
fn load_oauth_tokens_from_secrets_keyring_with_lock_held<K: KeyringStore + Clone + 'static>(
    ...
    let codex_home = find_antex_home().map_err(anyhow::Error::from)?;
...
fn compute_store_key(server_name: &str, server_url: &str) -> Result<String> {
    ...
    let payload = if enterprise_owned {
        ...
        // isolated by Antex profile as well as authenticated user and workspace.
        let codex_home = find_antex_home()?;
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_antex_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    use super::*;
    use antex_keyring_store::tests::MockKeyringStore;
    use antex_secrets::compute_keyring_account;
    use anyhow::Result;
    ...
    fn fallback_file_is_private_at_creation() -> Result<()> {
        use std::os::unix::fs::PermissionsExt;
        const CHILD: &str = "ANTEX_TEST_OAUTH_PERMISSIVE_UMASK";
