...
use antex_exec_server::RemoteExecServerConnectArgs;
...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    antex_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn control_client() -> anyhow::Result<antex_http_client::HttpClient> {
    Ok(antex_http_client::HttpClientBuilder::new().build_direct()?)
}
...
pub(crate) async fn spawn_exec_server() -> anyhow::Result<ExecServerProcess> {
    ...
    let mut child = Command::new(antex_utils_cargo_bin::cargo_bin("antex")?)
...
/// Reads the websocket URL printed by `antex exec-server --listen`.
