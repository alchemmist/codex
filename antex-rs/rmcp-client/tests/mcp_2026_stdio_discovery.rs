...
fn local_stdio_inherits_ca_certificate_variables() -> anyhow::Result<()> {
    ...
    let output = Command::new(std::env::current_exe()?)
        ...
        .env("ANTEX_CA_CERTIFICATE", "")
        .env("REQUESTS_CA_BUNDLE", requests_ca_bundle)
        .env("ANTEX_MCP_TEST_SERVER_CWD", &server_dir)
        .output()?;
...
async fn local_stdio_inherits_ca_certificate_variables_child() -> anyhow::Result<()> {
    let server = antex_utils_cargo_bin::cargo_bin("test_stdio_server")?;
    let source_dir = std::env::current_dir()?;
    let server_dir = std::env::var("ANTEX_MCP_TEST_SERVER_CWD")?;
    ...
    for (overrides, env_name, expected) in [
        ...
        (None, "ANTEX_CA_CERTIFICATE", None),
        (
...
-> anyhow::Result<()> {
    let server = antex_utils_cargo_bin::cargo_bin("test_mcp_2026_discovery_stdio_server")?;
    ...
    for executor in [false, true] {
        ...
        let client = RmcpClient::new_stdio_client_with_protocol_mode(
            ...
            Vec::new(),
            Some(HashMap::from([(
                OsString::from("ANTEX_MCP_PROTOCOL_VERSION"),
            ...
            let result = client
                .call_tool(
                    ...
                    Some(json!({
                        ...
                        "env_var": "ANTEX_MCP_PROTOCOL_VERSION",
                    })),
                    /*meta*/ None,
                    Some(Duration::from_secs(10)),
                )
