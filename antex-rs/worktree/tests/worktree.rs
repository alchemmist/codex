...
use antex_worktree::ManagedWorktree;
...
impl RepositoryFixture {
    fn new() -> Self {
        ...
        let codex_home = temp_root.join("antex-home");
...
fn creation_ignores_inherited_git_environment() {
    const CHILD_ROOT: &str = "ANTEX_WORKTREE_ENV_TEST_ROOT";
    if let Some(root) = std::env::var_os(CHILD_ROOT) {
        let root = PathBuf::from(root);
        let source = root.join("project");
        let source_cwd = source.join("nested/component");
        let settings =
            WorktreeSettings::from_desktop_config(&root.join("antex-home"), /*desktop*/ None)
        ...
        assert_eq!(
            worktree.head_sha,
            std::env::var("ANTEX_WORKTREE_ENV_TEST_HEAD").expect("fixture operation succeeds")
        );
        return;
    }
    ...
    child
        ...
        .env("ANTEX_WORKTREE_ENV_TEST_HEAD", run_git(&fixture.repository, &["rev-parse", "HEAD"]))
