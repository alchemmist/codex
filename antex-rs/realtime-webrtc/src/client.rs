...
impl VoiceHost {
    ...
    pub async fn connect(package: &AntexPackageLayout, build_commit: &str) -> Result<Self> {
        let root = package.package_dir.as_path().canonicalize()?;
        let name = if cfg!(windows) {
            "antex-voice-host.exe"
        } else {
            "antex-voice-host"
        };
        let path = root.join("antex-resources/voice/bin").join(name);
        ensure!(
        ...
        let SpawnedProcess {
            ...
            stderr_rx,
            exit_rx,
        } = antex_utils_pty::spawn_pipe_process(
            &path,
            &[],
            &root,
            &environment,
