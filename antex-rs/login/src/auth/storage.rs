...
use antex_agent_identity::AgentIdentityJwtClaims;
...
/// Expected structure for $ANTEX_HOME/auth.json.
...
impl FileAuthStorage {
    ...
    /// Attempt to read and parse the `auth.json` file in the given `ANTEX_HOME` directory.
...
static ANTEX_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("ANTEX_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("ANTEX_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Antex Auth";
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &ANTEX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.secrets_manager
            .set(&SecretScope::Global, &ANTEX_AUTH_SECRET_NAME, &serialized)
            .map_err(|err| {
                let message =
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &ANTEX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
