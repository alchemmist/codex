...
fn bedrock_api_key_debug_redacts_secret() {
    ...
    assert_eq!(
        format!("{:?}", AntexAuth::BedrockApiKey(auth)),
...
async fn login_with_bedrock_api_key_replaces_openai_auth() -> anyhow::Result<()> {
    ...
    assert_eq!(
        auth_manager.auth_cached().and_then(|auth| match auth {
            AntexAuth::BedrockApiKey(auth) => Some(auth),
            AntexAuth::ApiKey(_)
            | AntexAuth::Chatgpt(_)
            ...
            | AntexAuth::BedrockAccessKeys(_) => None,
        }),
        Some(bedrock_auth())
    );
    Ok(())
...
async fn access_keys_auth_round_trips_and_logs_out() -> anyhow::Result<()> {
    ...
    assert_eq!(
        auth_manager.auth_cached().and_then(|auth| match auth {
            AntexAuth::BedrockAccessKeys(auth) => Some(auth),
            AntexAuth::ApiKey(_)
            | AntexAuth::Chatgpt(_)
            | AntexAuth::ChatgptAuthTokens(_)
            | AntexAuth::Headers(_)
            | AntexAuth::AgentIdentity(_)
            | AntexAuth::PersonalAccessToken(_)
            | AntexAuth::BedrockApiKey(_) => None,
        }),
