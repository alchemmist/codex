//! Default Antex HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! `HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`antex_login::default_client`] from other crates in this
//! workspace.

use antex_http_client::BuildRouteAwareHttpClientError;
...
pub use antex_http_client::RequestBuilder as AntexRequestBuilder;
...
/// Parenthesis will be added by Antex. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "codex_cli_rs";
pub const ANTEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "ANTEX_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-codex-residency";

pub use antex_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(ANTEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(ANTEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn get_antex_user_agent() -> String {
...
/// Antex's route-aware system/PAC resolution.
...
/// Builds the default Antex HTTP client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the Antex
...
/// Builds the default Antex HTTP client for a concrete outbound route without blocking the
...
/// Builds an HTTP client for an auth endpoint without Antex default headers.
...
/// Builds the default Antex HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_antex_user_agent()) {
