...
use antex_utils_template::Template;
use base64::Engine;
use chrono::Utc;
use rand::RngCore;
...
// Keep in sync with the Antex CLI Hydra redirect URI allow-list.
...
pub struct ServerOptions {
    ...
    pub antex_streamlined_login: bool,
...
fn redact_sensitive_error_url(
    mut err: antex_http_client::HttpError,
) -> antex_http_client::HttpError {
...
/// Returns true when the OAuth callback represents a missing Antex entitlement.
fn is_missing_antex_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_antex_entitlement_error(error_code, error_description) {
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_antex_entitlement_error(code, error_description) {
            (
...
mod tests {
    ...
    use super::is_missing_antex_entitlement_error;
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        ...
        assert!(is_missing_antex_entitlement_error(
            "access_denied",
            error_description
        ));
