...
use antex_login::LoginCallbackResult;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let opts = ServerOptions {
        ...
        auth_route_config: antex_login::test_support::transport_default_auth_route_config(),
        client_id: antex_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
...
#[tokio::test]
async fn creates_missing_antex_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let mut opts = ServerOptions::new(
        ...
        antex_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_antex_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        codex_home: first_antex_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: antex_login::test_support::transport_default_auth_route_config(),
        client_id: antex_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_antex_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        codex_home: second_antex_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: antex_login::test_support::transport_default_auth_route_config(),
        client_id: antex_login::CLIENT_ID.to_string(),
        issuer,
