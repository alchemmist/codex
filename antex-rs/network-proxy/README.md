...
`antex-network-proxy` is Codex's local network policy enforcement proxy. It runs:
...
`antex-network-proxy` reads from Codex's merged `config.toml` (via `antex-core` config loading).
...
# bundle env vars pointing at immutable public files under $ANTEX_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p antex-network-proxy --
```
...
`antex-network-proxy` can be embedded as a library with a thin API:

```rust
use antex_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `antex-network-proxy` is embedded in managed Codex runtime, policy decisions emit structured
OTEL-compatible events with `target=antex_otel.network_proxy`.
...
This section documents the protections implemented by `antex-network-proxy`, and the boundaries of
