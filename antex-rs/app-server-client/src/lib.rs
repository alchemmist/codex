...
//!
//! This crate wraps [`antex_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](antex_app_server::in_process::InProcessClientHandle),
...
pub use antex_app_server::app_server_control_socket_path;
pub use antex_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use antex_app_server::in_process::InProcessServerEvent;
use antex_app_server::in_process::InProcessStartArgs;
use antex_app_server::in_process::LogDbLayer;
pub use antex_app_server::in_process::StateDbHandle;
...
use antex_app_server_protocol::Result as JsonRpcResult;
...
use antex_config::NoopThreadConfigLoader;
use antex_core::config::Config;
pub use antex_core::otel_init::build_provider as build_otel_provider;
...
/// module exists so clients can remove a direct `antex-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub mod config {
        pub use antex_core::config::*;

        pub mod edit {
            pub use antex_core::config::edit::*;
        }
    }
}
...
pub struct InProcessClientStartArgs {
    ...
    /// Whether auth loading should honor the `ANTEX_API_KEY` environment variable.
    pub enable_antex_api_key_env: bool,
...
impl InProcessClientStartArgs {
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_antex_api_key_env: self.enable_antex_api_key_env,
            initialize,
...
/// - [`antex_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            antex_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
impl AppServerClient {
    ...
    pub fn codex_home(&self, local_antex_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_antex_home.display().to_string(),
            )),
...
mod tests {
    ...
    use antex_app_server_protocol::ServerNotification;
    use antex_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use antex_core::init_state_db;
    use antex_protocol::config_types::Personality;
    use antex_uds::UnixListener;
    use antex_utils_absolute_path::AbsolutePathBuf;
    use futures::SinkExt;
    ...
    async fn build_test_config_for_antex_home(codex_home: &Path) -> Config {
        ...
        {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides_for_antex_home(
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config_for_antex_home(codex_home.path()).await);
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            antex_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            antex_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(antex_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: antex_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(antex_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: antex_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: antex_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: antex_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: antex_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<antex_app_server_protocol::ThreadReadResponse>(
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.codex_home(), Some("/server/.antex"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: antex_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
            ...
            match event {
                ...
                AppServerEvent::ServerNotification(notification) => match notification.as_ref() {
                    ...
                    ServerNotification::ItemCompleted(notification)
                        if matches!(
                            &notification.item,
                            antex_app_server_protocol::ThreadItem::AgentMessage { text, .. }
                                if text == "hello"
                        ) =>
                    {
                    ...
                    ServerNotification::TurnCompleted(notification)
                        if notification.turn.status
                            == antex_app_server_protocol::TurnStatus::Completed =>
                    {
