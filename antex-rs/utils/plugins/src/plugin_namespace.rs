...
use antex_exec_server_protocol::DISCOVERABLE_PLUGIN_MANIFEST_PATHS;
...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".antex-plugin")).expect("mkdir manifest");
        fs::write(
    ...
    fn ignores_unrelated_root_plugin_manifest_before_legacy_fallback() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let legacy_path = plugin_root.join(".antex-plugin/plugin.json");
    ...
    fn rejects_nonregular_legacy_plugin_manifest_before_lower_precedence_manifest() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let antex_path = plugin_root.join(".antex-plugin/plugin.json");
        let claude_path = plugin_root.join(ALTERNATE_PLUGIN_CLA_MANIFEST_RELATIVE_PATH);
        fs::create_dir_all(&antex_path).expect("nonregular Codex manifest");
    ...
    fn rejects_symlinked_legacy_plugin_manifest_before_lower_precedence_manifest() {
        ...
        let antex_path = plugin_root.join(".antex-plugin/plugin.json");
        let claude_path = plugin_root.join(ALTERNATE_PLUGIN_CLA_MANIFEST_RELATIVE_PATH);
        fs::create_dir_all(antex_path.parent().expect("Codex manifest parent"))
        ...
        fs::write(&claude_path, r#"{"name":"sample"}"#).expect("Claude manifest");
        std::os::unix::fs::symlink("../benign.json", &antex_path).expect("Codex manifest symlink");
    ...
    fn rejects_symlinked_legacy_plugin_manifest_directory_before_lower_precedence_manifest() {
        ...
        .expect("benign manifest");
        fs::write(&claude_path, r#"{"name":"sample"}"#).expect("Claude manifest");
        std::os::unix::fs::symlink(&manifest_directory, plugin_root.join(".antex-plugin"))
    ...
    fn preserves_antex_claude_cursor_legacy_precedence() {
        ...
        assert_eq!(
            ...
            Some(antex_path.clone())
        );
        fs::remove_file(antex_path).expect("remove Codex manifest");
