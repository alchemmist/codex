...
/// Filename used for the redacted `antex doctor --json` feedback attachment.
...
pub const ANTEX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME: &str = "codex-app-directory-cache.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_codex_api_key_present = tracing::field::debug(auth_env.antex_api_key_env_present),
        auth_env_codex_api_key_enabled = tracing::field::debug(auth_env.antex_api_key_env_enabled),
...
#[derive(Clone)]
pub struct AntexFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for AntexFeedback {
    fn default() -> Self {
        Self::new()
    }
}
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("antex_http_client::transport", LevelFilter::DEBUG)
                    .with_target("antex_api::sse", LevelFilter::DEBUG)
                    ...
                    .with_target("antex_api::responses_websocket_timing", LevelFilter::OFF)
                    .with_target("antex_core::post_sampling_token_estimate", LevelFilter::OFF),
            )
    }
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = AntexFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn logger_layer_filters_noisy_trace_payloads() {
        ...
        tracing::trace!(target: "antex_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "antex_http_client::transport", "transport-trace");
        tracing::trace!(target: "antex_api::sse", "sse-trace");
        tracing::trace!(target: "antex_api::sse::responses", "nested-sse-trace");
        tracing::debug!(target: "antex_http_client::transport", "transport-debug");
        tracing::debug!(target: "antex_api::sse::responses", "sse-debug");
    ...
    async fn feedback_upload_allows_slow_reports() {
        ...
        upload_test_feedback(&AntexFeedback::new(), &dsn, &[attachment])
            .await
    ...
    async fn feedback_upload_deadline_stops_retries_and_later_attachments() {
        ...
        let snapshot = AntexFeedback::new()
            .snapshot(/*session_id*/ None)
            .with_feedback_diagnostics(FeedbackDiagnostics::default());
    ...
    async fn feedback_upload_retries_diagnostics_without_replaying_core() {
        ...
        upload_test_feedback(&AntexFeedback::new(), &dsn, &attachments)
            .await
    ...
    async fn feedback_upload_stops_diagnostics_for_sentry_rate_limits() {
        for (accepted_before_limit, rate_limit) in [
            ...
            upload_test_feedback(&AntexFeedback::new(), &dsn, &[later])
                .await
    ...
    async fn feedback_upload_does_not_forward_private_data_to_redirect_target() {
        ...
        let error = upload_test_feedback(&AntexFeedback::new(), &dsn, &[])
            .await
    ...
    async fn feedback_upload_reports_transport_failures() {
        ...
        assert!(
            error
                .downcast_ref::<antex_http_client::RouteAwareRequestError>()
                .is_some_and(antex_http_client::RouteAwareRequestError::is_connect)
        );
    }
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        ...
        let snapshot_with_diagnostics = AntexFeedback::new()
            .snapshot(/*session_id*/ None)
        ...
        let attachments_without_diagnostics = AntexFeedback::new()
            .snapshot(/*session_id*/ None)
            .with_feedback_diagnostics(FeedbackDiagnostics::default())
    ...
    fn path_backed_attachments_use_binary_content_types() {
        ...
        let attachments = AntexFeedback::new()
            .snapshot(/*session_id*/ None)
            .feedback_attachments(
                /*include_logs*/ false,
                &[],
