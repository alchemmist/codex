...
`antex-http-client` is the low-level HTTP transport shared by Codex crates. It is the intended
...
- custom CA handling through `ANTEX_CA_CERTIFICATE` and `SSL_CERT_FILE`;
...
Higher-level retry, SSE, and request-attempt telemetry policy remains in `antex-client`.
...
```rust
use antex_http_client::ClientRouteClass;
use antex_http_client::ReqwestTransport;

let client = http_client_factory.build_client(api_url, ClientRouteClass::Api)?;
let transport = ReqwestTransport::from_http_client(client);
```

If the existing wrapper surface cannot support a use case, extend `antex-http-client` rather than
