...
pub type StateDbHandle = Arc<antex_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        antex_state::StateRuntime::init(sqlite.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    antex_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == antex_state::BackfillStatus::Complete {
            return Ok(());
        }
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match antex_state::StateRuntime::init(
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == antex_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<antex_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&antex_state::StateRuntime>,
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => antex_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => antex_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => antex_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<antex_state::ThreadRelationFilter>,
    archived: bool,
    section: Option<Option<&str>>,
    project_id: Option<Option<&str>>,
    search_term: Option<&str>,
) -> Option<antex_state::ThreadsPage> {
    let ctx = context?;
    if ctx.sqlite() != sqlite {
        warn!(
            "state db SQLite home mismatch: expected {}, got {}",
    ...
    if let Some(relation_filter) = relation_filter {
        let filters = antex_state::ThreadFilterOptions {
