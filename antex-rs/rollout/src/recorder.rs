//! Persist Antex session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use antex_protocol::protocol::GitInfo as ProtocolGitInfo;
use antex_protocol::protocol::HistoryPosition;
use antex_protocol::protocol::MultiAgentVersion;
use antex_protocol::protocol::SessionContextWindow;
...
use antex_utils_path as path_utils;
...
///
/// ```ignore
/// $ jq -C . ~/.antex/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.antex/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Antex home directory.
    #[allow(clippy::too_many_arguments)]
    ...
    async fn list_threads_with_db_fallback(
        ...
        antex_state::record_fallback("list_threads", "db_error", /*telemetry_override*/ None);
...
fn precompute_new_rollout_path(
    ...
    // Resolve ~/.antex/sessions/YYYY/MM/DD path.
...
impl From<antex_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: antex_state::ThreadsPage) -> Self {
...
fn thread_item_from_state_metadata(
    item: antex_state::ThreadMetadata,
    parent_thread_id: Option<ThreadId>,
) -> ThreadItem {
    ThreadItem {
