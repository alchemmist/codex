...
use crate::landlock::ANTEX_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    pub antex_linux_sandbox_exe: Option<&'a Path>,
...
pub struct SandboxDirectSpawnTransformRequest<'a> {
    ...
    pub windows_sandbox_proxy_settings_mode: antex_windows_sandbox::WindowsSandboxProxySettingsMode,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing antex-linux-sandbox executable path")
            }
...
impl SandboxManager {
        ...
        let (argv, arg0_override, pending_sandboxed_request) = match sandbox {
            ...
            SandboxType::MacosSeatbelt => {
                ...
                let mut args = create_seatbelt_command_args_with_profile(
                    ...
                    self.allowed_symlinked_antex_home.as_ref(),
                )
            ...
            SandboxType::LinuxSeccomp => {
                let pending = pending_sandboxed_request?;
                let exe = antex_linux_sandbox_exe
    ...
    pub fn transform_for_direct_spawn(
        ...
        #[cfg(target_os = "windows")]
        {
            let codex_home = antex_utils_home_dir::find_antex_home()
                .map_err(|err| SandboxTransformError::WindowsSandboxPreparation(err.to_string()))?;
            self.transform_for_direct_spawn_with_antex_home(request, codex_home.as_path())
        }
    ...
    #[cfg(target_os = "windows")]
    fn transform_for_direct_spawn_with_antex_home(
        &self,
        request: SandboxDirectSpawnTransformRequest<'_>,
        codex_home: &Path,
    ) -> Result<SandboxExecRequest, SandboxTransformError> {
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    codex_home: &Path,
    proxy_settings_mode: antex_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    let helper = antex_windows_sandbox::resolve_exe_for_launch(source.as_path(), codex_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        antex_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(ANTEX_LINUX_SANDBOX_ARG0) {
        os_string_to_command_component(exe.as_os_str().to_owned())
    } else {
        ANTEX_LINUX_SANDBOX_ARG0.to_string()
    }
}
