...
use antex_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.antex".to_string(),
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_antex_subpaths() {
    ...
    assert!(
        ...
        "expected explicit writable root .git/.antex carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_antex_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Seatbelt policy.
    let config_toml = dot_antex_canonical.join("config.toml");
    let output = Command::new(MACOS_PATH_TO_SEATBELT_EXECUTABLE)
        .args(&args)
        .current_dir(&cwd)
        .output()
...
fn seatbelt_protects_resolved_target_of_symlinked_metadata_directory() {
    ...
    let dot_antex = writable_root.join(".antex");
    ...
    symlink(&actual_config, &dot_antex).expect("create .codex symlink");
    let policy = restricted_write_policy(&[writable_root.as_path()]);
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
        command: vec![
            "/bin/sh".to_string(),
...
#[test]
fn create_seatbelt_args_block_first_time_dot_antex_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_antex = repo_root.join(".antex");
    let config_toml = dot_antex.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_antex.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_antex = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_antex_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_antex),
        "missing {expected_dot_antex}: {args:#?}"
    );
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".antex")).expect("create .codex");
    fs::write(
        vulnerable_root.join(".antex").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    ...
    let dot_antex_canonical = vulnerable_root_canonical.join(".antex");
