//! Binds provisioning requests to the packaged Antex client and its Windows user.
    ...
    match client_family {
        Some(client_family) => match service_family.as_deref() {
            ...
            None if FOREGROUND_MODE.load(Ordering::Acquire)
                && is_known_antex_package_family(&client_family) => {}
            #[cfg(debug_assertions)]
            None if FOREGROUND_MODE.load(Ordering::Acquire) => {
...
#[cfg(debug_assertions)]
fn is_known_antex_package_family(package_family: &str) -> bool {
    matches!(
        package_family,
