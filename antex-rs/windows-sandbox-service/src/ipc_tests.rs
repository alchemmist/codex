...
use super::wake;
use antex_windows_sandbox::FramedProvisioningMessage;
use antex_windows_sandbox::PROVISIONING_PROTOCOL_VERSION;
use antex_windows_sandbox::ProvisioningMessage;
use antex_windows_sandbox::SandboxProvisioningRequest;
use antex_windows_sandbox::SandboxProvisioningResponse;
use antex_windows_sandbox::WindowsSandboxProvisioningSettings;
use antex_windows_sandbox::WindowsSandboxProxyListeners;
use antex_windows_sandbox::read_provisioning_frame;
use antex_windows_sandbox::to_wide;
use antex_windows_sandbox::write_provisioning_frame;
use std::path::Path;
...
fn config_parse_errors_are_distinguished_from_policy_and_io_failures() {
    let schema_error = antex_core::config::deserialize_config_toml_with_base(
    ...
    let syntax_error = antex_config::io_error_from_config_error(
        std::io::ErrorKind::InvalidData,
        antex_config::config_error_from_toml("config.toml", contents, parse_error.clone()),
        Some(parse_error),
    );
...
fn provisioning_request_preserves_home_spaces_and_unicode() {
    let request = framed_request(SandboxProvisioningRequest {
        codex_home: "D:\\Codex Homes\\Jos\u{00e9}\\.antex".to_string(),
    ...
    assert_eq!(
        ...
        ProvisioningRequest {
            codex_home: PathBuf::from("D:\\Codex Homes\\Jos\u{00e9}\\.antex"),
            listeners: WindowsSandboxProxyListeners::default(),
            settings: WindowsSandboxProvisioningSettings::default(),
        }
    );
...
fn structured_provisioning_request_accepts_independent_and_additional_proxy_ports() {
    for (http_ports, socks_ports, proxy_ports) in [
        ...
        let request = framed_request(SandboxProvisioningRequest {
            codex_home: r"C:\Users\alice\.antex".to_string(),
        ...
        assert_eq!(
            validate_request(&request).unwrap(),
            ProvisioningRequest {
                codex_home: PathBuf::from(r"C:\Users\alice\.antex"),
                settings,
                listeners,
            }
        );
