...
`antex-otel` is the OpenTelemetry integration crate for Codex. It provides:

- Provider wiring for log/trace/metric exporters (`antex_otel::OtelProvider`
  and `antex_otel::provider`).
- Session-scoped business event emission via `antex_otel::SessionTelemetry`.
- Low-level metrics APIs via `antex_otel::metrics`.
- Trace-context helpers via `antex_otel::trace_context` and crate-root re-exports.
...
```rust
use antex_otel::config::OtelExporter;
use antex_otel::config::OtelHttpProtocol;
use antex_otel::config::OtelSettings;
use antex_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "antex-cli".to_string(),
...
`antex-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Codex-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use antex_otel::config::{OtelExporter, OtelHttpProtocol};
