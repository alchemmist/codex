...
fn bundled_cleanup_source(plugin_id: &str, server: &str, event: &str) -> PluginHookSource {
    ...
    PluginHookSource {
        ...
        source_path: plugin_root.join(".antex-plugin/plugin.json"),
...
fn executor_stop_hook_fixture() -> (
    ...
    let source = ExecutorPluginHookSource {
        ...
        manifest_path: "file:///plugins/computer-use/.antex-plugin/plugin.json"
            .parse()
            .expect("valid plugin manifest URI"),
        source_relative_path: ".antex-plugin/plugin.json#hooks[0]".to_string(),
        hooks: HookEventsToml {
...
async fn mcp_tool_hooks_expand_event_input_and_apply_pre_tool_decisions() {
    ...
    assert_eq!(
        ...
        antex_protocol::protocol::HookExecutionMode::Sync
    );
    assert_eq!(
        *calls.lock().expect("lock MCP calls"),
        vec![HookMcpCall {
