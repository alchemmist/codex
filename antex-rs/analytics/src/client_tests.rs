...
#[cfg(debug_assertions)]
use crate::events::AntexPluginMetadata;
#[cfg(debug_assertions)]
use crate::events::AntexPluginUsedEventRequest;
#[cfg(debug_assertions)]
use crate::events::AntexPluginUsedMetadata;
#[cfg(debug_assertions)]
...
use antex_app_server_protocol::CommandExecutionOutputDeltaNotification;
...
fn sample_artifact_operation_event(thread_id: &str) -> TrackEventRequest {
    TrackEventRequest::ArtifactOperation(antex_artifact_operation_event_request(
        TrackEventsContext {
...
fn sample_mcp_tool_call_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::McpToolCall(AntexMcpToolCallEventRequest {
        ...
        event_params: AntexMcpToolCallEventParams {
            base: AntexToolItemEventBase {
                ...
                runtime: AntexRuntimeMetadata {
                    antex_rs_version: "0.0.0".to_string(),
...
fn sample_plugin_used_track_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::PluginUsed(AntexPluginUsedEventRequest {
        event_type: "codex_plugin_used",
        event_params: AntexPluginUsedMetadata {
            plugin: AntexPluginMetadata {
                plugin_id: plugin_id.map(str::to_string),
                remote_plugin_id: None,
...
async fn capture_file_writes_exact_serialized_request() {
    ...
    let auth = antex_login::AntexAuth::create_dummy_chatgpt_auth_for_testing();
...
fn track_response_ignores_unserializable_thread_responses() {
    ...
    thread_start.cwd = antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn flush_is_noop_when_analytics_is_disabled() {
    let client = AnalyticsEventsClient::new(
        antex_login::AuthManager::from_auth_for_testing(
