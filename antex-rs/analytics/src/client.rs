...
use antex_protocol::items::CollabAgentToolCallStatus;
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::AntexCompactionEvent) {
    ...
    pub fn track_goal_event(&self, event: AntexGoalEvent) {
    ...
    pub fn track_turn_antex_error(&self, fact: TurnAntexErrorFact) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::TurnAntexError(
            Box::new(fact),
        )));
    }
...
async fn send_track_events(
    ...
    } else if !auth.uses_antex_backend() {
        return;
    }
    if events.is_empty() {
        return;
...
async fn send_track_events_request(
    ...
    let response = create_client()
        ...
        .headers(antex_model_provider::auth_provider_from_auth(auth).to_auth_headers())
