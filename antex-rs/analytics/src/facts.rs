...
pub use antex_protocol::error::AntexErrKind;
...
/// Measurements for one successfully decoded image at the point where Antex prepares it for
...
#[derive(Clone)]
pub struct TurnAntexErrorFact {
    pub(crate) turn_id: String,
    pub(crate) thread_id: String,
    pub(crate) error: TurnAntexError,
}

impl TurnAntexErrorFact {
    pub fn from_antex_err(thread_id: String, turn_id: String, error: &CodexErr) -> Self {
        Self {
            turn_id,
            thread_id,
            error: TurnAntexError::from_antex_err(error),
        }
    }
}

#[derive(Clone)]
pub(crate) struct TurnAntexError {
    pub(crate) kind: AntexErrKind,
    pub(crate) http_status_code: Option<u16>,
}

impl TurnAntexError {
    fn from_antex_err(error: &CodexErr) -> Self {
        Self {
            kind: error.into(),
...
#[derive(Clone)]
pub struct AntexTurnSteerEvent {
...
#[derive(Clone)]
pub struct AntexCompactionEvent {
    pub thread_id: String,
    pub turn_id: String,
    ...
    pub antex_error_kind: Option<AntexErrKind>,
    pub antex_error_http_status_code: Option<u16>,
...
#[derive(Clone)]
pub struct AntexGoalEvent {
    pub thread_id: String,
    pub turn_id: Option<String>,
    pub goal_id: String,
    pub event_kind: GoalEventKind,
    pub goal_status: antex_state::ThreadGoalStatus,
...
pub(crate) enum CustomAnalyticsFact {
    ...
    Compaction(Box<AntexCompactionEvent>),
    Goal(Box<AntexGoalEvent>),
    ...
    TurnAntexError(Box<TurnAntexErrorFact>),
