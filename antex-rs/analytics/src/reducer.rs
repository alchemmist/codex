...
use crate::events::AntexCollabAgentToolCallEventParams;
use crate::events::AntexCollabAgentToolCallEventRequest;
...
use crate::events::AntexControlToolCallEventParams;
use crate::events::AntexControlToolCallEventRequest;
use crate::events::AntexDynamicToolCallEventParams;
use crate::events::AntexDynamicToolCallEventRequest;
use crate::events::AntexFileChangeEventParams;
use crate::events::AntexFileChangeEventRequest;
use crate::events::AntexGoalEventRequest;
use crate::events::AntexHookRunEventRequest;
use crate::events::AntexImageGenerationEventParams;
use crate::events::AntexImageGenerationEventRequest;
use crate::events::AntexMcpToolCallEventParams;
use crate::events::AntexMcpToolCallEventRequest;
use crate::events::AntexOnboardingExternalAgentImportCompleteEventRequest;
use crate::events::AntexOnboardingExternalAgentImportCompleteMetadata;
...
use crate::events::AntexPluginInstallRequestedEventRequest;
use crate::events::AntexPluginMeasurementEventParams;
use crate::events::AntexPluginMeasurementEventRequest;
...
use crate::events::AntexTurnEventParams;
use crate::events::AntexTurnEventRequest;
use crate::events::AntexTurnSteerEventParams;
use crate::events::AntexTurnSteerEventRequest;
use crate::events::AntexWebSearchEventParams;
use crate::events::AntexWebSearchEventRequest;
...
use crate::events::antex_artifact_operation_event_request;
use crate::events::antex_compaction_event_params;
use crate::events::antex_goal_event_params;
use crate::events::antex_hook_run_metadata;
use crate::events::antex_plugin_install_requested_metadata;
...
use crate::facts::AntexGoalEvent;
...
use crate::facts::TurnAntexError;
...
use antex_app_server_protocol::ClientResponse;
use antex_app_server_protocol::CodexErrorInfo;
use antex_app_server_protocol::CollabAgentStatus;
...
use antex_app_server_protocol::CommandExecutionApprovalKind;
use antex_app_server_protocol::CommandExecutionSource;
use antex_app_server_protocol::CommandExecutionStatus;
use antex_app_server_protocol::DynamicToolCallOutputContentItem;
...
use antex_app_server_protocol::NetworkPolicyRuleAction;
use antex_app_server_protocol::PatchApplyStatus;
use antex_app_server_protocol::PatchChangeKind;
...
use antex_app_server_protocol::WebSearchAction;
use antex_git_utils::SanitizedGitUrl;
use antex_git_utils::collect_git_info;
use antex_git_utils::get_git_repo_root;
...
use antex_protocol::items::is_safe_plugin_relative_path;
use antex_protocol::models::PermissionProfile;
use antex_protocol::protocol::SessionSource;
use antex_protocol::protocol::SkillScope;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a AntexCompactionEvent) -> Self {
        Self {
    ...
    fn goal(input: &'a AntexGoalEvent) -> Self {
        Self {
            event_name: "goal",
            thread_id: &input.thread_id,
    ...
    fn tool_item(
        notification: &'a antex_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct TurnState {
    ...
    antex_error: Option<TurnAntexError>,
...
impl AnalyticsReducer {
    pub(crate) async fn ingest(&mut self, input: AnalyticsFact, out: &mut Vec<TrackEventRequest>) {
        match input {
            ...
            AnalyticsFact::Custom(input) => match input {
                ...
                CustomAnalyticsFact::TurnAntexError(input) => {
                    self.ingest_turn_antex_error(*input);
                }
    ...
    fn ingest_artifact_operation(
        ...
        out.push(TrackEventRequest::ArtifactOperation(
            antex_artifact_operation_event_request(input.tracking, input.operation),
        ));
    }
    ...
    fn ingest_code_mode_tool_call(
        ...
        match input {
            ...
            CodeModeToolCallFact::Completed {
                ...
                let event = TrackEventRequest::DynamicToolCall(AntexDynamicToolCallEventRequest {
                    event_type: "codex_dynamic_tool_call_event",
                    event_params: AntexDynamicToolCallEventParams {
                        base,
    ...
    fn ingest_control_tool_call(
        ...
        base.cell_id = cell_id;
        let event = TrackEventRequest::ControlToolCall(AntexControlToolCallEventRequest {
            event_type: "codex_control_tool_call_event",
            event_params: AntexControlToolCallEventParams {
                base,
    ...
    fn ingest_turn_antex_error(&mut self, input: TurnAntexErrorFact) {
        let TurnAntexErrorFact {
            turn_id,
            thread_id,
            error,
        } = input;
        ...
        turn_state.antex_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = antex_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(AntexAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = antex_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(AntexAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(AntexHookRunEventRequest {
            event_type: "codex_hook_run",
            event_params: antex_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(AntexPluginUsedEventRequest {
            event_type: "codex_plugin_used",
            event_params: antex_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            AntexPluginInstallRequestedEventRequest {
                event_type: "codex_plugin_install_requested",
                event_params: antex_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = AntexPluginEventRequest {
    ...
    fn ingest_external_agent_config_import_completed(
        ...
        out.push(TrackEventRequest::ExternalAgentConfigImportCompleted(
            AntexOnboardingExternalAgentImportCompleteEventRequest {
                event_type: "codex_onboarding_external_agent_import_complete",
                event_params: AntexOnboardingExternalAgentImportCompleteMetadata {
    ...
    fn ingest_plugin_measurements(
        ...
        out.extend(
            rows.into_iter()
                ...
                .map(|row| {
                    TrackEventRequest::PluginMeasurement(AntexPluginMeasurementEventRequest {
                        event_type: "codex_plugin_measurement_event",
                        event_params: AntexPluginMeasurementEventParams {
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: antex_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        thread_originator: Option<String>,
        out: &mut Vec<TrackEventRequest>,
    ...
    fn ingest_compaction(&mut self, input: AntexCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            AntexCompactionEventRequest {
                event_type: "codex_compaction_event",
                event_params: antex_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: AntexGoalEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
            self.thread_context_or_warn(AnalyticsDropSite::goal(&input))
        else {
            return;
        };
        out.push(TrackEventRequest::Goal(Box::new(AntexGoalEventRequest {
            event_type: "codex_goal_event",
            event_params: antex_goal_event_params(
                input,
                thread_metadata.session_id.clone(),
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: antex_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(AntexTurnSteerEventRequest {
            event_type: "codex_turn_steer_event",
            event_params: AntexTurnSteerEventParams {
    ...
    fn emit_review_event(
        ...
        out.push(TrackEventRequest::ReviewEvent(AntexReviewEventRequest {
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(AntexTurnEventRequest {
            event_type: "codex_turn_event",
            event_params: antex_turn_event_params(
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
...
fn tool_event_base_mut(event: &mut TrackEventRequest) -> Option<&mut AntexToolItemEventBase> {
    match event {
    ...
    match item {
        ThreadItem::CommandExecution {
            ...
            Some(TrackEventRequest::CommandExecution(
                AntexCommandExecutionEventRequest {
        ...
        ThreadItem::FileChange {
            ...
            Some(TrackEventRequest::FileChange(AntexFileChangeEventRequest {
                event_type: "codex_file_change_event",
                event_params: AntexFileChangeEventParams {
                    base,
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                AntexMcpToolCallEventRequest {
                    event_type: "codex_mcp_tool_call_event",
                    event_params: AntexMcpToolCallEventParams {
                        base,
        ...
        ThreadItem::DynamicToolCall {
            ...
            Some(TrackEventRequest::DynamicToolCall(
                AntexDynamicToolCallEventRequest {
                    event_type: "codex_dynamic_tool_call_event",
                    event_params: AntexDynamicToolCallEventParams {
                        base,
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                AntexCollabAgentToolCallEventRequest {
                    event_type: "codex_collab_agent_tool_call_event",
                    event_params: AntexCollabAgentToolCallEventParams {
                        base,
        ...
        ThreadItem::WebSearch(item) => {
            ...
            Some(TrackEventRequest::WebSearch(AntexWebSearchEventRequest {
                event_type: "codex_web_search_event",
                event_params: AntexWebSearchEventParams {
                    base,
        ...
        ThreadItem::ImageGeneration(item) => {
            ...
            Some(TrackEventRequest::ImageGeneration(
                AntexImageGenerationEventRequest {
                    event_type: "codex_image_generation_event",
                    event_params: AntexImageGenerationEventParams {
                        base,
...
fn file_change_counts(changes: &[antex_app_server_protocol::FileUpdateChange]) -> FileChangeCounts {
...
fn antex_turn_event_params(
    app_server_client: CodexAppServerClientMetadata,
    runtime: AntexRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> AntexTurnEventParams {
    let (
        Some(thread_id),
    ...
    let antex_error = turn_state.antex_error.as_ref();
    AntexTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
        ...
        antex_turn_source: turn_metadata.antex_turn_source(),
        app_server_client,
        runtime,
        submission_type,
        ephemeral,
        ...
        antex_error_kind: antex_error.map(|error| error.kind),
        antex_error_http_status_code: antex_error.and_then(|error| error.http_status_code),
...
fn analytics_turn_status(status: antex_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        antex_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        antex_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        antex_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        antex_app_server_protocol::TurnStatus::InProgress => None,
    }
}
    ...
    fn command_execution_script_paths_reject_unsafe_values() {
        assert_eq!(
            safe_plugin_relative_script_path(
                ...
                Some("/home/user/.antex/plugins/cache/openai-curated/sample/scripts/run.py"),
            ),
            None
        );
        assert_eq!(
