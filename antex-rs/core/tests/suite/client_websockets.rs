...
use antex_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use antex_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use antex_core::AntexResponsesMetadata;
use antex_core::ModelClient;
use antex_core::ModelClientSession;
...
use antex_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
use antex_core::test_support::with_parent_turn;
...
use antex_rollout_trace::ConversationPart;
use antex_rollout_trace::InferenceTraceContext;
use antex_rollout_trace::RawTraceEventPayload;
use antex_rollout_trace::TraceWriter;
...
fn responses_metadata(
    ...
    turn_id: Option<&str>,
    request_kind: TestAntexResponsesRequestKind,
) -> AntexResponsesMetadata {
...
fn turn_metadata(harness: &WebsocketTestHarness, turn_id: Option<&str>) -> AntexResponsesMetadata {
    responses_metadata(harness, turn_id, TestAntexResponsesRequestKind::Turn)
}
...
fn prewarm_metadata(
    ...
    responses_metadata(harness, turn_id, TestAntexResponsesRequestKind::Prewarm)
}

fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> AntexResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
        TestAntexResponsesRequestKind::WebsocketConnection,
    )
}
...
async fn responses_websocket_preserves_credit_usage_metadata() {
    ...
    let harness = websocket_harness_for_antex_backend(&server).await;
    ...
    assert_eq!(
        usage_metadata,
        Some(antex_protocol::ResponseUsageMetadata {
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        handshake.header(X_ANTEX_ROUTING_HINT_HEADER),
    ...
    assert_eq!(
        ...
        Some(antex_login::default_client::get_antex_user_agent())
    );
    assert_eq!(
    ...
    let stream_request_start_ms = body["client_metadata"]
        [X_ANTEX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_omits_routing_hint_for_provider_with_own_credentials() {
    ...
    assert_eq!(handshake.header(X_ANTEX_ROUTING_HINT_HEADER), None);
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::AntexTurnStarted {
            antex_turn_id: "turn-1".to_string(),
            thread_id: harness.thread_id.to_string(),
        })
        .expect("turn started");
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server
            .single_handshake()
            .header(X_ANTEX_ROUTING_HINT_HEADER),
        None
    );
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let token_event =
        wait_for_event(&test.antex, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.antex, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn websocket_harness_for_antex_backend(server: &WebSocketTestServer) -> WebsocketTestHarness {
...
async fn websocket_harness_with_provider_options_and_auth(
    ...
    let model_info = antex_core::test_support::construct_model_info_offline(MODEL, &config);
    let thread_id = ThreadId::new();
    let session_id = SessionId::new();
    let client_auth_manager = auth.map(antex_core::test_support::auth_manager_from_auth);
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "antex-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
