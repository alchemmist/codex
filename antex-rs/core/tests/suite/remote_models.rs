...
use antex_models_manager::model_info::BASE_INSTRUCTIONS;
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let manager = antex_core::test_support::models_manager_with_provider(
        codex_home.path().to_path_buf(),
        antex_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
...
async fn remote_models_config_context_window_override_clamps_to_max_context_window() -> Result<()> {
    ...
    let turn_started_event = wait_for_event(&antex, |event| {
        matches!(
            event,
...
async fn namespaced_model_slug_uses_catalog_metadata_without_fallback_warning() -> Result<()> {
    ...
    loop {
        let event = wait_for_event(&antex, |_| true).await;
        match event {
...
fn bundled_default_model_slug() -> String {
    antex_core::test_support::all_model_presets()
        .iter()
