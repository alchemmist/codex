...
async fn namespaced_custom_tool_call_preserves_namespace_through_dispatch_and_replay() -> Result<()>
{
    ...
    assert_eq!(
        ...
        json!([{
            "name": antex_code_mode::PUBLIC_TOOL_NAME,
            "arguments": input,
        }]),
    );
...
async fn collect_tools(availability: CommandToolAvailability) -> Result<Vec<String>> {
    ...
    let mut builder = match availability {
        CommandToolAvailability::Default => test_antex(),
        CommandToolAvailability::ManagedUnifiedExecDisabled => test_antex()
...
"#,
        ...
        CommandToolAvailability::ShellToolDisabled => test_antex().with_config(|config| {
            config
                .features
        ...
        CommandToolAvailability::ModelDisabled => {
            test_antex().with_model_info_override("gpt-5.4", |model| {
