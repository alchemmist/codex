use antex_config::types::Personality;
...
use antex_protocol::openai_models::default_input_modalities;
...
fn read_only_user_turn(test: &TestAntex, items: Vec<UserInput>, model: String) -> TurnInputRequest {
...
async fn first_turn_model_change_appends_model_instructions_developer_message(
    ...
    let mut builder = test_antex().with_model("gpt-5.2").with_config(|config| {
        config
            .features
...
async fn first_turn_after_empty_prefix_fork_preserves_inherited_base_instructions(
    ...
    let source_rollout_path = test.antex.rollout_path().expect("rollout path");
    let source_history =
        antex_rollout::RolloutRecorder::get_rollout_history(&source_rollout_path).await?;
...
async fn rollback_first_turn_model_change_removes_its_instructions(
    ...
    let mut builder = test_antex().with_model(initial_model);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    let test = match followup {
        RollbackFollowup::ColdResume => {
            let mut resume_builder = test_antex().with_model(switched_model);
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.antex, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.antex, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.antex, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
