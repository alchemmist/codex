...
use antex_core::config::set_project_trust_level;
use antex_core_plugins::store::PluginStore;
...
use antex_skills_extension::HostSkillsLoadInput;
use antex_skills_extension::SkillsExtensionConfig;
use antex_skills_extension::install;
use anyhow::Result;
...
fn write_sample_plugin_manifest_and_config_at_root(
    ...
    std::fs::create_dir_all(plugin_root.join(".antex-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
async fn build_analytics_plugin_test_antex(
    server: &MockServer,
...
async fn build_apps_enabled_plugin_test_antex(
    server: &MockServer,
    codex_home: Arc<TempDir>,
...
async fn persisted_remote_plugin_command_attribution_flows_through_turn_context(
    ...
    std::fs::write(
        &script_path,
        r#"printf '%s' '{"version":1,"measurements":[{"name":"files_scanned","value":7}]}' > "$ANTEX_PLUGIN_METRICS_OUTPUT"
"#,
    )?;
    ...
    let test_antex = builder.build_with_auto_env(&server).await?;
    let antex = Arc::clone(&test_antex.antex);
    let cwd = test_antex.config.cwd.clone();
    let session_model = test_antex.session_configured.model.clone();
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::read_only(), cwd.as_path());
    antex
        .start_or_steer_turn(
            TurnInputRequest::user_input(vec![antex_protocol::user_input::UserInput::Text {
    ...
    let begin = wait_for_event_match(&antex, |event| match event {
        EventMsg::ExecCommandBegin(event) if event.call_id == call_id => Some(event.clone()),
        _ => None,
    })
    .await;
    let end = wait_for_event_match(&antex, |event| match event {
...
async fn agent_plugin_skills_use_shared_catalog_and_direct_child_discovery() -> Result<()> {
    ...
    test_antex
        ...
        .await?;
    let warning = wait_for_event(&test_antex.antex, |ev| {
        matches!(
            ev,
    ...
    .await;
    wait_for_event(&test_antex.antex, |ev| {
...
async fn plugin_skill_product_policy_and_migrated_command_precedence_reach_agent_turns(
    ...
    if include_migrated_command {
        let migrated_skill_dir =
            plugin_root.join(".antex-plugin/migrated-command-skills/source-command-review");
...
"#,
    ...
    for (marketplace, skill) in [
        ...
        std::fs::create_dir_all(root.join(".antex-plugin"))?;
        std::fs::write(
            root.join(".antex-plugin/plugin.json"),
            r#"{"name":"sites"}"#,
        )?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn agent_plugin_root_mcp_stdio_tool_round_trip_expands_reserved_paths_and_antex_env_overlay()
-> Result<()> {
    skip_if_no_network!(Ok(()));
    let server = start_mock_server().await;
    ...
    let mut builder = test_antex().with_home(Arc::clone(&codex_home));
    let test_antex = builder.build_with_remote_and_local_env(&server).await?;
    wait_for_mcp_server(&test_antex.antex, "agent").await?;
    ...
    test_antex
        ...
        .await?;
    let end = wait_for_event(&test_antex.antex, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    let overlay_end = wait_for_event(&test_antex.antex, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    wait_for_event(&test_antex.antex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn curated_plugin_skills_follow_auth_switch() -> Result<()> {
    ...
    const FIXTURES: &[Fixture] = &[
        ...
        Fixture {
            ...
            target_auth: TargetAuth::NoAntexAuth,
            target_model_provider_id: AMAZON_BEDROCK_PROVIDER_ID,
            expected_target_loaded_plugin_skills: &[API_CURATED_PLUGIN_SKILL],
            expected_target_skill_description: "api description before",
        },
    ...
    async fn loaded_plugin_skills_for_config(test_antex: &TestAntex, config: &Config) -> String {
        let plugins_input = config.plugins_config_input();
        let plugins_manager = test_antex.thread_manager.plugins_manager();
        ...
        let skills_snapshot = test_antex
            .thread_manager
            .skills_service()
...
"#,
        ...
        let mut builder = test_antex()
            ...
            .with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing());
        let test_antex = builder.build_with_auto_env(&server).await?;
        let initial_skills = loaded_plugin_skills_for_config(&test_antex, &test_antex.config).await;
        ...
        let expected_auth_mode = match fixture.target_auth {
            ...
            TargetAuth::ApiKey => {
                antex_login::login_with_api_key(
                    codex_home.path(),
                    "test-api-key",
                    antex_login::AuthCredentialsStoreMode::File,
                    antex_login::AuthKeyringBackendKind::default(),
                )?;
                test_antex.thread_manager.auth_manager().reload().await;
                Some(AuthMode::ApiKey)
            }
            TargetAuth::BedrockApiKey => {
                antex_login::login_with_bedrock_api_key(
                    codex_home.path(),
            ...
            TargetAuth::NoAntexAuth => {
                test_antex.thread_manager.auth_manager().logout().await?;
                None
            }
        };
        assert_eq!(
            test_antex.thread_manager.auth_manager().get_api_auth_mode(),
            expected_auth_mode
        );
        test_antex.thread_manager.skills_service().clear_cache();
        let mut target_config = test_antex.config.clone();
        target_config.model_provider_id = fixture.target_model_provider_id.to_string();
        let target_skills = loaded_plugin_skills_for_config(&test_antex, &target_config).await;
        assert_loaded_plugin_skills(
            fixture.name,
            "target config",
            &target_skills,
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins(
    ...
    let test_antex =
        build_apps_enabled_plugin_test_antex(&server, codex_home, apps_server.chatgpt_base_url)
            .await?;
    let antex = Arc::clone(&test_antex.antex);
    wait_for_mcp_server(&antex, CODEX_APPS_MCP_SERVER_NAME).await?;

    antex
        .start_or_steer_turn(TurnInputRequest::user_input(vec![
            antex_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_chatgpt_auth() -> Result<()> {
    ...
    let test_antex =
        build_apps_enabled_plugin_test_antex(&server, codex_home, apps_server.chatgpt_base_url)
            .await?;
    let antex = Arc::clone(&test_antex.antex);
    wait_for_mcp_server(&antex, "sample").await?;
...
async fn system_marketplace_plugin_honors_layered_activation_and_mcp_policy(
    ...
    std::fs::create_dir_all(project.path().join(".antex"))?;
    std::fs::write(
        project.path().join(".antex/config.toml"),
        format!(
    ...
    let startup = wait_for_event_match(&test.antex, |event| match event {
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let codex_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(codex_home.as_ref());
    let test_antex = build_analytics_plugin_test_antex(&server, codex_home).await?;
    let antex = Arc::clone(&test_antex.antex);
    ...
    assert_eq!(
        ...
        serde_json::json!(antex_login::default_client::originator().value)
    );
