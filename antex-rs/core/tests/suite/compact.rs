...
fn replacement_history_from_rollout(path: &Path) -> Result<Vec<Value>> {
    ...
    {
        let entry = antex_rollout::parse_rollout_line(line)?;
...
async fn assert_compaction_uses_turn_lifecycle_id(antex: &std::sync::Arc<antex_core::AntexThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = antex.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions(
    ...
    // Build config pointing to the mock server and spawn Antex.
    ...
    let warning_event = wait_for_event(&antex, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Antex to flush rollout entries before inspecting the file.
    ...
    for line in text.lines() {
        ...
        let Ok(entry) = antex_rollout::parse_rollout_line(trimmed) else {
            continue;
        };
        match entry.item {
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    antex.submit(Op::Compact).await.expect("trigger compact");

    let completed = wait_for_event_match(&antex, |ev| match ev {
    ...
    wait_for_event(&antex, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&antex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    let hook_inputs = read_hook_inputs(&test.antex_home_path().join("pre_compact_block_log.jsonl"));
...
async fn compact_hooks_respect_matchers_and_post_runs_after_compaction() {
    ...
    let hook_inputs =
        read_hook_inputs(&test.antex_home_path().join("post_compact_manual_log.jsonl"));
    assert_eq!(hook_inputs.len(), 1);
    let input = &hook_inputs[0];
...
async fn manual_compact_records_durable_and_local_token_usage() {
    ...
    antex.submit(Op::Compact).await.unwrap();

    // First TokenCount: from the compact API call (usage.total_tokens = 0).
    let first = wait_for_event_match(&antex, |ev| match ev {
    ...
    let last = wait_for_event_match(&antex, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    let antex = builder.build(&server).await.unwrap().antex;
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        ...
        antex_protocol::models::ResponseItem::Compaction {
            id: None,
    ...
    let mut resume_builder = test_antex().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    });
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.antex).await;
...
async fn pre_sampling_compact_falls_back_from_retired_previous_model_after_rename() {
    ...
    let mut initial_builder = test_antex()
        .with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    let model_provider = openai_model_provider(&server);
    let mut resumed_builder = test_antex()
        .with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(retired_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.antex).await;
...
async fn auto_compact_persists_rollout_entries() {
    ...
    let expected_settings = antex.thread_settings_snapshot().await;
    antex.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&antex, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&antex, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&antex, |event| match event {
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let antex = builder.build(&server).await.unwrap();

    antex.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    antex.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    antex.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&antex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn manual_compaction_keeps_the_creation_time_global_instructions() -> Result<()> {
    ...
    assert_eq!(
        test.antex.instruction_sources().await,
...
-> Result<()> {
    ...
    test.antex.submit(Op::Shutdown).await?;
    wait_for_event(&test.antex, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    ...
    assert_eq!(
        resumed.antex.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
