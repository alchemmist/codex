...
use antex_extension_api::McpServerContribution;
use antex_extension_api::McpServerContributionContext;
use antex_extension_api::McpServerContributor;
...
async fn root_and_spawned_subagent_receive_distinct_mcp_session_sources() -> Result<()> {
    ...
    let test = core_test_support::test_antex::test_antex()
...
async fn timeout_refresh_replaces_pending_startup_and_reuses_ready_connection() -> Result<()> {
    ...
    test.antex.refresh_mcp_config(refresh_config).await;
...
"#,
    ...
    let refreshed_servers = user_config
        .get("mcp_servers")
        .cloned()
        .map(HashMap::<String, antex_config::types::McpServerConfig>::deserialize)
        .transpose()?
    ...
    test.antex.refresh_runtime_config(refresh_config).await;
    test.antex.submit(Op::RefreshMcpServers).await?;
...
async fn elevated_apps_catalog_limit_requires_host_owned_registration() -> Result<()> {
    ...
    for extension_id in [None, Some("hosted_plugin_runtime"), Some("test-extension")] {
        ...
        let test = builder.build_with_auto_env(&server).await?;
        let startup = wait_for_mcp_server(&test.antex, CODEX_APPS_MCP_SERVER_NAME).await;
...
async fn initially_empty_deferred_tool_world_state_is_not_rendered_or_persisted() -> Result<()> {
    ...
    let mut builder = core_test_support::test_antex::test_antex()
        .with_config(enable_deferred_tool_world_state_without_agents);
    let test = builder.build_with_auto_env(&server).await?;
...
async fn deferred_tool_world_state_survives_resume_without_duplicate_updates() -> Result<()> {
    ...
    wait_for_mcp_server(&initial.antex, CODEX_APPS_MCP_SERVER_NAME).await?;
    initial
        .submit_turn("inspect deferred tools before resume")
        .await?;

    initial.antex.ensure_rollout_materialized().await;
    initial.antex.flush_rollout().await?;
    ...
    drop(initial);
    wait_for_mcp_server(&resumed.antex, CODEX_APPS_MCP_SERVER_NAME).await?;
    resumed
...
async fn apps_guidance_and_deferred_namespace_appear_after_recovery_within_a_turn() -> Result<()> {
    ...
    test.antex
        ...
        .await?;
    let EventMsg::RequestUserInput(request) = wait_for_event(&test.antex, |event| {
        ...
        matches!(
            ...
            EventMsg::McpStartupUpdate(update)
                if update.server == CODEX_APPS_MCP_SERVER_NAME
                    && matches!(update.status, antex_protocol::protocol::McpStartupStatus::Ready)
        )
    })
    .await;
...
-> Result<()> {
    ...
    tokio::fs::remove_dir_all(test.antex_home_path().join("cache/codex_apps_tools")).await?;
    startup_control.fail_next_initialize_attempts(/*attempts*/ 1);
    test.antex.submit(Op::RefreshMcpServers).await?;
    test.antex
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
