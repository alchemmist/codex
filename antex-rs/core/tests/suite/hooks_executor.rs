...
async fn executor_interrupt_hook_runs_after_attachment() -> Result<()> {
    ...
    fixture.test.antex.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.test.antex, |event| {
...
async fn executor_stop_hook_rejects_mismatched_environment() -> Result<()> {
    ...
    fixture
        ...
        .await;
    wait_for_mcp_server(&fixture.test.antex, "node_repl").await?;
    assert_eq!(
        fixture
            .test
            .antex
    ...
    if hook_event == "SubagentStop" {
        let child = fixture
            ...
            .start_thread(StartThreadOptions {
                ...
                environments: Some(fixture.test.antex.environment_selections().await),
                ..StartThreadOptions::new(fixture.test.config.clone())
            })
            .await?;
        fixture.test.antex = child.thread;
...
async fn executor_plugin_hook_fixture(
    ...
    for (plugin_id, manifest) in plugins {
        ...
        let plugin_directory = plugin_root.join(".antex-plugin")?;
...
impl ExecutorHookFixture {
    ...
    async fn interrupt_running_command(&self) -> Result<()> {
        wait_for_event(&self.test.antex, |event| {
            matches!(event, EventMsg::ExecCommandBegin(_))
        })
        .await;
        self.test.antex.submit(Op::Interrupt).await?;
        wait_for_event(&self.test.antex, |event| {
            matches!(event, EventMsg::TurnAborted(_))
        })
        .await;
