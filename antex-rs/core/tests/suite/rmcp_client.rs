...
use antex_config::types::McpServerAuth;
...
use antex_core::EnvironmentConfig;
use antex_core::EnvironmentMcpPolicy;
use antex_core::TurnInputRequest;
use antex_core::config::Config;
use antex_core::windows_sandbox::WindowsSandboxLevelExt;
use antex_exec_server::CreateDirectoryOptions;
use antex_exec_server::Environment;
use antex_exec_server::HttpRedirectPolicy;
use antex_exec_server::HttpRequestParams;
use antex_features::Feature;
use antex_http_client::HttpClientBuilder;
use antex_login::AntexAuth;
use antex_mcp::CODEX_APPS_MCP_SERVER_NAME;
use antex_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
use antex_mcp::SandboxState;
...
use antex_protocol::mcp_policy::McpServerIdentity;
use antex_protocol::mcp_policy::McpServerRequirement;
use antex_protocol::mcp_policy::PluginMcpRequirements;
...
use antex_protocol::openai_models::ConfigShellToolType;
use antex_protocol::openai_models::ConfirmationPolicies;
use antex_protocol::openai_models::InputModality;
use antex_protocol::openai_models::ModelInfo;
use antex_protocol::openai_models::ModelVisibility;
use antex_protocol::openai_models::ModelsResponse;
use antex_protocol::openai_models::ReasoningEffortPreset;
use antex_protocol::openai_models::TruncationPolicyConfig;
...
use antex_protocol::protocol::McpStartupFailureReason;
use antex_protocol::protocol::McpStartupStatus;
...
use antex_utils_cargo_bin::cargo_bin;
...
use core_test_support::test_antex::test_env;
use core_test_support::test_antex::turn_permission_fields;
...
fn read_only_user_turn(fixture: &TestAntex, text: impl Into<String>) -> TurnInputRequest {
...
fn auto_approved_user_turn(fixture: &TestAntex, text: impl Into<String>) -> TurnInputRequest {
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        antex_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
async fn call_structured_tool(
    ...
    wait_for_event(&fixture.antex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.antex, |ev| {
    ...
    wait_for_event(&fixture.antex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn assert_openai_form_capability_advertisement(expected: bool) -> anyhow::Result<()> {
    ...
    wait_for_mcp_server(&fixture.antex, server_name).await?;
...
async fn mcp_namespace_instructions_are_preserved_without_hiding_tools() -> anyhow::Result<()> {
    ...
    let fixture = test_antex()
        ...
        .await?;
    wait_for_mcp_server(&fixture.antex, "bounded").await?;

    fixture
        .antex
        .start_or_steer_turn(read_only_user_turn(&fixture, "show the bounded MCP tools"))
        .await?;
    wait_for_event(&fixture.antex, |event| {
...
async fn environment_mcp_policy_filters_runtime_config_and_model_tools(
    ...
    .await?;
    let (pending_config, _) = fixture.antex.current_mcp_config_and_runtime_context().await;
    ...
    let (runtime_config, _) = fixture.antex.current_mcp_config_and_runtime_context().await;
    ...
    fixture
        ...
        .await?;
    let (failed_config, _) = fixture.antex.current_mcp_config_and_runtime_context().await;
...
async fn stdio_server_round_trip(server_name: &'static str, namespace: &str) -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.antex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
"#,
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&fixture.antex, "history").await?;
...
-> anyhow::Result<()> {
                ...
                [("paginated", "two-pages"), ("rejected", "oversized-cursor")]
            {
                insert_mcp_server(
                    ...
                    stdio_transport(
                        ...
                        Some(HashMap::from([
                            (
                                "ANTEX_MCP_PROTOCOL_VERSION".to_string(),
    ...
    let startup = loop {
        let event = fixture.antex.next_event().await?;
...
async fn apps_enabled_turn_skips_pending_optional_mcp_without_cached_tools() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(2), fixture.antex.shutdown_and_wait())
        .await
...
async fn interrupt_during_mcp_startup_preserves_user_input_in_history(
    ...
    fixture.antex.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.antex, |event| {
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta(
    ...
    let namespace = format!("mcp__{server_name}");
    let mut models = antex_models_manager::bundled_models_response()?;
    let model = models
        .models
        .iter_mut()
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            antex_linux_sandbox_exe: fixture.config.antex_linux_sandbox_exe.clone(),
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.antex, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: antex_protocol::openai_models::ReasoningEffort::Medium,
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "ANTEX_TEST_REMOTE_EXEC_SERVER_URL";
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Antex should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Antex can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip(mode: HeadersHelperMode) -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Antex will call the MCP echo tool
    ...
    // Phase 3: configure Antex with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Antex begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.antex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn streamable_http_configured_auth_precedes_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let configured_auth_fixture = test_antex()
        .with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
            insert_mcp_server(
                config,
    ...
    wait_for_mcp_server(&configured_auth_fixture.antex, "configured_auth").await?;
...
async fn streamable_http_chatgpt_auth_is_not_sent_to_configured_origin() -> anyhow::Result<()> {
    ...
    wait_for_mcp_server(&fixture.antex, "untrusted_origin").await?;
...
/// This test writes to a fallback credentials file in ANTEX_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire ANTEX_HOME through the code.
#[test]
#[serial(codex_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Antex will call the OAuth-backed
    ...
    // Phase 3: seed an isolated ANTEX_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _antex_home_guard = EnvVarGuard::set("ANTEX_HOME", temp_home.path().as_os_str());
    let unset_authorization_env_var = format!(
        "ANTEX_TEST_UNSET_MCP_OAUTH_AUTHORIZATION_{}",
        std::process::id()
    );
    ...
    // Phase 4: configure Antex with the OAuth-backed Streamable HTTP MCP
    ...
    let recovery_hint = if credential_config.is_local_environment() {
        format!("Run `antex mcp login {server_name}`.")
    } else {
    ...
    let startup = wait_for_event(&fixture.antex, |event| {
    ...
    let (contended_turn, refreshed_failure, refreshed_failed_servers) =
        tokio::time::timeout(Duration::from_secs(5), async {
            ...
            {
                match fixture.antex.next_event().await?.msg {
    ...
    let discovered_turn = tokio::time::timeout(Duration::from_secs(5), async {
        ...
        fixture
            ...
            .await?;
        loop {
            if let EventMsg::TurnComplete(turn) = fixture.antex.next_event().await?.msg {
    ...
    tokio::time::timeout(
        ...
        wait_for_mcp_server(&fixture.antex, discovered_server_name),
    )
    .await
    .context("the newly discovered OAuth server did not recover after its store was unlocked")??;

    assert!(
        antex_rmcp_client::delete_oauth_tokens(
            discovered_credential_name.as_ref(),
            http_server.url(),
            OAuthCredentialsStoreMode::File,
            antex_config::types::AuthKeyringBackendKind::default(),
        )
        .await?
    );
    fixture.antex.refresh_runtime_config(refreshed_config).await;
    ...
    // Phase 7: assert Antex begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.antex, |ev| {
        matches!(
            ev,
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Antex fixture.
...
async fn write_fallback_oauth_tokens(
    ...
    antex_rmcp_client::save_oauth_tokens(
        server_name,
        &tokens,
        OAuthCredentialsStoreMode::File,
        antex_config::types::AuthKeyringBackendKind::default(),
