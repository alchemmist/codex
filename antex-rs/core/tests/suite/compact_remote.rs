...
use antex_core::X_ANTEX_ROUTING_HINT_HEADER;
use antex_core::compact::SUMMARY_PREFIX;
...
use antex_login::auth::AgentIdentityAuth;
use antex_login::auth::AgentIdentityAuthRecord;
use antex_login::auth::BedrockApiKeyAuth;
use antex_model_provider_info::AMAZON_BEDROCK_GPT_5_5_MODEL_ID;
use antex_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use antex_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
use antex_protocol::dynamic_tools::DynamicToolFunctionSpec;
use antex_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use antex_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use core_test_support::test_antex::TestAntexBuilder;
use core_test_support::test_antex::TestAntexHarness;
use core_test_support::test_antex::test_antex as base_test_antex;
...
fn test_antex() -> TestAntexBuilder {
    base_test_antex().with_config(|config| {
...
fn remote_realtime_test_antex_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestAntexBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_antex()
        .with_auth(AntexAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(antex: &antex_core::AntexThread) -> Result<()> {
    antex
    ...
    wait_for_event_match(antex, |msg| match msg {
...
async fn close_realtime_conversation(antex: &antex_core::AntexThread) -> Result<()> {
    antex.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(antex, |msg| match msg {
...
async fn wait_for_turn_complete(antex: &antex_core::AntexThread) {
...
fn amazon_bedrock_test_antex() -> TestAntexBuilder {
    let auth = AntexAuth::BedrockApiKey(BedrockApiKeyAuth {
...
async fn amazon_bedrock_manual_compaction_uses_v2_responses_endpoint() -> Result<()> {
    ...
    let harness = TestAntexHarness::with_auto_env_builder(amazon_bedrock_test_antex()).await?;
    ...
    harness.test().antex.submit(Op::Compact).await?;
    wait_for_turn_complete(&harness.test().antex).await;
...
async fn remote_compact_v2_retains_metadata_from_resumed_history() -> Result<()> {
    ...
    initial.antex.shutdown_and_wait().await?;
    ...
    wait_for_turn_complete(&resumed.antex).await;
    resumed.antex.shutdown_and_wait().await?;
...
async fn remote_compact_v2_retains_only_client_developer_messages_when_enabled(
    ...
    let harness = TestAntexHarness::with_auto_env_builder(
        test_antex()
            .with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(move |config| {
                config
    ...
    let antex = &harness.test().antex;
    let rollout_path = antex.rollout_path().context("rollout path")?;
    antex
    ...
    harness.test().submit_turn("before compact").await?;
    antex.submit(Op::Compact).await?;
    wait_for_turn_complete(antex).await;
    harness.test().submit_turn("after compact").await?;
    ...
    antex.shutdown_and_wait().await?;
    let replacement_history = fs::read_to_string(&rollout_path)?
        .lines()
        .filter_map(|line| antex_rollout::parse_rollout_line(line).ok())
...
async fn remote_compact_v2_records_usage_before_output_validation() -> Result<()> {
    ...
    harness.test().submit_turn("before compact").await?;
    antex.submit(Op::Compact).await?;
    wait_for_event(antex, |event| matches!(event, EventMsg::Error(_))).await;
    wait_for_event(antex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    antex.shutdown_and_wait().await?;
...
async fn amazon_bedrock_automatic_compaction_uses_v2_responses_endpoint() -> Result<()> {
    ...
    let harness = TestAntexHarness::with_auto_env_builder(amazon_bedrock_test_antex().with_config(
        |config| {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestAntexHarness::with_builder(
        test_antex()
            .with_history_mode(ThreadHistoryMode::Paginated)
            .with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let antex = harness.test().antex.clone();
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestAntexHarness::with_builder(
        test_antex().with_auth(AntexAuth::AgentIdentity(
            AgentIdentityAuth::from_record(
                ...
                &antex_login::test_support::transport_default_auth_route_config(),
            )
            .await?,
        )),
    )
    ...
    assert_eq!(
        compact_request.header(X_ANTEX_ROUTING_HINT_HEADER),
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let uses_antex_backend = auth.uses_antex_backend();
    ...
    let harness = TestAntexHarness::with_builder(builder).await?;
    let antex = harness.test().antex.clone();
    let image_url =
    ...
    let compact_turn_id = antex.submit(Op::Compact).await?;
    wait_for_turn_complete(&antex).await;
    ...
    assert_eq!(
        normal_request.header(X_ANTEX_ROUTING_HINT_HEADER),
    ...
    let image_inputs = (1..=8)
        .map(|number| {
            ...
            Ok(UserInput::Image {
                ...
                detail: Some(antex_protocol::models::ImageDetail::Original),
            })
        })
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let mut builder = test_antex().with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let new_thread = test
        ...
        .await?;
    test.antex = new_thread.thread;
    test.session_configured = new_thread.session_configured;
    let antex = test.antex.clone();
...
async fn remote_compact_does_not_charge_inline_audio_payload_as_text() -> Result<()> {
    ...
    test.antex
        ...
        .await?;
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.antex, |event| {
    ...
    test.antex
        ...
        .await?;
    wait_for_turn_complete(&test.antex).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&antex, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&antex, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestAntexHarness::with_builder(
        test_antex()
            .with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_antex = baseline_harness.test().antex.clone();
    ...
    baseline_antex
        ...
        .await?;
    wait_for_event(&baseline_antex, |event| {
    ...
    baseline_antex.submit(Op::Compact).await?;
    wait_for_event(&baseline_antex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestAntexHarness::with_builder(
        test_antex()
            .with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_antex = override_harness.test().antex.clone();
    ...
    override_antex
        ...
        .await?;
    wait_for_event(&override_antex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_antex.submit(Op::Compact).await?;
    wait_for_event(&override_antex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    let harness = TestAntexHarness::with_builder(
        test_antex().with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let antex = harness.test().antex.clone();
    ...
    {
        let event = antex.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    antex
        ...
        .await?;
    wait_for_event(&antex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    antex.submit(Op::Compact).await?;
    wait_for_event(&antex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    antex.submit(Op::Shutdown).await?;
    wait_for_event(&antex, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    ...
    for line in rollout_text
        ...
        .filter(|l| !l.is_empty())
    {
        let Ok(entry) = antex_rollout::parse_rollout_line(line) else {
            continue;
        };
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    let mut start_builder = test_antex()
        .with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    initial.antex.submit(Op::Compact).await?;
    wait_for_event(&initial.antex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    initial.antex.submit(Op::Shutdown).await?;
    wait_for_event(&initial.antex, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    .await;

    let mut resume_builder = test_antex()
        .with_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.antex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.antex
        ...
        .await?;
    wait_for_event(&test.antex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_antex_builder(&realtime_server).with_config(|config| {
        config.model_auto_compact_token_limit = Some(200);
    });
    let test = builder.build(&server).await?;
    ...
    start_realtime_conversation(test.antex.as_ref()).await?;
    ...
    close_realtime_conversation(test.antex.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_antex_builder(&realtime_server).with_config({
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    let mut initial_builder = remote_realtime_test_antex_builder(&initial_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(initial.antex.as_ref()).await?;
    initial.submit_turn("USER_ONE").await?;
    close_realtime_conversation(initial.antex.as_ref()).await?;
    initial.antex.submit(Op::Shutdown).await?;
    wait_for_event(&initial.antex, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    ...
    let mut resume_builder = remote_realtime_test_antex_builder(&resumed_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(resumed.antex.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.antex.as_ref()).await?;
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_antex_builder(&realtime_server);
    let test = builder.build(&server).await?;
