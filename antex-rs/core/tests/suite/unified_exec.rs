...
use antex_protocol::config_types::ShellEnvironmentPolicyInherit;
...
use antex_protocol::protocol::ExecCommandStatus;
use antex_protocol::protocol::Op;
use antex_protocol::protocol::ThreadSettingsOverrides;
use antex_protocol::shell_environment::ANTEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use antex_protocol::user_input::UserInput;
use antex_utils_output_truncation::approx_tokens_from_byte_count;
...
async fn wait_for_raw_unified_exec_output(
    ...
    let content = wait_for_event_match(&test.antex, |event| match event {
...
async fn exec_command_hides_and_rejects_login_when_disabled() -> Result<()> {
    ...
    let builder = test_antex().with_model("gpt-5.4").with_config(|config| {
...
async fn exec_command_hides_and_rejects_tty_when_disabled() -> Result<()> {
    ...
    let builder = test_antex().with_model("gpt-5.4").with_cloud_config_bundle(
        CloudConfigBundleFixture::loader_with_enterprise_requirement(
            "[features]\nunified_exec_tty = false\n",
        ),
    );
    let harness = TestAntexHarness::with_auto_env_builder(builder).await?;
...
async fn exec_command_does_not_expose_configured_noise_auth_token() -> Result<()> {
    ...
    let builder = test_antex().with_model("gpt-5.4").with_config(|config| {
        config.permissions.shell_environment_policy.r#set.insert(
            ANTEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(),
            "configured-noise-token".to_string(),
        );
        config.permissions.shell_environment_policy.r#set.insert(
            ANTEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_ascii_lowercase(),
    ...
    let command = match core_test_support::test_target_os() {
        core_test_support::TestTargetOs::Linux | core_test_support::TestTargetOs::MacOs => {
            "if [ -n \"${ANTEX_EXEC_SERVER_NOISE_AUTH_TOKEN:-}\" ] || [ -n \"${codex_exec_server_noise_auth_token:-}\" ]; then echo leaked; else echo unset; fi"
        }
        core_test_support::TestTargetOs::Windows => {
            "if ($env:ANTEX_EXEC_SERVER_NOISE_AUTH_TOKEN) { Write-Output leaked } else { Write-Output unset }"
        }
    };
...
async fn exec_command_uses_installed_environment_shell_policy_with_explicit_overrides() -> Result<()>
{
    ...
    let command = match core_test_support::test_target_os() {
        core_test_support::TestTargetOs::Linux | core_test_support::TestTargetOs::MacOs => {
            r#"printf '%s:%s:%s:%s' "$KEEP" "${DROP:-missing}" "${OWNER_ONLY:-missing}" "$ANTEX_VERSION""#
        }
        core_test_support::TestTargetOs::Windows => {
            r#"if (Test-Path Env:DROP) { $drop = $env:DROP } else { $drop = 'missing' }; if (Test-Path Env:OWNER_ONLY) { $owner = $env:OWNER_ONLY } else { $owner = 'missing' }; Write-Output "${env:KEEP}:${drop}:${owner}:${env:ANTEX_VERSION}""#
        }
    };
    let arguments = json!({
        "cmd": command,
...
async fn unified_exec_intercepts_apply_patch_exec_command() -> Result<()> {
    ...
    wait_for_event(&antex, |event| match event {
...
async fn unified_exec_rejects_justification_without_sandbox_permissions() -> Result<()> {
    ...
    let mut builder = test_antex().with_model("gpt-5.2");
    ...
    let mut saw_exec_begin = false;
    wait_for_event(&test.antex, |event| match event {
        EventMsg::ExecCommandBegin(event) if event.call_id == call_id => {
            saw_exec_begin = true;
            false
        }
...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&test.antex, |msg| match msg {
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    let end_event = wait_for_event_match(&test.antex, |msg| match msg {
...
async fn unified_exec_emits_output_delta_for_exec_command() -> Result<()> {
    ...
    while !turn_completed || end_event.is_none() {
        match wait_for_event(&test.antex, |_| true).await {
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&test.antex, |_| true).await;
        match msg {
...
) -> Result<(TestAntex, PermissionProfile)> {
...
async fn wait_for_unified_exec_end(
...
) -> (antex_protocol::protocol::ExecCommandEndEvent, bool) {
    ...
    let end_event = loop {
        ...
        let event = tokio::time::timeout(remaining, test.antex.next_event())
            .await
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    let mut task_completed = false;
    loop {
        let event_msg = wait_for_event(&test.antex, |_| true).await;
        match event_msg {
...
async fn exec_command_clamps_model_requested_max_output_tokens_to_policy() -> Result<()> {
    ...
    let mut builder = test_antex().with_model("gpt-5.4").with_config(|config| {
...
async fn write_stdin_returns_exit_metadata_and_clears_session() -> Result<()> {
    ...
    let mut turn_completed = false;
    loop {
        let event = wait_for_event(&test.antex, |_| true).await;
        match event {
...
async fn unified_exec_keeps_long_running_session_after_turn_end() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&antex, |msg| match msg {
...
async fn unified_exec_interrupt_preserves_long_running_session() -> Result<()> {
    ...
    let _begin_event = wait_for_event_match(&antex, |msg| match msg {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    ...
    antex.submit(Op::Interrupt).await?;
    wait_for_event(&antex, |event| matches!(event, EventMsg::TurnAborted(_))).await;
    ...
    antex.submit(Op::CleanBackgroundTerminals).await?;
    wait_for_process_exit(&pid).await?;
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    loop {
        let event = test.antex.next_event().await.expect("event");
...
async fn managed_unified_exec_disable_runs_commands_without_retained_authority() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_antex().with_cloud_config_bundle(
        CloudConfigBundleFixture::loader_with_enterprise_requirement(
            r#"
[features]
unified_exec = false
...
"#
    ...
    let end_event = wait_for_event_match(&test.antex, |event| match event {
