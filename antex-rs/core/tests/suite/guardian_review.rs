...
use antex_core::SleepFuture;
use antex_core::TimeFuture;
...
use antex_extension_api::ToolLifecycleFuture;
use antex_extension_api::ToolStartInput;
...
use antex_protocol::openai_models::AutoReviewMessages;
...
#[test_case(AntexAuth::from_api_key("test-api-key"), "OpenAI", "/v1", true, "/v1/responses", true; "api_key_uses_responses")]
#[test_case(AntexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/codex", false, "/backend-api/codex/responses", true; "chatgpt_uses_responses_by_default")]
#[test_case(AntexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/codex", true, "/backend-api/codex/guardian", true; "chatgpt_uses_guardian_when_enabled")]
#[test_case(AntexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/v1", true, "/v1/responses", true; "custom_openai_url_uses_responses")]
#[test_case(AntexAuth::create_dummy_chatgpt_auth_for_testing(), "Custom", "/backend-api/codex", true, "/backend-api/codex/responses", true; "custom_provider_uses_responses")]
#[test_case(AntexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/codex", true, "/backend-api/codex/guardian", false; "server_without_tickets")]
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[test_case(AntexAuth::from_api_key("test-api-key"), true, "gpt-5.6-luna"; "api_key_uses_luna_with_responses_lite")]
#[test_case(AntexAuth::create_dummy_chatgpt_auth_for_testing(), true, "codex-auto-review"; "chatgpt_uses_codex_auto_review")]
#[test_case(AntexAuth::create_dummy_chatgpt_auth_for_testing(), false, "codex-auto-review"; "chatgpt_without_free_guardian")]
async fn guardian_session_prewarms_and_is_reused_for_first_review(
    ...
    let tickets_enabled = free_guardian && uses_antex_backend;
    let bundled_models = antex_models_manager::bundled_models_response()?.models;
...
async fn guardian_node_repl_policy_follows_production_approval_path(
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.antex, repl_server).await?;
...
async fn guardian_session_is_reused_for_consecutive_tool_reviews_without_prewarm() -> Result<()> {
    ...
    let mut builder = test_antex()
        ...
        .with_workspace_setup(|cwd, fs| async move {
            fs.write_file(
                &antex_utils_path_uri::PathUri::from_abs_path(&cwd.join("guardian-secret.txt")),
...
async fn guardian_denial_rejects_tool_call_with_rationale(
    ...
    let feedback = antex_feedback::guardian_review_failures(&[test.session_configured.thread_id])
        .attachment
...
async fn guardian_timeout_rejects_tool_call_with_acting_model_instructions(
    ...
    impl antex_extension_api::ApprovalReviewContributor for TimedOutReviewContributor {
        fn fast_decision<'a>(
            &'a self,
...
async fn cyber_model_guardian_denial_interrupts_turn_immediately() -> Result<()> {
    ...
    let warning = wait_for_event(&test.antex, |event| {
        matches!(
            event,
    ...
    let aborted = wait_for_event(&test.antex, |event| {
        matches!(event, EventMsg::TurnAborted(_))
    })
    .await;
