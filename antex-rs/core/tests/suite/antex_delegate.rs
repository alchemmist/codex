...
async fn antex_delegate_ignores_legacy_deltas() {
    skip_if_no_network!();
    ...
    loop {
        let ev = wait_for_event(&test.antex, |_| true).await;
        match ev {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn antex_delegate_rejects_escalation_requests_when_parent_can_prompt() {
    skip_if_no_network!();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn antex_delegate_rejects_legacy_mcp_approvals_without_prompting() {
    skip_if_no_network!();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn antex_delegate_rejects_skill_mcp_dependency_installation_without_prompting() {
