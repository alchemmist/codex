...
use antex_history::GuardianHistoryCheckpoint;
use antex_history::InitialHistory;
use antex_history::ResumedHistory;
use antex_history::RetainedContext;
use antex_history::RetainedContextEvent;
use antex_history::RolloutItem;
use antex_history::VerifiedAnswer;
use antex_history::VerifiedQuestionAnswer;
...
async fn load_context(test: &TestAntex, thread: &AntexThread) -> Result<Vec<RolloutItem>> {
    Ok(test
        .thread_store
...
async fn resume(test: &TestAntex, thread: &AntexThread) -> Result<Arc<AntexThread>> {
...
async fn retained_instructions_keep_identity_across_compaction_and_resume(
    ...
    let thread = resume(&test, &test.antex).await?;
    ...
    let user_messages = [initial.as_str(), STEER]
        ...
        .map(|(index, text)| {
            ...
            json!({
                ...
                "text": antex_guardian_context::truncate_text(text, /*max_tokens*/ 900),
...
async fn legacy_checkpoint_recovers_root_excerpt_before_discarding_backup(
    ...
    .await;
    let snapshot = test.antex.conversation_history_snapshot().await;
    let source = snapshot
        .items()
        .find(|item| matches!(item, ResponseItem::Message { role, .. } if role == "user"))
        .cloned()
        .context("original instruction")?;
    let mut expected = compact_and_assert_answers(&test, &test.antex, &[]).await?;
    let mut checkpoint = load_context(&test, &test.antex)
        .await?
        .into_iter()
        .rev()
        .find_map(|item| match item {
    ...
    test.antex
        ...
        .await?;
    let resumed = resume(&test, &test.antex).await?;
    assert_eq!(
        resumed
            .conversation_history_snapshot()
            .await
...
async fn retained_answers_rollback_only_the_steered_instruction() -> Result<()> {
    ...
    test.antex
        ...
        .await?;
    let mut thread = resume(&test, &test.antex).await?;
...
async fn forked_parent_instructions_do_not_become_local_authorization(
    ...
    assert!(
        root_before
            .messages
            .contains(&antex_core::GuardianRootMessage::User(
    ...
    assert_eq!(
        expected.as_ref().map(|context| context
            ...
            .map(|entry| match entry {
                antex_history::RetainedContextEntry::UserMessage(message) => message.text.as_str(),
                antex_history::RetainedContextEntry::VerifiedAnswer(_) =>
