...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.antex);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.antex, "after resume").await?;
    shutdown_thread(&resumed.antex).await?;
...
async fn submit_user_turn(antex: &Arc<AntexThread>, text: &str) -> Result<()> {
    antex
...
async fn submit_compact_turn(antex: &Arc<AntexThread>) -> Result<()> {
    antex.submit(Op::Compact).await?;
    let warning_event = wait_for_event(antex, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(antex: &Arc<AntexThread>) -> Result<()> {
