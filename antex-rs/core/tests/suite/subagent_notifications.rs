...
use antex_core::config::AgentRoleConfig;
...
use antex_protocol::openai_models::MultiAgentMessages;
...
async fn wait_for_spawned_thread_id(test: &TestAntex) -> Result<String> {
    let deadline = Instant::now() + Duration::from_secs(2);
    loop {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_antex::TestAntexBuilder,
    ) -> core_test_support::test_antex::TestAntexBuilder,
) -> Result<(
    TestAntex,
    String,
    ...
    let mut builder = configure_test(test_antex().with_config(move |config| {
        config
            .features
...
async fn subagent_stop_replaces_stop_and_skips_internal_subagents() -> Result<()> {
    ...
    let stop_inputs = read_hook_log(test.antex_home_path(), "stop_hook_log.jsonl")?;
    assert!(
        stop_inputs
            .iter()
    ...
    let subagent_stop_inputs_after_internal =
        read_hook_log(test.antex_home_path(), "subagent_stop_hook_log.jsonl")?;
    assert_eq!(subagent_stop_inputs_after_internal, subagent_stop_inputs);

    let stop_inputs_after_internal = read_hook_log(test.antex_home_path(), "stop_hook_log.jsonl")?;
...
async fn spawned_full_history_v2_child_uses_model_precedence_without_dropping_context(
        ...
        if !matches!(
            ...
            assert_eq!(
                child_developer_messages
                    .iter()
                    .filter(|message| {
                        message.contains(&format!("{expected_model} subagent role."))
                    })
                    .count(),
                1
            );
    ...
    if matches!(selection, FullHistoryV2ModelSelection::WorldStateIdentity) {
        ...
        let child_rollout = antex_rollout::RolloutRecorder::get_rollout_history(
            &child_thread
                .rollout_path()
...
async fn multi_agent_v2_spawn_sends_agent_message_to_child(
    ...
    let mut builder = test_antex().with_model(parent_model).with_config(|config| {
        config
            .features
            .enable(Feature::Collab)
            .expect("test config should allow feature update");
...
async fn plaintext_multi_agent_v2_completion_sends_agent_message(
    ...
    if let Some((parent_turn_id, child_turn_id, child_thread_id)) = expected_completed_activity {
        ...
        let rollout = antex_rollout::RolloutRecorder::get_rollout_history(
            &test.antex.rollout_path().expect("parent rollout path"),
        )
        .await?;
        assert!(
