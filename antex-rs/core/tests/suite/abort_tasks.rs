...
use antex_core::SuspendTurnOutcome;
...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    let fixture = test_antex()
        ...
        .await
        .unwrap();
    let antex = Arc::clone(&fixture.antex);
    ...
    wait_for_event(&antex, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    antex.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&antex, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
    antex.submit(Op::CleanBackgroundTerminals).await.unwrap();
}
...
async fn root_turn_suspension_preserves_unfinished_turn_history() {
    ...
    let antex_core::TurnInputSubmission::Started { turn_id } = submitted else {
    ...
    let items = rollout
        .lines()
        .map(|line| {
            antex_rollout::parse_rollout_line(line)
    ...
    assert_eq!(
        resumed
            .antex
            .recover_turn_if_idle(antex_core::RecoverTurnRequest {
                turn_id: turn_id.clone(),
        ...
        antex_core::StartIfIdleSubmission::Started {
            turn_id: turn_id.clone(),
        },
    );
    let completed = wait_for_event(&resumed.antex, |event| {
