...
use antex_utils_image::data_url_from_bytes;
use anyhow::Context;
use base64::Engine;
...
fn find_user_message_with_image(text: &str) -> Option<ResponseItem> {
    for line in text.lines() {
        ...
        let rollout = match antex_rollout::parse_rollout_line(trimmed) {
            Ok(rollout) => rollout,
            Err(_) => continue,
        };
    ...
    let expected = ResponseItem::Message {
        ...
        content: vec![
            ContentItem::InputText {
                text: antex_protocol::models::local_image_open_tag_text_with_path(
            ...
            ContentItem::InputText {
                text: antex_protocol::models::image_close_tag_text(),
            },
            ContentItem::InputText {
                text: "pasted image".to_string(),
            },
...
async fn resumed_history_only_emits_resize_notices_for_new_images() -> anyhow::Result<()> {
    ...
    let server = start_mock_server().await;
    let initial = test_antex().build_with_auto_env(&server).await?;
    ...
    let persisted_developer_metadata = fs::read_to_string(&rollout_path)?
        ...
        .filter_map(|line| {
            let RolloutItem::ResponseItem(envelope) = antex_rollout::parse_rollout_line(line)
