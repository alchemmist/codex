...
use antex_core::config::CurrentTimeReminderConfig;
use antex_extension_items::ExtensionItem;
use antex_extension_items::sleep::SleepItem;
...
async fn idle_response_items_include_pending_mailbox_in_first_request() -> anyhow::Result<()> {
    ...
    .await;
    let test = test_antex().build_with_auto_env(&server).await?;

    submit_queue_only_agent_mail(test.antex.as_ref(), "pending mailbox input").await;
    let submission = test
        .antex
    ...
    wait_for_turn_complete(test.antex.as_ref()).await;
...
async fn assert_idle_user_input_reaches_the_first_model_request(
    ...
    let user_message = core_test_support::wait_for_event_match(test.antex.as_ref(), |event| {
...
async fn build_antex(server: &StreamingSseServer) -> Arc<AntexThread> {
    test_antex()
...
async fn submit_user_input(antex: &AntexThread, text: &str) {
    antex
...
async fn submit_danger_full_access_user_turn(test: &TestAntex, text: &str) {
...
async fn steer_user_input(antex: &AntexThread, text: &str) {
...
async fn enqueue_queue_only_agent_mail(antex: &AntexThread, text: &str) {
    antex
...
async fn submit_queue_only_agent_mail(antex: &AntexThread, text: &str) {
    enqueue_queue_only_agent_mail(antex, text).await;
    antex
        .submit(Op::RealtimeConversationListVoices)
        .await
        .expect("submit list-voices barrier");
    wait_for_event(antex, |event| {
        matches!(event, EventMsg::RealtimeConversationListVoicesResponse(_))
    })
    .await;
}

async fn wait_for_reasoning_item_started(antex: &AntexThread) {
    wait_for_event(antex, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(antex: &AntexThread, text: &str) {
...
async fn wait_for_turn_complete(antex: &AntexThread) {
    wait_for_event(antex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(antex: &AntexThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(antex, |event| {
        matches!(
            event,
...
async fn wait_for_sleep_item_completed(antex: &AntexThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(antex, |event| {
        matches!(
            event,
...
impl antex_extension_api::ThreadLifecycleContributor<antex_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: antex_extension_api::ThreadStartInput<'a, antex_core::config::Config>,
...
async fn queue_only_agent_mail_wakes_sleeping_root_with_previous_turn_context() {
    ...
    .await;
    let mut extensions =
        antex_extension_api::ExtensionRegistryBuilder::<antex_core::config::Config>::new();
    ...
    enqueue_queue_only_agent_mail(&antex, CHILD_MESSAGE).await;
    wait_for_turn_complete(&antex).await;
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            ...
            RolloutItem::ResponseItem(envelope)
                if matches!(
                    &envelope.item,
                    antex_protocol::models::ResponseItem::AgentMessage { content, .. }
                        if content.iter().any(|content| matches!(
                            content,
                            antex_protocol::models::AgentMessageInputContent::InputText { text }
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    submit_user_input(&antex, INITIAL_PROMPT).await;
    wait_for_event(&antex, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&antex, STEER_PROMPT).await;
    wait_for_turn_complete(&antex).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&antex, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&antex, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&antex, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&antex, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&antex, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&antex, "new mailbox input").await;
    wait_for_sleep_item_completed(&antex, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&antex).await;
    ...
    antex.submit(Op::Shutdown).await.expect("shutdown session");
    wait_for_event(&antex, |event| matches!(event, EventMsg::ShutdownComplete)).await;

    let rollout_path = antex.rollout_path().expect("rollout path");
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let antex = build_antex(&server).await;

    submit_user_input(&antex, "first prompt").await;

    wait_for_reasoning_item_started(&antex).await;

    submit_queue_only_agent_mail(&antex, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&antex, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&antex, "second prompt").await;
    wait_for_turn_complete(&antex).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&antex, "second prompt").await;
...
async fn terminal_compaction_error_does_not_retry_pending_input(
    ...
    let test = test_antex()
        ...
        .await?;
    let antex = &test.antex;

    if failure_point == CompactionFailurePoint::PreTurn {
        submit_user_input(antex, "initial prompt").await;
        wait_for_turn_complete(antex).await;
    }
    if pending_input == PendingInputAfterFailure::QueuedMail {
        submit_queue_only_agent_mail(antex, PENDING_MESSAGE).await;
    }
    submit_user_input(antex, "prompt that needs compaction").await;
    tokio::time::timeout(
    ...
    .await?;
    match pending_input {
        PendingInputAfterFailure::Steer => steer_user_input(antex, PENDING_MESSAGE).await,
        ...
        PendingInputAfterFailure::TriggeringMail => {
            antex
                ...
                .await?;
            antex.submit(Op::RealtimeConversationListVoices).await?;
            wait_for_event(antex, |event| {
                matches!(event, EventMsg::RealtimeConversationListVoicesResponse(_))
            })
            .await;
    ...
    antex.flush_rollout().await?;
    let history = antex.load_history(/*include_archived*/ false).await?;
    ...
    submit_user_input(antex, "retry after quota resets").await;
    wait_for_agent_message(antex, "recovered answer").await;
    let completed = wait_for_event(antex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&antex, "resumed old task").await;
    wait_for_turn_complete(&antex).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&antex, "processed steered prompt").await;
    wait_for_turn_complete(&antex).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&antex, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&antex, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
