...
async fn mcp_tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    let mut builder = test_antex().with_config(move |config| {
        ...
        servers.insert(
            ...
            antex_config::types::McpServerConfig {
                auth: Default::default(),
                transport: antex_config::types::McpServerTransportConfig::Stdio {
...
async fn mcp_tool_output_limit_preserves_output_that_fits(
    ...
    let server = start_mock_server().await;
    let builder = test_antex().with_config(|config| config.tool_output_token_limit = Some(50));
    let (_fixture, output) =
...
async fn mcp_tool_output_limit_survives_resume(output_token_limit: Option<usize>) -> Result<()> {
    ...
    let server = start_mock_server().await;
    let builder = test_antex().with_config(|config| config.tool_output_token_limit = Some(50_000));
    ...
    fixture.antex.ensure_rollout_materialized().await;
    fixture.antex.flush_rollout().await?;
