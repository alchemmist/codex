...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db = test.antex.state_db().expect("state db enabled");
    assert!(
        !rollout_path.exists(),
...
async fn mcp_call_marks_thread_memory_mode_polluted_when_configured() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    wait_for_mcp_server(&test.antex, server_name).await?;
    let db = test.antex.state_db().expect("state db enabled");
    let thread_id = test.session_configured.thread_id;
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = antex_state::log_db::start(db.clone());
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = antex_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
