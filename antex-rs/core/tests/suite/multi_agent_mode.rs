...
use antex_protocol::openai_models::MultiAgentModeMessages;
...
async fn ultra_reasoning_uses_highest_non_ultra_and_proactive_mode() -> Result<()> {
    ...
    submit_turn(&test.antex, "hello", /*effort*/ None).await?;
...
async fn mode_hints_override_reasoning_effort(source: ModeHintSource) -> Result<()> {
    ...
    let test = test_antex()
        ...
        .build(&server)
        .await?;
    submit_turn(&test.antex, "explicit", Some(ReasoningEffort::High)).await?;
    submit_turn(&test.antex, "proactive", Some(ReasoningEffort::Ultra)).await?;
...
async fn catalog_proactive_mode_is_ultra_only(
    ...
    submit_turn(&test.antex, "hello", Some(effort)).await?;
...
async fn model_switch_refreshes_catalog_role_and_mode(
    ...
    let test = test_antex()
        ...
        .build_with_auto_env(&server)
        .await?;
    submit_turn(&test.antex, "first model", Some(effort.clone())).await?;
    ...
    .await?;
    submit_turn(&test.antex, "second model", Some(effort)).await?;
...
async fn changing_configured_mode_hint_to_empty_emits_no_update() -> Result<()> {
    ...
    .await;
    let initial = test_antex()
        .with_config(configure_custom_mode_hint)
        .build(&server)
        .await?;

    submit_turn(&initial.antex, "before resume", /*effort*/ None).await?;

    let mut resume_builder = test_antex().with_config(|config| {
        configure_multi_agent_v2(config);
        config.multi_agent_v2.multi_agent_mode_hint_text = Some(String::new());
    });
    let resumed = resume_builder.restart(&server, &initial).await?;
    drop(initial);
    submit_turn(&resumed.antex, "after resume", /*effort*/ None).await?;
...
async fn live_mode_change_appends_mode_without_reappending_usage_hint() -> Result<()> {
    ...
    submit_turn(&test.antex, "proactive", /*effort*/ None).await?;
    submit_turn(&test.antex, "explicit", Some(ReasoningEffort::High)).await?;
...
async fn leaving_ultra_after_cold_resume_emits_explicit_mode() -> Result<()> {
    ...
    let resumed = resume_builder.restart(&server, &initial).await?;
    drop(initial);
    submit_turn(&resumed.antex, "after resume", Some(ReasoningEffort::High)).await?;
