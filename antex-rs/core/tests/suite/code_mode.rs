...
use antex_tools::FreeformTool;
use antex_tools::FreeformToolFormat;
use antex_tools::FunctionCallError;
use antex_tools::JsonToolOutput;
use antex_tools::ResponsesApiNamespace;
use antex_tools::ResponsesApiNamespaceTool;
use antex_tools::ToolCall;
use antex_tools::ToolExecutor;
use antex_tools::ToolExecutorFuture;
use antex_tools::ToolName;
use antex_tools::ToolOutput;
use antex_tools::ToolPayload;
use antex_tools::ToolSpec;
use antex_web_search_extension::install as install_web_search_extension;
use anyhow::Result;
use base64::Engine;
...
async fn run_code_mode_turn_with_model_and_config(
    ...
    let builder = test_antex().with_model(model).with_config(move |config| {
...
"#,
    ...
    let auth = AntexAuth::from_api_key("dummy");
    let auth_manager = antex_core::test_support::auth_manager_from_auth(auth.clone());
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let mut builder = test_antex().with_model(model).with_config(move |config| {
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.antex, "rmcp").await?;
...
async fn code_mode_exec_holds_captured_result_during_elicitation() -> Result<()> {
    ...
    assert_eq!(
        test.antex.increment_out_of_band_elicitation_count().await?,
        1
    );
    assert_eq!(
        test.antex.increment_out_of_band_elicitation_count().await?,
    ...
    let release_elicitation = async {
        ...
        assert_eq!(
            test.antex.decrement_out_of_band_elicitation_count().await?,
            1
        );
        ...
        Box::pin(async move {
            if matches!(input.tool_name.name.as_str(), "exec_command" | "wait") {
                self.tickets.lock().unwrap().push((
                    ...
                    input
                        .turn_store
                        .get::<antex_protocol::guardian_ticket::GuardianTicket>()
...
impl ToolLifecycleContributor for InterruptedNestedToolObserver {
    fn on_tool_start<'a>(&'a self, input: ToolStartInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            let antex_extension_api::ToolCallSource::CodeMode { cell_id, .. } = input.source else {
                return;
            };
    ...
    fn on_tool_finish<'a>(&'a self, input: ToolFinishInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            if input.tool_name.name != "test_sync_tool"
                || !matches!(
                    input.source,
                    antex_extension_api::ToolCallSource::CodeMode { .. }
                )
            {
                return;
            }
...
async fn code_mode_node_repl_screenshots_can_be_captured_without_guardian_transcript_flags(
    ...
    let test = builder.build_with_auto_env(&server).await?;
    core_test_support::wait_for_mcp_server(&test.antex, repl_server).await?;
    let evidence = test
        .antex
            ...
            if reviewer_constraint.is_some_and(|value| value != "large_prompt") || check_detail {
                ...
                if reviewer_token_budget {
                    ...
                    reviewer.input_modalities =
                        vec![antex_protocol::openai_models::InputModality::Text];
                } else {
...
"#,
        ...
        test.antex
            .start_or_steer_turn(
                ...
                .with_thread_settings(ThreadSettingsOverrides {
                    environments: Some(antex_protocol::protocol::TurnEnvironmentSelections::new(
                        cwd,
                        Vec::new(),
                    )),
        ...
        let turn_id = wait_for_event_match(&test.antex, |event| match event {
