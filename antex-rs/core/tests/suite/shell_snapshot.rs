...
const SNAPSHOT_MARKER_VAR: &str = "ANTEX_SNAPSHOT_POLICY_MARKER";
...
async fn run_snapshot_command_with_options(
    ...
    let builder = test_antex().with_config(move |config| {
        config
            .features
    ...
    let begin = wait_for_event_match(&antex, |ev| match ev {
    ...
    let end = wait_for_event_match(&antex, |ev| match ev {
...
async fn run_tool_turn_on_harness(
    ...
    wait_for_event_match(&antex, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
#[cfg(unix)]
fn shell_snapshot_v2_prewarm_builder(profile_home: &Path) -> TestAntexBuilder {
...
#[cfg(unix)]
async fn run_no_shell_turn(harness: &TestAntexHarness) -> Result<()> {
...
async fn shell_snapshot_v2_recovers_after_failed_prewarm() -> Result<()> {
    ...
    harness.test().antex.shutdown_and_wait().await?;
    Ok(())
}
...
async fn shell_snapshot_v2_prewarm_stops_on_shutdown() -> Result<()> {
    ...
    let harness = TestAntexHarness::with_auto_env_builder(shell_snapshot_v2_prewarm_builder(
        profile_home.path(),
    ))
    .await?;
    run_no_shell_turn(&harness).await?;
...
async fn shell_snapshot_v2_prewarm_skips_ineligible_sessions(
    configure: fn(&mut antex_core::config::Config),
) -> Result<()> {
    skip_if_remote!(Ok(()), "profile fixture uses a host-local HOME directory");
    let profile_home = tempfile::tempdir()?;
    fs::write(
...
async fn shell_snapshot_v2_guardian_uses_its_resolved_permissions_and_tools(
    ...
    fs::write(
        profile_home.path().join(".bashrc"),
        "printf capture > \"$HOME/$ANTEX_THREAD_ID\"\n",
    )
    .await?;
    let builder =
        shell_snapshot_v2_prewarm_builder(profile_home.path()).with_config(move |config| {
...
async fn shell_snapshot_v2_preserves_legacy_snapshots_for_user_shell() -> Result<()> {
    ...
    let builder = test_antex().with_config(|config| {
        config
            .features
...
async fn unified_exec_snapshot_still_intercepts_apply_patch() -> Result<()> {
    ...
    wait_for_event(&antex, |ev| match ev {
