...
use antex_protocol::openai_models::CollaborationModeMessages;
use antex_protocol::openai_models::ModelMessages;
use antex_protocol::openai_models::ModelsResponse;
use antex_protocol::protocol::COLLABORATION_MODE_CLOSE_TAG;
...
fn model_with_collaboration_messages(
    ...
    default: Option<&str>,
    plan: Option<&str>,
) -> antex_protocol::openai_models::ModelInfo {
            ...
            .with_thread_settings(ThreadSettingsOverrides {
                ...
                summary: Some(
                    test.config
                        ...
                        .unwrap_or(antex_protocol::config_types::ReasoningSummary::Auto),
                ),
                collaboration_mode: Some(collaboration_mode),
                ..Default::default()
            }),
...
async fn cold_resume_refreshes_legacy_collaboration_snapshot_once(
    ...
    let builder_with_instructions = |instructions: &str| {
        ...
        test_antex().with_model(MODEL).with_config(move |config| {
            config.model_catalog = Some(ModelsResponse {
                models: vec![model],
            });
        })
    ...
    let legacy_rollout = std::fs::read_to_string(&rollout_path)?
        .lines()
        .map(|original_line| {
            let mut line = antex_rollout::parse_rollout_line(original_line)?;
