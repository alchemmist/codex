...
fn configure_multi_agent_v2_with_role(
    config: &mut antex_core::config::Config,
...
async fn cold_root_resume_restores_agent_identity_and_role_on_followup() -> Result<()> {
    ...
    let mut initial_builder = test_antex().with_config(move |config| {
        ...
        if let Some(thread_id) = initial_child_request
            ...
            .find_map(|request| {
                ...
                body["client_metadata"]["thread_id"]
                    .as_str()
                    .and_then(|thread_id| antex_protocol::ThreadId::from_string(thread_id).ok())
            })
        {
            break thread_id;
        }
    ...
    assert_eq!(
        ...
        initial.antex.config().await.model_provider,
...
"#
    ...
    assert_eq!(
        ...
        resumed.antex.config().await.model_provider,
    ...
    let body_for = |text: &str, thread: antex_protocol::ThreadId| {
        requests
            .iter()
            .find_map(|request| {
