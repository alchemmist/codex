...
use core_test_support::test_antex::RecordingUserInstructionsProvider;
use core_test_support::test_antex::TestAntexBuilder;
use core_test_support::test_antex::executor_path_uri;
...
async fn agents_instructions(mut builder: TestAntexBuilder) -> Result<String> {
...
async fn submit_thread_turn(thread: &Arc<antex_core::AntexThread>, prompt: &str) -> Result<()> {
    thread
...
async fn agents_override_is_preferred_over_agents_md() -> Result<()> {
    let instructions =
        agents_instructions(test_antex().with_workspace_setup(|cwd, fs| async move {
...
async fn runtime_trust_reload_refreshes_project_instructions() -> Result<()> {
    ...
    let mut untrusted_config = (*test.antex.config().await).clone();
    untrusted_config.active_project.trust_level = Some(TrustLevel::Untrusted);
    test.antex.refresh_runtime_config(untrusted_config).await;
    ...
    let mut trusted_config = (*test.antex.config().await).clone();
    trusted_config.active_project.trust_level = Some(TrustLevel::Trusted);
    test.antex.refresh_runtime_config(trusted_config).await;
...
-> Result<()> {
    ...
    let error = error
        .replace(&canonical_home_path, "$ANTEX_HOME")
        .replace(&home_path, "$ANTEX_HOME");
    insta::assert_snapshot!(error, @"
    Fatal error: Failed to initialize session: failed to load AGENTS.md instructions for environment `local`: failed to prepare fs sandbox: failed to prepare Seatbelt sandbox: writable root $ANTEX_HOME/visualizations contains symlink component $ANTEX_HOME/visualizations; symlinked writable roots are not supported.
    If this writable root is at or beneath ANTEX_HOME and you trust its symlink targets, set `allow_symlinked_codex_home = true` at the top level of `$ANTEX_HOME/config.toml` (normally `~/.antex/config.toml`) on the execution host, then restart Codex or its executor. This opt-out trusts targets outside ANTEX_HOME and targets changed between commands. It does not apply to other writable roots.
    ");
...
-> Result<()> {
    ...
    .await;
    let mut builder = test_antex().with_workspace_setup(|cwd, fs| async move {
        fs.write_file(
            &executor_path_uri(cwd.join(GLOBAL_AGENTS_FILENAME))?,
            PROJECT_INSTRUCTIONS.as_bytes().to_vec(),
            Default::default(),
    ...
    let EventMsg::Error(error) =
        wait_for_event(&test.antex, |event| matches!(event, EventMsg::Error(_))).await
    else {
        unreachable!();
    };
    assert!(
...
async fn loads_user_instructions_without_a_primary_environment() -> Result<()> {
    ...
    let provider = Arc::new(RecordingUserInstructionsProvider::new(Arc::new(
        AntexHomeUserInstructionsProvider::new(AbsolutePathBuf::try_from(
...
async fn fresh_thread_composes_global_before_project_and_reports_sources() -> Result<()> {
    ...
    assert_eq!(test.antex.instruction_sources().await, creation_sources);
...
async fn invalid_utf8_global_instructions_are_lossy() -> Result<()> {
    ...
    let mut builder = test_antex().with_home(home);
    let test = builder.build(&server).await?;
...
async fn cold_resume_invalidates_deleted_legacy_agents_md_once() -> Result<()> {
    ...
    let mut initial_builder = test_antex().with_home(Arc::clone(&home));
    let initial = initial_builder.build(&server).await?;

    // Assert the pre-resume thread reports the source used to create its snapshot.
    assert_eq!(
        initial.antex.instruction_sources().await,
    ...
    let mut resume_builder = test_antex().with_home(Arc::clone(&home));
...
async fn fork_injects_changed_agents_md_once() -> Result<()> {
    ...
    let mut builder = test_antex().with_home(Arc::clone(&home));
    let parent = builder.build(&server).await?;

    // Assert the parent reports the source used to create its snapshot.
    assert_eq!(
        parent.antex.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
        "parent reports the creation-time global source"
    );
    parent.submit_turn("persist instructions").await?;
    parent.antex.ensure_rollout_materialized().await;
    parent.antex.flush_rollout().await?;
    let rollout_path = parent.antex.rollout_path().expect("rollout path");
    ...
    fork_config.antex_self_exe = parent.config.antex_self_exe.clone();
    fork_config
        .features
