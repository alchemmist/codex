...
fn repo_root() -> std::path::PathBuf {
    antex_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = antex_utils_cargo_bin::cargo_bin("antex").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(repo_root())
        .arg("hello?");
    cmd.env("ANTEX_HOME", home.path())
        .env(ANTEX_ACCESS_TOKEN_ENV_VAR, PERSONAL_ACCESS_TOKEN)
        .env("ANTEX_AUTHAPI_BASE_URL", server.uri())
        .env_remove(ANTEX_API_KEY_ENV_VAR)
...
impl Drop for ChildProcessCleanupGuard {
    fn drop(&mut self) {
        #[cfg(unix)]
        {
            let _ = antex_utils_pty::process_group::kill_process_group(self.0);
        }
...
// Use this for new `antex exec` subprocess tests in this file. These commands
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Antex to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `antex exec --profile ...` preserves the active user config
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    cmd.arg("exec")
        ...
        .arg(&prompt);
    cmd.env("ANTEX_HOME", home.path())
        .env(ANTEX_API_KEY_ENV_VAR, "dummy");
    ...
    let bin2 = antex_utils_cargo_bin::cargo_bin("antex").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("ANTEX_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");

    let output2 = run_cli_command(&mut cmd2).unwrap();
    assert!(output2.status.success(), "resume antex-cli run failed");
