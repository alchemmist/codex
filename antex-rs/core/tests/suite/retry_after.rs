use antex_client::RetryOn;
use antex_client::RetryPolicy;
use antex_client::run_with_retry;
use antex_http_client::Request;
use antex_http_client::TransportError;
    ...
    S: Subscriber,
{
    fn on_event(&self, event: &Event<'_>, _context: Context<'_, S>) {
        if event.metadata().target() == "antex_http_client::transport" {
            self.record_request_after_retry();
            return;
        }

        if event.metadata().target() != "antex_otel.trace_safe" {
            return;
        }
...
async fn submit_user_input(test: &TestAntex, text: &str) -> Result<()> {
    test.antex
...
async fn wait_for_turn_completion(test: &TestAntex) {
    let EventMsg::TurnComplete(completed) = wait_for_event(&test.antex, |event| {
