...
async fn process_sse_emits_tracing_for_output_item() {
    ...
    let TestAntex { antex, .. } = test_antex().build(&server).await.unwrap();
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    wait_for_event(&antex, |ev| matches!(ev, EventMsg::TokenCount(_))).await;
...
async fn handle_exec_command_user_approved_records_tool_decision() {
    ...
    let approval_event =
        wait_for_event(&antex, |ev| matches!(ev, EventMsg::ExecApprovalRequest(_))).await;
