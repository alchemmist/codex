...
use antex_core::ModelClient;
use antex_core::NewThread;
use antex_core::Prompt;
use antex_core::ResponseEvent;
...
use antex_core::resolve_installation_id;
use antex_core::thread_store_from_config;
use antex_extension_api::empty_extension_registry;
use antex_features::Feature;
use antex_history::RolloutItem;
use antex_history::RolloutLine;
use antex_login::AntexAuth;
use antex_login::AuthKeyringBackendKind;
...
use antex_protocol::models::LocalShellAction;
use antex_protocol::models::LocalShellExecAction;
use antex_protocol::models::LocalShellStatus;
use antex_protocol::models::MessagePhase;
use antex_protocol::models::ReasoningItemContent;
use antex_protocol::models::ReasoningItemReasoningSummary;
use antex_protocol::models::ResponseItem;
use antex_protocol::models::WebSearchAction;
...
use core_test_support::TestAntexResponsesRequestKind;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> antex_core::AntexResponsesMetadata {
    ...
    test_responses_metadata(
        ...
        TestAntexResponsesRequestKind::Turn,
    )
}
...
fn assert_antex_client_metadata(
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_antex().build(&server).await.unwrap();
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    wait_for_event(&initial.antex, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    first.antex.submit(Op::Shutdown).await?;
    wait_for_event(&first.antex, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: antex_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = antex_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = antex_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![antex_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = antex_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Antex to resume from our file
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let codex_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_antex().with_model("gpt-5.4");
    let test = builder
        .resume(&server, codex_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_antex().with_auth(AntexAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    assert_eq!(request.header(X_ANTEX_ROUTING_HINT_HEADER), None);
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.antex_home_path().join(INSTALLATION_ID_FILENAME))
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let model = antex_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        antex_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(AntexAuth::from_api_key(
            "unused-api-key",
        ))),
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &antex_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_antex()
        .with_auth(AntexAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut builder = test_antex()
        .with_auth(create_dummy_antex_auth())
        .with_config(move |config| {
    ...
    assert_eq!(
        request.header(X_ANTEX_ROUTING_HINT_HEADER),
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = AntexAuth::from_auth_storage(
        codex_home.path(),
    ...
    let auth_manager = antex_core::test_support::auth_manager_from_auth(auth);
    ...
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        antex_core::build_models_manager(&config, auth_manager),
        antex_core::CodexAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(antex_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(antex_core::test_support::EmptyUserInstructionsProvider),
        /*analytics_events_client*/ None,
        antex_core::passthrough_image_store(),
    ...
    let NewThread { thread: antex, .. } = thread_manager
...
async fn powershell_shell_version_is_model_visible_only_when_enabled() -> anyhow::Result<()> {
    ...
    let shell_path = antex_shell_command::powershell::try_find_powershell_executable_blocking()
    ...
    for enabled in [false, true] {
        ...
        let user_shell = antex_shell_command::shell_detect::DetectedShell {
            shell_type: antex_shell_command::shell_detect::ShellType::PowerShell,
...
-> anyhow::Result<()> {
    ...
    .await;
    let TestAntex { antex, .. } = test_antex().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    .await;
    let TestAntex { antex, config, .. } = test_antex().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_does_not_store_and_preserves_prefixed_item_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        antex_core::test_support::auth_manager_from_auth(AntexAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let mut builder = test_antex()
        .with_auth(AntexAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let antex = builder
    ...
    wait_for_event(&antex, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let mut builder = test_antex();
    let antex_fixture = builder.build(&server).await?;
    let antex = antex_fixture.antex.clone();
    ...
    let token_event = wait_for_event(&antex, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&antex, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&antex, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&antex, |ev| matches!(ev, EventMsg::Error(_))).await;
...
async fn env_var_overrides_loaded_auth() {
    ...
    assert_eq!(request.headers.get(X_ANTEX_ROUTING_HINT_HEADER), None);
}

fn create_dummy_antex_auth() -> AntexAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Antex sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
