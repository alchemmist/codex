...
async fn partial_request_permissions_grants_do_not_preapprove_new_permissions() -> Result<()> {
    ...
    let antex_protocol::models::LegacyReadWriteRoots {
        read: approval_reads,
...
async fn request_permissions_session_grants_carry_across_turns() -> Result<()> {
    ...
    let completion_event = wait_for_event(&test.antex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
        )
...
async fn denied_child_permissions_require_fresh_approval(
    ...
    let harness =
        TestAntexHarness::with_auto_env_builder(test_antex().with_config(move |config| {
    ...
    test.antex.submit(decision).await?;
    wait_for_completion(test).await;
