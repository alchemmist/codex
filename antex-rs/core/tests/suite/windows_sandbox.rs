...
use antex_core::exec::process_exec_tool_call;
...
fn antex_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestAntexHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep ANTEX_HOME stable within
        ...
        std::fs::create_dir_all(&codex_home)
            .with_context(|| format!("create stable test ANTEX_HOME {}", codex_home.display()))?;
        return Ok(TestAntexHome::Persistent(codex_home));
    }

    Ok(TestAntexHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("antex-resources");
    ...
    for helper_name in ["antex-windows-sandbox-setup", "antex-command-runner"] {
        let helper = antex_utils_cargo_bin::cargo_bin(helper_name)?;
...
fn stage_windows_sandbox_cli(fixture_bin: &Path) -> anyhow::Result<(PathBuf, PathBuf)> {
    ...
    let resources_dir = fixture_bin.join("antex-resources");
    std::fs::create_dir_all(&resources_dir)?;

    let antex_source = antex_utils_cargo_bin::cargo_bin("antex")?;
    let antex = fixture_bin.join("antex.exe");
    std::fs::copy(&antex_source, &antex)
        .with_context(|| format!("copy {} to {}", antex_source.display(), antex.display()))?;
    for helper_name in ["antex-windows-sandbox-setup", "antex-command-runner"] {
        let helper = antex_utils_cargo_bin::cargo_bin(helper_name)?;
    ...
    let probe_source = antex_utils_cargo_bin::cargo_bin("antex-windows-managed-deny-probe")?;
...
fn windows_sandbox_cli_preserves_managed_deny_reads_across_launches() -> anyhow::Result<()> {
    let codex_home =
        antex_home_for_windows_sandbox_test("windows-cli-managed-deny-read-codex-home")?;
    ...
    let (antex, probe) = stage_windows_sandbox_cli(&bin)?;
    ...
    for launch in 1..=2 {
        let output = Command::new(&antex)
            .current_dir(&work)
            .env("ANTEX_HOME", codex_home.path())
            .env("ANTEX_WINDOWS_ALLOWED_TEXT", &allowed_text)
            .env("ANTEX_WINDOWS_DENIED_TEXT", &denied_text)
            .env("ANTEX_WINDOWS_ALLOWED_MODULE", &allowed_module)
            .env("ANTEX_WINDOWS_DENIED_MODULE", &denied_module)
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let codex_home =
        antex_home_for_windows_sandbox_test("windows-restricted-token-deny-read-codex-home")?;
    let _antex_home_guard = EnvVarGuard::set("ANTEX_HOME", codex_home.path().as_os_str());
...
async fn windows_elevated_does_not_create_missing_workspace_metadata() -> anyhow::Result<()> {
    let codex_home =
        antex_home_for_windows_sandbox_test("windows-elevated-missing-metadata-codex-home")?;
    let _antex_home_guard = EnvVarGuard::set("ANTEX_HOME", codex_home.path().as_os_str());
    ...
    for name in antex_protocol::permissions::PROTECTED_METADATA_PATH_NAMES {
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let codex_home = antex_home_for_windows_sandbox_test("windows-elevated-deny-read-codex-home")?;
    let _antex_home_guard = EnvVarGuard::set("ANTEX_HOME", codex_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;
    let workspace = TempDir::new()?;
    let cwd = dunce::canonicalize(workspace.path())?.abs();
    ...
    let bundled_skill_dir = user_profile.path().join(".antex/plugins/cache");
...
async fn windows_elevated_unified_exec_enforces_managed_deny_reads() -> anyhow::Result<()> {
    let codex_home =
        antex_home_for_windows_sandbox_test("windows-elevated-tool-runtime-deny-read-codex-home")?;
    let _antex_home_guard = EnvVarGuard::set("ANTEX_HOME", codex_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;

    let configured_antex_home = dunce::canonicalize(codex_home.path())?.abs();
    let builder = test_antex()
        .with_windows_cmd_shell()
        .with_config(move |config| {
            config.codex_home = configured_antex_home;
