...
async fn submit_user_turn(
    test: &core_test_support::test_antex::TestAntex,
    prompt: &str,
...
"#;
    ...
    let server = start_mock_server().await;
    let mut migrated_builder = test_antex().with_config(|config| {
    ...
    let migrated_policy_path = migrated.antex_home_path().join("rules/default.rules");
    assert_eq!(fs::read_to_string(&migrated_policy_path)?, MIGRATED_POLICY);
    assert_eq!(
        fs::read_to_string(migrated.antex_home_path().join(MIGRATION_MARKER_FILENAME))?,
        "v1\n"
    );

    let mut ignored_builder = test_antex().with_config(|config| {
        let policy_path = config.codex_home.join("rules/default.rules");
        fs::create_dir_all(policy_path.parent().expect("rules directory"))
            .expect("create rules directory");
        fs::write(policy_path, LEGACY_POLICY).expect("write legacy policy");
    ...
    let ignored_policy_path = ignored.antex_home_path().join("rules/default.rules");
...
async fn granular_complex_forced_rm_requests_approval_when_allowed() -> Result<()> {
    ...
    let approval_event = wait_for_event(&test.antex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
        )
