...
use antex_image_generation_extension::install as install_image_generation_extension;
...
fn responses_extensions(auth: &AntexAuth) -> Arc<ExtensionRegistry<Config>> {
...
fn configure_image_capable_model(model_info: &mut antex_protocol::openai_models::ModelInfo) {
...
async fn responses_lite_prepares_images() -> Result<()> {
    ...
    let mut builder = test_antex().with_model_info_override("gpt-5.4", |model_info| {
        model_info.use_responses_lite = true;
