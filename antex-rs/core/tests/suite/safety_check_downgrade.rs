...
fn disabled_text_turn(test: &TestAntex, text: &str) -> TurnInputRequest {
...
async fn openai_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let mut builder = test_antex().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.antex, |event| {
    ...
    let warning = wait_for_event(&test.antex, |event| matches!(event, EventMsg::Warning(_))).await;
    ...
    let _ = wait_for_event(&test.antex, |event| {
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.antex, |event| matches!(event, EventMsg::Error(_))).await;
