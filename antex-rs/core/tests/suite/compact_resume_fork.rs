...
//! request payload that Antex would send to the model and assert that the
...
fn seed_first_checkpoint_harness_metadata(path: &Path, retained_text: &str) -> Result<()> {
    ...
    for envelope in replacement_history {
        envelope.metadata = Some(AntexHarnessMetadata::default());
    }
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<AntexThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.antex)
}

async fn user_turn(conversation: &Arc<AntexThread>, text: &str) {
    conversation
...
async fn compact_conversation(conversation: &Arc<AntexThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<AntexThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<AntexThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = antex_core::test_support::auth_manager_from_auth(
