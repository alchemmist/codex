...
use antex_core::EnvironmentNetworkPolicy;
use antex_core::StartThreadOptions;
use antex_core::TurnInputRequest;
use antex_core::WaitForEnvironmentToolConfig;
use antex_core::compact::SUMMARIZATION_PROMPT;
use antex_core::config::Config;
use antex_core::config::Constrained;
use antex_core::windows_sandbox::WindowsSandboxLevelExt;
use antex_exec_server::CopyOptions;
use antex_exec_server::CreateDirectoryOptions;
use antex_exec_server::EnvironmentReadyInfo;
use antex_exec_server::ExecServerError;
...
use antex_exec_server::NoiseChannelPublicKey;
use antex_exec_server::NoiseRendezvousConnectBundle;
use antex_exec_server::NoiseRendezvousConnectProvider;
use antex_exec_server::REMOTE_ENVIRONMENT_ID;
use antex_exec_server::RemoteEnvironmentConfig;
...
use antex_extension_api::RenderedWorldStateFragment;
use antex_extension_api::ThreadLifecycleContributor;
use antex_extension_api::ThreadStartInput;
use antex_extension_api::WorldStateContributionInput;
use antex_extension_api::WorldStateSectionContribution;
...
use antex_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
fn test_antex_with_wait_for_environment() -> TestAntexBuilder {
    let mut extensions = ExtensionRegistryBuilder::new();
    extensions.thread_lifecycle_contributor(Arc::new(WaitForEnvironmentTestExtension));
    test_antex().with_extensions(Arc::new(extensions.build()))
}

async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestAntex> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = antex_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn wait_for_completion_without_patch_approval(test: &TestAntex) {
    let event = wait_for_event(&test.antex, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn environment_permissions_follow_configuration_ownership() -> Result<()> {
    ...
    test.antex
        ...
        .await?;
    let persisted_settings = wait_for_event_match(&test.antex, |event| match event {
        EventMsg::ThreadSettingsApplied(event) => Some(event.thread_settings.clone()),
        _ => None,
    })
    .await;
    let snapshot = test.antex.config_snapshot().await;
    ...
    test.antex
        .restore_thread_settings(test.antex.restorable_thread_settings().await)
        .await?;
    let (mcp_config, _) = test.antex.current_mcp_config_and_runtime_context().await;
    assert_eq!(
    ...
    for deferred_executor_enabled in [false, true] {
        for host_config_present in [false, true] {
            ...
            let builder = if host_config_present {
                test_antex_with_wait_for_environment()
            } else {
                test_antex()
            };
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    let initial_environments = test.antex.environment_selections().await;
    ...
    test.antex
        ...
        .await?;
    let request = wait_for_event_match(&test.antex, |event| match event {
    ...
    let preview = test
        .antex
        .preview_thread_settings_overrides(AntexThreadSettingsOverrides {
...
async fn deferred_executor_promotes_primary_environment_when_startup_completes() -> Result<()> {
    ...
    core_test_support::wait_for_mcp_server(&test.antex, "deferred").await?;
    test.antex
        .submit(Op::UserInputAnswer {
            id: request.turn_id,
            response: RequestUserInputResponse {
    ...
    let rollout = fs::read_to_string(test.antex.rollout_path().context("rollout path")?)?;
...
async fn shared_executor_keeps_ready_capability_roots_scoped_to_each_attachment() -> Result<()> {
    ...
    let mut builder = test_antex().with_extensions(Arc::new(extensions.build()));
    let test = builder.build_with_auto_env(&server).await?;
    let selection = test
        .antex
    ...
    assert_eq!(
        test.antex.inspect_selected_capability_roots().ready_roots,
        vec![root("first-root")]
    );
    assert_eq!(
        second
...
async fn owner_network_policy_rejects_unsupported_environment_authority() -> Result<()> {
    ...
    let selections = test.antex.environment_selections().await;
    ...
    assert_eq!(test.antex.environment_selections().await, selections);
    Ok(())
}
...
async fn ready_before_selection_resolves_resumed_thread_capability_root_after_wait(
    ...
    let remote_environment = tokio::spawn(antex_exec_server::run_remote_environment(
        remote_config,
        runtime_paths,
    ));
...
async fn deferred_executor_stays_pending_after_materialization() -> Result<()> {
    ...
    .await;
    let mut builder = test_antex_with_wait_for_environment().with_config(|config| {
        assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
    });
    ...
    test.antex.submit(Op::Interrupt).await?;
    wait_for_event(&test.antex, |event| {
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    .await;
    let mut builder = test_antex_with_wait_for_environment()
        .with_exec_server_url(format!("ws://{}", listener.local_addr()?))
        .with_config(|config| {
            assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
        });
    ...
    assert_eq!(test.antex.instruction_sources().await, vec![agents_path]);
...
async fn deferred_executor_compaction_preserves_then_updates_environment_once() -> Result<()> {
    ...
    test.antex.ensure_rollout_materialized().await;
    test.antex.flush_rollout().await?;
    let rollout_path = test.antex.rollout_path().context("rollout path")?;
