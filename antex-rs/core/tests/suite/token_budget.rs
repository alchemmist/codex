...
use antex_protocol::protocol::CONTEXT_WINDOW_CLOSE_TAG;
...
async fn assert_context_compaction_item_lifecycle(antex: &std::sync::Arc<antex_core::AntexThread>) {
...
async fn token_budget_guidance_precedes_standalone_context_window(
    ...
    let test = test_antex()
        .with_auth(AntexAuth::from_external_chatgpt_tokens(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn experimental_context_requires_antex_backend(
...
async fn token_budget_context_injects_plain_thread_hint_text() -> Result<()> {
    ...
    let test = test_antex()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test.antex, "notes").await?;
    let responses = mount_sse_sequence(
        &server,
        vec![sse(vec![
            ev_response_created("resp-1"),
...
async fn token_budget_context_uses_new_window_after_compaction(
    ...
    assert_context_compaction_item_lifecycle(&test.antex).await;
...
async fn token_budget_compaction_runs_compact_hooks() -> Result<()> {
    ...
    let pre_compact = wait_for_event_match(&test.antex, |event| match event {
    ...
    let post_compact = wait_for_event_match(&test.antex, |event| match event {
        EventMsg::HookCompleted(completed)
