...
use antex_core::RolloutRecorderParams;
...
#[tokio::test]
async fn find_handles_gitignore_covering_antex_home_directory() {
    let repo = TempDir::new().unwrap();
    let codex_home = repo.path().join(".antex");
    std::fs::create_dir_all(&codex_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".antex/**\n").unwrap();
    let id = Uuid::new_v4();
