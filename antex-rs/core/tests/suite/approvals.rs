...
use antex_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestAntex) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Antex helper binary
...
impl Expectation {
    fn verify(&self, test: &TestAntex, result: &CommandResult) -> Result<()> {
        match self {
...
fn assert_active_workspace_permission_profile(test: &TestAntex) {
    assert_eq!(
...
async fn wait_for_completion_without_approval(test: &TestAntex) {
    let event = wait_for_event(&test.antex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestAntex) {
...
async fn wait_for_spawned_thread(test: &TestAntex) -> Result<Arc<AntexThread>> {
...
"#,
    ...
    let command = r#"printf '%s\n%s\n%s\n' "$GH_HOST" "$GH_ENTERPRISE_TOKEN" "$ANTEX_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE""#;
    let event = shell_event(
        call_id,
        command,
        /*timeout_ms*/ 10_000,
...
/// same sandbox with `antex sandbox -P`. The nested invocation must also pass
...
printenv ANTEX_PERMISSION_PROFILE
touch {outside_path:?}
"#
        ),
    )?;
    ...
    assert_eq!(
        ...
        "the unsandboxed script should inherit ANTEX_PERMISSION_PROFILE from the shell command"
    );
    assert!(
        outside_path.exists(),
...
/// profile names before passing one to `antex sandbox -P`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
def main():
    args = parse_args()
    profile_name = os.environ.get("ANTEX_PERMISSION_PROFILE")
    if not profile_name:
        raise SystemExit("ANTEX_PERMISSION_PROFILE must not be empty")
    if profile_name not in ALLOWED_PROFILES:
        raise SystemExit("ANTEX_PERMISSION_PROFILE is not allowlisted")
...
"#
    ...
    assert_eq!(
        sandbox_argv.len(),
        9,
        "expected antex sandbox ... bash -lc CMD"
    );
    assert_eq!(
        sandbox_argv[..8],
        [
