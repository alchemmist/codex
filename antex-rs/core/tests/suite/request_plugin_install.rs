...
use antex_core_plugins::startup_sync::curated_plugins_repo_path;
...
use antex_protocol::approvals::ElicitationAction;
...
async fn start_gated_step_preparation(test: &TestAntex, server: &MockServer) -> Result<PathUri> {
...
async fn start_install_turn(test: &TestAntex, prompt: &str) -> Result<ElicitationRequestEvent> {
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::Disabled, test.config.cwd.as_path());
    test.antex
        .start_or_steer_turn(
    ...
    Ok(wait_for_event_match(&test.antex, |event| match event {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.4",
                "product_client_id": antex_login::default_client::originator().value,
            }
        })
    );
...
async fn run_remote_plugin_install_refresh_case(refreshed_tools: RefreshedAppsTools) -> Result<()> {
    ...
    drop(requests);
    test.antex.refresh_runtime_config(test.config.clone()).await;
