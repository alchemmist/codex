...
use antex_skills_extension::ExecutorSkillProvider;
use antex_skills_extension::HostSkillProvider;
use antex_skills_extension::OrchestratorSkillProvider;
use antex_skills_extension::SkillProvider;
use antex_skills_extension::SkillProviderSource;
...
use antex_utils_string::approx_token_count;
use anyhow::Result;
...
fn catalog_extensions(
    ...
    Arc<antex_extension_api::ExtensionRegistry<Config>>,
...
async fn rendered_catalogs_for_turns(
    ...
    for _ in 0..turn_count {
        test.antex
            ...
            .await?;
        loop {
            match core_test_support::wait_for_event(&test.antex, |_| true).await {
    ...
    // while core warnings are delivered through the TestAntex event stream.
...
async fn capability_sections_render_in_order_with_host_repo_and_plugin_skills() -> Result<()> {
    ...
    core_test_support::wait_for_event(&test.antex, |event| {
...
async fn agent_plugin_skill_prompt_stays_bounded_without_skills_extension() -> Result<()> {
    ...
    let mut builder = test_antex().with_home(codex_home);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    test.antex
        ...
        .await?;
    let warning = core_test_support::wait_for_event(&test.antex, |event| {
        matches!(
            event,
...
text({ names: result.skills.map(skill => skill.name), warnings: result.warnings, next_cursor: result.next_cursor });"#,
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.antex, CODEX_APPS_MCP_SERVER_NAME).await?;
