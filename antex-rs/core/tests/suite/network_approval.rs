...
use antex_core::config::NetworkProxySpec;
use antex_core::shell::ShellType;
use antex_core::shell::get_shell;
...
use antex_protocol::approvals::NetworkApprovalContext;
...
-> Result<()> {
    ...
    .await?;
    let EventMsg::RequestPermissions(request) = wait_for_event(&test.antex, |event| {
...
async fn cancelled_guardian_network_review_fails_closed_without_rewriting_turn_state() -> Result<()>
{
    ...
    while !saw_turn_aborted || !saw_guardian_aborted {
        let event = tokio::time::timeout(Duration::from_secs(5), test.antex.next_event())
            .await
...
async fn background_network_approval_uses_active_turn_after_original_turn_completes() -> Result<()>
{
    ...
    .await?;
    let EventMsg::TurnComplete(first_turn) = wait_for_event(&test.antex, |event| {
    ...
    assert_eq!(test.antex.list_background_terminals().await.len(), 1);
    ...
    .await?;
    let EventMsg::TurnStarted(active_turn) = wait_for_event(&test.antex, |event| {
    ...
    test.antex
        ...
        .await?;
    let assessment = wait_for_event(&test.antex, |event| {
        matches!(
            event,
            EventMsg::GuardianAssessment(assessment)
    ...
    assert_eq!(
        test.antex.list_background_terminals().await.len(),
        1,
        "approved network access must not terminate the background process"
    );
    test.antex.submit(Op::CleanBackgroundTerminals).await?;
...
async fn ambiguous_unattributed_network_request_is_not_assigned_to_active_calls() -> Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if test.antex.list_background_terminals().await.is_empty() {
                break;
            }
...
async fn managed_network_unified_exec_test(server: &wiremock::MockServer) -> Result<TestAntex> {
...
async fn wait_for_completion_without_network_prompt(test: &TestAntex) {
    let event = wait_for_event(&test.antex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_turn_complete(test: &TestAntex) {
    wait_for_event(&test.antex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
