...
async fn guardian_subagent_review_preserves_late_root_user_authorization(
        ...
        root_history_items.extend(
            [
                format!(
                    ...
                    antex_core::review_prompts::SUMMARY_PREFIX
                ),
    ...
    test.antex.inject_response_items(root_history_items).await?;
    ...
    test.antex
        ...
        .await?;
    let question = wait_for_event_match(&test.antex, |event| match event {
    ...
    let legacy_answer = antex_guardian_context::truncate_text(
        &format!(
            "{}{}",
    ...
    let expected_messages = match root_context {
        ...
        RootContext::RetainedAtMessageLimit => {
            ...
            messages.push(GuardianRootMessage::User(
                antex_guardian_context::truncate_text(
    ...
    let feedback_thread_ids = test
        ...
        .await?;
    let failures = antex_feedback::guardian_review_failures(&feedback_thread_ids);
