...
async fn tool_search_returns_deferred_tools_without_follow_up_tool_injection() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(begin) = wait_for_event(&test.antex, |event| {
    ...
    let EventMsg::McpToolCallEnd(end) = wait_for_event(&test.antex, |event| {
...
async fn tool_search_returns_deferred_v1_multi_agent_tools() -> Result<()> {
    ...
    let mut builder = test_antex().with_config(configure_search_capable_model);
    let test = builder.build_with_auto_env(&server).await?;
    test.submit_turn_with_approval_and_permission_profile(
