...
async fn codex_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    antex_returns_json_result("gpt-5.4".to_string()).await
}

async fn antex_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let TestAntex { antex, config, .. } = test_antex().build(&server).await?;
    ...
    let message = wait_for_event(&antex, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
