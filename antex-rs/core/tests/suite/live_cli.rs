...
//! `just test -p antex-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let codex_home = home.path().join(".antex");
    ...
    let mut cmd = Command::new(antex_utils_cargo_bin::cargo_bin("antex-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn antex-rs");
