...
use antex_protocol::protocol::ElicitationAction;
...
async fn wait_for_mcp_tool_call_item(
    ...
    wait_for_event_match(&test.antex, |event| {
...
async fn approved_mcp_tool_call_metadata_records_prior_user_input_request(
    ...
    if !strict_auto_review {
        let EventMsg::ElicitationRequest(request) = wait_for_event(&test.antex, |event| {
            matches!(
                event,
                EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
            )
...
"#
    ...
    let route_event = wait_for_event(&test.antex, |event| {
        matches!(
            event,
            EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
        )
...
async fn apps_default_writes_prompts_for_writes_but_not_reads() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(read_begin) = wait_for_event(&test.antex, |event| {
        matches!(event, EventMsg::McpToolCallBegin(_))
    })
    .await
    else {
    ...
    let next_route = wait_for_event(&test.antex, |event| {
        matches!(
            event,
