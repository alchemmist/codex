...
use antex_core::TimeProvider;
pub use antex_core::TurnInputRequest;
use antex_core::config::Config;
use antex_core::resolve_installation_id;
use antex_core::shell::Shell;
use antex_core::shell::get_shell_by_model_provided_path;
...
use antex_exec_server::RemoveOptions;
use antex_extension_api::ExtensionRegistry;
use antex_extension_api::LoadUserInstructionsFuture;
use antex_extension_api::UserInstructionsProvider;
...
use antex_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
use antex_utils_path_uri::PathConvention;
...
#[derive(Debug)]
pub struct TestEnv {
    environment: antex_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let environment =
            antex_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &antex_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                antex_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestAntexBuilder {
    ...
    pub fn with_auth(mut self, auth: AntexAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestAntex> {
    ...
    ///
    /// With no remote test configuration, or with `ANTEX_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `ANTEX_TEST_ENVIRONMENT=docker` or
    /// `ANTEX_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `ANTEX_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `ANTEX_TEST_REMOTE_ENV` Docker-container
    ...
    pub async fn restart(
        ...
        previous.antex.shutdown_and_wait().await?;
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let antex_linux_sandbox_exe = Some(
            crate::find_antex_linux_sandbox_exe()
                .context("should find binary for antex-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let antex_linux_sandbox_exe = None;
        let local_runtime_paths = antex_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            antex_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            antex_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            antex_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        environment_manager: Arc<antex_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestAntex> {
        ...
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(AntexHomeUserInstructionsProvider::new(
                    config.codex_home.clone(),
                ))
            });
        let auth_manager = antex_core::test_support::auth_manager_from_auth_with_home(
            auth.clone(),
        ...
        let models_manager = self
            .models_manager
            .clone()
            .unwrap_or_else(|| antex_core::build_models_manager(&config, auth_manager.clone()));
        let thread_manager = ThreadManager::new(
            ...
            antex_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    antex_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    antex_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = antex_utils_cargo_bin::cargo_bin("antex") {
            config.antex_self_exe = Some(path);
        } else if let Ok(path) = antex_utils_cargo_bin::cargo_bin("antex-exec") {
            // `antex-exec` also supports `--codex-run-as-apply-patch`, so use it
        ...
        {
            let antex = bin_dir.join("codex");
            let antex_exec = bin_dir.join("antex-exec");
            if antex.is_file() {
                config.antex_self_exe = Some(antex);
            } else if antex_exec.is_file() {
                config.antex_self_exe = Some(antex_exec);
            }
        }
...
pub struct TestAntex {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub antex: Arc<AntexThread>,
...
impl TestAntex {
    ...
    pub fn antex_home_path(&self) -> &Path {
    ...
    pub async fn submit_text_turn(&self, prompt: &str) -> Result<()> {
        ...
        wait_for_event(&self.antex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
        Ok(())
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.antex, |event| match event {
...
pub struct TestAntexHarness {
    server: MockServer,
    test: TestAntex,
}

impl TestAntexHarness {
    pub async fn with_builder(mut builder: TestAntexBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestAntexBuilder) -> Result<Self> {
        let server = start_mock_server().await;
...
pub fn test_antex() -> TestAntexBuilder {
    TestAntexBuilder {
        ...
        auth: AntexAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
