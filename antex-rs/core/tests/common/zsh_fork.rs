...
use crate::test_antex::TestAntex;
use crate::test_antex::TestAntexBuilder;
use crate::test_antex::test_antex;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = antex_utils_cargo_bin::cargo_bin("antex-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `antex-execve-wrapper` binary");
        return Ok(None);
    };
...
pub fn zsh_fork_test_builder(
    ...
    test_antex().with_config(move |config| {
