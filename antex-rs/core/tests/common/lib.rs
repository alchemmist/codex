...
use antex_arg0::Arg0PathEntryGuard;
use antex_utils_cargo_bin::CargoBinError;
...
pub use antex_core::test_support::TestAntexResponsesRequestKind;
pub use antex_core::test_support::responses_metadata;
use antex_utils_absolute_path::AbsolutePathBuf;
pub use antex_utils_absolute_path::test_support::PathBufExt;
pub use antex_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    antex_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    antex_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(antex_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = antex_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("antex-rs"));
...
/// avoids clobbering a developer’s real `~/.antex`.
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        antex_linux_sandbox_exe: Some(
            find_antex_linux_sandbox_exe().expect("should find binary for antex-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_antex_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    antex_utils_cargo_bin::cargo_bin("antex-linux-sandbox")
}

pub async fn wait_for_event<F>(
    antex: &AntexThread,
    predicate: F,
) -> antex_protocol::protocol::EventMsg
where
    F: FnMut(&antex_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(antex, predicate, Duration::from_secs(1)).await
}

/// Waits for a configured MCP server to finish startup and requires it to be ready.
pub async fn wait_for_mcp_server(antex: &AntexThread, server_name: &str) -> anyhow::Result<()> {
...
pub async fn submit_thread_settings(
    antex: &AntexThread,
    thread_settings: antex_protocol::protocol::ThreadSettingsOverrides,
) -> anyhow::Result<()> {
    use antex_protocol::protocol::EventMsg;
    ...
    let submission_id = antex.submit(Op::ThreadSettings { thread_settings }).await?;
    loop {
        let ev = timeout(Duration::from_secs(10), antex.next_event())
            .await
...
{
    let ev = wait_for_event(antex, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&antex_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), antex.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    antex_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    antex_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    antex_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    antex_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    antex_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! antex_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_antex_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("antex-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
