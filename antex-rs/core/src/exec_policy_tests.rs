...
use antex_config::RequirementsExecPolicy;
...
fn config_stack_for_dot_antex_folder(dot_antex_folder: &Path) -> ConfigLayerStack {
    let dot_antex_folder =
        AbsolutePathBuf::from_absolute_path(dot_antex_folder).expect("absolute dot_codex_folder");
...
async fn returns_empty_policy_when_no_policy_files_exist() {
    ...
    let config_stack = config_stack_for_dot_antex_folder(temp_dir.path());
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        antex_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        /*justification*/ None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(antex_config::Sourced::new(
            antex_config::RequirementsExecPolicy::new(requirements_exec_policy),
            antex_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
    let dot_antex_folder = AbsolutePathBuf::from_absolute_path(temp_dir.path())?;
    let layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project { dot_antex_folder },
        TomlValue::Table(Default::default()),
    );
...
async fn ignore_user_project_rules_keeps_system_policy_files() {
    ...
    .expect("write policy file");
    let config_file =
        AbsolutePathBuf::from_absolute_path(config_dir.join(antex_config::CONFIG_TOML_FILE))
...
async fn ignores_rules_from_untrusted_project_layers() -> anyhow::Result<()> {
    ...
    let project_dot_antex_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
    let layers = vec![ConfigLayerEntry::new_disabled(
        ConfigLayerSource::Project {
            dot_antex_folder: project_dot_antex_folder,
        },
        TomlValue::Table(Default::default()),
        "marked untrusted",
    )];
...
async fn exec_policies_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let root_rules = project_root.join(".antex").join(RULES_DIR_NAME);
    let nested_rules = nested.join(".antex").join(RULES_DIR_NAME);
...
async fn exec_policies_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let nested = project_root.join("nested");
    let rules_dir = project_root.join(".antex").join(RULES_DIR_NAME);
