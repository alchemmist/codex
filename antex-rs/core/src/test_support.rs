...
use antex_models_manager::collaboration_mode_presets;
...
pub fn auth_manager_from_auth(auth: AntexAuth) -> Arc<AuthManager> {
    AuthManager::from_auth_for_testing(auth)
}

pub fn auth_manager_from_auth_with_home(auth: AntexAuth, codex_home: PathBuf) -> Arc<AuthManager> {
...
) -> antex_protocol::error::Result<crate::NewThread> {
    thread_manager
...
#[derive(Clone, Copy)]
pub enum TestAntexResponsesRequestKind {
    Turn,
    Prewarm,
    WebsocketConnection,
}
...
pub fn responses_metadata(
    ...
    let request_kind = match request_kind {
        TestAntexResponsesRequestKind::Turn => Some(AntexResponsesRequestKind::Turn),
        TestAntexResponsesRequestKind::Prewarm => Some(AntexResponsesRequestKind::Prewarm),
        TestAntexResponsesRequestKind::WebsocketConnection => None,
    };
    AntexResponsesMetadata {
...
pub fn with_parent_turn(mut metadata: AntexResponsesMetadata, id: &str) -> AntexResponsesMetadata {
