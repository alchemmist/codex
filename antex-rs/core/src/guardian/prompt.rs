...
use antex_guardian_context::ComposedContext;
use antex_guardian_context::ContextTarget;
use antex_guardian_context::ConversationTranscriptConfig;
use antex_guardian_context::ConversationTranscriptEntry;
use antex_guardian_context::ConversationTranscriptEntryKind;
use antex_guardian_context::ConversationTranscriptOptions;
use antex_guardian_context::GuardianRootMessage;
use antex_guardian_context::SectionError;
use antex_guardian_context::SectionHistory;
use antex_guardian_context::SectionInput;
use antex_guardian_context::TranscriptEntryLimits;
use antex_guardian_context::TranscriptRetentionConfig;
use antex_guardian_context::default_registry;
    ...
    let user_messages = entries
        ...
        .filter_map(|(index, entry)| {
            matches!(entry.kind, ConversationTranscriptEntryKind::User).then_some(
                antex_guardian_context::UserMessageCost {
                    index,
        ...
        .collect::<Vec<_>>();
    let selection = antex_guardian_context::select_user_messages(
        &user_messages,
...
impl SectionHistory for GuardianReviewHistory<'_> {
    fn retained_context(&self) -> Option<&antex_history::RetainedContext> {
...
pub(crate) fn guardian_truncate_text(content: &str, token_cap: usize) -> (String, bool) {
    (
        antex_guardian_context::truncate_text(content, token_cap),
