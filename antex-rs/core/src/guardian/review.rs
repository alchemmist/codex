...
use antex_analytics::GuardianReviewDecision;
use antex_analytics::GuardianReviewFailureReason;
use antex_analytics::GuardianReviewTerminalStatus;
use antex_analytics::GuardianReviewTrackContext;
use antex_analytics::GuardianReviewedAction;
use antex_analytics::GuardianV2Event;
use antex_analytics::GuardianV2EventKind;
use antex_async_utils::THREAD_STACK_SIZE_BYTES;
use antex_core_plugins::PluginCommandAttribution;
use antex_extension_api::GuardianV2Enabled;
...
use antex_protocol::protocol::GuardianAssessmentDecisionSource;
...
async fn run_guardian_review(
                ...
                .is_some()
            {
                session
                    ...
                    .track_guardian_v2_event(GuardianV2Event {
                        ...
                        occurred_at_ms: antex_analytics::now_unix_millis(),
    ...
    antex_extension_api::SynchronousApprovalReviewer::review(
        &runtime,
        antex_protocol::approvals::GuardianReviewReason::Policy,
    )
    .await
}
...
pub(super) async fn run_synchronous_review(
    ...
    _review_reason: antex_protocol::approvals::GuardianReviewReason,
) -> ReviewDecision {
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == antex_protocol::openai_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info()
                ...
                .iter()
                .any(|preset| preset.effort == antex_protocol::openai_models::ReasoningEffort::Low),
