...
use antex_protocol::capabilities::CapabilityRootLocation;
use antex_protocol::capabilities::SelectedCapabilityRoot;
use antex_protocol::config_types::WindowsSandboxLevel;
use antex_protocol::mcp::ClientMcpExtensions;
use antex_protocol::mcp::MCP_APP_UI_EXTENSION_ID;
use antex_protocol::mcp::OPENAI_FORM_EXTENSION_ID;
...
async fn reserved_thread_id_is_used_without_changing_normal_id_generation() {
    ...
    config.codex_home = temp_dir.path().join("antex-home").abs();
    ...
    assert!(matches!(
        ...
        antex_protocol::error::CodexErrorDetails::InvalidRequest(message)
...
impl antex_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: antex_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> antex_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<antex_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> antex_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<antex_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> antex_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl antex_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: antex_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl antex_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: antex_extension_api::McpServerContributionContext<'a, Config>,
        ) -> antex_extension_api::ExtensionFuture<'a, Vec<antex_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(AntexAuth::from_api_key("dummy"));
...
async fn spawn_internal_guardian_session_preserves_windows_sandbox_proxy_settings() {
    ...
    assert_eq!(
        ...
        (
            ...
            antex_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
        )
    );
...
async fn spawn_internal_session_preserves_parent_lineage_without_forking_history() {
    ...
    impl antex_extension_api::ThreadLifecycleContributor<Config> for ParentLifecycleContributor {}

    impl antex_extension_api::McpServerContributor<Config> for ParentLifecycleContributor {
        fn id(&self) -> &'static str {
        ...
        fn contribute<'a>(
            &'a self,
            context: antex_extension_api::McpServerContributionContext<'a, Config>,
        ) -> antex_extension_api::ExtensionFuture<'a, Vec<antex_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    struct ParentInstructionsProvider(antex_extension_api::Instructions);

    impl antex_extension_api::UserInstructionsProvider for ParentInstructionsProvider {
        fn load_user_instructions(&self) -> antex_extension_api::LoadUserInstructionsFuture<'_> {
            Box::pin(async move {
                antex_extension_api::LoadedUserInstructions {
    ...
    let mut managed_exec_policy = antex_execpolicy::Policy::empty();
    managed_exec_policy
        .add_prefix_rule(&["rm".to_string()], antex_execpolicy::Decision::Forbidden)
        .expect("add managed execution restriction");
    let mut requirements = config.config_layer_stack.requirements().clone();
    requirements.exec_policy = Some(antex_config::Sourced::new(
        antex_execpolicy::RequirementsExecPolicy::new(managed_exec_policy),
        antex_config::RequirementSource::Unknown,
    ));
    requirements.additional_developer_instructions = Some(antex_config::Sourced::new(
    ...
    config.config_layer_stack = antex_config::ConfigLayerStack::new(
        config
            .config_layer_stack
    ...
    let parent_instructions = antex_extension_api::Instructions {
    ...
    reviewer_environment.config =
        EnvironmentConfigState::Ready(antex_protocol::protocol::EnvironmentConfig {
            ...
            exec_policy: Some(antex_execpolicy::RequirementsExecPolicy::new(
                antex_execpolicy::Policy::empty(),
            )),
            mcp_policy: None,
            network_policy: None,
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl antex_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
    ...
    impl antex_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        ) -> antex_extension_api::ExtensionFuture<'a, Vec<antex_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = antex_mcp::codex_apps_mcp_server_config(
                ...
                vec![antex_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = antex_extension_api::ExtensionDataInit::new();
    ...
    let selected_servers = |config: &antex_mcp::McpConfig| {
        antex_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let codex_apps_server = antex_mcp::configured_mcp_servers(&first_resolved.config)
        .remove(antex_mcp::CODEX_APPS_MCP_SERVER_NAME)
        .expect("Codex Apps server should be configured");
    let codex_apps_headers = match codex_apps_server.transport {
        antex_config::McpServerTransportConfig::StreamableHttp { http_headers, .. } => http_headers,
        antex_config::McpServerTransportConfig::Stdio { .. } => {
...
#[tokio::test]
async fn explicit_installation_id_skips_antex_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.codex_home = temp_dir.path().join("antex-home").abs();
    config.cwd = config.codex_home.abs();
