...
use antex_diagnostics::Gauge;
use antex_diagnostics::GaugeGuard;
use antex_exec_server::SelectedCapabilityRootsStatus;
...
use antex_protocol::turn_input::RecoverTurnRequest;
use antex_protocol::turn_input::StartIfIdleSubmission;
use antex_protocol::turn_input::SteerSubmission;
use antex_protocol::turn_input::SuspendTurnOutcome;
...
impl ThreadConfigSnapshot {
    ...
    pub fn sandbox_policy(&self) -> SandboxPolicy {
        antex_sandboxing::compatibility_sandbox_policy_for_permission_profile(
...
pub struct AntexThreadSettingsOverrides {
...
pub use antex_guardian_context::GuardianRootMessage;
...
/// (formerly called a conversation) in Antex.
impl AntexThread {
    pub(crate) fn new(
        session: Arc<Session>,
        io: SessionIo,
    ...
    pub async fn submit(&self, op: Op) -> AntexResult<String> {
    ...
    pub fn thread_extension_data(&self) -> &antex_extension_api::ExtensionData {
        &self.session.services.thread_extension_data
    }

    pub async fn shutdown_and_wait(&self) -> AntexResult<()> {
    ...
    pub(crate) async fn emit_thread_ready_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_ready(antex_extension_api::ThreadReadyInput {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(antex_extension_api::ThreadResumeInput {
                    session_store: &self.session.services.session_extension_data,
                    thread_store: &self.session.services.thread_extension_data,
                })
                .await;
    ...
    pub async fn start_turn_if_idle(
        &self,
        request: TurnInputRequest,
    ) -> AntexResult<StartIfIdleSubmission> {
        match self
    ...
    pub async fn suspend_turn_and_shutdown(&self) -> AntexResult<SuspendTurnOutcome> {
    ...
    /// callers that only hold a `AntexThread`.
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: AntexThreadSettingsOverrides,
    ...
    fn thread_settings_update(overrides: AntexThreadSettingsOverrides) -> SessionSettingsUpdate {
        let AntexThreadSettingsOverrides {
            environments,
    ...
    pub async fn next_event(&self) -> AntexResult<Event> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> AntexResult<()> {
    ...
    pub async fn restorable_thread_settings(&self) -> AntexThreadSettingsOverrides {
    ...
    ) -> std::collections::HashMap<String, antex_protocol::mcp::McpServerConnectionStatus> {
        self.session
            .services
            .mcp_runtime
    ...
    ) -> (antex_mcp::McpConfig, antex_mcp::McpRuntimeContext) {
    ...
    pub async fn current_mcp_config_and_runtime_context(
        &self,
    ) -> (Arc<antex_mcp::McpConfig>, antex_mcp::McpRuntimeContext) {
    ...
    pub async fn start_mcp_event_stream(
    ...
    ) -> anyhow::Result<antex_mcp::McpEventStream> {
        ...
        self.session.refresh_mcp_if_dirty().await;
        antex_mcp::McpResourceClient::new(Arc::clone(&self.session.services.mcp_runtime))
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> AntexResult<i64> {
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> AntexResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        if elicitations.count == 0 {
            return Err(CodexErr::InvalidRequest(
