...
use antex_connectors::merge::plugin_connector_to_app_info;
use antex_connectors::metadata::connector_install_url;
use antex_connectors::metadata::sanitize_name;
...
fn synthetic_links_are_exposed_to_the_agent_but_not_accessible_in_app_list() {
    let mut synthetic_tool = antex_app_tool("gmail_batch_read_email", "gmail", Some("Gmail"), &[]);
    ...
    let tools = vec![
        synthetic_tool,
        antex_app_tool("calendar_list_events", "calendar", Some("Calendar"), &[]),
    ];
