...
use crate::client::X_ANTEX_TURN_METADATA_HEADER;
...
use antex_api::RealtimeAudioFrame;
use antex_api::RealtimeContextAppendChannel;
use antex_api::RealtimeEvent;
use antex_api::RealtimeEventParser;
use antex_api::RealtimeSessionConfig;
use antex_api::RealtimeSessionMode;
use antex_api::RealtimeWebsocketClient;
use antex_api::RealtimeWebsocketConnection;
use antex_api::RealtimeWebsocketEvents;
use antex_api::RealtimeWebsocketWriter;
use antex_api::build_session_headers;
use antex_api::map_api_error;
...
use antex_login::read_openai_api_key_from_env;
...
use antex_protocol::protocol::RealtimeConversationSdpEvent;
use antex_protocol::protocol::RealtimeConversationStartedEvent;
...
use antex_utils_output_truncation::approx_bytes_for_tokens;
use antex_utils_string::approx_token_count;
use antex_utils_string::take_bytes_at_char_boundary;
use antex_utils_string::to_ascii_json_string;
use anyhow::Context;
...
impl RealtimeConversationManager {
    ...
    ) -> AntexResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> AntexResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> AntexResult<()> {
        let sender = {
    ...
    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> AntexResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
    ...
    pub(crate) async fn append_speech(&self, text: String) -> AntexResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> AntexResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
            guard.as_ref().map(|state| state.handoff.clone())
        };
    ...
    pub(crate) async fn shutdown(&self) -> AntexResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> AntexResult<PreparedRealtimeConversationStart> {
    ...
    {
        extra_headers.insert(X_ANTEX_TURN_METADATA_HEADER, metadata);
    }
...
) -> AntexResult<RealtimeSessionConfig> {
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> AntexResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
fn realtime_api_key(auth: Option<&AntexAuth>, provider: &ModelProviderInfo) -> AntexResult<String> {
    ...
    if let Some(api_key) = auth.and_then(AntexAuth::api_key) {
