...
/// Start an interactive sub-Antex thread and return its runtime and IO channels.
///
...
pub(crate) async fn run_antex_thread_interactive(
    mut config: Config,
    ...
    let extensions = if is_guardian_reviewer {
        antex_extension_api::empty_extension_registry()
    } else {
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_antex_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
