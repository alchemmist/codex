...
use antex_analytics::ImagePreparationFact;
use antex_analytics::ImagePreparationMetadata;
use antex_analytics::SubAgentThreadStartedInput;
use antex_analytics::TurnAntexErrorFact;
use antex_async_utils::OrCancelExt;
use antex_connectors::connector_runtime_context_key;
use antex_context_fragments::RenderedFragment;
use antex_exec_server::Environment;
use antex_exec_server::EnvironmentManager;
use antex_execpolicy::prefix_rule_migration;
use antex_extension_api::ConversationHistorySnapshot;
use antex_extension_api::ExtensionDataInit;
use antex_extension_api::LoadedUserInstructions;
use antex_extension_api::PromptSlot;
use antex_extension_api::TurnContextContributionInput;
use antex_features::FEATURES;
use antex_features::Feature;
use antex_features::unstable_features_warning_event;
use antex_history::RolloutItem;
use antex_hooks::Hooks;
use antex_hooks::HooksConfig;
use antex_login::AntexAuth;
use antex_login::AuthManager;
use antex_login::auth_env_telemetry::collect_auth_env_telemetry;
use antex_mcp::McpResourceClient;
use antex_mcp::McpRuntime;
use antex_mcp::McpRuntimeContext;
use antex_mcp::McpRuntimeInput;
use antex_models_manager::manager::RefreshStrategy;
use antex_models_manager::manager::SharedModelsManager;
use antex_network_proxy::NetworkProxy;
use antex_network_proxy::NetworkProxyAuditMetadata;
use antex_network_proxy::normalize_host;
use antex_otel::current_span_trace_id;
use antex_otel::current_span_w3c_trace_context;
use antex_otel::set_parent_from_w3c_trace_context;
use antex_protocol::ResponseUsageMetadata;
use antex_protocol::SessionId;
use antex_protocol::ThreadId;
use antex_protocol::approvals::ElicitationRequest;
use antex_protocol::approvals::ElicitationRequestEvent;
use antex_protocol::approvals::ExecPolicyAmendment;
use antex_protocol::approvals::NetworkPolicyAmendment;
use antex_protocol::approvals::NetworkPolicyRuleAction;
...
use antex_protocol::dynamic_tools::DynamicToolResponse;
use antex_protocol::dynamic_tools::DynamicToolSpec;
use antex_protocol::items::EnteredReviewModeItem;
use antex_protocol::items::SubAgentActivityItem;
use antex_protocol::items::TurnItem;
use antex_protocol::items::UserMessageItem;
use antex_protocol::models::ActivePermissionProfile;
use antex_protocol::models::AdditionalPermissionProfile;
...
use antex_protocol::openai_models::ModelPreset;
...
use antex_protocol::protocol::FileChange;
use antex_protocol::protocol::HasLegacyEvent;
...
use antex_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
use antex_protocol::protocol::MultiAgentVersion;
use antex_protocol::protocol::RawResponseItemEvent;
...
use antex_protocol::protocol::TurnAbortReason;
use antex_protocol::protocol::TurnContextItem;
use antex_protocol::protocol::TurnContextNetworkItem;
use antex_protocol::protocol::TurnEnvironmentSelection;
use antex_protocol::protocol::W3cTraceContext;
use antex_protocol::protocol::WorldStateItem;
...
use antex_protocol::request_permissions::RequestPermissionsArgs;
use antex_protocol::request_permissions::RequestPermissionsEvent;
use antex_protocol::request_permissions::RequestPermissionsResponse;
use antex_protocol::request_user_input::RequestUserInputArgs;
use antex_protocol::request_user_input::RequestUserInputResponse;
use antex_rmcp_client::ElicitationResponse;
use antex_rollout::state_db;
use antex_rollout_trace::AgentResultTracePayload;
use antex_rollout_trace::ThreadStartedTraceMetadata;
use antex_rollout_trace::ThreadTraceContext;
use antex_sandboxing::policy_transforms::intersect_permission_profiles_with_context;
use antex_shell_command::parse_command::parse_command;
use antex_terminal_detection::user_agent;
use antex_thread_store::CreateThreadParams;
use antex_thread_store::LiveThread;
use antex_thread_store::LiveThreadInitGuard;
use antex_thread_store::LocalThreadStore;
use antex_thread_store::PersistContext;
use antex_thread_store::ReadThreadParams;
use antex_thread_store::ResumeThreadParams;
use antex_thread_store::ThreadPersistenceMetadata;
use antex_thread_store::ThreadStore;
use antex_utils_audio::prepare_response_items as prepare_audio_response_items;
use antex_utils_git_discovery::GitRootDiscovery;
...
use crate::antex_thread::AntexThreadSettingsOverrides;
use crate::antex_thread::ThreadConfigSnapshot;
...
use antex_protocol::error::CodexErrorDetails;
use antex_protocol::error::Result as AntexResult;
#[cfg(test)]
use antex_protocol::exec_output::StreamOutput;
...
use antex_core_plugins::PluginCommandAttribution;
use antex_core_plugins::PluginsManager;
use antex_core_plugins::RecommendedPluginCandidatesInput;
...
use antex_mcp::effective_mcp_servers;
use antex_otel::SessionTelemetry;
use antex_otel::THREAD_STARTED_METRIC;
use antex_otel::TelemetryAuthMode;
use antex_protocol::ResponseItemId;
use antex_protocol::approvals::ExecApprovalKind;
...
use antex_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use antex_protocol::config_types::Settings;
use antex_protocol::config_types::WindowsSandboxLevel;
use antex_protocol::mcp::ClientMcpExtensions;
use antex_protocol::models::LocalImagePreparation;
...
use antex_protocol::protocol::ApplyPatchApprovalRequestEvent;
use antex_protocol::protocol::AskForApproval;
use antex_protocol::protocol::CodexErrorInfo;
use antex_protocol::protocol::DeprecationNoticeEvent;
...
use antex_protocol::protocol::ModelRerouteEvent;
use antex_protocol::protocol::ModelRerouteReason;
use antex_protocol::protocol::ModelVerification;
use antex_protocol::protocol::ModelVerificationEvent;
use antex_protocol::protocol::NetworkApprovalContext;
use antex_protocol::protocol::Op;
use antex_protocol::protocol::RateLimitSnapshot;
use antex_protocol::protocol::RawResponseCompletedEvent;
use antex_protocol::protocol::RequestUserInputEvent;
...
use antex_protocol::protocol::SessionNetworkProxyRuntime;
use antex_protocol::protocol::StreamErrorEvent;
use antex_protocol::protocol::Submission;
use antex_protocol::protocol::ThreadMemoryMode;
...
use antex_protocol::protocol::TurnModerationMetadataEvent;
...
use antex_protocol::turn_input::TurnStartOptions;
use antex_protocol::user_input::UserInput;
use antex_skills_extension::HostSkillsService;
use antex_tools::ToolName;
use antex_tools::UnifiedExecShellMode;
use antex_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use antex_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn antex_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<antex_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) windows_sandbox_proxy_settings_mode:
        antex_sandboxing::WindowsSandboxProxySettingsMode,
}
...
impl Session {
    ...
    ) -> BoxFuture<'static, AntexResult<(Arc<Self>, SessionIo)>> {
        Box::pin(async move {
            let parent_trace = match args.parent_trace {
                Some(trace) => {
                    if antex_otel::context_from_w3c_trace_context(&trace).is_some() {
                        Some(trace)
                    } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> AntexResult<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if crate::guardian::is_basic_session_source(&session_source) {
            let managed_policy = config
                ...
                .exec_policy
                .as_deref()
                .map_or_else(antex_execpolicy::Policy::empty, |policy| {
                    policy.as_ref().clone()
                });
        ...
        let refresh_strategy = if session_source.is_non_root_agent() {
            antex_models_manager::manager::RefreshStrategy::Offline
        } else {
            antex_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
...
impl SessionIo {
    ...
    pub(crate) async fn submit(&self, op: Op) -> AntexResult<String> {
    ...
    pub(crate) async fn submit_with_id(&self, mut sub: Submission) -> AntexResult<()> {
    ...
    pub(crate) async fn submit_turn_input(
        ...
        mode: TurnInputMode,
    ) -> AntexResult<TurnInputSubmission> {
        let id = new_submission_id();
    ...
    pub(crate) async fn shutdown_and_wait(&self) -> AntexResult<()> {
    ...
    pub(crate) async fn next_event(&self) -> AntexResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> AntexResult<PermissionProfileState> {
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &antex_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn antex_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn antex_network_proxy::BlockedRequestObserver>>,
    ...
    pub(crate) async fn restorable_thread_settings(&self) -> AntexThreadSettingsOverrides {
        let state = self.state.lock().await;
        state
            .session_configuration
    ...
    pub(crate) async fn user_instructions(&self) -> Option<antex_extension_api::Instructions> {
    ...
    pub(crate) fn track_turn_antex_error(&self, turn_context: &TurnContext, error: &CodexErr) {
        self.services
            .analytics_events_client
            .track_turn_antex_error(TurnAntexErrorFact::from_antex_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        self.services
            ...
            .record_antex_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &antex_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| antex_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => antex_rollout::existing_rollout_path(&path).await.is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    async fn capture_step_context_inner(
        ...
        let executor_capability_discovery = self
            ...
            .await?;
        let extension_data = antex_extension_api::ExtensionData::new(turn_context.sub_id.clone());
        ...
        let mut selected_plugins = self
            ...
            .get::<antex_extension_api::SelectedPluginSnapshot>()
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(antex_hooks::SessionStartSource::Compact);
        }
    }
            ...
            && turn_context.model_context_window().is_some()
        {
            ...
            separate_developer_sections.push(
                crate::context::TokenBudgetContext::new(
                    session_source
                        .get_agent_path()
                        .unwrap_or_else(antex_protocol::AgentPath::root),
    ...
    pub(crate) async fn record_user_prompt_and_emit_turn_item(
        ...
        self.record_annotated_conversation_items(
            ...
            vec![ResponseItemEnvelope {
                item: response_item,
                metadata: acceptance_order.map(|order| AntexHarnessMetadata {
    ...
    pub(crate) async fn notify_stream_error(
        ...
        antex_error: CodexErr,
    ) {
        let additional_details = antex_error.to_string();
        let codex_error_info = CodexErrorInfo::ResponseStreamDisconnected {
            http_status_code: antex_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<antex_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_config(
    ...
    HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::AntexHooks),
