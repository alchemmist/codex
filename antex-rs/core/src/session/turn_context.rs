...
use antex_core_plugins::ResolvedPluginMetricsOperation;
use antex_core_plugins::TrustedPluginRoots;
...
use antex_protocol::permissions::RawFileSystemSandboxPolicy;
...
use antex_sandboxing::policy_transforms::effective_permission_profile;
...
pub struct TurnContext {
    ...
    pub(crate) subagent_spawn_policy: antex_protocol::config_types::SubagentSpawnPolicy,
    ...
    pub(crate) extension_data: Arc<antex_extension_api::ExtensionData>,
...
impl TurnContext {
    ...
    pub(crate) fn apps_enabled(&self) -> bool {
        let uses_antex_backend = self
            .auth_manager
            .as_deref()
            .is_some_and(AuthManager::current_auth_uses_antex_backend);
        self.config
            .features
            .apps_enabled_for_auth(uses_antex_backend)
...
impl Session {
    ...
    pub(crate) fn make_turn_context(
        ...
        let extension_data = Arc::new(antex_extension_api::ExtensionData::new(sub_id.clone()));
    ...
    ) -> AntexResult<(Arc<TurnContext>, ThreadSettingsSnapshot)> {
    ...
    ) -> AntexResult<Option<(Arc<TurnContext>, ThreadSettingsSnapshot)>> {
        let service_tier_for_turn = updates.service_tier_for_turn.clone();
        let subagent_spawn_policy = updates.subagent_spawn_policy;
    ...
    async fn new_turn_context_from_configuration(
        ...
        if turn_context.config.model_provider_id == antex_model_provider_info::OPENAI_PROVIDER_ID {
            turn_context.cyber_access_program = options.cyber_access_program;
        }
        if let Some(subagent_spawn_policy) = options.subagent_spawn_policy {
    ...
    pub(crate) async fn maybe_emit_model_warnings_for_turn(&self, tc: &TurnContext) {
        ...
        if !tc.code_mode_available
            && matches!(
                ...
                antex_protocol::openai_models::ToolMode::CodeMode
                    | antex_protocol::openai_models::ToolMode::CodeModeOnly
            )
