...
use crate::responses_metadata::AntexResponsesRequestKind;
...
use antex_analytics::AppInvocation;
use antex_analytics::CompactionPhase;
use antex_analytics::CompactionReason;
use antex_analytics::InvocationType;
use antex_analytics::TurnResolvedConfigFact;
use antex_analytics::build_track_events_context;
use antex_async_utils::OrCancelExt;
use antex_connectors::AppToolPolicyEvaluator;
...
use antex_extension_api::TurnInputEnvironment;
use antex_features::Feature;
use antex_file_system::FindUpErrorPolicy;
use antex_file_system::find_nearest_ancestor_with_markers;
use antex_login::AntexAuth;
use antex_model_provider::RemoteCompactionSupport;
...
use antex_protocol::items::PlanItem;
...
use antex_protocol::protocol::AgentMessageContentDeltaEvent;
use antex_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use antex_protocol::protocol::PlanDeltaEvent;
use antex_protocol::protocol::ReasoningContentDeltaEvent;
use antex_protocol::protocol::ReasoningRawContentDeltaEvent;
use antex_protocol::protocol::SafetyBufferingEvent;
...
use antex_skills::ToolMentionKind;
use antex_skills::app_id_from_path;
use antex_skills::build_skill_name_counts;
use antex_skills::collect_explicit_skill_mentions;
use antex_skills::tool_kind_for_path;
use antex_skills_extension::HostSkillPrompts;
use antex_skills_extension::InjectedHostSkillPrompts;
use antex_thread_store::PersistContext;
use antex_tools::DiscoverableTool;
use antex_tools::ToolName;
use antex_tools::filter_request_plugin_install_discoverable_tools_for_client;
use antex_utils_path_uri::PathUri;
use antex_utils_stream_parser::AssistantTextChunk;
use antex_utils_stream_parser::AssistantTextStreamParser;
use antex_utils_stream_parser::ProposedPlanSegment;
use antex_utils_stream_parser::extract_proposed_plan_text;
use antex_utils_stream_parser::strip_citations;
use futures::prelude::*;
...
const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "antex_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
    ...
    .await
    {
        ...
        let error = err.to_antex_protocol_error();
    ...
    loop {
        ...
        let sampling_request_result: AntexResult<_> = async {
            ...
            let responses_metadata = sess
                .responses_metadata(turn_context.as_ref(), AntexResponsesRequestKind::Turn)
                .await;
        ...
        match sampling_request_result {
            ...
            {
                sess.track_turn_antex_error(turn_context.as_ref(), &antex_error);
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_antex_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_antex_error(turn_context.as_ref(), &e);
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &antex_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = antex_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
    ...
    sess.services
        ...
        .track_turn_resolved_config(TurnResolvedConfigFact {
            ...
            guardian_v2_enabled: sess
                ...
                .get::<antex_extension_api::GuardianV2Enabled>()
                .is_some(),
...
///
/// Returns `None` when the active authentication does not use the Antex backend, the provider is
...
async fn capture_current_model_fallback_step_context(
    ...
    previous_model: &str,
    cancellation_token: &CancellationToken,
) -> AntexResult<Option<Arc<StepContext>>> {
    let uses_antex_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(antex_login::AuthManager::current_auth_uses_antex_backend);
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = antex_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<antex_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &AntexResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> AntexResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        turn_context
            .extension_data
            .remove::<antex_protocol::guardian_ticket::GuardianTicket>();
        ...
        {
            antex_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt_input);
        }
...
pub(super) fn agent_message_text(item: &antex_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            antex_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: antex_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(antex_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &antex_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> AntexResult<SamplingRequestResult> {
    let turn_context = Arc::clone(&step_context.turn);
    feedback_tags!(
    ...
    let outcome: AntexResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(antex_async_utils::CancelErr::Cancelled) => {
                ...
                if analytics_tool_call_ids.len() < MAX_ANALYTICS_TOOL_CALL_IDS_PER_RESPONSE {
                    let call_id = match &item {
                        ...
                        | ResponseItem::ImageGenerationCall { id, .. } => {
                            id.as_ref().map(antex_protocol::ResponseItemId::as_str)
                        }
                        _ => None,
                    };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![antex_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                sess.services
                    ...
                    .track_code_mode_tool_call(
                        antex_analytics::CodeModeToolCallFact::SamplingResponseCompleted {
                            thread_id: sess.thread_id.to_string(),
                            turn_id: turn_context.sub_id.clone(),
