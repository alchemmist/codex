...
use antex_protocol::mcp::RequestId as ProtocolRequestId;
...
pub async fn inter_agent_communication(
    ...
    start_options: antex_protocol::turn_input::TurnStartOptions,
) {
...
pub async fn resolve_elicitation(
    ...
    decision: antex_protocol::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        antex_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        antex_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        antex_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
...
pub(super) async fn emit_thread_stop_lifecycle(sess: &Session) {
    for contributor in sess.services.extensions.thread_lifecycle_contributors() {
        contributor
            .on_thread_stop(antex_extension_api::ThreadStopInput {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(antex_rollout_trace::RolloutStatus::Completed);
    true
}
            ...
            match sub.op {
                ...
                Op::SuspendTurnAndShutdown { reply } => {
                    ...
                    let should_exit = matches!(
                        &result,
                        Ok(antex_protocol::turn_input::SuspendTurnOutcome::Suspended { .. })
                    );
                    let _ = reply.send(result);
                    should_exit
                }
                ...
                Op::InspectContext { reply } => {
                    ...
                    let _ = reply.send(antex_protocol::protocol::ContextInspection {
                        base_instructions,
                        items,
                        token_info,
