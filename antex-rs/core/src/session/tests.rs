...
use antex_analytics::CompactionPhase;
use antex_analytics::CompactionReason;
use antex_config::ConfigLayerStack;
use antex_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use antex_config::LoaderOverrides;
use antex_config::NetworkConstraints;
use antex_config::NetworkDomainPermissionToml;
use antex_config::NetworkDomainPermissionsToml;
use antex_config::RequirementSource;
use antex_config::Sourced;
use antex_config::loader::project_trust_key;
use antex_config::types::McpServerConfig;
use antex_config::types::McpServerTransportConfig;
use antex_config::types::ToolSuggestDisabledTool;
use antex_config::types::WindowsSandboxModeToml;
use core_test_support::test_antex::TurnInputRequest as ExternalTurnInputRequest;

use antex_features::Feature;
use antex_file_system::FileSystemSandboxContext;
use antex_http_client::ClientRouteClass;
use antex_http_client::HttpClientFactory;
use antex_http_client::OutboundProxyPolicy;
use antex_http_client::RouteAwareClientPool;
use antex_login::AntexAuth;
use antex_login::auth::AgentIdentityAuthPolicy;
...
use antex_models_manager::model_info;
use antex_models_manager::test_support::construct_model_info_offline_for_tests;
use antex_models_manager::test_support::get_model_offline_for_tests;
use antex_protocol::AgentPath;
use antex_protocol::ResponseItemId;
use antex_protocol::SessionId;
use antex_protocol::ThreadId;
use antex_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use antex_protocol::config_types::ServiceTier;
use antex_protocol::config_types::TrustLevel;
use antex_protocol::exec_output::ExecToolCallOutput;
use antex_protocol::models::AgentMessageInputContent;
use antex_protocol::models::FileSystemPermissions;
...
use antex_protocol::models::SandboxEnforcement;
use antex_protocol::openai_models::ModelServiceTier;
use antex_protocol::openai_models::ToolMode;
...
use antex_protocol::permissions::FileSystemSandboxPolicyContext;
...
use antex_protocol::request_permissions::PermissionGrantScope;
use antex_protocol::request_permissions::RequestPermissionProfile;
use antex_protocol::turn_input::TurnInput as SubmittedTurnInput;
use antex_protocol::turn_input::TurnInputMode;
use antex_protocol::turn_input::TurnInputRequest;
use antex_protocol::turn_input::TurnInputSubmission;
...
use antex_config::config_toml::ConfigToml;
use antex_config::config_toml::ProjectConfig;
use antex_config::permissions_toml::FilesystemPermissionToml;
use antex_config::permissions_toml::FilesystemPermissionsToml;
use antex_config::permissions_toml::NetworkToml;
use antex_config::permissions_toml::PermissionProfileToml;
use antex_config::permissions_toml::PermissionsToml;
use antex_execpolicy::Decision;
use antex_execpolicy::NetworkRuleProtocol;
use antex_execpolicy::Policy;
use antex_history::AntexHarnessMetadata;
use antex_history::CompactedItem;
use antex_history::InitialHistory;
use antex_history::ResponseItemEnvelope;
use antex_history::ResumedHistory;
use antex_history::RolloutItem;
use antex_network_proxy::NetworkProxyConfig;
use antex_otel::MetricsClient;
use antex_otel::MetricsConfig;
use antex_otel::TelemetryAuthMode;
...
use antex_protocol::items::HookPromptFragment;
use antex_protocol::items::build_hook_prompt_message;
use antex_protocol::models::BaseInstructions;
...
use antex_protocol::protocol::CreditsSnapshot;
use antex_protocol::protocol::GranularApprovalConfig;
use antex_protocol::protocol::HistoryPosition;
use antex_protocol::protocol::InterAgentCommunication;
use antex_protocol::protocol::MultiAgentVersion;
use antex_protocol::protocol::NetworkApprovalProtocol;
use antex_protocol::protocol::RateLimitSnapshot;
use antex_protocol::protocol::RateLimitWindow;
use antex_protocol::protocol::RealtimeAudioFrame;
use antex_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use antex_protocol::protocol::RealtimeVoice;
use antex_protocol::protocol::RealtimeVoicesList;
use antex_protocol::protocol::SessionMeta;
use antex_protocol::protocol::SessionMetaLine;
use antex_protocol::protocol::Submission;
use antex_protocol::protocol::ThreadRolledBackEvent;
use antex_protocol::protocol::ThreadSettingsOverrides;
use antex_protocol::protocol::TokenCountEvent;
use antex_protocol::protocol::TokenUsage;
use antex_protocol::protocol::TokenUsageInfo;
use antex_protocol::protocol::TokenUsageRecord;
use antex_protocol::protocol::TurnAbortedEvent;
use antex_protocol::protocol::TurnCompleteEvent;
use antex_protocol::protocol::TurnStartedEvent;
use antex_protocol::protocol::UserMessageEvent;
use antex_protocol::protocol::W3cTraceContext;
use antex_rmcp_client::ElicitationAction;
...
use core_test_support::test_antex::local;
...
use antex_protocol::mcp::CallToolResult as McpCallToolResult;
...
pub(crate) fn mcp_config_for_test(config: &crate::config::Config) -> Arc<antex_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &antex_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(antex_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl antex_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: antex_extension_api::WorldStateContributionInput<'a>,
        ) -> antex_extension_api::ExtensionFuture<
            'a,
            Vec<antex_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
    ...
    let mut builder = antex_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        antex_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
fn write_project_hooks(dot_antex: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_antex)?;
    std::fs::write(
        dot_antex.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        codex_home.join(antex_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<antex_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&antex_hooks::SessionStartRequest {
            session_id: thread_id,
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: antex_hooks::StartHookTarget::SessionStart {
                source: antex_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn antex_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { antex_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<TurnEnvironment> for ProbeToolRuntime {
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::ExecCommand {
                ...
                environment_id: antex_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> antex_sandboxing::SandboxablePreference {
            antex_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(antex_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: antex_tools::ToolName::plain("probe"),
    };
...
async fn get_base_instructions_no_user_content() {
    let prompt_path = antex_utils_cargo_bin::find_resource!(
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = antex_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    assert_eq!(
        config
            ...
            .map(antex_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    ...
    let config_toml_path = codex_home.join(CONFIG_TOML_FILE);
    let user_config: antex_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = antex_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = antex_hooks::list_hooks(antex_hooks::HooksConfig {
        feature_enabled: true,
    ...
    assert_eq!(
        ...
        antex_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: antex_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: antex_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: antex_config::MatcherGroup {
                matcher: None,
                hooks: vec![antex_config::HookHandlerConfig::Command {
        ...
        let identity = antex_config::TomlValue::try_from(identity)?;
        antex_config::version_for_toml(&identity)
    };
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    next_config.mcp_servers = antex_config::Constrained::normalized(
...
async fn reconstruct_history_uses_replacement_history_verbatim() {
    ...
    let replacement_history = vec![
        ResponseItemEnvelope {
            ...
            metadata: Some(AntexHarnessMetadata::default()),
        },
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = antex_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: antex_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_inter_agent_communication_preserves_item_id_in_rollout_and_resume() {
    let (mut session, turn_context, _rx) = make_session_and_context_with_auth_and_config_and_rx(
        AntexAuth::from_api_key("Test API Key"),
        Vec::new(),
        |_| {},
    )
    .await;
...
async fn prepares_resumed_history_before_installing_it() {
    ...
    assert_eq!(
        ...
        Some(AntexHarnessMetadata::default())
    );
}
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl antex_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a antex_extension_api::ExtensionData,
            thread_store: &'a antex_extension_api::ExtensionData,
            turn_store: &'a antex_extension_api::ExtensionData,
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl antex_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: antex_extension_api::TurnStartInput<'a>,
        ) -> antex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl antex_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: antex_extension_api::TurnErrorInput<'a>,
        ) -> antex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl antex_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &antex_extension_api::ExtensionData,
            thread_store: &antex_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: antex_protocol::protocol::NetworkAccess::Restricted,
    };
    let permission_profile = PermissionProfile::External {
        network: NetworkSandboxPolicy::Restricted,
    };
    let expected_permission_profile = permission_profile.clone();
    let mut builder = test_antex().with_config(move |config| {
        config
            .permissions
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_antex().with_config(|config| {
        ...
        config.permissions.approval_policy =
            antex_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.antex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    // Forking reads the persisted rollout JSONL, so force the completed source turn to disk
    // before snapshotting from it.
    initial.antex.ensure_rollout_materialized().await;
    initial
        .antex
        .flush_rollout()
        .await
    ...
    fork_config.permissions.approval_policy =
        antex_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "antex_core__antex_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: antex_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            antex_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            antex_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            antex_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(antex_protocol::account::PlanType::Plus),
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(antex_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(AntexAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("antex-tui".to_string()),
...
async fn session_configuration_apply_client_metadata_preserves_permissions() {
    ...
    let updated = configuration
        .apply(
            &SessionSettingsUpdate {
                app_server_client_name: Some("antex-tui".to_string()),
    ...
    assert_eq!(
        ...
        (Some("antex-tui".to_string()), Some("1.0.0".to_string())),
    );
}
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = antex_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        antex_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    ...
    std::fs::write(
        codex_home.path().join(antex_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&antex_exec_server::LOCAL_FS));
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(AntexAuth::from_api_key("Test API Key"));
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(antex_code_mode::DisabledCodeModeSessionProvider),
        Arc::new(antex_extension_api::ExtensionRegistryBuilder::new().build()),
        antex_extension_api::ExtensionDataInit::default(),
        ...
        Arc::new(antex_thread_store::LocalThreadStore::new(
            antex_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        antex_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
        /*external_time_provider*/ None,
        Some(config.multi_agent_version_from_features()),
        GitEnrichmentPolicy::Fresh,
        antex_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(antex_mcp::McpRuntime::empty(config.prefix_mcp_tool_names()));
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: antex_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(antex_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: antex_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: antex_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: antex_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(antex_thread_store::LocalThreadStore::new(
            antex_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            antex_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(antex_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(antex_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            antex_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(antex_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = antex_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(antex_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    ...
                    call_id,
                    antex_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(antex_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: antex_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_tool_rejects_invalid_requests(
    ...
    Arc::make_mut(&mut turn_context.config)
        .permissions
        .approval_policy = antex_config::Constrained::allow_any(AskForApproval::Never);
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(antex_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = antex_utils_path_uri::PathUri::from_abs_path(&second_cwd);
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        antex_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = antex_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(antex_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn antex_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    result_sender
        .try_send(
            serde_json::from_value::<antex_protocol::protocol::HookCompletedEvent>(json!({
                "turn_id": "turn-1",
                "run": {
    ...
    assert_eq!(
        antex_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl antex_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: antex_extension_api::ThreadStopInput<'a>,
        ) -> antex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl antex_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: antex_extension_api::ThreadStopInput<'a>,
        ) -> antex_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl antex_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: antex_extension_api::TurnAbortInput<'a>,
        ) -> antex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            antex_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let configured_servers = antex_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !antex_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        antex_mcp::configured_mcp_servers(rematerialized_old.config()).contains_key("refreshed"),
    ...
    assert!(
        antex_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_active_reviewer_and_latest_runtime_policy(
    ...
    if live_update {
        assert_eq!(
            session
                .apply_turn_settings(
                    &old_turn.sub_id,
                    antex_protocol::protocol::TurnSettingsUpdate {
                        approvals_reviewer: Some(ApprovalsReviewer::AutoReview),
                        ..Default::default()
                    },
                )
                .await,
            antex_protocol::protocol::TurnSettingsUpdateOutcome::Applied
        );
    } else {
        session
            .update_settings(SessionSettingsUpdate {
    ...
    let request = antex_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: antex_rmcp_client::Elicitation::Mcp(
...
async fn mcp_refresh_detects_shared_auth_manager_changes() {
    ...
    assert_eq!(
        ...
        Some(antex_protocol::auth::AuthMode::ApiKey)
    );
    session.refresh_mcp_if_dirty().await;
    assert!(
        session
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    let (session, turn_context) = make_session_and_context().await;
    let selected_root =
        |environment_id: &str, path: &str| antex_protocol::capabilities::SelectedCapabilityRoot {
            id: "shared-root".to_string(),
            location: antex_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    for selected_root in &selected_roots {
        let antex_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
        ...
        turn_environments.push(TurnEnvironment::new(
            ...
            Arc::new(
                antex_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(antex_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(antex_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
async fn build_initial_context_reuses_in_flight_recommendation_prewarm() {
    ...
    let (session, turn_context, _rx_event) = make_session_and_context_with_auth_and_config_and_rx(
        AntexAuth::create_dummy_chatgpt_auth_for_testing(),
        /*dynamic_tools*/ Vec::new(),
        |config| {
...
impl antex_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a antex_extension_api::ExtensionData,
        thread_store: &'a antex_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<antex_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    antex_extension_api::PromptFragment::developer_policy(
                        "prompt extension enabled",
                        antex_extension_api::ContentItemKind("test.prompt_extension".to_string()),
                    )
                })
                .into_iter()
                .collect()
...
-> Arc<antex_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = antex_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl antex_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: antex_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<antex_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            ...
            .then(|| {
                antex_extension_api::PromptFragment::developer_policy(
                    ...
                    antex_extension_api::ContentItemKind("test.turn_context".to_string()),
                )
            })
            .into_iter()
            .collect()
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<antex_thread_store::InMemoryThreadStore> {
    let store = Arc::new(antex_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn antex_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &antex_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
    ...
    impl antex_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: antex_extension_api::ThreadIdleInput<'a>,
        ) -> antex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = antex_protocol::user_input::TextElement::new(
        antex_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    session
        .emit_thread_idle_lifecycle_if_idle(antex_extension_api::ThreadIdleCause::Completed)
        .await;
...
async fn active_turn_keeps_first_root_when_mail_coalesces(inherited_root: Option<&str>) {
    ...
    .enumerate()
    {
        sess.input_queue
            .enqueue_mailbox_communication(
                communication,
                antex_protocol::turn_input::TurnStartOptions {
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(antex_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: antex_tools::ToolName::plain("exec_command"),
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let root_dot_antex = project_root.join(".antex");
    let nested_dot_antex = nested.join(".antex");

    std::fs::create_dir_all(&codex_home)?;
    std::fs::create_dir_all(&nested_dot_antex)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_antex)?;
    write_project_hooks(&nested_dot_antex)?;
    ...
    let expected_source_path = antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_antex.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_antex = project_root.join(".antex");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_antex)?;
