...
use antex_config::permissions_toml::NetworkUnixSocketPermissionToml;
use antex_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use antex_features::NetworkProxyDomainPermissionToml;
use antex_features::NetworkProxyModeToml;
use antex_features::NetworkProxyUnixSocketPermissionToml;
use antex_network_proxy::NetworkMode;
use antex_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use antex_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
use antex_protocol::permissions::project_roots_glob_pattern;
...
/// for Antex to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_antex_runtime(
    codex_home: &Path,
...
// Adding a new `:special_path` must not make older Antex versions reject the
