...
use antex_config::ConfigLayerEntry;
use antex_config::ConfigLayerSource;
use antex_config::ConfigLayerStack;
use antex_config::McpServerCommandMatcher;
use antex_config::McpServerIdentity;
use antex_config::McpServerRequirement;
use antex_config::McpServerValueMatcher;
use antex_config::ProfileV2Name;
use antex_config::RequirementSource;
use antex_config::Sourced;
use antex_config::config_toml::AgentRoleToml;
use antex_config::config_toml::AgentsToml;
use antex_config::config_toml::AutoReviewToml;
use antex_config::config_toml::ConfigToml;
use antex_config::config_toml::ExperimentalRequestUserInput;
use antex_config::config_toml::ProjectConfig;
use antex_config::config_toml::RealtimeConfig;
use antex_config::config_toml::RealtimeToml;
use antex_config::config_toml::RealtimeTransport;
use antex_config::config_toml::RealtimeWsMode;
use antex_config::config_toml::RealtimeWsVersion;
use antex_config::config_toml::ToolsToml;
...
use antex_config::permissions_toml::NetworkDomainPermissionToml;
use antex_config::permissions_toml::NetworkDomainPermissionsToml;
use antex_config::permissions_toml::NetworkMitmActionToml;
use antex_config::permissions_toml::NetworkMitmHookToml;
use antex_config::permissions_toml::NetworkMitmToml;
...
use antex_config::permissions_toml::WorkspaceRootsToml;
use antex_config::types::AppToolApproval;
use antex_config::types::ApprovalsReviewer;
use antex_config::types::BundledSkillsConfig;
use antex_config::types::FeedbackConfigToml;
use antex_config::types::HistoryPersistence;
use antex_config::types::McpServerEnvVar;
use antex_config::types::McpServerOAuthConfig;
use antex_config::types::McpServerToolConfig;
use antex_config::types::McpServerTransportConfig;
use antex_config::types::MemoriesConfig;
use antex_config::types::MemoriesToml;
use antex_config::types::ModelAvailabilityNuxConfig;
use antex_config::types::Notice;
use antex_config::types::NotificationCondition;
use antex_config::types::NotificationMethod;
use antex_config::types::Notifications;
use antex_config::types::OtelConfigToml;
use antex_config::types::OtelExporterKind;
use antex_config::types::ResumeCwdMode;
use antex_config::types::SandboxWorkspaceWrite;
use antex_config::types::SessionPickerViewMode;
use antex_config::types::SkillsConfig;
use antex_config::types::StartupPanelConfig;
use antex_config::types::ToolSuggestDisabledTool;
use antex_config::types::ToolSuggestDiscoverableType;
use antex_config::types::Tui;
...
use antex_config::types::WindowsToml;
use antex_exec_server::LOCAL_FS;
use antex_features::Feature;
use antex_features::FeaturesToml;
use antex_login::default_client::RESIDENCY_HEADER_NAME;
use antex_login::test_support::auth_manager_from_optional_auth;
use antex_model_provider::ProviderCapabilities;
use antex_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use antex_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use antex_model_provider_info::WireApi;
use antex_models_manager::bundled_models_response;
use antex_network_proxy::NetworkMode;
use antex_protocol::config_types::ModelProviderAuthInfo;
...
use antex_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use antex_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use antex_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
...
use antex_protocol::openai_models::MultiAgentRoleMessages;
...
use antex_protocol::protocol::NetworkAccess;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: antex_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
env_http_headers = { "x-openai-internal-codex-residency" = "ANTEX_TEST_UNSET_RESIDENCY_HEADER", "x-provider-env-header" = "ANTEX_TEST_UNSET_PROVIDER_HEADER" }
"#,
        )
        ...
        assert_eq!(
            environment_headers
                ...
                .map(String::as_str),
            Some("ANTEX_TEST_UNSET_RESIDENCY_HEADER")
        );
        assert_eq!(
        ...
        assert_eq!(
            environment_headers
                ...
                .map(String::as_str),
            Some("ANTEX_TEST_UNSET_PROVIDER_HEADER")
        );
...
"#,
    ...
    assert_eq!(
        ...
        antex_core_plugins::remote::RemotePluginServiceConfig::new(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        antex_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".antex"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".antex".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_local_path_with_cwd(&root.join(".antex"), cwd.as_path()),
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_local_path_with_cwd(&cwd.path().join(".antex"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".antex"] {
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    antex_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            antex_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    ...
    let project_dot_antex =
        AbsolutePathBuf::resolve_path_against_base("project/.antex", codex_home.path());
    ...
    let requirements = antex_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_antex_folder: project_dot_antex.clone(),
                },
                toml::toml! {
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_antex_folder: project_dot_antex,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
-> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(plugin_root.join(".antex-plugin"))?;
    std::fs::write(
        plugin_root.join(".antex-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
    std::fs::write(
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            antex_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
#[tokio::test]
async fn sqlite_home_defaults_to_antex_home_for_workspace_write() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
...
async fn config_applies_managed_auth_store_and_chatgpt_base_url() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        requirements,
        antex_config::ConfigRequirementsToml::default(),
    )?;
...
"#,
    ...
    let project_config_dir = workspace.path().join(".antex");
...
async fn to_mcp_config_flows_mcp_2026_feature_from_config() -> std::io::Result<()> {
    ...
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(mcp_config.protocol_mode, antex_mcp::McpProtocolMode::Legacy);

    let _ = config.features.enable(Feature::Mcp20260728);
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(
        mcp_config.protocol_mode,
        antex_mcp::McpProtocolMode::V20260728
    );
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/antex-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/antex-mcp""#),
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        antex_config::ConfigRequirementsToml {
...
"#,
    ...
    let config_layer_stack = antex_config::ConfigLayerStack::new(
        vec![antex_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".antex").join("agents");
...
"#;
    ...
    let antex_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        codex_home: antex_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        ...
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: antex_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = antex_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![antex_config::WebSearchModeRequirement::Cached]),
        application: None,
    ...
    let requirement_source = antex_config::RequirementSource::Unknown;
    ...
    let requirements = antex_config::ConfigRequirements {
        web_search_mode: antex_config::ConstrainedWithSource::new(
            constrained,
...
-> std::io::Result<()> {
    ...
    let instructions = "x".repeat(antex_utils_string::approx_bytes_for_tokens(
        /*tokens*/ 10_000,
    ));
...
"#,
    ...
    assert_eq!(
        ...
        Some(antex_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
fn test_tui_startup_panel_config() {
    ...
    assert_eq!(
        panel,
        antex_config::types::StartupPanelConfig {
            style: antex_config::types::StartupPanelStyle::Hacker,
            ...
            mascot_skin: antex_config::types::StartupMascotSkin::Ant01,
        }
    );
}
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    ...
    assert_eq!(
        warnings,
        vec![format!(
            "Environment value for `$ANTEX_SQLITE_HOME` is overridden by the required `sqlite_home` value {required:?} from {}.",
            RequirementSource::Unknown
        )]
    );
