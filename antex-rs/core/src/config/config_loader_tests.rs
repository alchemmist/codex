...
use antex_config::ConfigError;
use antex_config::ConfigLayerEntry;
use antex_config::ConfigLayerSource;
use antex_config::ConfigLoadError;
use antex_config::ConfigLoadOptions;
use antex_config::ConfigRequirements;
use antex_config::ConfigRequirementsToml;
use antex_config::ConfigRequirementsWithSources;
use antex_config::FilesystemDenyReadPattern;
use antex_config::LoaderOverrides;
use antex_config::RequirementSource;
use antex_config::RequirementsLayerEntry;
use antex_config::SessionThreadConfig;
use antex_config::StaticThreadConfigLoader;
use antex_config::ThreadConfigSource;
use antex_config::compose_requirements;
use antex_config::config_error_from_ignored_toml_fields;
use antex_config::config_error_from_toml;
use antex_config::config_error_from_typed_toml;
...
use antex_config::loader::load_requirements_toml;
...
async fn write_project_hook_config(
    dot_antex_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_antex_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_antex_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when ANTEX_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn ignore_project_config_skips_project_discovery() -> std::io::Result<()> {
    ...
    .await?;
    let project_config_dir = workspace.join(".antex");
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = antex_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        antex_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = antex_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = antex_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_packaged_defaults_when_other_layers_are_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when ANTEX_HOME/config.toml does not exist");
...
#[cfg(target_os = "macos")]
#[tokio::test]
async fn managed_preferences_requirements_resolve_paths_against_antex_home() -> anyhow::Result<()> {
    use base64::Engine;

    let tmp = tempdir()?;
    let codex_home = tmp.path().join("antex-home");
...
"#
    ...
    .await?;
    let expected_log_dir = AbsolutePathBuf::resolve_path_against_base("~/.antex/logs", &codex_home);
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![antex_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(antex_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(antex_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_antex_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let codex_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&codex_home).await?;
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            antex_config::SandboxModeRequirement::ReadOnly,
            antex_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    antex_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".antex")).await?;
    tokio::fs::create_dir_all(project_root.join(".antex")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".antex").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".antex").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_antex_folder } => Some(dot_antex_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".antex").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".antex").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".antex")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".antex")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".antex"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".antex"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".antex"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".antex"),
        Some("worktree-child"),
...
async fn forged_linked_worktree_does_not_inherit_repo_trust() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(attacker_root.join(".antex")).await?;
    tokio::fs::write(
    ...
    .await?;
    tokio::fs::write(
        attacker_root.join(".antex").join(CONFIG_TOML_FILE),
        r#"foo = "attacker"
"#,
    )
    .await?;
...
async fn malformed_untrusted_linked_worktree_does_not_read_root_hooks() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".antex")).await?;
    tokio::fs::create_dir_all(repo_root.join(".antex")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    tokio::fs::write(worktree_root.join(".antex").join(CONFIG_TOML_FILE), "foo =").await?;
    tokio::fs::write(repo_root.join(".antex").join(CONFIG_TOML_FILE), [0xff]).await?;
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".antex"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_antex_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    tokio::fs::write(project_root.join(".antex").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".antex").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".antex").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".antex").join("child.txt"),
        "child instructions",
    )
    .await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_antex_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_antex_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".antex"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
#[tokio::test]
async fn antex_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let codex_home = home_dir.join(".antex");
    tokio::fs::create_dir_all(&codex_home).await?;
    tokio::fs::write(
        codex_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn antex_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_antex = project_root.join(".antex");
    let nested_dot_antex = nested.join(".antex");

    tokio::fs::create_dir_all(&nested_dot_antex).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(project_root.join(".git/HEAD"), "ref: refs/heads/main\n").await?;
    tokio::fs::write(
        nested_dot_antex.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_antex).await?;
    make_config_for_test(
        &project_dot_antex,
        &project_root,
        TrustLevel::Trusted,
    ...
    .await?;
    let user_config_path = project_dot_antex.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_antex_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_antex)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let antex_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&antex_home_untrusted).await?;
    make_config_for_test(
        &antex_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = antex_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let antex_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&antex_home_unknown).await?;
    tokio::fs::write(
        antex_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_antex).await?;
    ...
    // `.antex` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_antex.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_antex.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.antex` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".antex")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".antex").join(CONFIG_TOML_FILE), "foo =").await?;
...
mod requirements_exec_policy_tests {
    ...
    use antex_config::ConfigRequirementsWithSources;
    use antex_config::RequirementSource;
    use antex_config::RequirementsExecPolicyDecisionToml;
    use antex_config::RequirementsExecPolicyParseError;
    use antex_config::RequirementsExecPolicyPatternTokenToml;
    use antex_config::RequirementsExecPolicyPrefixRuleToml;
    use antex_config::RequirementsExecPolicyToml;
    use antex_execpolicy::Decision;
    use antex_execpolicy::Evaluation;
    use antex_execpolicy::RuleMatch;
    use antex_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_antex_folder_with_requirements(
        dot_antex_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_antex_folder = AbsolutePathBuf::from_absolute_path(dot_antex_folder)
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_antex_folder_with_requirements(temp_dir.path(), requirements);
