...
use antex_network_proxy::build_config_state;
use antex_network_proxy::host_and_port_from_network_addr;
#[cfg(any(target_os = "windows", test))]
use antex_network_proxy::managed_proxy_ports;
use antex_network_proxy::normalize_host;
use antex_network_proxy::validate_policy_against_constraints;
...
impl NetworkProxySpec {
    ...
    pub fn windows_sandbox_proxy_listeners(
        ...
        antex_windows_sandbox::WindowsSandboxProvisioningSettings,
        antex_windows_sandbox::WindowsSandboxProxyListeners,
    )> {
        if !self.config.enabled {
            return Ok((
                antex_windows_sandbox::WindowsSandboxProvisioningSettings::default(),
                antex_windows_sandbox::WindowsSandboxProxyListeners::default(),
            ));
        }
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.upsert_domain_permission(
                host,
                if allow {
                    antex_network_proxy::NetworkDomainPermission::Allow
                } else {
                    antex_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
