...
use antex_agent_roles::load_agent_roles;
use antex_config::CloudConfigBundleLoader;
use antex_config::ConfigLayerSource;
use antex_config::ConfigLayerStack;
use antex_config::ConfigRequirements;
use antex_config::ConfigRequirementsToml;
use antex_config::ConstrainedWithSource;
use antex_config::FeatureRequirementsToml;
use antex_config::ManagedAuthPolicy;
use antex_config::McpServerRequirement;
use antex_config::PluginRequirementsToml;
use antex_config::ProfileV2Name;
use antex_config::ResidencyRequirement;
use antex_config::SandboxModeRequirement;
use antex_config::Sourced;
use antex_config::ThreadConfigLoader;
use antex_config::config_toml::ConfigToml;
use antex_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use antex_config::config_toml::ProjectConfig;
use antex_config::config_toml::RealtimeAudioConfig;
use antex_config::config_toml::RealtimeConfig;
use antex_config::config_toml::ThreadStoreToml;
use antex_config::config_toml::validate_model_providers;
use antex_config::loader::load_config_layers_state;
...
use antex_config::sandbox_mode_requirement_for_permission_profile;
use antex_config::types::ApprovalsReviewer;
use antex_config::types::AuthCredentialsStoreMode;
use antex_config::types::AuthKeyringBackendKind;
use antex_config::types::History;
use antex_config::types::McpServerConfig;
use antex_config::types::McpServerDisabledReason;
use antex_config::types::MemoriesConfig;
use antex_config::types::ModelAvailabilityNuxConfig;
use antex_config::types::Notice;
use antex_config::types::OAuthCredentialsStoreMode;
use antex_config::types::ResumeCwdMode;
use antex_config::types::SessionPickerViewMode;
use antex_config::types::ToolSuggestConfig;
use antex_config::types::ToolSuggestDisabledTool;
use antex_config::types::ToolSuggestDiscoverable;
use antex_config::types::TuiKeymap;
use antex_config::types::TuiNotificationSettings;
use antex_config::types::TuiPetAnchor;
use antex_config::types::UriBasedFileOpener;
use antex_config::types::VimModeStart;
use antex_config::types::WindowsSandboxModeToml;
use antex_core_plugins::PluginLoadOutcome;
use antex_core_plugins::PluginsConfigInput;
use antex_exec_server::ExecutorFileSystem;
use antex_exec_server::LOCAL_FS;
use antex_exec_server::ReadFileOptions;
use antex_features::CodeModeConfigToml;
use antex_features::CurrentTimeReminderConfigToml;
use antex_features::CurrentTimeReminderDeliveryMode;
use antex_features::CurrentTimeSource;
use antex_features::Feature;
use antex_features::FeatureConfigSource;
use antex_features::FeatureOverrides;
use antex_features::FeatureToml;
use antex_features::Features;
use antex_features::FeaturesToml;
use antex_features::MultiAgentV2ConfigToml;
use antex_features::NetworkProxyConfigToml;
use antex_features::SleepToolMode;
use antex_features::TokenBudgetConfigToml;
...
use antex_install_context::InstallContext;
use antex_login::AuthManagerConfig;
use antex_login::AuthRouteConfig;
use antex_mcp::DEFAULT_OPTIONAL_MCP_STARTUP_GRACE;
use antex_mcp::McpConfig;
use antex_mcp::McpPluginAttribution;
use antex_mcp::McpProtocolMode;
use antex_mcp::McpServerRegistration;
use antex_mcp::ResolvedMcpCatalog;
use antex_memories_read::memory_root;
use antex_model_provider::ProviderCapabilities;
use antex_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use antex_model_provider_info::ModelProviderInfo;
use antex_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use antex_model_provider_info::built_in_model_providers;
use antex_model_provider_info::merge_configured_model_providers;
use antex_models_manager::ModelsManagerConfig;
use antex_protocol::config_types::AltScreenMode;
use antex_protocol::config_types::AutoCompactTokenLimitScope;
...
use antex_protocol::config_types::Verbosity;
use antex_protocol::config_types::WebSearchConfig;
...
use antex_protocol::models::BaseInstructionsProvenance;
...
use antex_protocol::permissions::ReadDenyMatcher;
...
use antex_rmcp_client::McpOAuthRefreshMode;
pub use antex_thread_store::ExtraConfig;
use antex_utils_absolute_path::AbsolutePathBuf;
use antex_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_antex_runtime;
...
#[cfg(test)]
mod schema;
pub use antex_agent_roles::AgentRoleConfig;
pub use antex_config::ConfigLoadOptions;
pub use antex_config::Constrained;
pub use antex_config::ConstraintError;
pub use antex_config::ConstraintResult;
pub use antex_config::LoaderOverrides;
pub use antex_network_proxy::NetworkProxyAuditMetadata;
use antex_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use antex_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(antex_state::SQLITE_HOME_ENV).ok()?;
...
impl Permissions {
    ...
    /// selection. Internal Antex-only writable roots are intentionally excluded.
...
// when Antex is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<antex_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Antex will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Antex
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.antex/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Antex"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Antex '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    pub tui_startup_panel: antex_config::types::StartupPanelConfig,
    ...
    /// file (default): Use a file in the Antex home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Antex can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Antex unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/codex/blob/main/codex-rs/rmcp-client/src/oauth.rs#L2
    /// file: ANTEX_HOME/.credentials.json
    ///       This file will be readable to Antex and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Antex will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Antex state (defaults to `~/.antex` but can be
    /// overridden by the `ANTEX_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Antex SQLite databases.
    pub sqlite: antex_state::SqliteConfig,

    /// Directory where Antex writes log files (defaults to `$ANTEX_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.antex/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Antex executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub antex_self_exe: Option<PathBuf>,

    /// Path to the `antex-linux-sandbox` executable. This must be set if
    /// [`antex_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `antex-linux-sandbox`.
    pub antex_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `antex-execve-wrapper` executable used for shell
    ...
    /// Whether Antex-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for Antex updates on startup and surfaces update prompts.
    /// Set to `false` only if your Antex updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Antex product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Antex product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: antex_config::types::OtelConfig,
}
...
impl ConfigBuilder {
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&antex_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = antex_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    antex_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(antex_config::io_error_from_config_error(
...
impl Config {
    pub fn sqlite_config(&self) -> &antex_state::SqliteConfig {
        &self.sqlite
    }

    /// Whether Guardian may use the unmetered Antex inference endpoints.
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &antex_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            antex_linux_sandbox_exe: self.antex_linux_sandbox_exe.clone(),
            ...
            connector_snapshot:
                antex_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_antex_home(
    ...
    /// Load a default configuration for a specific Antex home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_antex_home(
        codex_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let cli_layer = antex_config::build_cli_overrides_layer(&cli_overrides);
        antex_config::merge_toml_values(&mut merged, &cli_layer);
    ...
    /// ignores user settings. For example, the `antex exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `antex_self_exe`, `antex_linux_sandbox_exe`, and
...
/// Patch `ANTEX_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(codex_home: &Path, provider: &str) -> std::io::Result<()> {
    antex_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &antex_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            antex_protocol::permissions::FileSystemSandboxEntry {
                path: antex_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: antex_protocol::permissions::FileSystemAccessMode::Deny,
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&antex_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(antex_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_antex_runtime(
            &codex_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            codex_home,
            sqlite: antex_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
            config_layer_stack,
            history,
    ...
    pub fn bundled_skills_enabled(&self) -> bool {
        antex_config::bundled_skills_enabled_from_stack(&self.config_layer_stack)
    }
...
/// Returns the path to the Antex configuration directory, which can be
/// specified by the `ANTEX_HOME` environment variable. If not set, defaults to
/// `~/.antex`.
///
/// - If `ANTEX_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `ANTEX_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_antex_home() -> std::io::Result<AbsolutePathBuf> {
    antex_utils_home_dir::find_antex_home()
}

/// Returns the path to the folder where Antex logs are stored. Does not verify
