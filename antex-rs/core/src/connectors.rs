...
pub use antex_connectors::AppBranding;
pub use antex_connectors::AppInfo;
pub use antex_connectors::AppMetadata;
use antex_connectors::ConnectorDirectoryCacheContext;
use antex_connectors::ConnectorDirectoryCacheKey;
use antex_connectors::apps_config_from_layer_stack;
...
use antex_mcp::McpStartupPolicy;
use antex_mcp::ToolInfo;
use antex_mcp::ToolPluginProvenance;
use antex_mcp::effective_mcp_servers;
use antex_mcp::tool_plugin_provenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = antex_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        antex_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    if !config
        .features
        .apps_enabled_for_auth(auth.as_ref().is_some_and(AntexAuth::uses_antex_backend))
    {
        return Some(Vec::new());
    }
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let environment_manager = EnvironmentManager::from_antex_home(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    let codex_apps_auth_manager =
        antex_mcp::host_owned_codex_apps_enabled(&mcp_config, auth.as_ref())
...
fn accessible_connectors_cache_key(
    ...
    let account_id = auth.and_then(AntexAuth::get_account_id);
    let chatgpt_user_id = auth.and_then(AntexAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(AntexAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + antex_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    antex_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        ...
        Some(antex_connectors::accessible::AccessibleConnectorTool {
    ...
    antex_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn mcp_approvals_reviewer_from_layers(
    config_layer_stack: &antex_config::ConfigLayerStack,
