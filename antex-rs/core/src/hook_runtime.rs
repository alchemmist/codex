...
use antex_analytics::CompactionTrigger;
use antex_analytics::HookRunFact;
...
use antex_core_plugins::executor_plugin_hook_sources;
use antex_hooks::InterruptRequest;
use antex_hooks::PermissionRequestDecision;
use antex_hooks::PermissionRequestOutcome;
use antex_hooks::PermissionRequestRequest;
use antex_hooks::PostToolUseOutcome;
use antex_hooks::PostToolUseRequest;
use antex_hooks::PreToolUseOutcome;
use antex_hooks::PreToolUseRequest;
use antex_hooks::SessionStartOutcome;
use antex_hooks::StartHookTarget;
use antex_hooks::StopHookTarget;
use antex_hooks::StopOutcome;
use antex_hooks::SubagentHookContext;
use antex_hooks::UserPromptSubmitOutcome;
use antex_hooks::UserPromptSubmitRequest;
use antex_hooks::hook_execution_mode_label;
use antex_hooks::hook_handler_type_label;
use antex_mcp::CODEX_APPS_MCP_SERVER_NAME;
use antex_otel::HOOK_RUN_DURATION_METRIC;
use antex_otel::HOOK_RUN_METRIC;
use antex_plugin::ExecutorPluginHookSource;
use antex_protocol::items::FunctionCallOutputItem;
...
use antex_protocol::protocol::HookCompletedEvent;
...
use antex_protocol::protocol::HookOutputEntryKind;
use antex_protocol::protocol::HookRunStatus;
use antex_protocol::protocol::HookRunSummary;
use antex_protocol::protocol::HookSource;
use antex_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    antex_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = antex_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = antex_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = antex_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = antex_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(antex_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: antex_hooks::HookEvent::AfterAgent {
                event: antex_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            antex_hooks::HookResult::Success => continue,
            antex_hooks::HookResult::FailedContinue(error) => (error, false),
            antex_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(antex_protocol::protocol::ErrorEvent {
        misalignment: None,
        message,
...
fn hook_run_analytics_payload(
    ...
    turn_context: &TurnContext,
    completed: &HookCompletedEvent,
) -> (antex_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use antex_protocol::protocol::HookHandlerType;
    use antex_protocol::protocol::HookRunStatus;
    use antex_protocol::protocol::HookScope;
    use antex_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    antex_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
    ...
    async fn hook_lifecycle_notifications_hide_builtin_and_async_runs_but_preserve_metrics() {
        ...
        assert!(matches!(
            started.msg,
            antex_protocol::protocol::EventMsg::HookStarted(event)
        ...
        assert!(matches!(
            completed.msg,
            antex_protocol::protocol::EventMsg::HookCompleted(event)
                if event.run.id == synchronous_run.id
        ));
        assert!(events.try_recv().is_err());
