...
use antex_analytics::ArtifactOperation;
use antex_analytics::ArtifactOperationLifecycle;
use antex_analytics::build_track_events_context;
use antex_apply_patch::AppliedPatchDelta;
use antex_core_plugins::PluginCommandAttribution;
use antex_core_plugins::recognize_artifact_operation;
use antex_otel::ARTIFACT_OPERATION_EXPECTED_OUTPUT_COUNT_METRIC;
use antex_otel::ARTIFACT_OPERATION_STARTED_METRIC;
...
use antex_protocol::items::CommandExecutionStatus;
...
use antex_utils_string::truncate_middle_with_token_budget;
...
impl ToolEmitter {
    ...
    pub async fn finish(
        ...
        let (event, result) = match out {
            ...
            Err(ToolError::Antex(err)) => match err.details() {
...
mod tests {
    ...
    async fn assert_failed_apply_patch_tracks_committed_delta(
        ...
        let delta = antex_apply_patch::apply_patch(
    ...
    async fn denied_apply_patch_tracks_committed_delta() {
        ...
        assert_failed_apply_patch_tracks_committed_delta(
            Err(ToolError::Antex(CodexErr::Sandbox(SandboxErr::Denied {
