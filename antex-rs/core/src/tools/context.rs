...
use antex_protocol::models::function_call_output_content_items_to_text;
use antex_tools::LoadableToolSpec;
...
pub enum ToolCallSource {
    ...
    CodeMode {
        ...
        /// debugging the JS/runtime bridge, but it is not the Antex tool call id
    ...
    pub(crate) async fn originating_item_id(&self) -> Option<ResponseItemId> {
        if let ToolCallSource::CodeMode { cell_id, .. } = &self.source {
            return self
                ...
                .services
                .code_mode_service
                .cell_originating_item_id(&antex_code_mode::CellId::new(cell_id.clone()));
        }
