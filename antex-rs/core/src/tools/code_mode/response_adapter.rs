use antex_code_mode::ImageDetail as CodeModeImageDetail;
...
pub(super) fn into_function_call_output_content_items(
    items: Vec<antex_code_mode::FunctionCallOutputContentItem>,
...
impl IntoProtocol<FunctionCallOutputContentItem>
    for antex_code_mode::FunctionCallOutputContentItem
{
    fn into_protocol(self) -> FunctionCallOutputContentItem {
        let value = self;
        match value {
            antex_code_mode::FunctionCallOutputContentItem::InputText { text } => {
            ...
            antex_code_mode::FunctionCallOutputContentItem::InputAudio { audio_url } => {
