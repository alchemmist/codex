...
impl ToolOrchestrator {
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let attempt_with_network_approval = SandboxAttempt {
            ...
            antex_linux_sandbox_exe: attempt.antex_linux_sandbox_exe,
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let sandbox_manager = sandbox_manager.with_allowed_symlinked_antex_home(
            environment
                .environment
                .local_runtime_paths()
                .and_then(|paths| paths.allowed_symlinked_antex_home.clone()),
        );
        ...
        let initial_attempt = SandboxAttempt {
            ...
            workspace_roots,
            antex_linux_sandbox_exe: turn_ctx.config.antex_linux_sandbox_exe.as_ref(),
        ...
        match first_result {
            ...
            Err(ToolError::Antex(err)) => {
                ...
                if network_policy_decision.is_some() && network_approval_context.is_none() {
                    ...
                    return Err(ToolError::Antex(err));
                }
                ...
                let retry_antex_linux_sandbox_exe = if unsandboxed_allowed {
                    None
                } else {
                    turn_ctx.config.antex_linux_sandbox_exe.as_ref()
                };
                let retry_attempt = SandboxAttempt {
                    ...
                    sandbox_cwd: &sandbox_policy_cwd,
                    workspace_roots,
                    antex_linux_sandbox_exe: retry_antex_linux_sandbox_exe,
                    use_legacy_landlock: sandbox_config.use_legacy_landlock,
                    windows_sandbox_level: sandbox_config.windows_sandbox_level,
                    windows_sandbox_private_desktop: sandbox_config.windows_sandbox_private_desktop,
                    network_denial_cancellation_token: None,
