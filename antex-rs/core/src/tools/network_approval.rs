...
use antex_network_proxy::BlockedRequest;
...
use antex_network_proxy::NetworkRequestCancellation;
use antex_network_proxy::NetworkRequestCancellationReason;
use antex_network_proxy::NetworkRequestDisconnect;
...
use antex_sandboxing::record_network_sandbox_violation;
...
impl NetworkApprovalService {
    ...
    pub(crate) async fn handle_inline_policy_request(
        ...
        let approval_decision = match session.request_approval(action, approval_context).await {
            ...
            Err(ToolError::Antex(err)) => {
                let telemetry_decision = if matches!(
                    err.details(),
                    antex_protocol::error::CodexErrorDetails::TurnAborted
                ) {
...
pub(crate) async fn begin_network_approval(
    ...
    let execution_proxy = network
        ...
        .map_err(|err| {
            ToolError::Antex(antex_protocol::error::CodexErr::Io(io::Error::other(
