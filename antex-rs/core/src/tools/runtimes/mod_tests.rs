...
#[cfg(target_os = "macos")]
use antex_network_proxy::ANTEX_PROXY_GIT_SSH_COMMAND_MARKER;
use antex_network_proxy::CUSTOM_CA_ENV_KEYS;
use antex_network_proxy::ConfigReloader;
use antex_network_proxy::ConfigReloaderFuture;
use antex_network_proxy::ConfigState;
use antex_network_proxy::NetworkProxy;
use antex_network_proxy::NetworkProxyConfig;
use antex_network_proxy::NetworkProxyConstraints;
use antex_network_proxy::NetworkProxyState;
use antex_network_proxy::PROXY_ACTIVE_ENV_KEY;
use antex_network_proxy::PROXY_ENV_KEYS;
#[cfg(target_os = "macos")]
use antex_network_proxy::PROXY_GIT_SSH_COMMAND_ENV_KEY;
use antex_protocol::config_types::WindowsSandboxLevel;
use antex_protocol::models::PermissionProfile;
use antex_sandboxing::SandboxManager;
...
async fn test_network_proxy_with_config(
    ...
    let state =
        antex_network_proxy::build_config_state(config, NetworkProxyConstraints::default())?;
    NetworkProxy::builder()
        .state(Arc::new(NetworkProxyState::with_reloader(
            state,
            Arc::new(StaticReloader),
        )))
        .managed_by_antex(/*managed_by_antex*/ false)
...
async fn explicit_escalation_prepares_exec_without_managed_network() -> anyhow::Result<()> {
    ...
    let command = antex_sandboxing::SandboxCommand {
...
fn runtime_path_prepends_records_runtime_path_prepend() {
    ...
    runtime_path_prepends.prepend(&mut env, PathBuf::from("/package/antex-path").as_path());

    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/antex-path:/usr/bin:/bin"),
        "runtime PATH prepend should update the live exec environment"
    );
    assert_eq!(
        runtime_path_prepends.entries,
        vec!["/package/antex-path"],
...
fn runtime_path_prepends_drops_empty_path_entries() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        ":/usr/bin:/package/antex-path::/bin:".to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
...
fn apply_zsh_fork_path_prepend_uses_shell_parent() {
    ...
    apply_zsh_fork_path_prepend(
        ...
        PathBuf::from("/package/antex-resources/zsh/bin/zsh").as_path(),
    );

    let expected = "/package/antex-resources/zsh/bin:/usr/bin:/bin";
    assert_eq!(env.get("PATH").map(String::as_str), Some(expected));
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
            entries: vec!["/package/antex-resources/zsh/bin".to_string()]
        }
    );
}
...
fn apply_zsh_fork_path_prepend_moves_existing_shell_parent_to_front() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        "/usr/bin:/package/antex-resources/zsh/bin:/bin:/package/antex-resources/zsh/bin"
            .to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
    ...
    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/antex-resources/zsh/bin:/usr/bin:/bin")
    );
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
...
#[test]
fn explicit_escalation_keeps_user_proxy_env_without_antex_marker() {
    let env = HashMap::from([
        (
...
#[test]
fn maybe_wrap_shell_lc_with_snapshot_restores_antex_metadata_from_env() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport ANTEX_THREAD_ID='parent-thread'\nexport ANTEX_VERSION='old-version'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s|%s' \"$ANTEX_THREAD_ID\" \"$ANTEX_VERSION\"".to_string(),
    ];
    let env = HashMap::from([
        ("ANTEX_THREAD_ID".to_string(), "nested-thread".to_string()),
        (
...
fn maybe_wrap_shell_lc_with_snapshot_restores_permission_profile_from_env() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport ANTEX_PERMISSION_PROFILE='parent-profile'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printenv ANTEX_PERMISSION_PROFILE".to_string(),
    ];
    let env = HashMap::from([(
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env(ANTEX_PERMISSION_PROFILE_ENV_VAR, "current-profile")
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_unsets_absent_permission_profile() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport ANTEX_PERMISSION_PROFILE='stale-profile'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove(ANTEX_PERMISSION_PROFILE_ENV_VAR)
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_restores_apply_patch_rollout_state() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS='stale'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printenv ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS".to_string(),
    ];
    let env = HashMap::from([(
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env(ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR, "1")
        .output()
        .expect("run rewritten command");
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove(ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR)
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_restores_reserved_metrics_output_env() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport ANTEX_PLUGIN_METRICS_OUTPUT='/stale/path'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s' \"${ANTEX_PLUGIN_METRICS_OUTPUT-unset}\"".to_string(),
    ];
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_refreshes_antex_proxy_git_ssh_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{ANTEX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
    );
    let fresh_command = format!(
        "{ANTEX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:48081 %h %p'"
    );
    std::fs::write(
        &snapshot_path,
        format!(
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_clears_stale_antex_git_ssh_command_without_live_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{ANTEX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
...
fn run_snapshot_path_probe_with_runtime_path_prepend(
    ...
    let package_path_dir = dir.path().join("antex-path");
...
fn maybe_wrap_shell_lc_with_snapshot_preserves_unset_override_variables() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport ANTEX_TEST_UNSET_OVERRIDE='snapshot-value'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "if [ \"${ANTEX_TEST_UNSET_OVERRIDE+x}\" = x ]; then printf 'set:%s' \"$ANTEX_TEST_UNSET_OVERRIDE\"; else printf 'unset'; fi".to_string(),
        ];
    let explicit_env_overrides = HashMap::from([(
        "ANTEX_TEST_UNSET_OVERRIDE".to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove("ANTEX_TEST_UNSET_OVERRIDE")
        .output()
        .expect("run rewritten command");
    assert!(output.status.success(), "command failed: {output:?}");
    assert_eq!(String::from_utf8_lossy(&output.stdout), "unset");
