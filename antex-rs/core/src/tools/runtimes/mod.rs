...
use crate::exec_env::ANTEX_SESSION_ID_ENV_VAR;
use crate::exec_env::ANTEX_THREAD_ID_ENV_VAR;
use crate::exec_env::ANTEX_VERSION_ENV_VAR;
use crate::sandboxing::SandboxPermissions;
use crate::shell::Shell;
use crate::shell::ShellType;
use antex_apply_patch::ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
...
pub(crate) use antex_network_proxy::is_managed_proxy_env_var;
pub(crate) use antex_network_proxy::strip_managed_proxy_env;
...
pub(crate) fn exec_env_for_sandbox_permissions(
    ...
    // defensive cleanup for a caller that passes an environment already prepared by Antex.
...
/// PATH entries owned by Antex runtime setup.
///
...
pub(crate) fn prepare_powershell_command_for_elevated_windows_sandbox(
    ...
    prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
        ...
        |path| {
            antex_shell_command::shell_detect::fallback_powershell_shell_for_elevated_windows_sandbox(
                path,
            )
        },
    )
...
fn prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
    ...
    windows_sandbox_level: WindowsSandboxLevel,
    environment_is_remote: bool,
    find_fallback: impl FnOnce(&Path) -> Option<antex_shell_command::shell_detect::DetectedShell>,
) -> Vec<String> {
...
/// runtime-only vars like `ANTEX_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains Antex-owned PATH entries already applied to
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    for key in [
        ...
        "CODEX_THREAD_ID",
        "CODEX_VERSION",
        ANTEX_SESSION_ID_ENV_VAR,
        ANTEX_THREAD_ID_ENV_VAR,
        ANTEX_VERSION_ENV_VAR,
        ANTEX_PERMISSION_PROFILE_ENV_VAR,
        ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR,
...
fn build_proxy_env_exports(env: &HashMap<String, String>) -> (String, String) {
    let mut keys = PROXY_ENV_KEYS
        .iter()
        .copied()
        .chain(antex_network_proxy::brokered_credential_env_keys(env))
    ...
    let git_blocks = build_antex_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_antex_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", ANTEX_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
...
mod prepare_powershell_command_tests {
    ...
    fn local_elevated_powershell_uses_discovered_fallback() {
        ...
        let rewritten = prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
            ...
            /*environment_is_remote*/ false,
            |_| {
                Some(antex_shell_command::shell_detect::DetectedShell {
