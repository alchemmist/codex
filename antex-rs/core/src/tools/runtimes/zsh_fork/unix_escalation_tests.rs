...
use antex_execpolicy::PolicyParser;
...
use antex_shell_escalation::EscalationExecution;
use antex_shell_escalation::EscalationPermissions;
use antex_shell_escalation::ResolvedPermissionProfile;
...
async fn unsandboxed_intercepted_exec_strips_managed_network_env() -> anyhow::Result<()> {
    ...
    let executor = CoreShellCommandExecutor {
        sandbox_manager: antex_sandboxing::SandboxManager::new(),
        command: Vec::new(),
        cwd: workdir.clone(),
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(antex_execpolicy::Policy::empty())),
        ...
        stopwatch: antex_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = antex_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = antex_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    let config_toml_path = turn_context
        .config
        .codex_home
        .join(antex_config::CONFIG_TOML_FILE);
    ...
    let expected_hook_command =
        antex_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    ...
    session
        ...
        .await;
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(antex_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        antex_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        antex_shell_escalation::EscalationDecision::Escalate(
            antex_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, antex_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        antex_shell_escalation::EscalationDecision::Deny {
