...
use antex_execpolicy::MatchOptions;
...
use antex_protocol::protocol::NetworkPolicyRuleAction;
...
use antex_shell_command::bash::parse_shell_lc_plain_commands;
use antex_shell_escalation::EscalateServer;
use antex_shell_escalation::EscalationDecision;
use antex_shell_escalation::EscalationExecution;
use antex_shell_escalation::EscalationPermissions;
use antex_shell_escalation::EscalationPolicy;
use antex_shell_escalation::EscalationPolicyFuture;
use antex_shell_escalation::EscalationSession;
use antex_shell_escalation::ExecResult;
use antex_shell_escalation::PreparedExec;
use antex_shell_escalation::ResolvedPermissionProfile;
use antex_shell_escalation::ShellCommandExecutor;
use antex_shell_escalation::ShellCommandExecutorFuture;
use antex_shell_escalation::Stopwatch;
...
pub(crate) async fn prepare_unified_exec_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        antex_linux_sandbox_exe: ctx.step_context.turn.config.antex_linux_sandbox_exe.clone(),
...
impl CoreShellActionProvider {
    ...
    async fn prompt(
            ...
            .await
        {
            ...
            Err(ToolError::Antex(err)) => Err(err.into()),
        }
    }
...
struct CoreShellCommandExecutor {
    ...
    network: Option<antex_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["ANTEX_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            antex_linux_sandbox_exe: self.antex_linux_sandbox_exe.as_deref(),
