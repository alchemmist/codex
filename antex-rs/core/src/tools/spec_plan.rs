...
use antex_tools::TOOL_SEARCH_TOOL_NAME;
use antex_tools::ToolCall as ExtensionToolCall;
use antex_tools::ToolEnvironmentMode;
use antex_tools::ToolExecutor;
use antex_tools::ToolExposures;
use antex_tools::ToolName;
use antex_tools::ToolSearchInfo;
use antex_tools::ToolSpec;
use antex_tools::UnifiedExecShellMode;
use antex_tools::can_request_original_image_detail;
use antex_tools::collect_code_mode_exec_prompt_tool_definitions;
use antex_tools::collect_request_plugin_install_entries;
use antex_tools::default_namespace_description;
use antex_tools::request_user_input_available_modes;
...
pub(crate) fn finalize_tool_router(
    ...
    if code_mode_enabled {
        for tool_name in [
            ToolName::plain(antex_code_mode::PUBLIC_TOOL_NAME),
            ToolName::plain(antex_code_mode::WAIT_TOOL_NAME),
        ] {
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && antex_code_mode::is_code_mode_nested_tool(spec.name())
        && code_mode_tool_names
            .get(&antex_code_mode::normalize_code_mode_identifier(
                &antex_tools::code_mode_name_for_tool_name(tool_name),
            ))
            .is_some_and(|winner| winner == tool_name)
    {
        antex_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
    ...
    provider.uses_openai_actor_authorization()
        || (provider.requires_openai_auth
            && turn_context
                .auth_manager
                .as_deref()
                .is_some_and(AuthManager::current_auth_uses_antex_backend))
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure.is_available_in_code_mode()
        && antex_code_mode::is_code_mode_nested_tool(&antex_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn register_code_mode_executors(
    ...
    for tool in registry.entries() {
        ...
        if !antex_code_mode::is_code_mode_nested_tool(&code_mode_name) {
            continue;
        }
        match code_mode_tool_names.entry(antex_code_mode::normalize_code_mode_identifier(
            &code_mode_name,
        )) {
            Entry::Vacant(entry) => {
    ...
    let execute_handler = CodeModeExecuteHandler::new(
        create_code_mode_tool(
            ...
            if unified_image_budget_enabled(&turn_context.config.features, model_info) {
                antex_code_mode::ImageDetailVisibility::Hidden
            } else {
                antex_code_mode::ImageDetailVisibility::Visible
            },
        ),
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, antex_code_mode::ToolNamespaceDescription> {
    let mut namespace_descriptions = BTreeMap::new();
    for spec in specs {
        let ToolSpec::Namespace(namespace) = spec else {
            continue;
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| antex_code_mode::ToolNamespaceDescription {
...
fn compare_code_mode_tools(
    left: &antex_code_mode::ToolDefinition,
    right: &antex_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, antex_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &antex_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, antex_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
