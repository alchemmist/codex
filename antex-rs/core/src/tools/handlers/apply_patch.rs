...
use antex_apply_patch::ApplyPatchAction;
use antex_apply_patch::ApplyPatchFileChange;
use antex_apply_patch::ApplyPatchFileUpdateMode;
use antex_apply_patch::Hunk;
use antex_apply_patch::StreamingPatchParser;
...
use antex_protocol::protocol::PatchApplyUpdatedEvent;
use antex_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use antex_sandboxing::policy_transforms::merge_permission_profiles;
use antex_sandboxing::policy_transforms::normalize_additional_permissions;
use antex_sandboxing::policy_transforms::normalize_additional_permissions_with_context;
...
fn format_update_chunks_for_progress(chunks: &[antex_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    file_paths: &[PathUri],
    file_system_sandbox_policy: &antex_protocol::permissions::FileSystemSandboxPolicy,
...
async fn effective_patch_permissions(
    ...
    antex_protocol::permissions::FileSystemSandboxPolicy,
) {
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match antex_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match antex_apply_patch::verify_apply_patch_args_with_mode(
            args,
        ...
        .await
        {
            antex_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            antex_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            antex_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            antex_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    let sandbox = turn_environment.sandbox_context(/*additional_permissions*/ None);
    match antex_apply_patch::maybe_parse_apply_patch_verified_with_mode(
        command,
        cwd,
        apply_patch_file_update_mode(turn),
        fs,
    ...
    .await
    {
        ...
        antex_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
