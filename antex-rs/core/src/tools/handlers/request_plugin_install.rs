...
use antex_analytics::PluginInstallRequestSource;
use antex_analytics::PluginInstallRequested;
use antex_analytics::PluginInstallRequestedPlugin;
...
use antex_tools::DiscoverableToolAction;
use antex_tools::DiscoverableToolType;
use antex_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use antex_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use antex_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use antex_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use antex_tools::RequestPluginInstallArgs;
use antex_tools::RequestPluginInstallResult;
use antex_tools::ToolName;
use antex_tools::ToolSpec;
use antex_tools::all_requested_connectors_picked_up;
use antex_tools::build_request_plugin_install_elicitation_request;
use antex_tools::filter_request_plugin_install_discoverable_tools_for_client;
use antex_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("antex-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in antex-tui yet".to_string(),
            ));
        }
        ...
        let tool = if self.presentation == ToolSuggestPresentation::RecommendationContext {
            ...
            match antex_core_plugins::hydrate_selected_recommended_plugin_install_metadata(
                &plugins_config,
                auth.as_ref(),
                tool,
            )
...
async fn persist_disabled_install_request(
    codex_home: &antex_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    tool: &DiscoverableTool,
    auth: Option<&antex_login::AntexAuth>,
) -> bool {
    match tool {
