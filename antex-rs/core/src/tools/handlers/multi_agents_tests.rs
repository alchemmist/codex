...
use antex_protocol::protocol::FileSystemAccessMode;
use antex_protocol::protocol::FileSystemPath;
use antex_protocol::protocol::FileSystemSandboxEntry;
use antex_protocol::protocol::FileSystemSandboxPolicy;
use antex_protocol::protocol::InterAgentCommunication;
use antex_protocol::protocol::ItemCompletedEvent;
use antex_protocol::protocol::NetworkSandboxPolicy;
...
fn invocation(
    ...
    ToolInvocation {
        ...
        tool_name: antex_tools::ToolName::plain(tool_name),
...
async fn wait_for_recorded_user_input(thread: &crate::AntexThread, expected: &[UserInput]) {
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    antex_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_spawn_fork_turns_all_applies_agent_type_override() {
    ...
    let mut turn = turn;
    turn.config = Arc::new(config);
    turn.multi_agent_version = antex_protocol::protocol::MultiAgentVersion::V2;
...
async fn spawn_agent_reapplies_runtime_sandbox_after_role_config() {
    ...
    let sandbox_runtime = core_test_support::test_antex::test_antex()
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let thread = manager
        .resume_thread_with_history(
            ...
            AuthManager::from_auth_for_testing(AntexAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    config.antex_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);
