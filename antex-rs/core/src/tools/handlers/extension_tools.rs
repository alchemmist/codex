...
use antex_tools::ConversationHistory;
use antex_tools::ExtensionTurnItem;
use antex_tools::ResponsesApiNamespaceTool;
use antex_tools::ToolCall as ExtensionToolCall;
use antex_tools::ToolEnvironment;
use antex_tools::ToolName;
use antex_tools::ToolSearchInfo;
use antex_tools::ToolSpec;
use antex_tools::TurnItemEmissionFuture;
use antex_tools::TurnItemEmitter;
...
pub(crate) struct ExtensionToolAdapter(
    Arc<dyn for<'call> antex_tools::ToolExecutor<ExtensionToolCall<'call>>>,
);

impl ExtensionToolAdapter {
    pub(crate) fn new(
        executor: Arc<dyn for<'call> antex_tools::ToolExecutor<ExtensionToolCall<'call>>>,
    ) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall<'_> {
    ...
    let antex_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    use antex_extension_items::web_search::WebSearchItem;
    use antex_protocol::items::TurnItem;
    ...
    use antex_tools::ExtensionTurnItem;
    use antex_tools::ToolCallSource as ExtensionToolCallSource;
    ...
    impl<'call> antex_extension_api::ToolExecutor<antex_tools::ToolCall<'call>>
        for StubExtensionExecutor
    {
        fn tool_name(&self) -> antex_tools::ToolName {
            antex_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> antex_tools::ToolSpec {
            antex_tools::ToolSpec::Function(antex_tools::ResponsesApiTool {
                ...
                strict: true,
                parameters: antex_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle<'a>(
            &'a self,
            _call: antex_tools::ToolCall<'call>,
        ) -> antex_tools::ToolExecutorFuture<'a>
        where
            'call: 'a,
        {
            Box::pin(async {
                Ok(
                    Box::new(antex_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn antex_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<antex_tools::ToolCall<'static>>>>,
        ...
        for CapturingExtensionExecutor
    {
        ...
        fn handle<'a>(
            &'a self,
            call: antex_tools::ToolCall<'call>,
        ) -> antex_tools::ToolExecutorFuture<'a>
        where
            'call: 'a,
        {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: antex_tools::ToolCall<'_>,
        ) -> Result<Box<dyn antex_tools::ToolOutput>, antex_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: antex_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = antex_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        antex_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        antex_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
