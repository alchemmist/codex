...
pub fn sandbox_setup_is_complete(codex_home: &Path) -> bool {
    antex_windows_sandbox::sandbox_setup_is_complete(codex_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_antex_home: &Path) -> bool {
    false
}
...
pub fn prepare_elevated_sandbox(
    ...
    if !sandbox_setup_is_complete(codex_home) {
        let permissions =
            antex_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
                permission_profile,
                workspace_roots,
            )?;
        antex_windows_sandbox::run_elevated_setup(antex_windows_sandbox::SandboxSetupRequest {
    ...
    antex_windows_sandbox::run_setup_refresh(
        permission_profile,
        workspace_roots,
        command_cwd,
        env_map,
...
) -> std::io::Result<antex_windows_sandbox::WindowsSandboxProvisioningSettings> {
    let Some(network) = network.filter(|network| network.enabled()) else {
        return Ok(antex_windows_sandbox::WindowsSandboxProvisioningSettings::default());
    };
    Ok(antex_windows_sandbox::WindowsSandboxProvisioningSettings {
...
pub fn run_elevated_provisioning_setup(
    ...
    antex_windows_sandbox::run_elevated_provisioning_setup(
        codex_home,
        real_user,
...
pub fn run_legacy_setup_preflight(
    ...
    antex_windows_sandbox::run_windows_sandbox_legacy_preflight(
        permission_profile,
        workspace_roots,
        codex_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    antex_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        permission_profile,
        workspace_roots,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let setup_antex_home = codex_home.clone();
...
pub fn emit_windows_sandbox_setup_failure_metrics(
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        #[cfg(target_os = "windows")]
        {
            ...
            if let Some(failure) = antex_windows_sandbox::extract_setup_failure(_err) {
                code_tag = Some(failure.code.as_str().to_string());
                message_tag = Some(antex_windows_sandbox::sanitize_setup_metric_tag_value(
                    &failure.message,
                ));
            }
            ...
            let metric_name =
                if antex_windows_sandbox::extract_setup_failure(_err).is_some_and(|failure| {
                    matches!(
                        failure.code,
                        antex_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
                    )
                }) {
