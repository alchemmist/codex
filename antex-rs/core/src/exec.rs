...
use antex_protocol::protocol::ExecCommandOutputDeltaEvent;
use antex_protocol::protocol::ExecOutputStream;
use antex_sandboxing::SandboxCommand;
use antex_sandboxing::SandboxManager;
use antex_sandboxing::SandboxTransformRequest;
use antex_sandboxing::SandboxType;
use antex_sandboxing::SandboxablePreference;
use antex_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use antex_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use antex_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use antex_sandboxing::record_filesystem_sandbox_violation;
#[cfg(test)]
use antex_sandboxing::resolve_windows_elevated_filesystem_overrides;
#[cfg(test)]
use antex_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use antex_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
#[cfg(any(test, target_os = "windows"))]
use antex_sandboxing::windows_sandbox_uses_elevated_backend;
...
use antex_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: antex_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: antex_protocol::config_types::WindowsSandboxLevel,
...
pub fn build_exec_request(
    ...
    let request = manager
        .transform(SandboxTransformRequest {
            ...
            antex_linux_sandbox_exe: antex_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        antex_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = antex_otel::global() {
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_antex_home;
    use antex_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use antex_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            antex_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let codex_home = find_antex_home().map_err(|err| {
        CodexErr::Io(io::Error::other(format!(
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                antex_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        antex_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                antex_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
