...
fn sandbox_detection_ignores_network_policy_text_in_non_sandbox_mode() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"ANTEX_NETWORK_POLICY_DECISION {"decision":"ask","reason":"not_allowed","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
    assert!(!is_likely_sandbox_denied(SandboxType::None, &output));
}
...
fn sandbox_detection_ignores_network_policy_text_with_zero_exit_code() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"ANTEX_NETWORK_POLICY_DECISION {"decision":"ask","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
...
async fn exec_full_buffer_capture_ignores_expiration() -> Result<()> {
    ...
    let output = exec(
        ExecParams {
            command,
            cwd: antex_utils_absolute_path::AbsolutePathBuf::current_dir()?,
            expiration: 1.into(),
...
async fn process_exec_tool_call_preserves_full_buffer_capture_policy() -> Result<()> {
    ...
    let cwd = antex_utils_absolute_path::AbsolutePathBuf::current_dir()?;
...
fn windows_restricted_token_rejects_managed_root_write_profiles() {
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        antex_protocol::permissions::FileSystemSandboxEntry {
            path: antex_protocol::permissions::FileSystemPath::Special {
                value: antex_protocol::permissions::FileSystemSpecialPath::Root,
            },
            access: antex_protocol::permissions::FileSystemAccessMode::Write,
...
fn windows_elevated_allows_split_restricted_read_policies() {
    ...
    let docs = antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        antex_protocol::permissions::FileSystemSandboxEntry {
            path: docs.into(),
            access: antex_protocol::permissions::FileSystemAccessMode::Read,
            missing_path_behavior: None,
        },
    ]);
    let permission_profile = PermissionProfile::from_runtime_permissions(
...
fn windows_restricted_token_rejects_split_only_filesystem_policies() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        antex_protocol::permissions::FileSystemSandboxEntry {
            path: antex_protocol::permissions::FileSystemPath::Special {
                value: antex_protocol::permissions::FileSystemSpecialPath::project_roots(
                    /*subpath*/ None,
                ),
            },
            access: antex_protocol::permissions::FileSystemAccessMode::Write,
        ...
        antex_protocol::permissions::FileSystemSandboxEntry {
            path: antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs)
                .expect("absolute docs")
                .into(),
            access: antex_protocol::permissions::FileSystemAccessMode::Read,
            missing_path_behavior: None,
...
fn windows_restricted_token_supports_full_read_split_write_read_carveouts() {
    ...
    // `.antex`, so the restricted-token overlay only needs the extra read-only
    // docs carveout.
...
fn windows_elevated_supports_split_write_read_carveouts() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            additional_deny_read_paths: vec![],
            additional_deny_write_paths: vec![
                antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_docs)
                    .expect("absolute docs"),
            ],
        }))
    );
...
fn windows_workspace_defaults_do_not_hide_explicit_metadata_carveouts() {
    ...
    for name in antex_protocol::permissions::PROTECTED_METADATA_PATH_NAMES {
        ...
        explicit_policy
            .entries
            .push(antex_protocol::permissions::FileSystemSandboxEntry {
                path: antex_protocol::permissions::FileSystemPath::Special {
...
fn windows_elevated_supports_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        antex_protocol::permissions::FileSystemSandboxEntry {
            path: antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&blocked)
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
                    expected_blocked.clone(),
                )
                .expect("absolute blocked"),
            ],
            additional_deny_write_paths: vec![
                antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_blocked)
                    .expect("absolute blocked"),
            ],
        }))
    );
...
fn windows_elevated_supports_unreadable_globs() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(secret)
...
fn windows_elevated_rejects_reopened_writable_descendants() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        antex_protocol::permissions::FileSystemSandboxEntry {
            path: antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&nested)
...
fn process_exec_tool_call_uses_platform_sandbox_for_network_only_restrictions() {
    let expected = antex_sandboxing::get_platform_sandbox(/*windows_sandbox_enabled*/ false)
    ...
    assert_eq!(
        select_process_exec_tool_sandbox_type(
            ...
            antex_protocol::config_types::WindowsSandboxLevel::Disabled,
...
async fn kill_child_process_group_kills_grandchildren_on_timeout() -> Result<()> {
    ...
    let params = ExecParams {
        ...
        network_environment_id: None,
        sandbox_permissions: SandboxPermissions::UseDefault,
        windows_sandbox_level: antex_protocol::config_types::WindowsSandboxLevel::Disabled,
        windows_sandbox_private_desktop: false,
        justification: None,
        arg0: None,
    };
