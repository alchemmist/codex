...
use antex_analytics::CompactionImplementation;
use antex_analytics::CompactionPhase;
use antex_analytics::CompactionReason;
use antex_analytics::CompactionStrategy;
...
use crate::client::X_ANTEX_INSTALLATION_ID_HEADER;
use crate::client::X_ANTEX_PARENT_THREAD_ID_HEADER;
use crate::client::X_ANTEX_TURN_METADATA_HEADER;
use crate::client::X_ANTEX_WINDOW_ID_HEADER;
...
const RESERVED_METADATA_KEYS: &[&str] = &[
    antex_protocol::guardian_ticket::GUARDIAN_TICKET_METADATA_KEY,
...
pub(crate) enum AntexResponsesRequestKind {
    Turn,
    Prewarm,
...
impl AntexResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            AntexResponsesRequestKind::Turn => ("turn", None),
            AntexResponsesRequestKind::Prewarm => ("prewarm", None),
            AntexResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            AntexResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_thread_identity(self) -> bool {
        !matches!(self, AntexResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of Antex metadata sent to ResponsesAPI.
///
/// The full Antex turn metadata blob is transported canonically as
...
/// truth.
#[derive(Clone, Debug)]
pub struct AntexResponsesMetadata {
    /// Opaque runtime receipt; deliberately omitted from metadata projections.
    pub(crate) guardian_ticket: Option<antex_protocol::guardian_ticket::GuardianTicket>,
    ...
    pub(crate) request_kind: Option<AntexResponsesRequestKind>,
...
impl AntexResponsesMetadata {
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        let mut client_metadata = HashMap::from([
            (
                X_ANTEX_INSTALLATION_ID_HEADER.to_string(),
                self.installation_id.clone(),
            ),
            (SESSION_ID_KEY.to_string(), self.session_id.clone()),
            (THREAD_ID_KEY.to_string(), self.thread_id.clone()),
            (X_ANTEX_WINDOW_ID_HEADER.to_string(), self.window_id.clone()),
        ]);
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_ANTEX_PARENT_THREAD_ID_HEADER.to_string(),
        ...
        {
            client_metadata.insert(X_ANTEX_TURN_METADATA_HEADER.to_string(), turn_metadata_json);
        }
        client_metadata
    }
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        insert_header(&mut headers, X_ANTEX_WINDOW_ID_HEADER, &self.window_id);
        ...
        if self.has_turn_metadata()
            && let Ok(turn_metadata_json) = to_ascii_json_string(&AntexTurnMetadataPayload {
                tool_namespaces_info: None,
    ...
    fn turn_metadata_payload(&self) -> AntexTurnMetadataPayload<'_> {
        ...
        let has_thread_identity =
            request_kind.is_none_or(AntexResponsesRequestKind::has_thread_identity);
        let has_request_identity =
            request_kind.is_some_and(AntexResponsesRequestKind::has_thread_identity);
        AntexTurnMetadataPayload {
            ...
            compaction,
            // Extra metadata enriches the Antex turn metadata blob, not literal top-level
            // Responses client_metadata. Product metadata is validated while loading config;
            // app-server metadata has reserved Antex-owned keys filtered when it enters turn state.
            extra: &self.extra,
        }
    }
}
...
#[derive(Serialize)]
struct AntexTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]
