...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> AntexThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> AntexResult<String> {
        let state = self.upgrade()?;
    ...
    pub(crate) fn ensure_agent_known(&self, agent_id: ThreadId) -> AntexResult<AgentMetadata> {
        self.state
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> AntexResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
        let thread = state.get_thread(agent_id).await?;
    ...
    fn prepare_thread_spawn(
        ...
        agent_role: Option<String>,
        preferred_agent_nickname: Option<String>,
    ) -> AntexResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> AntexResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> AntexResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> AntexResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
    ...
    async fn persist_thread_spawn_edge_for_source(
        &self,
        child_thread: &crate::AntexThread,
        child_thread_id: ThreadId,
        session_source: Option<&SessionSource>,
    ) {
        ...
        if let Err(err) = agent_graph_store
            .upsert_thread_spawn_edge(
                parent_thread_id,
                child_thread_id,
                antex_agent_graph_store::ThreadSpawnEdgeStatus::Open,
            )
            .await
        {
