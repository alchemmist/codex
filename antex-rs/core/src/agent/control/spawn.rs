...
use antex_protocol::intersect_effective_permission_profiles;
...
) -> AntexResult<Option<Vec<RolloutItem>>> {
    match history_mode {
...
impl AgentControl {
    ...
    pub(crate) async fn restore_v2_agent_metadata(
        ...
        let descendant_ids = match agent_graph_store
            .list_thread_spawn_descendants(
                root_thread_id,
                Some(antex_agent_graph_store::ThreadSpawnEdgeStatus::Open),
            )
            .await
        {
    ...
    ) -> AntexResult<crate::thread_manager::NewThread> {
                ...
                RolloutItem::Compacted(compacted) => {
                    ...
                    compacted.retained_context =
                        (config.features.enabled(Feature::GuardianThreadContext)
                            && multi_agent_version == MultiAgentVersion::V2)
                            .then(antex_history::RetainedContext::default);
    ...
    async fn resume_single_agent_from_rollout(
        ...
        thread_id: ThreadId,
        session_source: SessionSource,
    ) -> AntexResult<(ThreadId, MultiAgentVersion)> {
        let state = self.upgrade()?;
        let stored_thread = state
            .read_stored_thread(ReadThreadParams {
                thread_id,
