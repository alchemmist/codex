...
use antex_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    ...
    async fn new_with_config(home: TempDir, config: Config) -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(antex_exec_server::EnvironmentManager::default_for_tests()),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<AntexThread>) {
        let new_thread = self
            .manager
    ...
    async fn start_paginated_thread(&self) -> (ThreadId, Arc<AntexThread>) {
        let new_thread = self
            .manager
...
async fn persisted_originator(thread: &AntexThread) -> String {
...
async fn wait_for_recorded_user_message(thread: &AntexThread, needle: &str) {
...
async fn wait_for_subagent_notification(parent_thread: &Arc<AntexThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<AntexThread>, message: &str) {
...
async fn spawn_agent_fork_from_paginated_parent_uses_model_context_prefix() {
    ...
    let lines = std::fs::read_to_string(&rollout_path)
        ...
        .lines()
        .map(|line| antex_rollout::parse_rollout_line(line).expect("parse rollout line"))
        .collect::<Vec<_>>();
...
async fn spawn_agent_without_fork_from_paginated_parent_stays_fresh_and_paginated() {
    ...
    child_thread
        ...
        .await
        .expect("child rollout should flush");
    let meta = antex_rollout::read_session_meta_line(
        &child_thread
            .rollout_path()
            .expect("child rollout should exist"),
    )
...
async fn spawn_agent_fork_strips_parent_usage_hints_from_compacted_history(
    ...
    let answer_event: antex_history::RetainedContextEvent = serde_json::from_value(serde_json::json!({
    ...
    let mut retained_context = antex_history::RetainedContext::default();
    ...
    parent_thread
        ...
        .persist_rollout_items(&[
            RolloutItem::Compacted(CompactedItem {
                ...
                guardian_history: Some(antex_history::GuardianHistoryCheckpoint(vec![
    ...
    let mut inherited_context = antex_history::RetainedContext::default();
...
async fn spawn_agent_full_fork_legacy_compaction_rebuilds_child_instructions_once() {
    ...
    for (case, parent_developer_instructions) in [
        ...
        parent_config.config_layer_stack = antex_config::ConfigLayerStack::new(
            parent_config
                .config_layer_stack
