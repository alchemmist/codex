...
use antex_protocol::items::DynamicToolCallStatus;
use antex_protocol::items::ImageGenerationItem;
...
#[test]
fn promotes_successful_antex_app_mcp_calls_and_splits_active_transcripts() {
    let mut state = started_state();
    observe_realtime(
        &mut state,
        RealtimeEvent::OutputTranscriptDelta(RealtimeTranscriptDelta {
