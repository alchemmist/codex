...
async fn project_layers_do_not_override_project_root_markers() {
    ...
    config.cwd = nested.abs();
    let project_layer = |dot_antex_folder: AbsolutePathBuf, marker: &str| {
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![
            project_layer(root.path().join(".antex").abs(), ".ignored-root-marker"),
            project_layer(config.cwd.join(".antex"), ".ignored-nested-marker"),
        ],
