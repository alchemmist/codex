...
use antex_config::types::AppConfig;
use antex_config::types::AppToolConfig;
use antex_config::types::AppToolsConfig;
use antex_config::types::ApprovalsReviewer;
use antex_config::types::AppsConfigToml;
...
fn approval_config(turn_context: &TurnContext) -> antex_mcp::McpConfig {
...
"#,
    ...
    let hook_list = antex_hooks::list_hooks(HooksConfig {
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_ANTEX_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        ...
        serde_json::json!({
            "callId": "call-custom",
            crate::X_ANTEX_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
async fn mcp_tool_call_request_meta_uses_the_issuing_step(
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            "callId": "call-b",
            crate::X_ANTEX_TURN_METADATA_HEADER: expected,
    ...
    assert_eq!(
        turn_context
            ...
            .to_responses_metadata(
                ...
                "window".to_string(),
                crate::responses_metadata::AntexResponsesRequestKind::Turn,
            )
            .turn_metadata_value()
...
async fn guardian_mcp_tool_call_request_meta_excludes_actor_confirmation_policy() {
    for session_source in [
        ...
        let expected = Some(serde_json::json!({
            ...
            crate::X_ANTEX_TURN_METADATA_HEADER: expected_mcp_turn_metadata(&turn_context),
        }));
...
async fn codex_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        request.id,
        antex_protocol::mcp::RequestId::String("codex_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        antex_protocol::approvals::ElicitationRequest::Url { .. }
    ));
...
#[tokio::test]
async fn persist_antex_app_tool_approval_writes_tool_override() {
    ...
    persist_antex_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_antex_dir = project_dir.path().join(".antex");
    std::fs::create_dir_all(&project_antex_dir).expect("create project .codex dir");
    std::fs::write(
        project_antex_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    let contents = std::fs::read_to_string(project_antex_dir.join(CONFIG_TOML_FILE))
