...
use antex_api::HostedFileUploadContext;
use antex_config::ConfigLayerSource;
use antex_config::types::AppToolApproval;
use antex_connectors::AppToolPolicy;
use antex_connectors::AppToolPolicyEvaluator;
use antex_connectors::AppToolPolicyInput;
use antex_extension_api::McpToolContext;
use antex_features::Feature;
use antex_mcp::CODEX_APPS_MCP_SERVER_NAME;
use antex_mcp::MCP_TOOL_CODEX_APPS_META_KEY;
use antex_mcp::McpPermissionPromptAutoApproveContext;
use antex_mcp::PreparedMcpCall;
use antex_mcp::SandboxState;
use antex_mcp::ToolInfo;
use antex_mcp::auth_elicitation_completed_result;
use antex_mcp::build_auth_elicitation_plan;
use antex_mcp::mcp_permission_prompt_is_auto_approved;
use antex_protocol::ResponseItemId;
use antex_protocol::approvals::ElicitationRequest;
use antex_protocol::items::McpToolCallError;
use antex_protocol::items::McpToolCallItem;
use antex_protocol::items::McpToolCallStatus;
use antex_protocol::items::TurnItem;
use antex_protocol::mcp::CONFIRMATION_POLICIES_META_KEY;
use antex_protocol::mcp::CallToolResult;
use antex_protocol::mcp::is_node_repl_backed_server;
use antex_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use antex_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use antex_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use antex_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use antex_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use antex_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use antex_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use antex_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use antex_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use antex_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use antex_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use antex_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use antex_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use antex_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use antex_protocol::request_user_input::RequestUserInputAnswer;
use antex_protocol::request_user_input::RequestUserInputArgs;
use antex_protocol::request_user_input::RequestUserInputQuestion;
use antex_protocol::request_user_input::RequestUserInputQuestionOption;
...
use antex_utils_output_truncation::TruncationPolicy;
use antex_utils_output_truncation::truncate_text;
...
async fn handle_approved_mcp_tool_call(
    ...
    .await;
    maybe_track_antex_app_used(sess, turn_context, &server, &metadata).await;
...
async fn maybe_request_codex_apps_auth_elicitation(
    ...
    let install_url = connector_id.map(|connector_id| {
        antex_connectors::metadata::connector_install_url(
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        antex_linux_sandbox_exe: prepared_call.config().antex_linux_sandbox_exe.clone(),
        sandbox_cwd,
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                antex_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == antex_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn maybe_track_antex_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    metadata: &McpToolApprovalMetadata,
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, antex_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_ANTEX_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }
...
async fn maybe_request_mcp_tool_approval(
    ...
    Some(
        match sess.request_approval(action, approval_context).await {
            ...
            Err(ToolError::Antex(_)) => ReviewDecision::Abort,
        },
    )
}
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == CODEX_APPS_MCP_SERVER_NAME {
        ...
        persist_antex_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_antex_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, antex_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
