...
use antex_analytics::TurnProfileFact;
use antex_analytics::TurnTokenUsageFact;
use antex_context_fragments::RenderedFragment;
use antex_otel::SessionTelemetry;
use antex_otel::TURN_E2E_DURATION_METRIC;
use antex_otel::TURN_MEMORY_METRIC;
use antex_otel::TURN_NETWORK_PROXY_METRIC;
use antex_otel::TURN_TOKEN_USAGE_METRIC;
use antex_otel::TURN_TOOL_CALL_METRIC;
use antex_otel::TURN_UNIFIED_EXEC_RUNNING_PROCESSES_METRIC;
...
pub(crate) type SessionTaskResult = AntexResult<Option<String>>;
...
///
/// Implementations encapsulate a specific Antex workflow (regular chat,
...
impl Session {
    ...
    pub(crate) fn has_outstanding_durable_sleep(&self) -> bool {
        self.services
            ...
            .get::<antex_extension_items::sleep::SleepItem>()
            .is_some()
    }
        ...
        let (last_agent_message, abort_reason) = match task_result {
            ...
            Err(err) => {
                ...
                self.emit_turn_error_lifecycle(
                    turn_context.as_ref(),
                    err.to_antex_protocol_error(),
                )
                .await;
                self.track_turn_antex_error(turn_context.as_ref(), &err);
                self.send_event(
                    turn_context.as_ref(),
