...
//!
//! `ModelClient` is intended to live for the lifetime of a Antex session and holds the stable
...
use antex_api::AgentIdentityTelemetry;
use antex_api::ApiError;
use antex_api::AuthProvider;
use antex_api::CompactClient as ApiCompactClient;
use antex_api::CompactionInput as ApiCompactionInput;
use antex_api::Compression;
use antex_api::MemoriesClient as ApiMemoriesClient;
use antex_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use antex_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use antex_api::Provider as ApiProvider;
use antex_api::RawMemory as ApiRawMemory;
use antex_api::RealtimeCallClient as ApiRealtimeCallClient;
use antex_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use antex_api::Reasoning;
use antex_api::ReasoningContext;
use antex_api::RequestTelemetry;
use antex_api::ReqwestTransport;
use antex_api::ResponseCreateWsRequest;
use antex_api::ResponsesApiRequest;
use antex_api::ResponsesClient as ApiResponsesClient;
use antex_api::ResponsesEndpoint;
use antex_api::ResponsesOptions as ApiResponsesOptions;
use antex_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use antex_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use antex_api::ResponsesWsRequest;
use antex_api::SharedAuthProvider;
use antex_api::SseTelemetry;
use antex_api::StreamOptions;
use antex_api::TransportError;
use antex_api::WebsocketTelemetry;
use antex_api::auth_header_telemetry;
use antex_api::build_session_headers;
use antex_api::create_text_param_for_request;
use antex_api::response_create_client_metadata;
...
use antex_login::RefreshTokenError;
use antex_login::UnauthorizedRecovery;
use antex_login::default_client::add_originator_header;
use antex_login::default_client::create_client_for_route;
use antex_otel::SessionTelemetry;
use antex_otel::current_span_w3c_trace_context;
use antex_protocol::ResponseItemId;
use antex_protocol::auth::AuthMode;
use async_channel::Sender;

use antex_protocol::ThreadId;
use antex_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use antex_protocol::config_types::Verbosity as VerbosityConfig;
...
use antex_protocol::protocol::Event as ProtocolEvent;
...
use antex_rollout_trace::CompactionTraceContext;
use antex_rollout_trace::InferenceTraceAttempt;
use antex_rollout_trace::InferenceTraceContext;
use antex_tools::create_tools_json_for_responses_api;
use antex_tools::create_tools_json_for_responses_lite;
use antex_tools::create_tools_raw_json_for_responses_api;
...
use crate::responses_metadata::AntexResponsesMetadata;
use crate::responses_metadata::subagent_header_value;
use crate::util::emit_feedback_auth_recovery_tags;
use antex_feedback::FeedbackRequestTags;
use antex_feedback::emit_feedback_request_tags_with_auth_env;
use antex_login::auth::AgentIdentityAuthPolicy;
use antex_login::auth_env_telemetry::AuthEnvTelemetry;
use antex_login::auth_env_telemetry::collect_auth_env_telemetry;
use antex_model_provider::AgentIdentitySessionFallback;
use antex_model_provider::ProviderAuthScope;
use antex_model_provider::ProviderUnauthorizedRecovery;
use antex_model_provider::SharedModelProvider;
use antex_model_provider::create_model_provider;
#[cfg(test)]
use antex_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
use antex_model_provider_info::ModelProviderInfo;
use antex_model_provider_info::WireApi;
use antex_protocol::error::CodexErr;
use antex_protocol::error::Result;
use antex_response_debug_context::extract_response_debug_context;
use antex_response_debug_context::extract_response_debug_context_from_api_error;
use antex_response_debug_context::telemetry_api_error_message;
use antex_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_ANTEX_INSTALLATION_ID_HEADER: &str = "x-codex-installation-id";
pub const X_ANTEX_ROUTING_HINT_HEADER: &str = "x-codex-routing-hint";
pub const X_ANTEX_TURN_STATE_HEADER: &str = "x-codex-turn-state";
pub const X_ANTEX_TURN_METADATA_HEADER: &str = "x-codex-turn-metadata";
pub const X_ANTEX_PARENT_THREAD_ID_HEADER: &str = "x-codex-parent-thread-id";
pub const X_ANTEX_WINDOW_ID_HEADER: &str = "x-codex-window-id";
...
const X_ANTEX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-codex-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_codex_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_ANTEX_RESPONSES_LITE_HEADER: &str =
...
///
/// This holds configuration and state that should be shared across turns within a Antex session
...
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Antex turn. Reusing it across turns would replay
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Antex session. Per-turn values
    ...
    pub fn new(
        ...
        let antex_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.antex_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), antex_api_key_env_enabled);
    ...
    fn prompt_cache_key(&self, responses_metadata: &AntexResponsesMetadata) -> String {
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &AntexResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(AntexAuth::auth_mode),
        ...
        if let Ok(header_value) = HeaderValue::from_str(&responses_metadata.installation_id) {
            extra_headers.insert(X_ANTEX_INSTALLATION_ID_HEADER, header_value);
        }
        ...
        if let Some(header_value) = self.build_routing_hint_header(
            ...
            extra_headers.insert(X_ANTEX_ROUTING_HINT_HEADER, header_value);
        }
    ...
    fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: antex_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    fn responses_endpoint(&self, auth: Option<&AntexAuth>, model: &str) -> ResponsesEndpoint {
        if self.free_guardian_enabled
            && crate::guardian::is_basic_session_source(&self.state.session_source)
            && self.uses_antex_backend(auth)
            && self.state.provider.info().supports_antex_backend_routes()
    ...
    fn uses_antex_backend(&self, auth: Option<&AntexAuth>) -> bool {
        let provider = self.state.provider.info();
        auth.is_some_and(AntexAuth::uses_antex_backend)
            && provider.is_openai()
    ...
    fn set_guardian_ticket_request(
        ...
        if self.free_guardian_enabled
            ...
            && matches!(
                auth,
                Some(AntexAuth::Chatgpt(_) | AntexAuth::ChatgptAuthTokens(_))
            )
            && self.uses_antex_backend(auth)
            && self.state.provider.info().supports_antex_backend_routes()
        {
    ...
    fn build_routing_hint_header(
        ...
        if !self.uses_antex_backend(auth) {
            return None;
        }
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: antex_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &AntexResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth)
                ...
                .connect(
                    &self.http_client_factory,
                    headers,
                    antex_login::default_client::default_headers(),
                    /*turn_state*/ None,
    ...
    async fn build_websocket_headers(
        ...
        if let Some(routing_hint) = &responses_metadata.routing_hint {
            headers.insert(X_ANTEX_ROUTING_HINT_HEADER, routing_hint.clone());
        }
        if let Some(header_value) = self.generate_attestation_header_for().await {
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&AntexAuth>) -> Compression {
    ...
    async fn stream_responses_api(
        ...
        loop {
                ...
                )
            {
                options
                    .extra_headers
                    .insert(X_ANTEX_ROUTING_HINT_HEADER, header_value);
            }
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            if let Some(turn_state) = self.turn_state.get() {
                client_metadata.insert(X_ANTEX_TURN_STATE_HEADER.to_string(), turn_state.clone());
            }
            match self
    ...
    /// Permanently disables WebSockets for this Antex session and resets WebSocket state.
    ///
...
fn stamp_ws_stream_request_start_ms(request: &mut ResponsesWsRequest<'_>) {
    ...
    payload
        ...
        .get_or_insert_with(HashMap::new)
        .insert(
            X_ANTEX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY.to_string(),
...
///
/// These headers implement Antex-specific conventions:
///
...
fn build_responses_headers(
    ...
    {
        headers.insert(X_ANTEX_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn add_responses_lite_header(headers: &mut ApiHeaderMap, use_responses_lite: bool) {
    if use_responses_lite {
        headers.insert(
            X_OPENAI_INTERNAL_ANTEX_RESPONSES_LITE_HEADER,
            HeaderValue::from_static("true"),
        );
    }
}
...
fn map_response_stream(
    api_stream: antex_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let antex_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = antex_api::ResponseStream {
        rx_event,
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: antex_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a AntexResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
    endpoint: ResponsesEndpoint,
}
