...
use antex_git_utils::get_git_remote_urls_assume_git_repo;
use antex_git_utils::get_has_changes_in_repo;
use antex_git_utils::get_head_commit_hash;
...
const ANTEX_VERSION_KEY: &str = "codex_version";
...
pub async fn detached_memory_responses_metadata(
    ...
    let mut metadata = AntexResponsesMetadata {
        turn_id: Some(turn_id.clone()),
        root_turn_id: Some(turn_id),
        request_kind: Some(AntexResponsesRequestKind::Memory),
        ...
        .await,
        ..AntexResponsesMetadata::new(installation_id, session_id, thread_id, window_id)
    };
...
impl antex_analytics::TurnAnalyticsMetadata for TurnMetadataState {
...
impl TurnMetadataState {
    ...
    fn responses_metadata_template(&self) -> AntexResponsesMetadata {
    ...
    fn mcp_metadata_template(&self) -> AntexResponsesMetadata {
        let mut extra = self
            .responsesapi_client_metadata
            .read()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
