...
use antex_analytics::TurnAnalyticsMetadata;
...
use antex_sandboxing::get_platform_sandbox;
...
fn test_turn_responses_metadata_json(state: &TurnMetadataState, window_id: &str) -> String {
    test_responses_metadata_json(state, window_id, AntexResponsesRequestKind::Turn)
}
...
fn test_compaction_responses_metadata_json(
    ...
    test_responses_metadata_json(
        state,
        window_id,
        AntexResponsesRequestKind::Compaction(compaction),
    )
}
...
async fn detached_memory_responses_metadata_starts_an_independent_root_turn() {
    ...
    let thread_manager = crate::ThreadManager::with_models_provider_for_tests(
        antex_login::AntexAuth::from_api_key("test"),
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    assert_eq!(
        (state.turn_trigger(), state.antex_turn_source()),
        (
            Some("goal".to_string()),
    ...
    for (configured, client, expected) in [
        ...
        assert_eq!(state.antex_turn_source(), expected);
    }
}
