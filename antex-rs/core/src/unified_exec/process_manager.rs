...
use crate::antex_thread::BackgroundTerminalInfo;
use crate::exec_env::ANTEX_PERMISSION_PROFILE_ENV_VAR;
use crate::exec_env::ANTEX_THREAD_ID_ENV_VAR;
use crate::exec_env::ANTEX_VERSION_ENV_VAR;
...
use antex_core_plugins::PLUGIN_METRICS_OUTPUT_ENV_VAR;
use antex_core_plugins::PluginCommandAttribution;
use antex_core_plugins::PluginMetricsSidecar;
use antex_core_plugins::strip_output_env;
use antex_network_proxy::NetworkPolicyDecider;
...
use antex_protocol::protocol::TerminalInteractionEvent;
use antex_protocol::shell_environment::is_non_inheritable_env_var;
use antex_sandboxing::SandboxCommand;
use antex_shell_command::is_dangerous_command::DangerousCommandPlatform;
...
pub(super) fn exec_env_policy_from_shell_policy(
...
) -> antex_exec_server::ExecEnvPolicy {
    ...
    exclude.extend([
        ...
        antex_apply_patch::ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR.to_string(),
        PLUGIN_METRICS_OUTPUT_ENV_VAR.to_string(),
    ]);
    let mut r#set = policy.r#set.clone();
    r#set.retain(|key, _| {
        ![
            ANTEX_PERMISSION_PROFILE_ENV_VAR,
            ANTEX_VERSION_ENV_VAR,
            antex_apply_patch::ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR,
...
fn exec_server_env_for_request(
    ...
    Option<antex_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    antex_exec_server::ExecParams {
        ...
        metadata: tool_ctx.map(|ctx| antex_exec_server::ExecMetadata {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Antex(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        network_proxy_launch: Option<antex_network_proxy::RemoteNetworkProxyLaunchConfig>,
        environment_id: Option<&str>,
        exec_server_env_config: Option<ExecServerEnvConfig>,
        shell_snapshot: Option<antex_exec_server::ShellSnapshotRequest>,
        windows_sandbox_proxy_settings_mode: antex_sandboxing::WindowsSandboxProxySettingsMode,
        tty: bool,
        ...
        .map_err(|err| match err {
            UnifiedExecError::SandboxDenied { output, .. } => {
                ToolError::Antex(CodexErr::Sandbox(SandboxErr::Denied {
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        let network_proxy_restricting_sid = {
            ...
            {
                if request.sandbox == antex_sandboxing::SandboxType::WindowsRestrictedToken {
                    request
                        .network
                        .as_ref()
                        .map(|network| {
        ...
        let windows_sandbox =
            if request.sandbox == antex_sandboxing::SandboxType::WindowsRestrictedToken {
                Some(antex_sandboxing::WindowsSandboxSpawnRequest {
        ...
        let spawn_result = antex_sandboxing::spawn_process(antex_sandboxing::SpawnRequest {
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            ANTEX_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Antex(err) => match err.details() {
