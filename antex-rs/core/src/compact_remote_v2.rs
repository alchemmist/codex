...
use antex_context_fragments::set_annotated_content;
use antex_context_fragments::to_annotated_content;
...
use antex_protocol::items::ContextCompactionItem;
...
use antex_rollout_trace::CompactionCheckpointTracePayload;
use antex_rollout_trace::InferenceTraceContext;
use antex_utils_output_truncation::approx_token_count;
...
async fn run_remote_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    antex_analytics::CompactionStatus::Interrupted,
                    Some(&error),
                    analytics_details,
                )
                .await;
    ...
    let antex_error = result.as_ref().err();
    if result.is_ok() {
        let post_compact_outcome = run_post_compact_hooks(sess, turn_context, trigger).await;
        if let PostCompactHookOutcome::Stopped = post_compact_outcome {
            attempt
                .track(sess.as_ref(), status, antex_error, analytics_details)
                .await;
    ...
    match result {
        ...
        Err(err) => {
            sess.track_turn_antex_error(turn_context, &err);
...
) -> AntexResult<RemoteCompactionV2Output> {
    let turn_context = &step_context.turn;
...
fn build_v2_compacted_history(
    ...
    prompt_input_metadata: Vec<Option<AntexHarnessMetadata>>,
    compaction_output: ResponseItem,
...
mod tests {
    ...
    fn response_stream(events: Vec<AntexResult<ResponseEvent>>) -> ResponseStream {
    ...
    fn retained_history_truncation_preserves_metadata() {
        ...
        assert_eq!(truncated[0].metadata, Some(AntexHarnessMetadata::default()));
    }
    ...
    async fn collect_compaction_output_accepts_additional_output_items() {
        ...
        let stream = response_stream(vec![
            ...
            Ok(ResponseEvent::Completed {
                ...
                usage_metadata: Some(antex_protocol::ResponseUsageMetadata {
