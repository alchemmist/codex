...
///
/// 1. The process was spawned by Antex as part of a shell tool call.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    let inherited_fd = env
        .get(antex_shell_escalation::ESCALATE_SOCKET_ENV_VAR)
    ...
    // If this Antex process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            if detach_from_tty {
                antex_utils_pty::process_group::detach_from_tty()?;
            }
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                antex_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            // macOS cannot receive the fd with close-on-exec set atomically.
            #[cfg(target_os = "macos")]
            antex_utils_pty::pty::close_inherited_fds_except(inherited_fd.as_slice());
            Ok(())
        });
    }
