...
use crate::antex_thread::AntexThread;
...
use antex_agent_graph_store::AgentGraphStore;
use antex_agent_graph_store::LocalAgentGraphStore;
...
use antex_attachment_store::AttachmentStore;
use antex_attachment_store::InlineAttachmentStore;
use antex_code_mode::CodeModeSessionProvider;
use antex_code_mode::DisabledCodeModeSessionProvider;
use antex_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use antex_protocol::mcp::OPENAI_STANDARD_FORM_INPUT_EXTENSION_ID;
...
use antex_thread_store::InMemoryThreadStore;
use antex_thread_store::LoadThreadHistoryParams;
use antex_thread_store::LocalThreadStore;
use antex_thread_store::LocalThreadStoreConfig;
use antex_thread_store::MoveThreadToSectionParams;
use antex_thread_store::PreparedFork;
use antex_thread_store::ReadThreadByRolloutPathParams;
use antex_thread_store::ReadThreadParams;
use antex_thread_store::StoredModelContext;
use antex_thread_store::StoredThread;
use antex_thread_store::ThreadMetadataPatch;
use antex_thread_store::ThreadStore;
use antex_thread_store::ThreadStoreError;
use antex_thread_store::UpdateThreadMetadataParams;
...
struct TempAntexHomeGuard {
    path: PathBuf,
}

impl Drop for TempAntexHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created Antex thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<AntexThread>,
...
pub struct ThreadManager {
    ...
    _test_antex_home_guard: Option<TempAntexHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<antex_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<AntexThread>>>>,
            ...
            if has_state_db && background_migration_enabled {
                ...
                tokio::spawn(async move {
                    ...
                    if compression_enabled {
                        antex_rollout::spawn_rollout_compression_worker(codex_home);
                    }
                });
            } else if compression_enabled {
                antex_rollout::spawn_rollout_compression_worker(config.codex_home.to_path_buf());
            }
            store
        }
...
impl ThreadManager {
    ...
    /// Construct with a dummy AuthManager containing the provided AntexAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        manager._test_antex_home_guard = Some(TempAntexHomeGuard { path: codex_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided AntexAuth and codex home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let absolute_antex_home = match AbsolutePathBuf::from_absolute_path_checked(&codex_home) {
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite: antex_state::SqliteConfig::new_for_testing(absolute_antex_home),
    ...
    pub async fn list_models(
        ...
        http_client_factory: antex_http_client::HttpClientFactory,
    ) -> Vec<ModelPreset> {
        self.state
            .models_manager
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> AntexResult<Arc<AntexThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `AntexThread`/`LiveThread`, so metadata changes stay ordered
    ...
    pub async fn start_thread(&self, options: StartThreadOptions) -> AntexResult<NewThread> {
    ...
    /// as `Arc<AntexThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<AntexThread>> {
    ...
    pub async fn remove_thread_if_matches(
        &self,
        thread_id: &ThreadId,
        expected: &Arc<AntexThread>,
    ) -> Option<Arc<AntexThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> AntexResult<Arc<AntexThread>> {
    ...
    ) -> AntexResult<StoredModelContext> {
        let thread_id = params.thread_id;
        self.thread_store
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<AntexThread>> {
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(ANTEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    async fn spawn_thread(&self, request: ThreadSpawnRequest) -> AntexResult<NewThread> {
    ...
    async fn finalize_thread_spawn(
...

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(AntexThread::new(
                    session,
                    io,
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> antex_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return antex_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(antex_rollout_trace::ThreadTraceContext::disabled)
    }
}
