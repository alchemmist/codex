...
pub use antex_prompts::SUMMARIZATION_PROMPT;
pub use antex_prompts::SUMMARY_PREFIX;
...
async fn run_compact_task_inner_impl(
    ...
    let responses_metadata = sess
        .responses_metadata(
            turn_context.as_ref(),
            AntexResponsesRequestKind::Compaction(compaction_metadata),
        )
        .await;
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        sess.services
            ...
            .track_compaction(AntexCompactionEvent {
                ...
                status,
                antex_error_kind: antex_error.map(Into::into),
                antex_error_http_status_code: antex_error
...
pub(crate) fn compaction_status_from_result<T>(result: &AntexResult<T>) -> CompactionStatus {
    match result {
...
pub(crate) struct CompactedUserMessage {
    ...
    harness_metadata: Option<AntexHarnessMetadata>,
}
