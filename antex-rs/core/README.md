...
`bazel test //antex-rs/core:core-all-wine-exec-test`.
...
Note that `antex-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.antex` read-only.
...
Expects the binary containing `antex-core` to run the equivalent of `codex sandbox` when `arg0` is `antex-linux-sandbox`. See the `antex-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `antex-resources/bwrap`
...
Expects the binary containing `antex-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--antex-run-as-apply-patch`. See the
`antex-arg0` crate for details.
