//! Proxy-aware WebSocket connection setup shared by Antex API clients.
...
use antex_http_client::BuildCustomCaTransportError;
use antex_http_client::HttpClientFactory;
use antex_http_client::OutboundProxyRoute;
use antex_http_client::build_rustls_client_config_with_custom_ca;
...
/// Selects whether WebSocket TLS follows Antex custom-CA policy or Tungstenite defaults.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum WebSocketTlsMode {
    /// Build an explicit TLS configuration from native roots and configured Antex custom CAs.
    ExplicitAntexTls,
...
impl WebSocketConnector {
    /// Creates a connector using native roots and any configured Antex custom CA bundle.
    pub fn new(
        http_client_factory: &HttpClientFactory,
    ) -> Result<Self, BuildCustomCaTransportError> {
        Self::new_with_tls_mode(http_client_factory, WebSocketTlsMode::ExplicitAntexTls)
    }

    /// Creates a connector with explicit Antex TLS or the transport's existing TLS defaults.
    ///
    /// HTTPS proxy connections still build Antex TLS configuration when they establish their
    ...
    pub fn new_with_tls_mode(
        ...
        let tls_config = match tls_mode {
            WebSocketTlsMode::ExplicitAntexTls => {
