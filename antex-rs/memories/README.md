...
Runtime orchestration for Phase 1 and Phase 2 still lives in `antex-core` under
`antex-rs/core/src/memories/`.

## Crates

- `antex-rs/memories/read` (`antex-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `antex-rs/memories/write` (`antex-memories-write`) owns the write path:
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.antex/memories/.git` by `antex-git-utils`
