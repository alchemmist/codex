...
use antex_git_utils::diff_since_latest_init;
use antex_git_utils::reset_git_repository;
...
use antex_rollout::RolloutLine;
use antex_state::Phase2JobClaimOutcome;
...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_antex(&server, home.clone()).await?;
    trigger_memories_startup(&test).await;
...
-> anyhow::Result<()> {
    ...
    core_test_support::submit_thread_settings(
        ...
        antex_protocol::protocol::ThreadSettingsOverrides {
            ...
            permission_profile: Some(antex_protocol::models::PermissionProfile::workspace_write()),
            ..Default::default()
        },
    )
    .await?;
...
async fn run_memory_phase_one_model_request_test(
    ...
    let test = build_test_antex_with_memories_config(server, Arc::clone(&home), memories).await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let test = build_test_antex_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
...
async fn build_test_antex(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
) -> anyhow::Result<TestAntex> {
    build_test_antex_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_antex_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestAntex> {
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<antex_state::StateRuntime>> {
    let db = antex_state::StateRuntime::init(
        antex_state::SqliteConfig::new_for_testing(home.path().abs()),
        "test-provider".into(),
    )
    .await?;
...
async fn trigger_memories_startup(test: &TestAntex) {
    let config_snapshot = test.antex.config_snapshot().await;
...
) -> (Arc<MemoryStartupContext>, Arc<antex_core::config::Config>) {
    let config_snapshot = test.antex.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> antex_models_manager::manager::SharedModelsManager {
        self.delegate
...
) -> anyhow::Result<antex_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.antex.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        antex_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_antex(test: &TestAntex) -> anyhow::Result<()> {
    test.antex.submit(Op::Shutdown {}).await?;
    wait_for_event(&test.antex, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    Ok(())
}
