...
use antex_core::content_items_to_text;
use antex_core::detached_memory_responses_metadata;
...
pub(crate) struct SpawnedConsolidationAgent {
    ...
    pub(crate) thread: Arc<AntexThread>,
}
...
impl StageOneRequestContext {
    pub(crate) fn start_timer(&self, name: &str) -> Option<antex_otel::Timer> {
...
fn build_session_telemetry(
    ...
    let auth_mode = auth.map(AntexAuth::auth_mode).map(TelemetryAuthMode::from);
    let account_id = auth.and_then(AntexAuth::get_account_id);
    let account_email = auth.and_then(AntexAuth::get_account_email);
        ...
        while let Some(message) = stream.next().await.transpose()? {
            match message {
                ...
                ResponseEvent::OutputItemDone(item) => {
                    if result.is_empty()
                        && let antex_protocol::models::ResponseItem::Message { content, .. } = item
