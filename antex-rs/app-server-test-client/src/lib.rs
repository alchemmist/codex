...
use antex_app_server_protocol::JSONRPCMessage;
use antex_app_server_protocol::JSONRPCNotification;
use antex_app_server_protocol::JSONRPCRequest;
use antex_app_server_protocol::JSONRPCResponse;
...
use antex_otel::OtelProvider;
use antex_otel::current_span_w3c_trace_context;
use antex_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "antex-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $ANTEX_HOME/config.toml to get a trace URL!";
...
struct Cli {
    ...
    /// spawning `antex app-server` as a child process.
    #[arg(long, env = "ANTEX_BIN", global = true)]
    antex_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--codex-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "ANTEX_APP_SERVER_URL", global = true)]
    url: Option<String>,
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `antex app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/antex-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `antex app-server --listen`.
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a Antex-managed Amazon Bedrock API key.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "ANTEX_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let antex_bin = antex_bin.unwrap_or_else(|| PathBuf::from("antex"));
            serve(&antex_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(antex_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(antex_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(antex_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --antex-bin and does not support --url");
            }
            let antex_bin = antex_bin.context("plugin-analytics-smoke requires --antex-bin")?;
            plugin_analytics_smoke::run(&antex_bin, &config_overrides, &plugin_id, capture_file)
        }
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --antex-bin and does not support --url"
                );
            }
            let antex_bin =
                antex_bin.context("plugin-analytics-mutation-smoke requires --antex-bin")?;
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!("plugin-remote-uninstall requires --antex-bin and does not support --url");
            }
            let antex_bin = antex_bin.context("plugin-remote-uninstall requires --antex-bin")?;
...
fn resolve_endpoint(antex_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if antex_bin.is_some() && url.is_some() {
        bail!("--antex-bin and --url are mutually exclusive");
    }
    if let Some(antex_bin) = antex_bin {
        return Ok(Endpoint::SpawnAntex(antex_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if antex_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --antex-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(antex_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(antex_bin);
        if let Some(antex_bin_parent) = antex_bin.parent() {
            let mut path = OsString::from(antex_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", antex_bin.display()))?;
...
fn serve(antex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/antex-app-server-test-client");
    ...
    let mut cmdline = format!(
        ...
        shell_quote(&antex_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started antex app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnAntex(antex_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: antex_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut AntexClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = AntexClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = AntexClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (antex_bin, url) {
        (Some(_), Some(_)) => bail!("--antex-bin and --url are mutually exclusive"),
        (Some(antex_bin), None) => {
            let server = BackgroundAppServer::spawn(&antex_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve antex-app-server-test-client binary path")?;
...
impl AntexClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnAntex(antex_bin) => Self::spawn_stdio(antex_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(antex_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(antex_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let antex_bin_display = antex_bin.display();
        let mut cmd = Command::new(antex_bin);
        if let Some(antex_bin_parent) = antex_bin.parent() {
            let mut path = OsString::from(antex_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut antex_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{antex_bin_display}` app-server"))?;

        let stdin = antex_app_server
            .stdin
            .take()
            .context("antex app-server stdin unavailable")?;
        let stdout = antex_app_server
            .stdout
            .take()
            .context("antex app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: antex_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `antex-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: antex_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
                antex_streamlined_login: false,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: antex_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to antex app-server")?;
                    return Ok(());
                }
                bail!("antex app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from antex app-server")?;
                if bytes == 0 {
                    bail!("antex app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = antex_core::otel_init::build_provider(
            &config,
...
impl Drop for AntexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[antex app-server exited: {status}]");
            return;
        }
