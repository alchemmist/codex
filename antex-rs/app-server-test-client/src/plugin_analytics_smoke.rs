...
pub(super) const ANALYTICS_CAPTURE_ENV_VAR: &str = "ANTEX_ANALYTICS_EVENTS_CAPTURE_FILE";
const TEST_USER_CONFIG_ENV_VAR: &str = "ANTEX_APP_SERVER_TEST_USER_CONFIG_FILE";
...
pub(super) fn run(
    ...
    let mut client = AntexClient::spawn_stdio_with_env(antex_bin, &overrides, &child_environment)?;
...
fn run_plugin_turn(client: &mut AntexClient, expected: &ExpectedPlugin) -> Result<String> {
...
fn plugin_installed(client: &mut AntexClient) -> Result<PluginInstalledResponse> {
