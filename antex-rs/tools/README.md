...
`antex-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `antex-core`.
...
compatibility-sensitive orchestration in `antex-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `antex-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `antex-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `antex-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `antex-core`.
3. Leave compatibility-sensitive adapters in `antex-core` while downstream
...
`antex-core`, that is a sign to revisit the extraction boundary before adding
more here.
