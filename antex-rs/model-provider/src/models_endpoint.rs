...
use antex_login::AuthEnvTelemetry;
use antex_login::AuthManager;
use antex_login::collect_auth_env_telemetry;
use antex_login::default_client::create_client_for_route_async;
use antex_model_provider_info::ModelProviderInfo;
use antex_models_manager::manager::ModelsEndpointClient;
use antex_models_manager::manager::ModelsEndpointFuture;
use antex_otel::TelemetryAuthMode;
use antex_protocol::error::CodexErr;
use antex_protocol::error::Result as CoreResult;
...
impl OpenAiModelsEndpoint {
    ...
    async fn auth(&self) -> Option<AntexAuth> {
    ...
    async fn uses_antex_backend(&self) -> bool {
        self.auth()
            .await
            .as_ref()
            .is_some_and(AntexAuth::uses_antex_backend)
    }
    ...
    async fn list_models(
        ...
        let _timer =
            antex_otel::start_global_timer("codex.remote_models.fetch_update.duration_ms", &[]);
        let auth = self.auth().await;
        let auth_mode = auth.as_ref().map(AntexAuth::auth_mode);
        ...
        let agent_identity_telemetry = if let Some(AntexAuth::AgentIdentity(auth)) = auth.as_ref() {
    ...
    fn auth_env(&self) -> AuthEnvTelemetry {
        let antex_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.antex_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, antex_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    fn uses_antex_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(OpenAiModelsEndpoint::uses_antex_backend(self))
    }
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            ...
            auth.env_antex_api_key_present = self.auth_env.antex_api_key_env_present,
            auth.env_antex_api_key_enabled = self.auth_env.antex_api_key_env_enabled,
...
mod tests {
    ...
    use antex_login::default_client::create_client;
