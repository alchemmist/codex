...
use antex_login::default_client::ResidencyRequirement;
use antex_login::default_client::read_default_client_residency_requirement;
use antex_model_provider_info::ModelProviderInfo;
use antex_models_manager::cache::ModelsCache;
use antex_models_manager::manager::OpenAiModelsManager;
use antex_models_manager::manager::SharedModelsManager;
use antex_models_manager::manager::StaticModelsManager;
use antex_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that Antex may expose at runtime.
///
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether Antex should have a unified provider-specific auth
    ...
    fn map_api_error(&self, error: ApiError) -> CodexErr {
        antex_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(&self) -> ModelProviderFuture<'_, antex_protocol::error::Result<Provider>> {
        Box::pin(async move {
            let auth = self.auth().await;
            let mut provider = self
                .info()
                .to_api_provider(auth.as_ref().map(AntexAuth::auth_mode))?;
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, antex_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, antex_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    /// Returns request credentials, optionally scoped to a Antex session task.
    fn api_auth_for_scope(
        &self,
        scope: ProviderAuthScope,
    ) -> ModelProviderFuture<'_, antex_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| antex_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
        ...
        let account = if self.info.requires_openai_auth {
            self.auth_manager
                ...
                .and_then(|auth_manager| {
                    ...
                    if matches!(auth, AntexAuth::Headers(_)) {
                        return None;
                    }
                    Some(auth)
                })
                .map(|auth| match &auth {
                    AntexAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    AntexAuth::BedrockApiKey(_) | AntexAuth::BedrockAccessKeys(_) => {
...
mod tests {
    ...
    use antex_model_provider_info::AwsAuthRefreshConfig;
    use antex_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    use antex_utils_redacted_string::RedactedString;
    ...
    fn test_antex_home() -> std::path::PathBuf {
    ...
    fn bedrock_api_key_auth() -> AntexAuth {
        AntexAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    async fn shared_bedrock_auth_refresh_is_reused_only_for_matching_configuration() {
        ...
        const HELPER_ARG: &str = "ANTEX_BEDROCK_SHARED_AUTH_REFRESH_COMMAND";
        const SUBPROCESS_ARG: &str = "ANTEX_BEDROCK_SHARED_AUTH_REFRESH_SUBPROCESS";
        ...
        assert_eq!(
            ...
            Ok(ProviderAccountState {
                account: Some(ProviderAccount::AmazonBedrock {
                    uses_antex_managed_credentials: false,
                }),
                requires_openai_auth: false,
            })
        );
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_antex_home(), /*config_model_catalog*/ None);
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = antex_models_manager::bundled_models_response()
