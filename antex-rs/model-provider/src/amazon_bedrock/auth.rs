...
use antex_aws_auth::AwsAccessKeys;
...
pub(super) fn auth_source(
        ...
        .is_some_and(|aws| aws.profile.is_some())
    {
        ...
        Some(AntexAuth::BedrockApiKey(_))
    ) {
        BedrockAuthSource::ManagedBearerToken
    } else if matches!(
        auth_manager.and_then(AuthManager::auth_cached),
        Some(AntexAuth::BedrockAccessKeys(_))
    ) {
...
pub(super) async fn resolve_auth_method(
    ...
    match source {
        ...
        BedrockAuthSource::ManagedBearerToken => {
            let Some(AntexAuth::BedrockApiKey(auth)) = managed_auth else {
        ...
        BedrockAuthSource::ConfiguredAwsProfile => {
            ...
            let context = AwsAuthContext::load_profile(config)
                .await
                .map_err(aws_auth_error_to_antex_error)?;
            Ok(BedrockAuthMethod::AwsSdkAuth { context })
        }
        BedrockAuthSource::ManagedAccessKeys => {
            let Some(AntexAuth::BedrockAccessKeys(auth)) = managed_auth else {
                return Err(CodexErr::Fatal(
...
fn aws_auth_error_to_antex_error(error: AwsAuthError) -> CodexErr {
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer Antex agent requests fail even though raw Responses requests work.
...
impl AuthProvider for BedrockSigV4AuthProvider {
    ...
    fn apply_auth(&self, request: Request) -> antex_api::AuthProviderFuture<'_> {
...
mod tests {
    ...
    fn bedrock_auth_source_distinguishes_static_environment_credentials() {
        ...
        let managed_auth =
            AuthManager::from_auth_for_testing(AntexAuth::BedrockApiKey(BedrockApiKeyAuth {
                api_key: "managed-bedrock-api-key".to_string(),
                region: "us-east-1".to_string(),
            }));
        let managed_access_keys = AuthManager::from_auth_for_testing(AntexAuth::BedrockAccessKeys(
