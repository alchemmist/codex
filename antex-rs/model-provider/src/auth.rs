...
use antex_agent_identity::AgentIdentityKey;
use antex_agent_identity::authorization_header_for_agent_task;
use antex_api::AgentIdentityTelemetry;
use antex_api::AuthError;
use antex_api::AuthHeadersFuture;
...
use antex_login::auth::AgentIdentityAuthError;
...
struct AuthManagerAuthProvider {
    ...
    expected_auth: AntexAuth,
}

impl AuthManagerAuthProvider {
    fn is_expected_auth(&self, auth: &AntexAuth) -> bool {
        auth.uses_antex_backend()
    ...
    fn current_auth(&self) -> Option<AntexAuth> {
        self.auth_manager
            .auth_cached()
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> antex_protocol::error::Result<SharedAuthProvider> {
    ...
    if matches!(
        auth,
        Some(AntexAuth::BedrockApiKey(_) | AntexAuth::BedrockAccessKeys(_))
    ) {
...
) -> antex_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(AntexAuth::AgentIdentity(agent_identity_auth)) = auth {
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(AntexAuth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> antex_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party Antex auth snapshot.
pub fn auth_provider_from_auth(auth: &AntexAuth) -> SharedAuthProvider {
    match auth {
        AntexAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        AntexAuth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        ...
        | AntexAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
#[cfg(test)]
mod tests {
    use antex_agent_identity::generate_agent_key_material;
    ...
    use antex_login::auth::login_with_chatgpt_auth_tokens;
    ...
    ) -> (PathBuf, Arc<AuthManager>, AntexAuth) {
    ...
    fn custom_provider_does_not_inherit_ambient_auth_headers() {
        ...
        let ambient_auth = AntexAuth::Headers(AuthHeaders::new(ambient_headers));
    ...
    fn custom_provider_does_not_inherit_ambient_bedrock_auth() {
        let provider =
            create_oss_provider_with_base_url("http://localhost:11434/v1", WireApi::Responses);
        let ambient_auth = AntexAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    fn custom_provider_uses_command_resolved_auth() {
        ...
        let command_auth = AntexAuth::from_api_key("command-token");
    ...
    fn openai_provider_preserves_ambient_auth_headers() {
        ...
        let ambient_auth = AntexAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = AntexAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = AntexAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&AntexAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_antex_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);
