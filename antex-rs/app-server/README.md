...
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$ANTEX_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake (also supported on Windows)
...
opens exactly one raw stream connection to `$ANTEX_HOME/app-server-control/app-server-control.sock`
...
- `thread/context/read` — inspect the exact base instructions, current model-visible history items, cached token usage, and the latest complete logical Responses API request for a loaded thread. This method is experimental.
...
- `memory/reset` — experimental; clear the current `ANTEX_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
...
- `turn/settings/update` — experimental; publish a reviewer or model-settings patch to the exact live task identified by `threadId` and `turnId`, regardless of task kind. Model, effort, and summary updates require `step_model_switching`; reviewer and service-tier updates do not. Returns `status: "applied"` or `status: "targetUnavailable"`, or a request error if rejected. Future-thread settings and already captured steps are unchanged. Parent-owned Multi-Agent V2 subagents reject direct settings updates.
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.antex/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
Enable `capabilities.experimentalApi`; model, effort, and summary updates also require the
...
Reviewer and service-tier updates do not require `step_model_switching`. A service-tier change applies to the next model request in the current turn; it does not restart an in-flight request. Set
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.antex/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.antex/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.antex/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.antex/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.antex/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.antex/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
- **Personal access token (`personalAccessToken`)**: Codex uses a ChatGPT-backed personal access token loaded outside the app-server login RPCs, such as with `codex login --with-access-token` or `ANTEX_ACCESS_TOKEN`.
...
active user config. Neither flow changes `$ANTEX_HOME/.env`. Existing loaded sessions keep their
...
profile or environment credentials leaves existing keys in `$ANTEX_HOME/.env` unchanged.
...
any Codex-managed credentials, and leaves AWS-managed credentials and `$ANTEX_HOME/.env` unchanged.
...
`$ANTEX_HOME/.env` are left untouched.
...
5. Verify the protocol crate:
   ...
   ```bash
   just test -p antex-app-server-protocol
   ```
