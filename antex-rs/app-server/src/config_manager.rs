...
use antex_cloud_config::cloud_config_bundle_loader;
...
/// Shared app-server entry point for loading effective Antex configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let merged_cli_overrides = cli_overrides
            ...
            .collect::<Vec<_>>();
        let mut config = antex_core::config::ConfigBuilder::default()
            .codex_home(self.codex_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.antex_self_exe = self.arg0_paths.antex_self_exe.clone();
        config.antex_linux_sandbox_exe = self.arg0_paths.antex_linux_sandbox_exe.clone();
