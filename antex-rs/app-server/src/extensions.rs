...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn antex_skills_extension::SkillProvider>,
    ...
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = CodexErr> + 'static,
{
    ...
    antex_history_notes_extension::install(&mut builder, auth_manager.clone());
    if let Some(state_db) = state_db {
        antex_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            antex_otel::global(),
            ...
            |config: &Config| GoalExtensionConfig {
                enabled: config.features.enabled(antex_features::Feature::Goals),
    ...
    antex_web_search_extension::install(&mut builder, auth_manager.clone());
    antex_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
        Some(config.codex_home.clone())
    });
    let skill_providers = antex_skills_extension::SkillProviders::new()
        .with_executor_provider(executor_skill_provider)
        .with_orchestrator_provider(Arc::new(
            antex_skills_extension::OrchestratorSkillProvider::new(),
        ))
...
#[cfg(test)]
mod tests {
    use antex_protocol::protocol::ThreadGoal as CoreThreadGoal;
    use antex_protocol::protocol::ThreadGoalStatus;
    use antex_protocol::protocol::ThreadGoalUpdatedEvent;
