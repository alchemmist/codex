...
//!
//! # Relationship to `antex-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `antex-app-server-client`,
...
use antex_app_server_protocol::AgentMessageDelivery;
...
pub use antex_rollout::StateDbHandle;
pub use antex_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `antex-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_antex_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let auth_manager =
        AuthManager::shared_from_config(args.config.as_ref(), args.enable_antex_api_key_env)
            .await
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let state_db = antex_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        client._test_antex_home = Some(codex_home);
        client
    }
