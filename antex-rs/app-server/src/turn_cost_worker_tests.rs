use super::*;
use antex_backend_client::ApiKeyResponseCost;
...
async fn worker_starts_with_otlp_metrics_exporter_without_log_exporter() {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(AntexAuth::from_api_key("sk-test"));
...
async fn turn_cost_failures_do_not_log_response_bodies(status: u16) {
    ...
    let mut runtime = test_runtime(
        &server,
        AuthManager::from_auth_for_testing(AntexAuth::from_api_key("sk-test")),
    )
    .await;
