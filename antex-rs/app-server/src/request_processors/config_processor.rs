...
use antex_app_server_protocol::AllowDenyRequirement;
use antex_app_server_protocol::AutoReviewRequirements;
use antex_app_server_protocol::BrowserUseAccessApprovalLifetime;
use antex_app_server_protocol::BrowserUseOriginPolicy;
use antex_app_server_protocol::BrowserUseRequirements;
use antex_app_server_protocol::CliAuthCredentialsStoreMode;
use antex_app_server_protocol::ClientResponsePayload;
use antex_app_server_protocol::ComputerUseMacosRequirements;
use antex_app_server_protocol::ComputerUseRequirements;
use antex_app_server_protocol::ComputerUseWindowsExeRequirement;
use antex_app_server_protocol::ComputerUseWindowsRequirements;
use antex_app_server_protocol::ConfigBatchWriteParams;
use antex_app_server_protocol::ConfigReadParams;
use antex_app_server_protocol::ConfigReadResponse;
use antex_app_server_protocol::ConfigRequirements;
use antex_app_server_protocol::ConfigRequirementsReadResponse;
use antex_app_server_protocol::ConfigValueWriteParams;
use antex_app_server_protocol::ConfigWriteErrorCode;
use antex_app_server_protocol::ConfigWriteResponse;
use antex_app_server_protocol::ConfiguredHookHandler;
use antex_app_server_protocol::ConfiguredHookMatcherGroup;
use antex_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use antex_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use antex_app_server_protocol::FeedbackRequirements;
use antex_app_server_protocol::InAppBrowserRequirements;
use antex_app_server_protocol::JSONRPCErrorError;
use antex_app_server_protocol::ManagedHooksRequirements;
use antex_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use antex_app_server_protocol::ModelsRequirements;
use antex_app_server_protocol::NetworkDomainPermission;
use antex_app_server_protocol::NetworkRequirements;
use antex_app_server_protocol::NetworkUnixSocketPermission;
...
use antex_config::HookEventsToml;
use antex_config::HookHandlerConfig as CoreHookHandlerConfig;
use antex_config::ManagedHooksRequirementsToml;
use antex_config::MatcherGroup as CoreMatcherGroup;
use antex_config::ResidencyRequirement as CoreResidencyRequirement;
use antex_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use antex_core::ThreadManager;
use antex_features::Feature;
use antex_features::canonical_feature_for_key;
use antex_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<antex_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = antex_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ...
    ConfigRequirements {
        application: requirements.application.map(|application| {
            antex_app_server_protocol::ApplicationRequirements {
                network: application.network.map(|network| {
                    antex_app_server_protocol::ApplicationNetworkRequirements {
        ...
        cli_auth_credentials_store: requirements.cli_auth_credentials_store.map(
            |mode| match mode {
                antex_config::types::AuthCredentialsStoreMode::File => {
                    CliAuthCredentialsStoreMode::File
                }
                antex_config::types::AuthCredentialsStoreMode::Keyring => {
                    CliAuthCredentialsStoreMode::Keyring
                }
                antex_config::types::AuthCredentialsStoreMode::Auto => {
                    CliAuthCredentialsStoreMode::Auto
                }
                antex_config::types::AuthCredentialsStoreMode::Ephemeral => {
        ...
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(antex_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(antex_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            antex_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            antex_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: antex_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: antex_config::BrowserUseRequirementsToml,
...
fn map_browser_use_origin_policy_to_api(
    policy: antex_config::BrowserUseOriginPolicyToml,
...
fn map_allow_deny_requirement_to_api(
    requirement: antex_config::AllowDenyRequirementToml,
) -> AllowDenyRequirement {
    match requirement {
        antex_config::AllowDenyRequirementToml::Allow => AllowDenyRequirement::Allow,
        antex_config::AllowDenyRequirementToml::Deny => AllowDenyRequirement::Deny,
    }
}

fn map_browser_use_access_approval_lifetime_to_api(
    lifetime: antex_config::BrowserUseAccessApprovalLifetimeToml,
) -> BrowserUseAccessApprovalLifetime {
    match lifetime {
        antex_config::BrowserUseAccessApprovalLifetimeToml::Turn => {
            BrowserUseAccessApprovalLifetime::Turn
        }
        antex_config::BrowserUseAccessApprovalLifetimeToml::Thread => {
...
) -> antex_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => antex_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: antex_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(antex_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(antex_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(antex_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: antex_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        antex_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        antex_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: antex_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        antex_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        antex_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use antex_app_server_protocol::FeedbackRequirements;
    use antex_app_server_protocol::WindowsSandboxSetupMode;
    use antex_config::AllowDenyRequirementToml;
    use antex_config::AutoReviewRequirementsToml;
    use antex_config::BrowserUseAccessApprovalLifetimeToml;
    use antex_config::BrowserUseOriginPolicyToml;
    use antex_config::BrowserUseRequirementsToml;
    use antex_config::ComputerUseMacosRequirementsToml;
    use antex_config::ComputerUseRequirementsToml;
    use antex_config::ComputerUseWindowsExeRequirementToml;
    use antex_config::ComputerUseWindowsRequirementsToml;
    use antex_config::ConfigRequirementsToml;
    use antex_config::ModelsRequirementsToml;
    use antex_config::NewThreadModelDefaultsToml;
    use antex_config::WindowsRequirementsToml;
