use super::*;
use antex_core::config::permission_profile_catalog;
use antex_hooks::HookListEntryHandler;
...
) -> Vec<antex_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            antex_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    antex_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| antex_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[antex_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[antex_skills::SkillError],
) -> Vec<antex_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| antex_app_server_protocol::SkillErrorInfo {
                ...
                async move {
                    let config = match self.load_latest_config(Some(cwd.clone())).await {
                        ...
                        Err(error) => {
                            ...
                            return (
                                index,
                                antex_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![antex_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                    ...
                    let skills_input = antex_skills_extension::HostSkillsLoadInput::new(
                        config.cwd.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(antex_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![antex_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let hooks_enabled = config.features.enabled(Feature::AntexHooks);
            let plugin_hooks = if hooks_enabled && config.features.enabled(Feature::Plugins) {
                let plugins_input = config.plugins_config_input();
                let plugin_outcome = plugins_manager.plugins_for_config(&plugins_input).await;
                antex_core_plugins::PluginHookLoadOutcome {
                ...
                antex_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = antex_hooks::list_hooks(antex_hooks::HooksConfig {
