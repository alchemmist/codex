...
//! the configured Antex executable as a subprocess, accepts only valid JSON from
//! `antex doctor --json --feedback`, derives a small set of Sentry tags, and otherwise
...
use antex_feedback::FeedbackAttachment;
...
/// Runs `antex --cd <workspace> doctor --json --feedback` and returns a best-effort
/// feedback attachment.
///
/// Failure to spawn Antex, finish before the timeout, or parse JSON means the
...
mod tests {
    ...
    fn doctor_command_keeps_missing_workspace_out_of_process_cwd() {
        ...
        assert_eq!(
            ...
            [("ANTEX_HOME".as_ref(), Some(codex_home.path().as_os_str()))]
        );
    }
