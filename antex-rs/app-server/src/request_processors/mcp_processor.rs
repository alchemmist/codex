...
use antex_mcp::ReadResourceRequestParams;
...
impl McpRequestProcessor {
    ...
    async fn mcp_server_oauth_login_response(
        ...
        let effective_servers = antex_mcp::effective_mcp_servers(&mcp_config, auth.as_ref());
        ...
        let discovered_scopes = if scopes.is_none() && server.scopes.is_none() {
            discover_supported_scopes(
                ...
                antex_rmcp_client::OAuthDiscoveryTimeout::Requested,
                redirect_mode,
            )
            .await
        } else {
    ...
    async fn list_mcp_server_status_response(
        request_id: String,
        params: ListMcpServerStatusParams,
        mcp_config: antex_mcp::McpConfig,
        auth: Option<AntexAuth>,
        runtime_context: McpRuntimeContext,
        mcp_manager: Arc<McpManager>,
        thread: Option<Arc<antex_core::AntexThread>>,
    ...
    async fn read_mcp_resource(
        ...
        if let Some(thread_id) = thread_id {
            ...
            tokio::spawn(async move {
                let origin_call_id =
                    origin_call_id.filter(|_| server == antex_mcp::CODEX_APPS_MCP_SERVER_NAME);
...
fn mcp_operation_error(error: anyhow::Error) -> JSONRPCErrorError {
    match antex_rmcp_client::mcp_error(&error) {
