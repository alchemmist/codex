...
use antex_login::LoginOnboardingEntrypoint;
use antex_login::login_with_bedrock_access_keys;
use antex_model_provider::is_supported_amazon_bedrock_region;
use chrono::DateTime;
...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "ANTEX_APP_SERVER_LOGIN_ISSUER";
// The development success-page redirect remains debug-only.
#[cfg(debug_assertions)]
const LOGIN_OPEN_APP_URL_OVERRIDE_ENV_VAR: &str = "ANTEX_APP_SERVER_DEV_OPEN_APP_URL";
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth
                .as_ref()
                .map(AntexAuth::api_auth_mode)
                .map(auth_mode_to_api),
            plan_type: auth.as_ref().and_then(AntexAuth::account_plan_type),
        }
    }
            ...
            LoginAccountParams::Chatgpt {
                ...
                let login_success_page = if use_hosted_login_success_page {
                    ...
                    LoginSuccessPage::Hosted {
                        url: ANTEX_OPEN_APP_URL.parse().map_err(|err| {
                ...
                self.login_chatgpt_v2(request_id, antex_streamlined_login, login_success_page)
                    .await;
            }
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self
            .login_chatgpt_response(antex_streamlined_login, login_success_page)
            .await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self
            .login_chatgpt_common(antex_streamlined_login, login_success_page)
            .await?;
    ...
    async fn get_account_rate_limits_response(
        ...
        let usage_request = async {
            if params.supports_luna_reserve
                && auth.auth_mode() == antex_protocol::auth::AuthMode::Chatgpt
    ...
    async fn get_account_token_usage_response(
        ...
        if let Some(thread_id) = thread_id {
            ...
            let thread_usage = match usage {
                Ok(usage) => Some(antex_app_server_protocol::ThreadUsage {
                    ...
                    groups: usage
                        ...
                        .into_iter()
                        .map(
                            |group| antex_app_server_protocol::ThreadUsageBreakdownGroup {
                                model: group.model,
...
mod tests {
    use super::*;
    use antex_backend_client::TokenUsageProfileDailyBucket;
    use antex_backend_client::TokenUsageProfileStats;
    use http::StatusCode;
