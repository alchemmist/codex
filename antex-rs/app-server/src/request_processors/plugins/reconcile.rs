...
use antex_app_server_protocol::PluginReconcileChangedPlugin;
use antex_app_server_protocol::PluginReconcileParams;
use antex_app_server_protocol::PluginReconcileResponse;
...
impl PluginRequestProcessor {
    ...
    pub(crate) async fn plugin_reconcile(
        ...
        if !plugins_input.plugins_enabled
            || !auth.as_ref().is_some_and(AntexAuth::uses_antex_backend)
        {
