        ...
        tokio::spawn(async move {
            ...
            let setup_result = async {
                ...
                if setup_mode == CoreWindowsSandboxSetupMode::Elevated
                    ...
                    && !antex_login::is_workload_identity_selected()
                {
                    let provisioning = match config
                        ...
                        .map_or_else(
                            || {
                                Ok((
                                    antex_windows_sandbox::WindowsSandboxProvisioningSettings::from_environment(
                                        &setup_request.permission_profile,
                                        &setup_request.env_map,
                                    ),
                                    antex_windows_sandbox::WindowsSandboxProxyListeners::from_environment(
                                        &setup_request.permission_profile,
                                        &setup_request.env_map,
                                    ),
                                ))
                            },
                            antex_core::config::NetworkProxySpec::windows_sandbox_proxy_listeners,
                        )
                    {
                    ...
                    if let Some((settings, listeners)) = provisioning {
                        ...
                        tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
                            if antex_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
                            ...
                            antex_windows_sandbox::provision_windows_sandbox_via_service(
                        ...
                        .inspect_err(|error| {
                            antex_core::windows_sandbox::emit_windows_sandbox_setup_failure_metrics(
                                setup_mode,
                ...
                antex_core::windows_sandbox::run_windows_sandbox_setup(setup_request).await
            }
            .await;
...
fn resolve_allowed_windows_sandbox_setup_mode(
    requirements: &antex_config::ConfigRequirements,
