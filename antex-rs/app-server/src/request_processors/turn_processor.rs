...
use super::*;
use antex_agent_extension::AgentInvocation;
use antex_agent_extension::AgentRun;
use antex_agent_extension::AgentRunner;
...
use antex_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use antex_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
use antex_protocol::protocol::TurnSettingsUpdate;
use antex_protocol::protocol::TurnSettingsUpdateOutcome;
use antex_skills::system_cache_root_dir;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<antex_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = antex_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if turn_has_input && started {
            ...
            if config_snapshot.is_primary_environment_configured() {
                antex_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<antex_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(antex_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(antex_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        Ok(antex_protocol::protocol::ThreadSettingsOverrides {
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != antex_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match reason {
                    ...
                    NotSubmittedReason::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            antex_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            antex_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn prepare_realtime_conversation_thread(
        ...
        request_id: &ConnectionRequestId,
        thread_id: &str,
    ) -> Result<Option<(ThreadId, Arc<AntexThread>)>, JSONRPCErrorError> {
    ...
    async fn start_detached_review(
        ...
        if matches!(
            ...
            antex_protocol::protocol::ThreadHistoryMode::Paginated
        ) {
            return Err(invalid_request(
