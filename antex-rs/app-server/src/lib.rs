...
use antex_code_mode::GrpcCodeModeSessionProvider;
use antex_config::LoaderOverrides;
use antex_config::NoopThreadConfigLoader;
use antex_core::config::Config;
use antex_core::config::UnsupportedUntrustedApprovalPolicyError;
...
use antex_analytics::AppServerRpcTransport;
...
use antex_app_server_protocol::TextPosition as AppTextPosition;
use antex_app_server_protocol::TextRange as AppTextRange;
use antex_config::ConfigLayerSource;
use antex_config::ConfigLoadError;
use antex_config::TextRange as CoreTextRange;
use antex_core::ExecPolicyError;
...
use antex_rollout::state_db as rollout_state_db;
...
const OTEL_SERVICE_NAME: &str = "antex-app-server";
#[cfg(debug_assertions)]
const TEST_USER_CONFIG_FILE_ENV_VAR: &str = "ANTEX_APP_SERVER_TEST_USER_CONFIG_FILE";
...
async fn shutdown_signal() -> IoResult<ShutdownSignal> {
    ...
    #[cfg(windows)]
    {
        ...
        let daemon_signal = async {
            let result = antex_app_server_transport::daemon_shutdown_signal().await;
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.all_layers_low_to_high() {
        let ConfigLayerSource::Project { dot_antex_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_antex_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
        ...
        .await
    {
        Ok(config) => {
            let auth_manager =
                AuthManager::shared_from_config(&config, /*enable_antex_api_key_env*/ false)
                    .await
    ...
    let otel = antex_core::otel_init::build_provider(
        &config,
    ...
    antex_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    antex_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    ...
    let feedback = AntexFeedback::new();
    ...
    if let Some(metrics) = otel.as_ref().and_then(antex_otel::OtelProvider::metrics) {
        antex_home_metrics::spawn(&config, metrics.clone(), transport_shutdown_token.clone());
    }
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !antex_state::is_sqlite_corruption_error(&err)
        ...
        let backups = antex_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
