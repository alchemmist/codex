...
use antex_app_server_protocol::ExperimentalApi;
...
use antex_app_server_protocol::experimental_required_message;
...
use antex_thread_store::LocalQueueStore;
use antex_thread_store::QueueStore;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: AntexFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store = antex_core::thread_store_from_config(config.as_ref(), state_db.clone());
        ...
        let executor_skill_provider: Arc<dyn antex_skills_extension::SkillProvider> = Arc::new(
            antex_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ...
            let manager = ThreadManager::new(
                config.as_ref(),
                auth_manager.clone(),
                antex_core::build_models_manager(config.as_ref(), auth_manager.clone()),
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let antex_request = deserialize_client_request(request);
                let result = match antex_request {
                    Ok(antex_request) => {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = antex_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = antex_request.experimental_reason()
        ...
        let event_stream_ready = match &antex_request {
        ...
        let serialization_scope = antex_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: antex_request.id().clone(),
        };
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match antex_request {
            ...
            ClientRequest::ThreadContextRead { params, .. } => {
                self.thread_processor.thread_context_read(params).await
            }
            ...
            ClientRequest::McpServerEventStreamStart { params, .. } => {
                ...
                session
                    ...
                    .await?;
                Ok(Some(
                    antex_app_server_protocol::McpServerEventStreamStartResponse {}.into(),
                ))
            }
            ClientRequest::McpServerEventStreamStop { params, .. } => {
                session
                    ...
                    .await;
                Ok(Some(
                    antex_app_server_protocol::McpServerEventStreamStopResponse {}.into(),
                ))
            }
