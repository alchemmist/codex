...
use antex_rollout::append_thread_name;
...
async fn thread_fork_defers_inherited_active_goal_until_next_turn() -> Result<()> {
    ...
    assert_eq!(
        forked_goal.status,
        antex_state::ThreadGoalStatus::BudgetLimited
    );
    assert_eq!(
        state_db
            .thread_goals()
...
async fn assert_thread_fork_freezes_active_paginated_turn_as_interrupted(
    ...
    assert!(matches!(
        ...
        [
            ...
            &response_item.item,
            antex_protocol::models::ResponseItem::Message { role, .. }
...
#[tokio::test]
async fn pathless_ephemeral_thread_rejects_antex_home_path_after_reload() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(codex_home.path())?;
    ...
    let resume_id = app_server
        .send_thread_resume_request(ThreadResumeParams {
            ...
            path: Some(antex_home_path.clone()),
            ..Default::default()
        })
        .await?;
