...
use antex_config::types::OtelHttpProtocol;
use antex_core::config::ConfigBuilder;
use antex_core_plugins::loader::curated_plugin_cache_version;
...
fn set_metrics_exporter(config: &mut antex_core::config::Config) {
...
"#,
    ...
    std::fs::write(
        &script_path,
        r#"test -n "$ANTEX_PLUGIN_METRICS_OUTPUT"
sleep "${1:-0.3}"
printf '%s' '{"version":1,"measurements":[{"name":"findings","value":3,"dimensions":{"severity":"high"}},{"name":"files_scanned","value":17}]}' > "$ANTEX_PLUGIN_METRICS_OUTPUT"
"#,
    )?;
...
"#,
    ...
    if remote {
        assert_eq!(
            ...
            antex_exec_server::REMOTE_ENVIRONMENT_ID
        );
    }
