...
async fn computer_use_scoring_follows_model_review_requirement(
    ...
    let ordinary_model = antex_core::test_support::construct_model_info_offline(MODEL, &config);
    let mut reviewed_model =
        antex_core::test_support::construct_model_info_offline(REVIEWED_MODEL, &config);
