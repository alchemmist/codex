use antex_app_server_protocol::ActivePermissionProfile;
use antex_app_server_protocol::ApprovalsReviewer;
...
use antex_app_server_protocol::SandboxPolicy as AppSandboxPolicy;
...
use antex_app_server_protocol::ThreadActiveFlag;
...
use antex_core::ARCHIVED_SESSIONS_SUBDIR;
use antex_features::Feature;
use antex_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use antex_rollout::RolloutRecorder;
use antex_rollout::append_rollout_item_to_path;
use antex_rollout::read_session_meta_line;
...
const ANTEX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Codex, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    let antex_home_path = normalized_existing_path(codex_home.path())?;
    mock_responses_config(&server.uri()).write(&antex_home_path)?;
    let config_path = antex_home_path.join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("personality = true\n", "personality = true\ngoals = true\n"),
    ...
    let sqlite_home = antex_home_path
        .as_path()
        .to_str()
        .expect("test codex home should be utf-8");
    let mut mcp = TestAppServer::builder()
        .with_antex_home(&antex_home_path)
        .without_managed_config()
        .with_env_overrides(&[("ANTEX_SQLITE_HOME", Some(sqlite_home))])
        .build_initialized()
        .await?;
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "antex unarchive {conversation_id}` to unarchive it first"
            )),
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            antex_state::GoalAccountingMode::ActiveOnly,
...
async fn thread_goal_keeps_original_root_until_external_objective_edit() -> Result<()> {
    ...
    let get_id = mcp
        ...
        .await?;
    let _: antex_app_server_protocol::ThreadGoalGetResponse =
...
async fn thread_goal_lifecycle_emits_analytics_and_clear_deletes_goal() -> Result<()> {
    ...
    let get_id = mcp
        ...
        .await?;
    let get: antex_app_server_protocol::ThreadGoalGetResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(get_id)).await??;
...
"#;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![antex_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(ANTEX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
