...
use antex_app_server_protocol::GuardianRiskLevel;
use antex_app_server_protocol::GuardianUserAuthorization;
...
async fn guardians_retain_evidence_after_compaction_and_discard_it_after_rollback(
    ...
    .into_iter()
    .map(|(model, hash)| {
        let mut info = antex_core::test_support::construct_model_info_offline(model, &config);
