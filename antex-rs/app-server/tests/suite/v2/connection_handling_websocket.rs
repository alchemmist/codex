...
async fn thread_start_routes_project_exec_policy_warning_to_requester() -> Result<()> {
    ...
    let rules_dir = project.path().join(".antex/rules");
...
pub(super) async fn spawn_websocket_server_with_args(
    ...
    let program = antex_utils_cargo_bin::cargo_bin("antex-app-server")
    ...
    #[cfg(windows)]
    cmd.env(
        antex_app_server_transport::DAEMON_SHUTDOWN_FILE_ENV,
