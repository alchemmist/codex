...
async fn process_spawn_returns_before_exit_and_emits_exit_notification() -> Result<()> {
    ...
    let command = if cfg!(windows) {
        vec![
            ...
            "-Command".to_string(),
            concat!(
                "[IO.File]::WriteAllText($env:ANTEX_PROCESS_EXEC_PROBE_FILE, 'process'); ",
                "while (!(Test-Path -LiteralPath $env:ANTEX_PROCESS_EXEC_RELEASE_FILE)) { ",
        ...
        vec![
            ...
            "-c".to_string(),
            concat!(
                "printf process > \"$ANTEX_PROCESS_EXEC_PROBE_FILE\"; ",
                "while [ ! -e \"$ANTEX_PROCESS_EXEC_RELEASE_FILE\" ]; do sleep 0.05; done; ",
    ...
    let env = HashMap::from([
        (
            "ANTEX_PROCESS_EXEC_PROBE_FILE".to_string(),
            Some(probe_file.display().to_string()),
        ),
        (
            "ANTEX_PROCESS_EXEC_RELEASE_FILE".to_string(),
