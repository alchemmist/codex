...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": antex_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
async fn review_start_with_detached_delivery_returns_new_thread_id() -> Result<()> {
    ...
    let canonical_antex_home = std::fs::canonicalize(codex_home.path())?.try_into()?;
    let review_skill_path = system_cache_root_dir(&canonical_antex_home)
        .join("review-agent")
        .join("SKILL.md");
