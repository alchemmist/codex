...
use antex_app_server_protocol::DesktopOnboardingEntrypoint;
...
use antex_login::CLIENT_ID_OVERRIDE_ENV_VAR;
...
const LOGIN_ISSUER_ENV_VAR: &str = "ANTEX_APP_SERVER_LOGIN_ISSUER";
const LOGIN_OPEN_APP_URL_ENV_VAR: &str = "ANTEX_APP_SERVER_DEV_OPEN_APP_URL";
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(antex_app_server_protocol::ThreadStartParams {
            model: Some("mock-model".to_string()),
            ..Default::default()
        })
        .await?;
    let thread: antex_app_server_protocol::ThreadStartResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(thread_req)).await??;

    let turn_req = mcp
        .send_turn_start_request(antex_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![antex_app_server_protocol::UserInput::Text {
    ...
    .await?;
    let _: antex_app_server_protocol::TurnStartResponse =
