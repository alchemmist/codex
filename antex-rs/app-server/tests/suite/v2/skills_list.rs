...
use antex_app_server_protocol::SkillScope;
use antex_app_server_protocol::SkillsChangedNotification;
...
async fn skills_list_uses_each_cwds_bundled_skills_configuration() -> Result<()> {
    ...
    for (cwd, enabled) in [(disabled_cwd.path(), false), (enabled_cwd.path(), true)] {
        ...
        std::fs::create_dir_all(cwd.join(".antex"))?;
        std::fs::write(
            cwd.join(".antex/config.toml"),
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let repo_skill_dir = cwd.path().join(".antex/skills/repo-skill");
    ...
    let mut mcp = TestAppServer::builder()
        .with_antex_home(codex_home.path())
        .without_auto_env()
        .with_env_overrides(&[(ANTEX_EXEC_SERVER_URL_ENV_VAR, Some("none"))])
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
async fn skills_list_refreshes_externally_updated_plugin_versions() -> Result<()> {
    ...
    std::fs::create_dir_all(source.path().join(".antex-plugin"))?;
    std::fs::create_dir_all(source.path().join("skills"))?;
    std::fs::write(
        source.path().join(".antex-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
    std::fs::write(
        codex_home.path().join("config.toml"),
...
-> Result<()> {
    ...
    let skill_dir = cwd.path().join(".antex/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),
