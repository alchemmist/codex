...
async fn automatic_upgrade_isolates_git_while_explicit_install_preserves_configuration(
    ...
    let plugin_root = plugin_source.path().join(".antex-plugin");
    ...
    let plugin_root = codex_home
        .path()
        .join("plugins/cache/trusted/toolkit/1.0.0/.antex-plugin/plugin.json");
    ...
    std::fs::write(
        plugin_source.path().join(".antex-plugin/plugin.json"),
    ...
    assert!(
        codex_home
            .path()
            .join("plugins/cache/trusted/toolkit/1.1.0/.antex-plugin/plugin.json")
            .is_file(),
