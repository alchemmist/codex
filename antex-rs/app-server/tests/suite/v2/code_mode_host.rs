...
async fn code_mode_model_output_uses_structured_host_timing(
    ...
    let mut host = match transport {
        "grpc" => Some(
            Command::new(antex_utils_cargo_bin::cargo_bin("antex-code-mode-host")?)
    ...
    let mut builder = TestAppServer::builder()
        ...
        .with_json_logging("antex_code_mode::timing=info,antex_core::tools::parallel=info");
...
async fn app_server_shares_flag_selected_grpc_code_mode_host_across_threads() -> Result<()> {
    let host_program = antex_utils_cargo_bin::cargo_bin("antex-code-mode-host")?;
