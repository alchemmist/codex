...
#[cfg(unix)]
use antex_app_server_protocol::ExternalAgentConfigImportTypeResult;
...
async fn external_agent_config_import_skips_repository_redirect_after_detection() -> Result<()> {
    ...
    let repo_config_dir = repo_root.join(".antex");
...
async fn external_agent_config_secondary_source_imports_session_and_plugin_end_to_end() -> Result<()>
{
    ...
    std::fs::create_dir_all(configured_plugin_root.join(".antex-plugin"))?;
    std::fs::write(
  ...
  "plugins": [{
    ...
    std::fs::write(
        configured_plugin_root.join(".antex-plugin/plugin.json"),
...
"#,
    ...
    assert_eq!(
        marketplace
            .path
            .as_ref()
            .map(antex_config::AbsolutePathBuf::as_path),
...
-> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ...
            ("ANTEX_SQLITE_HOME", Some(sqlite_home_dir.as_str())),
        ])
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
    ...
    let state_db = antex_state::StateRuntime::init(
        antex_state::SqliteConfig::new_for_testing(sqlite_home.path().abs()),
        "mock_provider".into(),
    )
    .await?;
...
async fn external_agent_config_detects_and_imports_project_memory_files() -> Result<()> {
    ...
    let memory_diff = antex_git_utils::diff_since_latest_init(&memory_root).await?;
    ...
    antex_memories_write::workspace::reset_memory_workspace_baseline(&memory_root).await?;
    ...
    assert_eq!(
        ...
        vec![
            (
                antex_git_utils::GitBaselineChangeStatus::Deleted,
                "extensions/external_agent_import/resources/project-a/MEMORY.md",
            ),
            (
                antex_git_utils::GitBaselineChangeStatus::Deleted,
...
async fn external_agent_config_import_reports_failed_sync_import_in_completion() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ("HOME", Some(home_dir.as_str())),
            (
                "ANTEX_ANALYTICS_EVENTS_CAPTURE_FILE",
...
async fn external_agent_config_import_reinstalls_plugins_from_known_marketplaces() -> Result<()> {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        Some(vec![antex_app_server_protocol::PluginsMigration {
