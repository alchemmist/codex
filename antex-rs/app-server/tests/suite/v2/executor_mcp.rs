...
async fn selected_executor_discovers_browser_mcp_with_executor_only_bearer_token() -> Result<()> {
    ...
    let mut executor = Command::new(antex_utils_cargo_bin::cargo_bin("antex")?)
        .args(["exec-server", "--listen", "ws://127.0.0.1:0"])
        .stdout(Stdio::piped())
        .kill_on_drop(true)
        .env("ANTEX_HOME", executor_home.path())
...
async fn guardian_review_does_not_discover_executor_mcp() -> Result<()> {
    ...
    let antex_bin = toml::Value::String(
        antex_utils_cargo_bin::cargo_bin("antex")?
            .to_string_lossy()
            .into_owned(),
    );
    ...
    std::fs::write(
        codex_home.path().join("environments.toml"),
        format!(
            "default = \"{EXECUTOR_ID}\"\ninclude_local = false\n\n[[environments]]\nid = \"{EXECUTOR_ID}\"\nprogram = {antex_bin}\nargs = [\"exec-server\", \"--listen\", \"stdio\"]\n[environments.env]\nANTEX_HOME = {executor_home_value}\n"
        ),
    )?;
    let mut app_server = TestAppServer::builder()
        .with_antex_home(codex_home.path())
...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    MockResponsesConfig::new(&responses_server.uri())
        ...
        .write(codex_home.path())?;
    let executor_config: antex_config::types::McpServerConfig = serde_json::from_value(json!({
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".antex-plugin"))?;
    std::fs::write(
        plugin.path().join(".antex-plugin/plugin.json"),
