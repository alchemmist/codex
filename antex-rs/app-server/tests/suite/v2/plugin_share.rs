    ...
    let request_id = mcp
        .send_plugin_list_request(PluginListParams {
            ...
            marketplace_kinds: Some(vec![
                antex_app_server_protocol::PluginListMarketplaceKind::Local,
            ]),
            force_refetch: false,
        })
        .await?;
...
async fn plugin_share_update_targets_updates_share_targets() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: antex_app_server_protocol::PluginShareDiscoverability::Unlisted,
        }
    );
    Ok(())
}
...
async fn plugin_share_update_targets_publishes_workspace_plugin() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: antex_app_server_protocol::PluginShareDiscoverability::Listed,
        }
    );
    Ok(())
}
...
fn write_test_plugin(root: &Path, plugin_name: &str) -> std::io::Result<PathBuf> {
    ...
    write_file(
        &plugin_path.join(".antex-plugin/plugin.json"),
