...
use antex_protocol::protocol::SessionSource as CoreSessionSource;
...
use antex_state::DirectionalThreadSpawnEdgeStatus;
...
fn set_rollout_cwd(path: &Path, cwd: &Path) -> Result<()> {
    ...
    let mut rollout_line = antex_rollout::parse_rollout_line(first_line)?;
...
async fn thread_list_respects_cwd_filters() -> Result<()> {
    ...
    let mut mcp = init_mcp(codex_home.path()).await?;
    let request_id = mcp
        .send_thread_list_request(antex_app_server_protocol::ThreadListParams {
            originators: None,
            cursor: None,
            limit: Some(10),
            sort_key: None,
...
"#,
    ...
    state_db
        ...
        .await?;
    let rollout_config = antex_rollout::RolloutConfig {
        codex_home: codex_home.path().to_path_buf(),
        sqlite: antex_state::SqliteConfig::new_for_testing(codex_home.path().abs()),
        cwd: codex_home.path().to_path_buf(),
        model_provider_id: "mock_provider".to_string(),
        generate_memories: false,
    };
    let repaired_page = antex_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        antex_core::ThreadSortKey::CreatedAt,
        antex_core::SortDirection::Desc,
        &[],
...
async fn thread_search_returns_content_matches() -> Result<()> {
    ...
    let mut mcp = init_mcp(codex_home.path()).await?;
    let request_id = mcp
        .send_thread_search_request(antex_app_server_protocol::ThreadSearchParams {
            cursor: None,
            limit: Some(10),
            sort_key: None,
            sort_direction: None,
    ...
    for thread_id in [&older_match, &newer_match] {
        let request_id = mcp
            .send_thread_section_move_request(ThreadSectionMoveParams {
                ...
                section_id: Some(antex_state::PINNED_THREAD_SECTION_ID.to_string()),
...
async fn thread_list_sort_recency_at_uses_state_db_order_with_provider_filter() -> Result<()> {
    ...
    antex_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        antex_core::ThreadSortKey::CreatedAt,
        antex_core::SortDirection::Desc,
        antex_core::INTERACTIVE_SESSION_SOURCES.as_slice(),
        /*model_providers*/ None,
        /*cwd_filters*/ None,
        "mock_provider",
        /*search_term*/ None,
...
async fn thread_originator_is_preserved_in_list_read_and_resume(originator: &str) -> Result<()> {
    ...
    let started: antex_app_server_protocol::ThreadStartedNotification = timeout(
        DEFAULT_READ_TIMEOUT,
    ...
    let _: TurnStartResponse = mcp
        ...
        .await?;
    let completed: antex_app_server_protocol::TurnCompletedNotification = timeout(
        DEFAULT_READ_TIMEOUT,
