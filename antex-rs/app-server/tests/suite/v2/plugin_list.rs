...
use antex_app_server_protocol::HookTrustStatus;
...
use antex_app_server_protocol::PluginAuthPolicy;
use antex_app_server_protocol::PluginDisabledReason;
use antex_app_server_protocol::PluginInstallParams;
use antex_app_server_protocol::PluginInstallPolicy;
use antex_app_server_protocol::PluginInstallPolicySource;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "ANTEX_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
...
async fn plugin_rpcs_reject_repository_spoofing_openai_curated() -> Result<()> {
    ...
    std::fs::create_dir_all(repository.path().join("attacker/.antex-plugin"))?;
    std::fs::write(
        repository.path().join("attacker/.antex-plugin/plugin.json"),
        r#"{"name":"attacker"}"#,
    )?;
    let marketplace_path =
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.antex-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.antex-plugin/plugin.json"),
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    disabled_reason: None,
                    eligible_plan_types: None,
                    interface: Some(antex_app_server_protocol::PluginInterface {
...
async fn plugin_list_omitted_cwds_excludes_server_project_config() -> Result<()> {
    ...
    std::fs::create_dir_all(codex_home.path().join(".antex"))?;
  ...
  "plugins": [
    ...
    std::fs::write(
        codex_home.path().join(".antex/config.toml"),
        format!(
...
async fn plugin_list_force_refetch_waits_for_same_path_local_plugin_upgrade() -> Result<()> {
    ...
    let source_manifest = marketplace_root
        .path()
        .join("sample-plugin/.antex-plugin/plugin.json");
...
"#,
    ...
    let installed_manifest = plugin_cache.join("1.1.0/.antex-plugin/plugin.json");
    assert!(
...
async fn plugin_list_refreshes_plugins_from_each_cwd(
    ...
    for ((repo, name), version) in repos.iter().zip(names).zip(versions) {
        for directory in [".git", ".antex", ".agents/plugins", "sample/.antex-plugin"] {
            std::fs::create_dir_all(repo.join(directory))?;
        }
        std::fs::write(
            repo.join("sample/.antex-plugin/plugin.json"),
    ...
    for (name, version) in &expected_sources {
        let manifest = codex_home.path().join(format!(
            "plugins/cache/{name}/sample/{version}/.antex-plugin/plugin.json"
        ));
        if !force_refetch {
...
async fn plugin_catalogs_skip_invalid_project_config_and_report_cwd_error() -> Result<()> {
    ...
    for repo in [&invalid_repo, &valid_repo] {
        for directory in [".git", ".antex", ".agents/plugins"] {
            std::fs::create_dir_all(repo.join(directory))?;
        }
        set_project_trust_level(codex_home.path(), repo, TrustLevel::Trusted)?;
    }
    std::fs::write(invalid_repo.join(".antex/config.toml"), "invalid = [\n")?;
    std::fs::write(
        valid_repo.join(".antex/config.toml"),
...
"#,
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".antex"))?;
    std::fs::write(
        workspace_enabled.path().join(".antex/config.toml"),
...
async fn plugin_list_returns_installed_git_source_interface_from_cache(
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(cached_plugin_root.join(".antex-plugin"))?;
    std::fs::write(
...
"#;
    ...
    if configured_in_later_cwd {
        ...
        std::fs::write(later_repo.path().join(".antex/config.toml"), plugin_config)?;
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    wait_for_path_exists(&installed_path.join(".antex-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(installed_path.join(".antex-plugin/plugin.json"))?,
    )?;
    assert_eq!(
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    wait_for_path_exists(&new_path.join(".antex-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(new_path.join(".antex-plugin/plugin.json"))?,
    )?;
    assert_eq!(
        installed_plugin_manifest["version"],
        serde_json::json!("1.2.3")
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled(
    ...
    let cwds = if project_configuration != ProjectPluginConfiguration::None {
        ...
        std::fs::write(repo.path().join(".antex/config.toml"), config)?;
        std::fs::write(
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        antex_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
...
#[tokio::test]
async fn plugin_list_includes_api_curated_marketplace_for_bedrock_without_antex_auth() -> Result<()>
{
    let codex_home = TempDir::new()?;
    std::fs::write(
        codex_home.path().join("config.toml"),
...
"#,
    ...
    wait_for_path_exists(
        &codex_home.path().join(
            "plugins/cache/created-by-me-remote/private-linear/1.2.3/.antex-plugin/plugin.json",
        ),
    )
    .await?;
    wait_for_remote_installed_snapshot_request(&server).await?;
...
"#,
    ...
    wait_for_path_exists(
        &codex_home
            .path()
            .join("plugins/cache/workspace-directory/no-hooks/1.2.3/.antex-plugin/plugin.json"),
    )
    .await?;
...
async fn assert_disabled_remote_plugin_metadata(
    ...
    assert_eq!(
        plugin.availability,
        antex_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
...
"#,
    ...
    std::fs::write(
        repo.path().join(".antex/config.toml"),
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.antex-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
