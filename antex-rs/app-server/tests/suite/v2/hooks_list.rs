...
fn command_hook_hash(
    ...
    let Ok(value) = antex_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    antex_config::version_for_toml(&value)
}
...
fn write_project_hook_config(dot_antex_folder: &std::path::Path, command: &str) -> Result<()> {
    std::fs::create_dir_all(dot_antex_folder)?;
    std::fs::write(
        dot_antex_folder.join("config.toml"),
        format!(
            r#"[features]
hooks = true
...
async fn plugin_upgrade_refreshes_hook_runtime_for_loaded_session() -> Result<()> {
  ...
  "plugins": [{
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .enable_feature(Feature::AntexHooks)
    ...
    let trust_id = mcp
        ...
        .await?;
    let _: antex_app_server_protocol::ConfigWriteResponse =
    ...
    std::fs::write(
        plugin_source.join(".antex-plugin/plugin.json"),
...
async fn hooks_list_shows_discovered_plugin_mcp_tool_hook() -> Result<()> {
  ...
  "hooks": {
    ...
    let identity = NormalizedHookIdentity {
        ...
        group: antex_config::MatcherGroup {
            matcher: Some("Bash".to_string()),
            hooks: vec![antex_config::HookHandlerConfig::McpTool {
    ...
    assert_eq!(
        ...
        vec![HooksListEntry {
            ...
            hooks: vec![HookMetadata {
                ...
                enabled: true,
                is_managed: false,
                current_hash: antex_config::version_for_toml(&identity),
                trust_status: HookTrustStatus::Untrusted,
            }],
            warnings: Vec::new(),
            errors: Vec::new(),
...
"#,
    ...
    std::fs::create_dir_all(workspace.path().join(".antex"))?;
    std::fs::write(
...
"#,
    ...
    let project_config_path =
        AbsolutePathBuf::try_from(workspace.path().join(".antex/config.toml"))?;
    assert_eq!(
        data,
        vec![
            HooksListEntry {
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".antex"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".antex"), "echo worktree hook")?;
    ...
    let repo_config_path =
        AbsolutePathBuf::from_absolute_path(repo_root.join(".antex/config.toml"))?;
