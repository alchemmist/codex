...
use antex_app_server_protocol::AppInfo;
use antex_app_server_protocol::AppMetadata;
...
use antex_app_server_protocol::ScheduledTaskSchedule;
use antex_app_server_protocol::ScheduledTaskSummary;
use antex_app_server_protocol::ScheduledTaskWeekday;
use antex_app_server_protocol::SkillInterface;
...
async fn plugin_read_returns_canonical_openai_curated_marketplace_name() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.join("demo-plugin/.antex-plugin"))?;
    std::fs::write(
        repo_root.join("demo-plugin/.antex-plugin/plugin.json"),
        r#"{
  "name": "demo-plugin",
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.antex-plugin"))?;
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.antex-plugin/plugin.json"),
...
async fn plugin_read_agent_plugin_excludes_nested_skills() -> Result<()> {
    ...
    assert_eq!(
        ...
        vec![antex_app_server_protocol::SkillSummary {
