...
use antex_app_server_protocol::ThreadRealtimeAudioChunk;
...
use antex_app_server_protocol::ThreadRealtimeInitialItem;
use antex_app_server_protocol::ThreadRealtimeItem;
...
impl RealtimeE2eHarness {
    ...
    async fn start_webrtc_realtime(&mut self, offer_sdp: &str) -> Result<StartedWebrtcRealtime> {
        self.start_webrtc_realtime_with_antex_response_routing(
            offer_sdp,
    ...
    async fn start_webrtc_realtime_with_antex_response_items(
        &mut self,
        offer_sdp: &str,
    ...
    async fn start_webrtc_realtime_with_antex_response_routing(
        &mut self,
        offer_sdp: &str,
    ...
    async fn start_websocket_realtime(&mut self) -> Result<ThreadRealtimeStartedNotification> {
        self.start_websocket_realtime_with_antex_responses_as_items(
            /*codex_responses_as_items*/ None,
        )
        .await
    }

    async fn start_websocket_realtime_with_antex_response_items(
        &mut self,
    ...
    async fn start_websocket_realtime_with_antex_responses_as_items(
        &mut self,
        codex_responses_as_items: Option<bool>,
    ) -> Result<ThreadRealtimeStartedNotification> {
        let start_request_id = self
...
async fn webrtc_v3_start_posts_live_session_and_joins_without_session_update() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_antex_response_routing(
            "v=offer\r\n",
            /*client_managed_handoffs*/ None,
            /*codex_responses_as_items*/ None,
            /*codex_response_handoff_mode*/ None,
...
#[tokio::test]
async fn webrtc_v1_ignores_antex_response_handoff_mode() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn webrtc_v1_handoff_request_delegates_context_and_manual_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_antex_response_items("v=offer\r\n")
        .await?;
    assert_eq!(started.started.version, RealtimeConversationVersion::V1);
    assert_call_create_multipart(
        harness.call_capture.single_request(),
...
async fn realtime_automatic_standalone_output_is_item_and_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_websocket_realtime_with_antex_response_items()
        .await?;
    assert_eq!(started.version, RealtimeConversationVersion::V2);
    assert_eq!(
...
async fn websocket_v2_background_agent_steering_ack_requests_response_create() -> Result<()> {
    ...
    // Phase 4: release the gated delegated turn. Antex should then continue
...
async fn realtime_starts_on_forked_and_resumed_threads(
    ...
    if let RealtimeThreadLoad::ColdResume = load {
        ...
        harness.mcp = TestAppServer::builder()
            .with_antex_home(harness._antex_home.path())
            .build_initialized_with_timeout(DEFAULT_TIMEOUT)
            .await?;
