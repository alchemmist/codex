...
use antex_app_server_protocol::AppConfig;
use antex_app_server_protocol::AppLinkConfig;
use antex_app_server_protocol::AppLinksConfig;
use antex_app_server_protocol::AppToolApproval;
use antex_app_server_protocol::ApprovalsReviewer;
use antex_app_server_protocol::AppsConfig;
use antex_app_server_protocol::AppsDefaultConfig;
use antex_app_server_protocol::AskForApproval;
use antex_app_server_protocol::BrowserUseAccessApprovalLifetime;
use antex_app_server_protocol::BrowserUseConfig;
use antex_app_server_protocol::BrowserUseOriginPolicy;
use antex_app_server_protocol::BrowserUseOriginPolicyConfig;
use antex_app_server_protocol::BrowserUseRequirements;
use antex_app_server_protocol::CliAuthCredentialsStoreMode;
use antex_app_server_protocol::ComputerUseConfig;
use antex_app_server_protocol::ComputerUseMacosConfig;
use antex_app_server_protocol::ComputerUseMacosRequirements;
use antex_app_server_protocol::ComputerUseRequirements;
use antex_app_server_protocol::ComputerUseWindowsConfig;
use antex_app_server_protocol::ComputerUseWindowsExeConfig;
...
use antex_app_server_protocol::ConfigEdit;
use antex_app_server_protocol::ConfigLayerSource;
...
use antex_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use antex_app_server_protocol::ToolsV2;
...
use antex_protocol::config_types::WebSearchContextSize;
use antex_protocol::config_types::WebSearchLocation;
use antex_protocol::config_types::WebSearchToolConfig;
...
"#,
    ...
    let antex_home_path = codex_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(antex_home_path.join("config.toml"))?;
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_antex_folder: project_config.clone(),
        }
    );
    assert_eq!(
        origins
    ...
    assert_eq!(
        origins
            ...
            .name,
        ConfigLayerSource::Project {
            dot_antex_folder: project_config,
        }
    );
...
async fn config_read_respects_managed_project_root_markers() -> Result<()> {
    ...
    let workspace = TempDir::new()?;
    let ancestor_config = workspace.path().join(".antex");
    let child = workspace.path().join("child");
    let child_config = child.join(".antex");
    for dir in [
    ...
    let mut app_server = TestAppServer::builder()
        .with_antex_home(codex_home.path())
        .without_auto_env()
        .with_env_overrides(&[("ANTEX_APP_SERVER_MANAGED_CONFIG_PATH", Some(&managed_path))])
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    ...
    let project_layers = layers
        ...
        .into_iter()
        .filter_map(|layer| {
            if let ConfigLayerSource::Project { dot_antex_folder } = layer.name {
                Some((dot_antex_folder, layer.config, layer.disabled_reason))
            } else {
                None
            }
        })
...
fn assert_layers_user_then_optional_system(
    layers: &[antex_app_server_protocol::ConfigLayer],
