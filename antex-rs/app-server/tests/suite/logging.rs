...
fn standalone_app_server_emits_json_info_events() -> Result<()> {
    let codex_home = TempDir::new()?;
    let event = app_server_json_shutdown_event("antex-app-server", &[], codex_home.path())?;
...
async fn sqlite_log_metrics_exports_do_not_create_log_cycles() -> Result<()> {
    let quiet_period = antex_state::log_db::LogSinkQueueConfig::default().flush_interval
    ...
    for status in [200, 500] {
        ...
        let _app_server = TestAppServer::builder()
            ...
            .with_env_overrides(&[
                ...
                (
                    antex_app_server_transport::REMOTE_CONTROL_DISABLED_ENV_VAR,
                    Some("1"),
                ),
            ])
        ...
        let exports = timeout(export_timeout, async {
            loop {
                ...
                if requests.iter().any(|request| {
                    String::from_utf8_lossy(&request.body).contains(antex_state::LOG_WRITE_METRIC)
                }) {
                    break requests.len();
                }
        ...
        let expected_metrics = [
            (antex_state::LOG_WRITE_METRIC, "sum"),
            (antex_state::LOG_WRITE_DURATION_METRIC, "histogram"),
            (antex_state::LOG_WRITE_BYTES_METRIC, "histogram"),
            (antex_state::LOG_WRITE_ENTRIES_METRIC, "histogram"),
            (antex_state::LOG_WRITE_MAX_ENTRY_BYTES_METRIC, "histogram"),
        ];
        ...
        for (name, kind) in expected_metrics {
            ...
            for point in points {
                ...
                if matches!(
                    name,
                    antex_state::LOG_WRITE_BYTES_METRIC
                        | antex_state::LOG_WRITE_MAX_ENTRY_BYTES_METRIC
                ) {
        ...
        let write_count = observed_metrics[antex_state::LOG_WRITE_METRIC].0;
        ...
        .await?;
        let persisted_logs = state.query_logs(&antex_state::LogQuery::default()).await?;
        assert_eq!(
            observed_metrics[antex_state::LOG_WRITE_ENTRIES_METRIC].1,
            persisted_logs.len() as f64,
            "exported batch sizes must equal the number of persisted SQLite log rows"
        );
        assert!(
            observed_metrics[antex_state::LOG_WRITE_BYTES_METRIC].1
                >= observed_metrics[antex_state::LOG_WRITE_MAX_ENTRY_BYTES_METRIC].1,
...
async fn sqlite_log_metrics_grpc_exports_do_not_create_log_cycles(grpc_status: &str) -> Result<()> {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |request: &wiremock::Request| {
            let metric_name = antex_state::LOG_WRITE_METRIC.as_bytes();
            if request
                .body
...
async fn app_server_emits_structured_tool_call_timing_event() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        .with_antex_home(codex_home.path())
        .with_json_logging("warn,antex_core::tools::parallel=info")
        .build_initialized()
        .await?;
    ...
    assert_eq!(
        ...
        json!({
            ...
            "target": "antex_core::tools::parallel",
        })
    );
