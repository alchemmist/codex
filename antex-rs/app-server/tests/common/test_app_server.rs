...
use antex_app_server_protocol::FsCopyParams;
use antex_app_server_protocol::FsCreateDirectoryParams;
use antex_app_server_protocol::FsGetMetadataParams;
use antex_app_server_protocol::FsReadDirectoryParams;
use antex_app_server_protocol::FsReadFileParams;
use antex_app_server_protocol::FsRemoveParams;
use antex_app_server_protocol::FsUnwatchParams;
use antex_app_server_protocol::FsWatchParams;
use antex_app_server_protocol::FsWriteFileParams;
...
use antex_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use antex_app_server_protocol::PluginSearchParams;
use antex_app_server_protocol::PluginSkillReadParams;
use antex_app_server_protocol::PluginUninstallParams;
use antex_app_server_protocol::ProcessKillParams;
use antex_app_server_protocol::ProcessSpawnParams;
use antex_app_server_protocol::ProjectImportParams;
use antex_app_server_protocol::ProjectListParams;
use antex_app_server_protocol::ProjectReadParams;
use antex_app_server_protocol::RemoteControlClientsListParams;
use antex_app_server_protocol::RemoteControlClientsRevokeParams;
use antex_app_server_protocol::RemoteControlPairingStartParams;
use antex_app_server_protocol::RemoteControlPairingStatusParams;
...
use antex_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use antex_exec_server::ANTEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
...
use core_test_support::test_antex::TestEnv;
...
pub struct TestAppServer {
    ...
    // removing an owned ANTEX_HOME that may still be its cwd on Windows.
    ...
    _owned_antex_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "codex-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "ANTEX_APP_SERVER_DISABLE_MANAGED_CONFIG";
#[cfg(windows)]
...
impl TestAppServer {
    /// Starts building a server with a temporary ANTEX_HOME and the standard
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.env_remove(ANTEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = loop {
            ...
            if !process
                ...
                || retries == 2
            {
                break process.context("antex-mcp-server proc should start")?;
            }
            retries += 1;
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: antex_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    /// Uses this existing ANTEX_HOME instead of a temporary one.
    pub fn with_antex_home(mut self, codex_home: &Path) -> Self {
    ...
    /// Builds a server with a temporary ANTEX_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (codex_home, owned_antex_home) = match codex_home {
            Some(codex_home) => (codex_home, None),
            None => {
                let owned_antex_home = TempDir::new()?;
                (
                    owned_antex_home.path().to_path_buf(),
                    Some(owned_antex_home),
                )
            }
        };
        ...
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            antex_utils_cargo_bin::cargo_bin("exec-server")
                ...
                let mut auto_env_overrides = vec![
                    (
                        ANTEX_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        ANTEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        ANTEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                    (ANTEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(), None),
                    (
                        ANTEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                ];
        ...
        let mut program = match program {
            ...
            None => antex_utils_cargo_bin::cargo_bin("antex-app-server")
                .context("should find binary for antex-app-server")?,
        };
        let mut owned_install_dir = None;
        if !custom_program
            && antex_utils_cargo_bin::runfiles_available()
            && let Ok(code_mode_host_program) =
                antex_utils_cargo_bin::cargo_bin("antex-code-mode-host")
        {
        ...
        app_server._owned_antex_home = owned_antex_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `antex-app-server` child process.
        //
