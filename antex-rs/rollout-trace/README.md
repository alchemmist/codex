...
> `ANTEX_ROLLOUT_TRACE_ROOT` is set. Those local bundles can contain prompts,
...
```mermaid
flowchart TD
    subgraph Runtime["antex-core runtime"]
...
one from `ANTEX_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
`antex_rollout_trace::replay_bundle` directly.
