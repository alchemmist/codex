//! Mapping from Antex protocol events into raw rollout-trace events.
//!
...
//! `antex-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `antex-rollout-trace`.
//!
...
use antex_protocol::protocol::PatchApplyStatus;
use antex_protocol::protocol::SubAgentActivityEvent;
...
use crate::AntexTurnId;
use crate::ExecutionStatus;
use crate::RawTraceEventPayload;

pub(crate) struct AntexTurnTraceEvent {
    pub context_turn_id: AntexTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn antex_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<AntexTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let antex_turn_id = event.turn_id.clone();
            Some(AntexTurnTraceEvent {
                context_turn_id: antex_turn_id.clone(),
                payload: RawTraceEventPayload::AntexTurnStarted {
                    antex_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(AntexTurnTraceEvent {
                context_turn_id: antex_turn_id.clone(),
                payload: RawTraceEventPayload::AntexTurnEnded {
                    antex_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a antex_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a antex_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a antex_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a antex_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a antex_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a antex_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a antex_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a antex_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [antex_protocol::parse_command::ParsedCommand],
