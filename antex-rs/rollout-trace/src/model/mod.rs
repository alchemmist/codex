...
/// Antex conversation/session UUID.
...
/// Process/session ID returned by Antex's terminal runtime.
...
/// Canonical reduced graph for one Antex rollout.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct RolloutTrace {
    pub schema_version: u32,
    /// Unique identity for this trace capture.
    ///
    /// `rollout_id` names the Antex rollout/session being observed. `trace_id`
    ...
    #[serde(rename = "codex_turns")]
    pub antex_turns: BTreeMap<AntexTurnId, AntexTurn>,
