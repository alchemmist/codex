...
///
/// A Antex turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Antex turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        antex_turn_id: AntexTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                antex_turn_id: attempt.context.antex_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps antex-core focused on
    ...
    /// Records a provider stream that Antex intentionally stopped consuming.
    ///
...
/// Antex received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        antex_turn_id: Some(context.antex_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.antex_turn_id, "turn-1");
