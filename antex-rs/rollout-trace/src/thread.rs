...
use crate::protocol_event::antex_turn_trace_event;
...
pub const ANTEX_ROLLOUT_TRACE_ROOT_ENV: &str = "ANTEX_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `ANTEX_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Antex
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(ANTEX_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Antex turn lifecycle events from protocol lifecycle events.
    pub fn record_antex_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            antex_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Antex did while executing that boundary.
    pub fn record_tool_call_event(&self, antex_turn_id: impl Into<AntexTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(antex_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_antex_turn_id: impl Into<AntexTurnId>,
    ...
    pub fn record_antex_turn_started(&self, antex_turn_id: impl Into<AntexTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let antex_turn_id = antex_turn_id.into();
        context.append_with_context_best_effort(
            antex_turn_id.clone(),
            RawTraceEventPayload::AntexTurnStarted {
                antex_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    /// Builds reusable inference trace context for one Antex turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        antex_turn_id: AntexTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            antex_turn_id: Some(antex_turn_id),
        };
