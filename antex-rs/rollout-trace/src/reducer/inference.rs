...
impl TraceReducer {
    ...
    /// If a call is still `Running`, Antex stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.antex_turn_id == antex_turn_id
