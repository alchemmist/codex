...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.antex_turns.get(&started.antex_turn_id) else {
            bail!(
                "compaction request {} referenced unknown codex turn {}",
                started.compaction_request_id,
                started.antex_turn_id
            );
        };
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                antex_turn_id: started.antex_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.antex_turns.get(&antex_turn_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown codex turn {antex_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but codex turn {antex_turn_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) antex_turn_id: String,
