...
impl TraceReducer {
    ...
    /// Starts a Antex turn inside an existing thread.
    pub(super) fn start_antex_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.antex_turns.contains_key(&antex_turn_id) {
            bail!("duplicate codex turn start for {antex_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.antex_turns.insert(
            antex_turn_id.clone(),
            AntexTurn {
                antex_turn_id,
                thread_id,
    ...
    /// Marks a Antex turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_antex_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.antex_turns.get(&antex_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "codex turn end for {antex_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.antex_turns.get_mut(&antex_turn_id) else {
            bail!("codex turn end referenced unknown turn {antex_turn_id}");
        };
