...
//!
//! The history is stored at `~/.antex/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.antex`.
...
pub async fn append_entry(
    ...
    // Resolve `~/.antex/history.jsonl` and ensure the parent directory exists.
