...
fn focus_gained_with_unanswered_palette_queries_preserves_immediate_input() -> Result<()> {
    ...
    let mut terminal = PtyAntex::start(&repo_root, codex_home, &[])?;
...
async fn interactive_startup_honors_antex_home_symlink_opt_out() -> Result<()> {
    ...
    let mut terminal = PtyAntex::start(
        &workspace_path,
        codex_home,
...
impl PtyAntex {
    pub(super) fn start(
        ...
        let antex = antex_utils_cargo_bin::cargo_bin("antex-tui")
            .or_else(|_| antex_utils_cargo_bin::cargo_bin("antex"))?;
        let child = Command::new(antex)
            .args(extra_args)
            .arg("--no-alt-screen")
            .arg("-C")
            .arg(repo_root)
            ...
            .stderr(slave)
            .spawn()
            .context("start Antex in focus-test pseudo-terminal")?;
    ...
    pub(super) fn wait_for_startup(&mut self) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if self.palette_answered
                && (self.screen_contains("Antex") || self.screen_contains("alchemmist antex"))
            {
                return Ok(());
            }
            ...
            if let Some(status) = self.child.try_wait()? {
                bail!(
                    "Antex exited before the focus test started ({status}); screen:\n{}",
        ...
        bail!(
            "Antex did not initialize within {:?}; screen:\n{}",
            STARTUP_TIMEOUT,
            self.screen_contents(),
        );
    }
...
pub(super) fn write_test_config(codex_home: &Path, repo_root: &Path) -> Result<()> {
    ...
    std::fs::write(codex_home.join("config.toml"), config)
        .context("write focus-test Antex configuration")?;
    std::fs::write(
