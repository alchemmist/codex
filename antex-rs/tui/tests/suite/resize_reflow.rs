...
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let antex = antex_binary(&repo_root)?;
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("ANTEX_HOME={}", codex_home.path().display()))
    ...
    let antex_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!antex_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&antex_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&antex_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&antex_pane)?;
    let restored_row =
...
async fn tmux_scrolled_composer_resize_preserves_visible_draft_text() -> Result<()> {
    ...
    let baseline =
        wait_for_capture_contains(&antex_pane, &final_line, Duration::from_secs(/*secs*/ 15))?;
        ...
        [("narrowed", "28", "9", 2), ("restored", "44", "14", 6)]
    {
        ...
        let capture = capture_pane(&antex_pane)?;
...
fn antex_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = antex_utils_cargo_bin::cargo_bin("antex") {
        return Ok(path);
    }

    let fallback = repo_root.join("antex-rs/target/debug/codex");
    anyhow::ensure!(
        fallback.is_file(),
        "antex binary is unavailable; run `cargo build -p antex-cli` first"
    );
    Ok(fallback)
}
