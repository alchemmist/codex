...
fn live_001_commit_on_overflow() {
    ...
    let mut term =
        antex_tui::Terminal::with_options(backend).expect("terminal should be constructed");
    let area = Rect::new(
    ...
    let mut rb = antex_tui::RowBuilder::new(/*target_width*/ 20);
    ...
    antex_tui::insert_history_lines(&mut term, lines)
