...
use antex_config::types::StartupPanelStyle;
use antex_otel::RuntimeMetricTotals;
use antex_otel::RuntimeMetricsSummary;
use antex_protocol::ThreadId;
use antex_protocol::account::PlanType;
use antex_protocol::error::UnexpectedResponseError;
...
fn mcp_tools_output_from_statuses_renders_status_only_servers() {
    let statuses = vec![McpServerStatus {
        ...
        auth_status: antex_app_server_protocol::McpAuthStatus::Unknown,
    }];
...
fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
    ...
    let cmd = "set -o pipefail\ncargo test -p antex-tui --quiet".to_string();
    let call_id = "c1".to_string();
    let mut cell = ExecCell::new(
        ExecCall {
            call_id: call_id.clone(),
