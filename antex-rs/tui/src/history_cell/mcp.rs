...
use antex_app_server_protocol::McpServerConnectionStatus;
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<antex_protocol::mcp::CallToolResult, String>,
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, antex_protocol::mcp::Tool>,
...
pub(crate) fn new_mcp_tools_output_from_statuses(
    ...
    for status in statuses {
        let (label, style) = match status.runtime_status {
            ...
            None if matches!(
                status.auth_status,
                antex_app_server_protocol::McpAuthStatus::NotLoggedIn
            ) =>
            {
                (
                    "authentication required",
