...
impl App {
    ...
    pub(super) async fn apply_permission_profile_selection(
        ...
        self.sync_active_thread_permission_settings_to_cached_session()
            .await;
        self.app_event_tx
            .send(AppEvent::AntexOp(AppCommand::override_turn_context(
                /*cwd*/ None,
                approval_policy,
                approvals_reviewer,
    ...
    pub(super) async fn read_effective_config_after_overridden_write(
        ...
        match crate::config_update::read_effective_config(app_server.request_handle(), cwd).await {
            ...
            Err(err) => {
                ...
                self.chat_widget.add_error_message(format!(
                    "{setting} were saved, but Antex could not refresh the effective config: {err}"
                ));
                None
            }
        }
...
fn windows_sandbox_mode_from_effective_config(
    effective_config: &ConfigReadResponse,
) -> Option<antex_config::types::WindowsSandboxModeToml> {
...
mod tests {
    ...
    #[tokio::test]
    async fn update_reasoning_effort_keeps_new_session_default() {
    ...
    #[tokio::test]
    async fn advanced_reasoning_keeps_configured_defaults_for_new_threads() {
        let mut app = make_test_app().await;
        app.config.model = Some("gpt-5.4".to_string());
        app.config.model_reasoning_effort = Some(ReasoningEffortConfig::Low);

        app.on_apply_advanced_reasoning("gpt-5.2", ReasoningEffortConfig::Ultra);
    ...
    async fn resume_model_settings_preserves_only_explicit_model_overrides() {
        ...
        let profile = "work"
            .parse::<antex_config::ProfileV2Name>()
    ...
    -> Result<()> {
        let mut app = make_test_app().await;
        let codex_home = tempdir()?;
        let required_policy = antex_protocol::protocol::AskForApproval::Never;
