...
mod key_chords;
#[path = "tests/live_service_tier_tests.rs"]
mod live_service_tier_tests;
...
use antex_utils_absolute_path::test_support::PathExt;
...
use antex_app_server_client::AppServerPath;
use antex_app_server_protocol::AdditionalFileSystemPermissions;
use antex_app_server_protocol::AdditionalNetworkPermissions;
use antex_app_server_protocol::AdditionalPermissionProfile;
use antex_app_server_protocol::AgentMessageDeltaNotification;
...
use antex_app_server_protocol::FileUpdateChange;
use antex_app_server_protocol::ItemStartedNotification;
use antex_app_server_protocol::JSONRPCErrorError;
use antex_app_server_protocol::McpServerElicitationRequest;
...
use antex_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use antex_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use antex_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use antex_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use antex_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use antex_app_server_protocol::PatchChangeKind;
use antex_app_server_protocol::PermissionsRequestApprovalParams;
use antex_app_server_protocol::RequestId as AppServerRequestId;
...
use antex_app_server_protocol::TokenUsageBreakdown;
...
use antex_app_server_protocol::UserInput as AppServerUserInput;
...
use antex_protocol::models::NetworkPermissions;
use antex_protocol::models::PermissionProfile;
use antex_protocol::openai_models::MODEL_SPECIALTY_CYBER;
use antex_protocol::protocol::EventMsg;
use antex_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
use antex_protocol::protocol::MultiAgentVersion;
use antex_protocol::protocol::SessionMeta;
use antex_protocol::protocol::SessionSource as RolloutSessionSource;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let antex_app_server_client::AppServerEvent::ServerNotification(notification) = event
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: antex_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn external_editor_writable_directory_rejected_snapshot() -> Result<()> {
    ...
    let fallback_home = dirs::home_dir()
        ...
        .join(".antex")
        .abs();
    let workspace_antex_home = app.chat_widget.config_ref().cwd.join(".antex");
    let permission_profile = PermissionProfile::workspace_write_with(
        &[codex_home, fallback_home, workspace_antex_home],
        antex_protocol::permissions::NetworkSandboxPolicy::Restricted,
...
async fn enqueue_primary_thread_session_replays_buffered_approval_after_attach() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        antex_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::ServerRequestResolved(
                antex_app_server_protocol::ServerRequestResolvedNotification {
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_antex_backend_auth: false,
        model_catalog: app.model_catalog.clone(),
        feedback: antex_feedback::AntexFeedback::new(),
        is_first_run: false,
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::AntexOp(Op::UserTurn { items, .. }) => {
                assert!(
...
async fn history_lookup_response_is_routed_to_requesting_thread() -> Result<()> {
    ...
    let history_config = antex_message_history::HistoryConfig::new(
        app.local_settings.codex_home.clone(),
        &app.local_settings.history,
    );
    antex_message_history::append_entry("local prompt", thread_id, &history_config).await?;
    let (log_id, _) = antex_message_history::history_metadata(&history_config).await;
    ...
    let cursor = antex_message_history::HistoryBatchCursor::new(/*end_offset*/ 1);
...
async fn active_history_batch_is_delivered_without_replay_buffering() -> Result<()> {
    ...
    let event = HistoryLookupResponse::Batch {
        ...
        next_older_cursor: Some(antex_message_history::HistoryBatchCursor::new(
            /*end_offset*/ 0,
        )),
    };
...
async fn subagent_lifecycle_updates_active_agents_status_line_immediately() -> Result<()> {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        subagent_activity_notification(
            ...
            first_thread_id,
            "/root/first",
            antex_app_server_protocol::SubAgentActivityKind::Started,
        ),
    )));
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        subagent_activity_notification(
            ...
            second_thread_id,
            "/root/second",
            antex_app_server_protocol::SubAgentActivityKind::Interrupted,
        ),
    )));
    assert_eq!(app.chat_widget.status_line_text(), None);
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: antex_app_server_protocol::CollabAgentTool::Wait,
                status: antex_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemCompleted(antex_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: antex_app_server_protocol::CollabAgentTool::SendInput,
                status: antex_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    antex_app_server_protocol::CollabAgentState {
                        status: antex_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
    ...
    runtime.block_on(async {
        ...
        assert!(
            std::iter::from_fn(|| app_event_rx.try_recv().ok())
                .any(|event| matches!(event, AppEvent::AntexOp(Op::UserTurn { .. })))
        );
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let codex_home = tempdir()?;
        app.config.codex_home = codex_home.path().to_path_buf().abs();
        app.config.sqlite = antex_state::SqliteConfig::new_for_testing(codex_home.path().abs());
        ...
        let state_db = antex_state::StateRuntime::init(
            antex_state::SqliteConfig::new_for_testing(codex_home.path().abs()),
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::AntexOp(op)) => op,
        other => panic!("expected AntexOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
async fn side_defers_subagent_approval_overlay_until_side_exits() -> Result<()> {
    ...
    app.enqueue_thread_request(
        ...
        ServerRequest::DynamicToolCall {
            ...
            params: antex_app_server_protocol::DynamicToolCallParams {
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        ...
        vec![
            antex_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            antex_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            antex_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            antex_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: antex_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: antex_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: antex_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                updated_at: 2,
                recency_at: Some(2),
                status: antex_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: antex_app_server_protocol::SessionSource::Unknown,
...
async fn background_side_cleanup_removes_local_state_and_ignores_late_events() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        antex_app_server_client::AppServerEvent::ServerRequest(Box::new(exec_approval_request(
            side_thread_id,
            "turn-1",
            "item-1",
            Some("approval-1"),
    ...
    let resolution = app
        ...
        .take_resolution(
            ...
            Op::ExecApproval {
                ...
                turn_id: None,
                decision: antex_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            },
        )
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            antex_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        antex_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let codex_home = app_server
        .antex_home_path(&app.chat_widget.config_ref().codex_home)
        .expect("antex home");
...
async fn transcript_reflow_restyles_existing_user_message_after_theme_change() {
    ...
    app.config.terminal_resize_reflow.max_rows =
        crate::legacy_core::config::TerminalResizeReflowMaxRows::Disabled;
...
fn subagent_activity_notification(
    ...
    agent_path: &str,
    kind: antex_app_server_protocol::SubAgentActivityKind,
) -> ServerNotification {
    ServerNotification::ItemCompleted(antex_app_server_protocol::ItemCompletedNotification {
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        codex_error_info: Some(AppServerAntexErrorInfo::ActiveTurnNotSteerable {
...
async fn remote_exec_resume_current_cwd_is_rejected() -> Result<()> {
    ...
    app.environment_manager = Arc::new(
        EnvironmentManager::create_for_tests(
            ...
            Some(antex_exec_server::ExecServerRuntimePaths::new(
                std::env::current_exe()?,
                /*antex_linux_sandbox_exe*/ None,
            )?),
        )
        .await,
    );
...
async fn in_app_resume_uses_configured_or_explicit_cwd() -> Result<()> {
    let temp_dir = tempdir()?;
    let codex_home = temp_dir.path().join("antex-home");
...
async fn remembered_current_cwd_stays_at_launch_across_in_app_resumes() -> Result<()> {
    ...
    crate::legacy_core::config::edit::ConfigEditsBuilder::for_config(&config)
        .set_resume_cwd(antex_config::types::ResumeCwdMode::Current)
        .apply()
        .await
...
async fn prompt_edit_forks_before_selected_prompt_and_preserves_source() -> Result<()> {
    ...
    for (turn_id, message, images, local_images) in [
        ...
        for item in [
            ...
            antex_rollout::append_rollout_item_to_path(&source_path, &item).await?;
        }
    }
        ...
        .await?;
    {
        ...
        for item in selected_turn.items {
            store.push_notification(ServerNotification::ItemCompleted(
                antex_app_server_protocol::ItemCompletedNotification {
...
async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
    ...
    let replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_antex_backend_auth: app.chat_widget.has_antex_backend_auth(),
        model_catalog: app.model_catalog.clone(),
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(Box::new(
                ServerNotification::ItemStarted(
                    antex_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: antex_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                antex_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                antex_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn changing_cyber_model_reasoning_preserves_selected_permissions() {
    Box::pin(async {
        ...
        assert!(
            app.apply_permission_profile_selection(PermissionProfileSelection {
                profile_id: antex_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
                    .to_string(),
        ...
        for effort in [ReasoningEffortConfig::High, ReasoningEffortConfig::Ultra] {
            ...
            assert_eq!(
                settings
                    ...
                    .expect("active permission profile")
                    .id,
                antex_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
            );
            assert_eq!(
...
async fn selecting_cyber_model_falls_back_to_user_when_auto_review_is_unavailable() {
    ...
    assert_eq!(
        ...
        Some(antex_app_server_protocol::AskForApproval::OnRequest)
    );
    assert_eq!(
        params.approvals_reviewer,
        Some(antex_app_server_protocol::ApprovalsReviewer::User)
    );
}
...
async fn selecting_cyber_model_respects_auto_review_requirements() {
    Box::pin(async {
        let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
        let requirements_toml = antex_config::ConfigRequirementsToml {
            allowed_approval_policies: Some(vec![AskForApproval::UnlessTrusted.to_core()]),
            ..Default::default()
        };
        let mut requirements_with_sources = antex_config::ConfigRequirementsWithSources::default();
        requirements_with_sources.merge_unset_fields(
            antex_config::RequirementSource::Unknown,
            requirements_toml.clone(),
        );
        let requirements = antex_config::ConfigRequirements::try_from(requirements_with_sources)
            .expect("reviewer requirements");
        app.config.config_layer_stack =
            antex_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            active_permission_profile: Some(
                antex_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
...
async fn clear_only_ui_reset_allows_active_skill_warning_to_render_again() {
    ...
    let error = SkillErrorInfo {
        path: test_path_buf("/tmp/project/.antex/skills/abc/SKILL.md"),
