                ...
                let result = app_server
                    ...
                    .map(|response| {
                        let token_info = response.used_tokens.map(|used_tokens| {
                            antex_protocol::protocol::TokenUsageInfo {
                                total_token_usage: antex_protocol::protocol::TokenUsage {
                        ...
                        antex_protocol::protocol::ContextInspection {
                            base_instructions: antex_protocol::models::BaseInstructions {
...
&self.local_settings,
                    ...
                    if self.chat_widget.thread_id() == Some(thread_id)
                        ...
                        && let Err(error) = app_server
                            .thread_goal_set(
                                thread_id,
                                /*objective*/ None,
                                Some(antex_app_server_protocol::ThreadGoalStatus::Paused),
                                /*token_budget*/ None,
                            )
                            .await
                    {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    .await
                    {
                        ...
                        Ok(_) => {
                            ...
                            {
                                self.app_event_tx.send(AppEvent::AntexOp(
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "Antex can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            } else {
            ...
            AppEvent::AgentsDaemonStarted { result } => match result {
                Ok(()) => self.chat_widget.add_info_message(
                    "Background server started. Run `antex agents` in another terminal; this session remains unchanged."
                        .to_string(),
                    /*hint*/ None,
                ),
                Err(error) => self
