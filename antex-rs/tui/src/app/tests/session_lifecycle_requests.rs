...
use antex_app_server_protocol::ClientNotification;
use antex_app_server_protocol::ClientRequest;
use antex_app_server_protocol::JSONRPCError;
...
use antex_protocol::items::ExitedReviewModeItem;
...
async fn start_recording_app_server_with_history(
    ...
    let embedded = crate::start_embedded_app_server(
        antex_arg0::Arg0DispatchPaths::default(),
        config.clone(),
        Vec::new(),
        loader_overrides,
        /*strict_config*/ false,
        antex_config::CloudConfigBundleLoader::default(),
        antex_feedback::AntexFeedback::new(),
        /*log_db*/ None,
        state_db,
...
async fn external_transport_registers_dynamic_tools_and_finds_task_mentions() -> Result<()> {
    let (app, _antex_home) = make_history_test_app().await?;
    ...
    for params in starts {
        ...
        assert_eq!(params["dynamicTools"][0]["name"], "antex_tui");
        assert_eq!(
...
async fn archive_current_thread_reports_success_only_after_archiving() -> Result<()> {
    let (mut app, _antex_home) = make_history_test_app().await?;
    let thread_id = ThreadId::from_string(
        &create_fake_rollout(
...
async fn local_daemon_registers_approval_gated_mcp_tools_for_both_start_paths() -> Result<()> {
    ...
    app.config
        .web_search_mode
        .set(antex_protocol::config_types::WebSearchMode::Live)?;
    std::fs::write(
    ...
    let inventory: antex_app_server_protocol::ListMcpServerStatusResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::McpServerStatusList {
            request_id: AppServerRequestId::String("tui-tool-inventory".to_string()),
            params: antex_app_server_protocol::ListMcpServerStatusParams {
                cursor: None,
                limit: None,
                detail: Some(antex_app_server_protocol::McpServerStatusDetail::ToolsAndAuthOnly),
    ...
    let tools = &inventory
        .data
        .iter()
        .find(|server| server.name == "antex_tui")
    ...
    for params in &starts {
        ...
        let server = &params["config"]["mcp_servers.antex_tui"];
        assert!(
            server["url"]
                .as_str()
    ...
    let mcp_url = starts[0]["config"]["mcp_servers.antex_tui"]["url"]
        .as_str()
        .expect("MCP server URL");
    let unauthorized = antex_http_client::HttpClientBuilder::new()
        .build_direct()?
        .post(mcp_url)
        .send()
        .await?;
    ...
    app.config
        .web_search_mode
        .set(antex_protocol::config_types::WebSearchMode::Disabled)?;
    ...
    assert_eq!(
        resumed["config"]["mcp_servers.antex_tui"],
        starts[0]["config"]["mcp_servers.antex_tui"]
    );
    app_server
        .resume_thread(
            &app.local_settings,
    ...
    assert_eq!(
        reattached["config"]["mcp_servers.antex_tui"],
        starts[0]["config"]["mcp_servers.antex_tui"]
    );
    app_server
    ...
    assert_eq!(
        forked["config"]["mcp_servers.antex_tui"],
        starts[0]["config"]["mcp_servers.antex_tui"]
    );
    let authorization =
        starts[0]["config"]["mcp_servers.antex_tui"]["http_headers"]["Authorization"]
            .as_str()
            .expect("MCP bearer token");
    let client = antex_http_client::HttpClientBuilder::new().build_direct()?;
    ...
    assert_eq!(
        child["config"]["mcp_servers.antex_tui"],
        starts[0]["config"]["mcp_servers.antex_tui"]
    );
...
async fn local_mcp_respects_configured_servers_and_managed_requirements() -> Result<()> {
    for scenario in ["conflicting", "blocked", "mismatched", "allowed"] {
        ...
        if scenario == "conflicting" {
            let raw = serde_json::from_value::<antex_config::RawMcpServerConfig>(
            ...
            servers.insert(
                ...
                antex_config::McpServerConfig::try_from(raw)
            ...
            if matches!(scenario, "mismatched" | "allowed") {
                let requirement = if scenario == "allowed" {
                    antex_config::McpServerRequirement::Url(
                        antex_protocol::mcp_policy::McpServerValueMatcher::Prefix {
                            value: "http://127.0.0.1:".to_string(),
                        },
                    )
                } else {
                    antex_config::McpServerRequirement::Identity {
                        identity: antex_config::McpServerIdentity::Url {
            ...
            let requirements = antex_config::ConfigRequirements {
                mcp_servers: Some(antex_config::Sourced::new(
                    allowed_servers,
                    antex_config::RequirementSource::Unknown,
                )),
                ..Default::default()
            };
            app.config.config_layer_stack = antex_config::ConfigLayerStack::new(
                Vec::new(),
                requirements,
        ...
        if scenario == "allowed" {
            ...
            assert!(start["config"]["mcp_servers.antex_tui"].is_object());
        } else {
            assert_eq!(
                start["dynamicTools"][0]["tools"].as_array().map(Vec::len),
                Some(6)
            );
            assert!(start["config"]["mcp_servers.antex_tui"].is_null());
        }
        app_server.shutdown().await?;
        proxy.await??;
    }
...
async fn embedded_server_rejects_unowned_dynamic_tool_calls() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        antex_app_server_client::AppServerEvent::ServerRequest(Box::new(
...
async fn dynamic_tool_requests_ignore_other_namespaces_and_dispatch_tui_namespace() -> Result<()> {
        ...
        antex_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::DynamicToolCall {
                ...
                params: antex_app_server_protocol::DynamicToolCallParams {
                    ...
                    namespace: Some("antex_tui".to_string()),
                    tool: "list_threads".to_string(),
                    arguments: serde_json::json!({}),
                },
            },
    ...
    app_server
        ...
        .await?;
    let project: antex_app_server_protocol::ProjectCreateResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ProjectCreate {
            request_id: AppServerRequestId::String("create-source-project".to_string()),
            params: antex_app_server_protocol::ProjectCreateParams {
                name: "Source project".to_string(),
                roots: vec![antex_app_server_protocol::ProjectRoot {
        ...
        .await?;
    let _: antex_app_server_protocol::ThreadMetadataUpdateResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ThreadMetadataUpdate {
            request_id: AppServerRequestId::String("assign-source-project".to_string()),
            params: antex_app_server_protocol::ThreadMetadataUpdateParams {
        ...
        .await?;
    let source_settings: antex_app_server_protocol::ThreadResumeResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ThreadResume {
            request_id: AppServerRequestId::String("read-source-sandbox".to_string()),
            params: antex_app_server_protocol::ThreadResumeParams {
                thread_id: creation_source.to_string(),
                ..antex_app_server_protocol::ThreadResumeParams::default()
            },
        })
        .await?;
    ...
    spawn_approved_task_tool_call(
        ...
        antex_app_server_protocol::DynamicToolCallParams {
            thread_id: creation_source.to_string(),
            turn_id: "turn-1".to_string(),
...
async fn remote_legacy_history_start_negotiates_once_for_resume_and_fork() -> Result<()> {
    ...
    let response = crate::dynamic_tools::execute(
        ...
        antex_app_server_protocol::ThreadStartParams::default(),
        status_updates,
        /*app_event_tx*/ None,
    )
    .await;
                ...
                app.handle_app_server_event(
                    ...
                    AppServerEvent::ServerNotification(Box::new(
                        ServerNotification::ThreadStatusChanged(
                            antex_app_server_protocol::ThreadStatusChangedNotification {
                                thread_id: started.session.thread_id.to_string(),
...
async fn changing_directory_preserves_project_trust_permissions_history_and_hooks() -> Result<()> {
    use antex_protocol::config_types::TrustLevel as T;
    use serde_json::json;
    use std::fs;
    ...
    for directory in [&trusted, &unknown, &untrusted, &mismatch, &failed] {
        fs::create_dir_all(directory.join(".antex"))?;
        fs::write(directory.join(".antex/config.toml"), "")?;
    }
    let contents = "developer_instructions = \"destination policy\"\nmodel_reasoning_effort = \"high\"\napproval_policy = \"on-request\"\n[tui]\ntheme = \"dracula\"\n[tui.keymap.global]\nopen_transcript = \"f12\"";
    fs::write(trusted.join(".antex/config.toml"), contents)?;
    ...
    fs::write(trusted.join(".antex/hooks.json"), hooks)?;
    let contents = "default_permissions = \"dev\"\n[permissions.dev.filesystem]\n\":root\" = \"write\"\n[tui.keymap.global]\nopen_transcript = \"ctrl-l\"";
    fs::write(mismatch.join(".antex/config.toml"), contents)?;
    ...
    let meta = antex_rollout::read_session_meta_line(&forked_rollout).await?;
    ...
    let source = antex_utils_path_uri::PathUri::from_abs_path(&agents.abs());
