...
pub(super) fn configure_reserve_catalog(app: &mut App) {
    ...
    init.has_antex_backend_auth = true;
...
async fn luna_reserve_entry_dispatches_an_already_queued_turn_with_accepted_settings() -> Result<()>
{
    for mode in [ModeKind::Default, ModeKind::Plan] {
        ...
        let pending = std::iter::from_fn(|| events.try_recv().ok())
            .find(|event| matches!(event, AppEvent::AntexOp(AppCommand::UserTurn { .. })))
        ...
        let turn: antex_app_server_protocol::TurnStartParams =
    ...
    for has_credits in [false, true] {
        ...
        let mut pending = std::iter::from_fn(|| events.try_recv().ok())
            .find_map(|event| match event {
                AppEvent::AntexOp(op @ AppCommand::UserTurn { .. }) => Some(op),
                _ => None,
            })
        ...
        recovered.rate_limits.credits = Some(antex_app_server_protocol::CreditsSnapshot {
            has_credits,
            unlimited: false,
            balance: None,
        });
        ...
        assert!(
            !queued.iter().any(|event| matches!(
                event,
                AppEvent::AntexOp(_) | AppEvent::PersistModelSelection { .. }
            )),
