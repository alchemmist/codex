...
async fn luna_reserve_periodic_refresh_adapts_without_an_experiment_banner() -> Result<()> {
    ...
    app.config.sqlite = antex_state::SqliteConfig::new_for_testing(home.path().abs());
...
-> Result<()> {
    ...
    rolling.rate_limit_reached_type =
        Some(antex_app_server_protocol::RateLimitReachedType::WorkspaceMemberCreditsDepleted);
    app.handle_app_server_event(
        &session,
        antex_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::AccountRateLimitsUpdated(
                antex_app_server_protocol::AccountRateLimitsUpdatedNotification {
    ...
    app.handle_app_server_event(
        ...
        antex_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::AccountUpdated(
                antex_app_server_protocol::AccountUpdatedNotification {
                    auth_mode: None,
                    plan_type: None,
                },
            ),
...
async fn backend_banner_reset_redemption_rejects_pre_reset_content() -> Result<()> {
    ...
    app.handle_event(
        ...
        AppEvent::RateLimitResetCreditConsumed {
            ...
            credit_id: None,
            result: Ok(
                antex_app_server_protocol::ConsumeAccountRateLimitResetCreditResponse {
                    outcome:
                        antex_app_server_protocol::ConsumeAccountRateLimitResetCreditOutcome::Reset,
                },
            ),
        },
    )
    ...
    recovered.rate_limit_reset_credits =
        Some(antex_app_server_protocol::RateLimitResetCreditsSummary {
            available_count: 0,
            credits: None,
        });
    app.handle_event(
