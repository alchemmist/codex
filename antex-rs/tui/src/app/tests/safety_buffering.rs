...
fn next_user_turn_event(
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        if let AppEvent::AntexOp(turn @ AppCommand::UserTurn { .. }) = event {
            return turn;
        }
    }
...
"#,
    ...
    insta::assert_snapshot!(app.chat_widget.composer_text_with_pending(), @"Keep the accepted steer");
    assert!(
        std::iter::from_fn(|| app_event_rx.try_recv().ok())
            .all(|event| !matches!(event, AppEvent::AntexOp(AppCommand::UserTurn { .. })))
    );
...
"#,
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            antex_state::GoalAccountingMode::ActiveOrStopped,
