...
async fn lost_mutation_reply_preserves_work_without_resubmitting() -> Result<()> {
    ...
    app.chat_widget.show_selection_view(SelectionViewParams {
        items: vec![SelectionItem {
            ...
            actions: vec![Box::new(|tx| tx.send(AppEvent::AntexOp(Op::Interrupt)))],
            ..Default::default()
        }],
        ..Default::default()
    });
    assert!(matches!(
        app.handle_event(&mut tui, &mut session, AppEvent::AntexOp(op))
            .await?,
    ...
    let history = tokio::time::timeout(Duration::from_secs(/*secs*/ 5), async {
        loop {
            if let Ok(text) =
                ...
                && let Ok(entry) =
                    serde_json::from_str::<antex_message_history::HistoryEntry>(&text)
            {
                break entry;
            }
    ...
    let mut keymap = antex_config::types::TuiKeymap::default();
    keymap.chat.edit_queued_message = Some(antex_config::types::KeybindingsSpec::One(
        antex_config::types::KeybindingSpec("ctrl-x up".into()),
    ));
    let runtime =
    ...
    let draft = rendered
        .split_once("queued before disconnect")
        .map(|(_, suffix)| format!("queued before disconnect{suffix}"))
        .unwrap()
        .split("\n\n")
        .next()
        .unwrap()
