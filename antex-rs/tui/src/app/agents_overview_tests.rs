...
use antex_app_server_protocol::CurrentTimeReadParams;
use antex_app_server_protocol::ReasoningSummaryTextDeltaNotification;
...
use antex_config::types::KeybindingSpec;
use antex_config::types::KeybindingsSpec;
...
async fn overview_composer_preserves_editing_and_routes_focus() {
    ...
    view.handle_paste(format!(
        "x{}",
        " ".repeat(antex_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS)
    ));
    view.handle_key_event(KeyCode::Enter.into());
...
async fn shared_overview_keeps_rows_and_replays_changes_over_stale_reads() -> Result<()> {
    ...
    for event in [
        AppServerEvent::ServerNotification(Box::new(ServerNotification::ThreadReverted(
            antex_app_server_protocol::ThreadRevertedNotification {
                thread_id: retained.to_string(),
            },
        ))),
...
async fn shared_overview_seeds_once_and_retains_locally_resumed_history() -> Result<()> {
    ...
    for day in 1..=22 {
        let source = match day {
            3 => antex_protocol::protocol::SessionSource::Custom("atlas".to_string()),
            4 => antex_protocol::protocol::SessionSource::Custom("chatgpt".to_string()),
            5 => antex_protocol::protocol::SessionSource::Exec,
            6 => antex_protocol::protocol::SessionSource::Mcp,
            _ => antex_protocol::protocol::SessionSource::Cli,
        };
...
async fn overview_selection_applies_user_permissions_only_to_unloaded_threads() -> Result<()> {
    ...
    server_config
        .permissions
        .approval_policy
        .set(antex_protocol::protocol::AskForApproval::Never)?;
    ...
    let loaded = app_server
        ...
        .await?
        .session;
    app.harness_overrides.sandbox_mode = Some(antex_protocol::config_types::SandboxMode::ReadOnly);
    app.harness_overrides.approval_policy =
        Some(antex_protocol::protocol::AskForApproval::UnlessTrusted);
    app.config
        .permissions
    ...
    for (thread_id, expected_permissions, expected_approval) in [
        ...
        (
            ...
            antex_app_server_protocol::AskForApproval::UnlessTrusted,
        ),
        (
            thread_ids[2],
            PermissionProfile::read_only(),
        ...
        if thread_id == thread_ids[2] {
            ...
            app.harness_overrides.sandbox_mode =
                Some(antex_protocol::config_types::SandboxMode::WorkspaceWrite);
            app.harness_overrides.approval_policy =
                Some(antex_protocol::protocol::AskForApproval::Never);
            app.runtime_permission_profile_override =
                Some(RuntimePermissionProfileOverride::from_config(&app.config));
            app.runtime_approval_policy_override = Some(RuntimeApprovalPolicyOverride::Explicit(
                AskForApproval::UnlessTrusted,
    ...
    assert_eq!(
        ...
        (
            Some(thread_ids[3]),
            antex_app_server_protocol::ThreadStatus::NotLoaded
        ),
    );
    app_server.shutdown().await?;
    Ok(())
...
async fn overview_cold_resume_honors_working_directory_selection() -> Result<()> {
    for (mode, cli_cwd, runtime_cwd) in [
        ...
        crate::legacy_core::config::set_project_trust_level(
            app.config.codex_home.as_path(),
            &chosen,
            antex_protocol::config_types::TrustLevel::Trusted,
        )
        ...
        if expected_states.len() < targets.len() {
            ...
            app.chat_widget.handle_server_notification(
                ServerNotification::TurnStarted(
                    antex_app_server_protocol::TurnStartedNotification {
                        ...
                        turn: antex_app_server_protocol::Turn {
                            ...
                            items: Vec::new(),
                            status: antex_app_server_protocol::TurnStatus::InProgress,
                            error: None,
                            started_at: None,
                            completed_at: None,
                            duration_ms: None,
