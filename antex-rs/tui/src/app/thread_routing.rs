...
impl App {
            ...
            ServerRequest::McpServerElicitationRequest { request_id, params } => {
                    ...
                    )
                {
                    ...
                    match &params.request {
                        antex_app_server_protocol::McpServerElicitationRequest::Form {
                            message,
                            ..
                        ...
                        antex_app_server_protocol::McpServerElicitationRequest::OpenAiForm {
                            ..
                        }
                        | antex_app_server_protocol::McpServerElicitationRequest::OpenAiElicitationForm { .. }
                        | antex_app_server_protocol::McpServerElicitationRequest::Url { .. } => {
                            self.app_event_tx.resolve_elicitation(
                                thread_id,
                                params.server_name.clone(),
                                request_id.clone(),
                                antex_app_server_protocol::McpServerElicitationAction::Decline,
                                /*content*/ None,
                                /*meta*/ None,
                            );
                            None
    ...
    pub(super) fn append_message_history_entry(&self, thread_id: ThreadId, text: String) {
        ...
        tokio::spawn(async move {
            if let Err(err) =
                antex_message_history::append_entry(&text, thread_id, &history_config).await
            {
                tracing::warn!(
                    thread_id = %thread_id,
                    error = %err,
    ...
    pub(super) async fn lookup_message_history_entry(
        ...
        tokio::spawn(async move {
            let entry_opt = tokio::task::spawn_blocking(move || {
                antex_message_history::lookup(log_id, offset, &history_config)
            })
            .await
            .unwrap_or_else(|err| {
    ...
    pub(super) async fn lookup_message_history_batch(
        &mut self,
        thread_id: ThreadId,
        cursor: antex_message_history::HistoryBatchCursor,
        log_id: u64,
    ) -> Result<()> {
        let history_config = antex_message_history::HistoryConfig::new(
            self.local_settings.codex_home.clone(),
        ...
        tokio::spawn(async move {
            let event = match tokio::task::spawn_blocking(move || {
                antex_message_history::lookup_batch(log_id, cursor, &history_config)
            })
            .await
            {
                Ok(Ok(batch)) => {
        ...
        match op {
            ...
            AppCommand::ListSkills { cwds, force_reload } => {
                self.handle_skills_list_result(
                    app_server
                        .skills_list(antex_app_server_protocol::SkillsListParams {
                            cwds: cwds.clone(),
    ...
    fn startup_request_may_open_protected_view(&self, request: &ServerRequest) -> bool {
        ...
        match &params.request {
            antex_app_server_protocol::McpServerElicitationRequest::Form { .. } => true,
            antex_app_server_protocol::McpServerElicitationRequest::OpenAiForm { .. }
            | antex_app_server_protocol::McpServerElicitationRequest::OpenAiElicitationForm {
                ..
            } => false,
            ...
            Some(
                content
                    ...
                    .filter_map(|item| match item {
                        antex_app_server_protocol::UserInput::Text { text, .. } => {
