...
use antex_app_server_protocol::CreditsSnapshot as CoreCreditsSnapshot;
use antex_app_server_protocol::RateLimitSnapshot;
use antex_app_server_protocol::RateLimitWindow;
use antex_app_server_protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in ordered_snapshots {
        ...
        let combine_non_antex_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_antex_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_antex_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_antex_single_limit_renders_combined_row() {
        let now = Local::now();
        let antex = RateLimitSnapshotDisplay {
        ...
        let rows = match compose_rate_limit_data_many(&[antex, other], now) {
    ...
    #[test]
    fn non_antex_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "codex-other".to_string(),
            normal_model_slug: None,
