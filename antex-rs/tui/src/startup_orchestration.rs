...
pub(super) async fn run_main_inner(
    ...
    // `oss` model provider.
    let overrides_cli = antex_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    if !std::io::stdin().is_terminal() || !std::io::stdout().is_terminal() {
        let validation_target = app_server_target_for_launch(
            ...
            std::env::var_os(antex_exec_server::ANTEX_EXEC_SERVER_URL_ENV_VAR).as_deref(),
        )?;
        let validation_environment_manager =
            if should_load_configured_environments(&loader_overrides, &validation_target) {
                EnvironmentManager::prepare_from_antex_home(&codex_home).await
            } else {
    ...
    let initial_screen = if cli.resume_picker || cli.fork_picker || cli.agents_overview {
        ...
        && startup_preflight::should_delay_startup_composer_for_first_login(
            &codex_home,
            antex_config::loader::system_config_toml_file(),
            || antex_config::loader::has_local_managed_configuration(&codex_home),
    ...
    let prepared_environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            startup_draft
                .run_until(EnvironmentManager::prepare_from_antex_home(&codex_home))
                .await?
        } else {
            startup_draft
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        antex_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(antex_otel::OtelProvider::metrics) {
        let _ = antex_otel::record_process_start_once(metrics, otel_originator.as_str());
        ...
        let telemetry =
            antex_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = antex_state::install_process_db_telemetry(telemetry);
    }
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("antex_core=info,antex_tui=info,antex_rmcp_client=info")
        });
    ...
    let feedback = antex_feedback::AntexFeedback::new();
