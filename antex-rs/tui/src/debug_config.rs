...
use antex_config::NetworkUnixSocketPermissionToml;
use antex_config::RequirementSource;
use antex_config::ResidencyRequirement;
use antex_config::SandboxModeRequirement;
use antex_config::WebSearchModeRequirement;
use antex_config::format_config_layer_source;
...
mod tests {
    ...
    use antex_config::ConstraintError;
    use antex_config::FeatureRequirementsToml;
    use antex_config::FilesystemConstraints;
    use antex_config::HookEventsToml;
    use antex_config::HookHandlerConfig;
    use antex_config::LoaderOverrides;
    use antex_config::ManagedHooksRequirementsToml;
    use antex_config::MatcherGroup;
    use antex_config::McpServerIdentity;
    use antex_config::McpServerRequirement;
    use antex_config::NetworkConstraints;
    use antex_config::NetworkDomainPermissionToml;
    use antex_config::NetworkDomainPermissionsToml;
    use antex_config::NetworkHeaderInjectionToml;
    use antex_config::NetworkUnixSocketPermissionToml;
    use antex_config::NetworkUnixSocketPermissionsToml;
    use antex_config::RequirementSource;
    use antex_config::ResidencyRequirement;
    use antex_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        ...
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.antex")
        } else {
            absolute_path("/repo/.antex")
        };
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Antex\\requirements.toml")
        } else {
        ...
        let sqlite_home = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.antex\\state")
        } else {
            absolute_path("/home/alice/.antex/state")
        };
        let log_dir = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.antex\\logs")
        } else {
            absolute_path("/home/alice/.antex/logs")
        };
        let model_catalog_json = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.antex\\models.json")
        } else {
            absolute_path("/home/alice/.antex/models.json")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.antex\\config.toml")
        } else {
            absolute_path("/home/alice/.antex/config.toml")
        };
        let stack = ConfigLayerStack::new(
