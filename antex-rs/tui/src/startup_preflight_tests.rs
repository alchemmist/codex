...
fn startup_delays_composer_for_homes_without_authentication_state() -> std::io::Result<()> {
    ...
    let codex_home = temporary_directory.path().join("antex-home");
    ...
    assert!(should_delay_startup_composer_for_first_login(
        ...
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| (name == antex_login::ANTEX_ACCESS_TOKEN_ENV_VAR).then(|| "  ".into()),
    ));
    ...
    assert!(should_delay_startup_composer_for_first_login(
        ...
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| (name == "ANTEX_HOME").then(|| "/custom/home".into()),
    ));
    assert!(!should_delay_startup_composer_for_first_login(
        &codex_home,
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| { (name == antex_login::ANTEX_ACCESS_TOKEN_ENV_VAR).then(|| "access-token".into()) },
    ));
    for disabled_credential in [
        antex_login::OPENAI_API_KEY_ENV_VAR,
        antex_login::ANTEX_API_KEY_ENV_VAR,
    ] {
        assert!(should_delay_startup_composer_for_first_login(
            &codex_home,
            Ok(system_config_path.clone()),
    ...
    assert!(!should_delay_startup_composer_for_first_login(
        ...
        |name| match name {
            "ANTEX_HOME" => Some("/custom/home".into()),
            antex_login::ANTEX_ACCESS_TOKEN_ENV_VAR => Some("access-token".into()),
            _ => None,
        },
    ));
...
fn startup_keeps_composer_when_home_state_cannot_be_confirmed() -> std::io::Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(!should_delay_startup_composer_for_first_login(
            &blocking_parent.join("antex-home"),
            Ok(system_config_path),
...
fn startup_keeps_composer_for_workload_identity_markers() -> std::io::Result<()> {
    ...
    for marker in [
        antex_protocol::shell_environment::OPENAI_FEDERATION_RULE_ID_ENV_VAR,
        antex_protocol::shell_environment::OPENAI_IDENTITY_TOKEN_FILE_ENV_VAR,
    ] {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(!should_delay_startup_composer_for_first_login(
            ...
            || panic!("workload identity should not probe managed configuration"),
            |name| {
                (name == antex_protocol::shell_environment::OPENAI_IDENTITY_TOKEN_FILE_ENV_VAR)
