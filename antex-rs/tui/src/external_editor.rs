...
pub(crate) async fn run_editor(
    ...
    let default_antex_home = dirs::home_dir().map(|home| home.join(".antex"));
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    let project_antex_home = cwd.join(".antex");
    let mut candidate_homes = vec![codex_home];
    if let Some(default_antex_home) = default_antex_home.as_deref() {
        candidate_homes.push(default_antex_home);
    }
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    candidate_homes.push(&project_antex_home);
