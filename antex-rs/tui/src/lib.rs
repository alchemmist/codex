...
// The standalone `antex-tui` binary prints a short help message before the
...
use antex_app_server_client::RemoteAppServerClient;
use antex_app_server_client::RemoteAppServerConnectArgs;
pub use antex_app_server_client::RemoteAppServerEndpoint;
use antex_app_server_protocol::Account as AppServerAccount;
...
use antex_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
pub(crate) use antex_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "antex-tui.log";
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = antex_state::runtime_db_path_for_corruption_error(&err)
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let codex_home = find_antex_home().wrap_err("failed to resolve ANTEX_HOME")?;
            antex_app_server_client::app_server_control_socket_path(&codex_home)
...
async fn connect_remote_app_server(
    ...
    let app_server = RemoteAppServerClient::connect(RemoteAppServerConnectArgs {
        endpoint,
        client_name: "antex-tui".to_string(),
...
async fn maybe_probe_default_daemon_socket(codex_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = antex_app_server_client::app_server_control_socket_path(codex_home).ok()?;
    #[cfg(windows)]
    let (validated_path, _directory) =
        antex_uds::validate_private_socket_path(socket_path.as_path()).ok()?;
    #[cfg(windows)]
    ...
    match tokio::time::timeout(
        ...
        antex_uds::UnixStream::connect(probe_path),
    )
    .await
    {
        Ok(Ok(_stream)) => {
...
async fn resolve_startup_resume_or_fork_cwd(
    tui: &mut Tui,
    config: &Config,
    state_db: Option<&antex_state::StateRuntime>,
...
async fn run_ratatui_app(
         ...
         terminal_restore_guard: &mut TerminalRestoreGuard| {
            ...
            Ok(AppExitInfo {
                ...
                exit_reason: ExitReason::Fatal(format!(
                    "No saved session found with ID {id_str}. Run `antex {action}` without an ID to choose from existing sessions."
                )),
            })
        };
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_antex_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
...
async fn load_bootstrap_config_or_exit(
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        antex_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
"#
    ...
    fn write_session_rollout(
        ...
        let session_meta = antex_protocol::protocol::SessionMeta {
            ...
            source: antex_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(antex_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    async fn startup_services_use_final_cloud_managed_http_policy() -> color_eyre::Result<()> {
            ...
            [(true, false), (false, true)]
        {
            ...
            let prepared_environment_manager =
                EnvironmentManager::prepare_from_antex_home(codex_home.path()).await?;
            ...
            let cloud_config_bundle =
                antex_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: antex_app_server_client::app_server_control_socket_path(&codex_home)?,
            }
        );
        Ok(())
    }
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let codex_home = TempDir::new()?;
        let socket_path =
            antex_app_server_client::app_server_control_socket_path(codex_home.path())?;
        #[cfg(windows)]
        {
            let parent = socket_path.as_path().parent().expect("socket parent");
            std::fs::create_dir_all(parent)?;
            let listener = antex_uds::UnixListener::bind(socket_path.as_path()).await?;
            assert!(
                maybe_probe_default_daemon_socket(codex_home.path())
                    .await
                    .is_none()
        ...
        antex_uds::prepare_private_socket_directory(
            socket_path.as_path().parent().expect("socket parent"),
        )
        .await?;
        let _listener = antex_uds::UnixListener::bind(socket_path.as_path()).await?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            antex_app_server_client::AppServerClient::InProcess(
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        let sqlite =
            antex_state::SqliteConfig::new_for_testing(occupied_sqlite_home.as_path().abs());
    ...
    -> color_eyre::Result<()> {
        ...
        let sqlite = antex_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
        ...
        assert!(
            antex_state::sqlite_error_detail_is_corruption(startup_error.detail()),
