...
const MACOS_APP_TOOLTIP: &str =
    "Run `antex app` to open the Desktop app (it installs on macOS if needed).";
...
const OTHER_TOOLTIP_NON_MAC: &str = "*New* Build faster with Antex.";
const FREE_GO_TOOLTIP: &str =
    "*New* For a limited time, Antex is included in your plan for free – let’s build together.";
...
/// Pick a random tooltip to show to the user when starting Antex.
...
pub(crate) mod announcement {
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // Antex currently publishes CLI builds for macOS, Windows, and Linux.
                Self::Linux
            }
        }
    }
    ...
    pub(crate) fn parse_announcement_tip_toml(
        ...
        for raw in announcements {
            ...
            if tip.version_matches(ANTEX_CLI_VERSION)
...
mod tests {
    ...
    fn desktop_app_tooltip_uses_supported_platform_launcher() {
        ...
        if linux_app_tooltip(LinuxDesktopSession::current()).is_some() {
            ...
            insta::assert_snapshot!(tooltip, @"Run `antex app` to open the Desktop app (it installs on macOS if needed).");
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for Antex TUI.
...
[[announcements]]
content = "Welcome to Antex! Check out the new onboarding flow."
