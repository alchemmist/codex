            ...
            if matches!(
                ...
                self.handle_turn_completed_notification(
                    TurnCompletedNotification {
                        ...
                        turn: Turn {
                            id: turn_id,
                            items_view: antex_app_server_protocol::TurnItemsView::NotLoaded,
                            items: Vec::new(),
                            status,
                            error,
                            started_at,
    ...
    pub(super) fn handle_thread_item(
        ...
        match item {
                ...
                self.on_agent_message_item_completed(
                    AgentMessageItem {
                        ...
                        memory_citation: memory_citation.map(|citation| {
                            antex_protocol::memory_citation::MemoryCitation {
                                entries: citation
                                    .entries
                                    .into_iter()
                                    .map(|entry| {
                                        antex_protocol::memory_citation::MemoryCitationEntry {
                                            path: entry.path,
            ...
            item @ ThreadItem::CommandExecution {
                status: antex_app_server_protocol::CommandExecutionStatus::InProgress,
                ..
            } => self.on_command_execution_started(item),
            item @ ThreadItem::CommandExecution {
                source: ExecCommandSource::Agent | ExecCommandSource::UnifiedExecStartup,
                status:
                    antex_app_server_protocol::CommandExecutionStatus::Completed
                    | antex_app_server_protocol::CommandExecutionStatus::Failed,
                ..
            ...
            item @ ThreadItem::McpToolCall {
                status: antex_app_server_protocol::McpToolCallStatus::InProgress,
                ..
            ...
            ThreadItem::WebSearch(item) => {
                ...
                self.on_web_search_end(
                    ...
                    item.query,
                    item.action
                        .unwrap_or(antex_app_server_protocol::WebSearchAction::Other),
                );
            }
            ...
            ThreadItem::FunctionCallOutput {
                    ...
                    )
                {
                    self.add_to_history(history_cell::PrefixedWrappedHistoryCell::new(
                        format!("Sent by Antex from task {source_thread_id}\n{prompt}"),
                        "• ".dim(),
                        "  ",
                    ));
                }
