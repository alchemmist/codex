...
use antex_app_server_protocol::SkillsListEntry;
...
use antex_utils_plugins::mention_syntax::TOOL_MENTION_SIGIL;
...
pub(crate) fn find_app_mentions(
    ...
    for app in apps.iter().filter(|app| is_app_mentionable(app)) {
        let slug = antex_connectors::metadata::connector_mention_slug(app);
