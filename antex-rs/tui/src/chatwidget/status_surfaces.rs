...
impl ChatWidget {
    ...
    /// Clears the terminal title Antex most recently wrote, if any.
    ///
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_antex_folder } => {
                    dot_antex_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    pub(super) fn status_line_value(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::AntexVersion => Some(ANTEX_CLI_VERSION.to_string()),
            ...
            StatusLineItem::ActiveAgents => (self.active_subagent_count > 0)
                .then(|| format!("Agents {}", self.active_subagent_count)),
        }
    }
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            ...
            StatusSurfacePreviewItem::ActiveAgents => StatusLineItem::ActiveAgents,
            ...
            StatusSurfacePreviewItem::AntexVersion => StatusLineItem::AntexVersion,
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::AntexVersion => self
                .status_line_value_for_item(StatusLineItem::AntexVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
...
fn five_hour_status_window(
    ...
    find_primary_antex_window(snapshot, "5h")
...
fn find_primary_antex_window<'a>(
    snapshot: &'a RateLimitSnapshotDisplay,
    label: &str,
) -> Option<(&'a RateLimitWindowDisplay, bool)> {
