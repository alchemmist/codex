...
impl ChatWidget {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        let result = antex_windows_sandbox::apply_world_writable_scan_and_denies_for_permissions(
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        header.push(*Box::new(
            Paragraph::new(if allow_unelevated {
                vec![
                    line!["Set up the Antex agent sandbox to protect your files and control network access. Learn more <https://developers.openai.com/codex/windows>"],
                ]
            } else {
                vec![
                    line!["Your organization requires the default Antex agent sandbox to continue. Set it up to protect your files and control network access."],
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            lines.push(line![
                "You can still use Antex in a non-admin sandbox. It carries greater risk if prompt injected."
            ]);
        } else {
            lines.push(line![
                "Your organization requires the default sandbox before Antex can continue."
            ]);
        }
        lines.push(line![
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                name: "Use Antex with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
