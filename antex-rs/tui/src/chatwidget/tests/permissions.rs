...
async fn permission_discovery_uses_server_catalog_and_keeps_remote_custom_selection_disabled() {
    ...
    discovery
        .profiles
        .push(antex_app_server_protocol::PermissionProfileSummary {
...
fn windows_sandbox_requirements_stack(
    ...
    let requirements_toml = antex_config::ConfigRequirementsToml {
        windows: Some(antex_config::WindowsRequirementsToml {
...
pub(super) fn requirements_stack(
    requirements_toml: antex_config::ConfigRequirementsToml,
) -> ConfigLayerStack {
...
async fn profile_permissions_selection_popup_with_disallowed_full_access_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.explicit_permission_profile_mode = true;
    chat.config.config_layer_stack = requirements_stack(antex_config::ConfigRequirementsToml {
...
async fn startup_windows_sandbox_prompt_blocks_disallowed_unelevated_fallback() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(
        popup.contains("Your organization requires the default Antex agent sandbox"),
...
async fn permissions_selection_sends_approvals_reviewer_in_override_turn_context() {
    ...
    let op = std::iter::from_fn(|| rx.try_recv().ok())
        .find_map(|event| match event {
            AppEvent::AntexOp(op @ Op::OverrideTurnContext { .. }) => Some(op),
            _ => None,
        })
