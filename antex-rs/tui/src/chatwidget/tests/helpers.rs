...
pub(super) async fn test_config() -> Config {
    ...
    let codex_home = tempfile::Builder::new()
        ...
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_antex_home(codex_home.clone(), Vec::new())
            .await
            .expect("config");
    // Keep generic UI snapshots stable when the bundled catalog default changes.
    config.model = Some("gpt-5.6-sol".to_string());
    config.codex_home = codex_home.abs();
    config.sqlite = antex_state::SqliteConfig::new_for_testing(codex_home.as_path().abs());
...
pub(super) fn test_model_catalog(_config: &Config) -> Arc<ModelCatalog> {
    Arc::new(
        ModelCatalog::new(crate::test_support::TEST_MODEL_PRESETS.clone())
            ...
            antex_models_manager::collaboration_mode_presets::builtin_collaboration_mode_presets(),
        ),
    )
}
...
pub(super) async fn make_chatwidget_manual_with_auth(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, super::AntexOpTarget::Direct(op_tx));
...
fn token_usage_breakdown(usage: TokenUsage) -> antex_app_server_protocol::TokenUsageBreakdown {
    antex_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    antex_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: antex_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = antex_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            antex_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(antex_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    antex_app_server_protocol::PluginHookSummary {
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    let mut entries = vec![antex_app_server_protocol::HookOutputEntry {
        kind: antex_app_server_protocol::HookOutputEntryKind::Warning,
        text: "Heads up from the hook".to_string(),
    }];
    if event_name != antex_app_server_protocol::HookEventName::Interrupt {
        entries.push(antex_app_server_protocol::HookOutputEntry {
