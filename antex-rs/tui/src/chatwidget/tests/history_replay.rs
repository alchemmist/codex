...
use antex_app_server_protocol::NetworkAccess;
...
async fn forked_thread_history_line_without_name_shows_id_once_snapshot() {
    ...
    chat.config.codex_home =
        antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(temp.path())
...
async fn app_server_forked_thread_history_line_uses_app_server_title_snapshot() {
    ...
    assert!(
        ...
        "app-server fork title lookup should not read local ANTEX_HOME"
    );
        ...
        chat.handle_server_notification(
            ServerNotification::ItemCompleted(ItemCompletedNotification {
                ...
                item: AppServerThreadItem::McpToolCall {
                    ...
                    tool: "js".to_string(),
                    status: antex_app_server_protocol::McpToolCallStatus::Failed,
                    arguments: json!({"title": "Inspect workspace"}),
                    app_context: None,
                    mcp_app_resource_uri: None,
                    plugin_id: None,
                    read_only_hint: None,
                    result: Some(Box::new(antex_app_server_protocol::McpToolCallResult {
                        content: vec![
...
async fn deferred_mcp_lifecycle_events_keep_fifo_after_stream_finishes() {
    ...
    chat.on_mcp_tool_call_completed(AppServerThreadItem::McpToolCall {
        ...
        server: "copilot-bridge".to_string(),
        tool: "copilot".to_string(),
        status: antex_app_server_protocol::McpToolCallStatus::Completed,
        arguments: json!({"action": "wait"}),
        app_context: None,
        mcp_app_resource_uri: None,
        plugin_id: None,
    ...
    for (summary_index, delta) in [
        ...
        chat.handle_server_notification(
            ServerNotification::ReasoningSummaryPartAdded(
                antex_app_server_protocol::ReasoningSummaryPartAddedNotification {
                    thread_id: "thread-1".to_string(),
                    turn_id: "turn-1".to_string(),
                    item_id: "reasoning-1".to_string(),
                    summary_index,
