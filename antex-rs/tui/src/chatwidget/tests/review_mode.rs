...
async fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::AntexOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::AntexOp(Op::Review { target }) => {
            assert_eq!(
                target,
                ReviewTarget::Custom {
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::AntexOp should be sent
...
async fn interrupted_turn_after_goal_budget_limited_uses_budget_message_snapshot() {
    ...
    chat.handle_server_notification(
        antex_app_server_protocol::ServerNotification::TurnStarted(
    ...
    chat.handle_server_notification(
        antex_app_server_protocol::ServerNotification::ThreadGoalUpdated(
            antex_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: antex_app_server_protocol::ThreadGoal {
                    ...
                    status: antex_app_server_protocol::ThreadGoalStatus::BudgetLimited,
    ...
    chat.handle_server_notification(
        antex_app_server_protocol::ServerNotification::TurnCompleted(
            antex_app_server_protocol::TurnCompletedNotification {
                thread_id: "thread-1".to_string(),
                turn: antex_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    items_view: antex_app_server_protocol::TurnItemsView::Full,
                    items: Vec::new(),
                    status: antex_app_server_protocol::TurnStatus::Interrupted,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
