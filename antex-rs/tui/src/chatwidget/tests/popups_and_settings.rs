...
use antex_app_server_protocol::HookErrorInfo;
use antex_app_server_protocol::HooksListEntry;
use antex_app_server_protocol::HooksListResponse;
use antex_app_server_protocol::MarketplaceLoadErrorInfo;
...
async fn plugin_detail_popup_snapshot_labels_personal_marketplace_as_local() {
    ...
    let mut plugin = plugins_test_detail(
        ...
        &[
            (antex_app_server_protocol::HookEventName::PreToolUse, 1),
            (antex_app_server_protocol::HookEventName::Stop, 2),
        ],
        &["Figma", "Slack"],
        &["figma-mcp", "docs-mcp"],
    );
...
async fn experimental_features_popup_snapshot() {
    ...
    let mut config = antex_config::types::TuiKeymap::default();
    config.list.accept = Some(antex_config::types::KeybindingsSpec::One(
        antex_config::types::KeybindingSpec("ctrl-x enter".to_string()),
    ));
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &antex_feedback::FeedbackDiagnostics::new(vec![antex_feedback::FeedbackDiagnostic {
