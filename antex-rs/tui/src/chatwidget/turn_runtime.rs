...
impl ChatWidget {
    ...
    pub(super) fn on_task_complete(
        ...
        // still preserve live-turn plan state after thread switch replay.
        if !from_replay {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        codex_error_info: &AppServerAntexErrorInfo,
    ) -> bool {
        matches!(
            codex_error_info,
            AppServerAntexErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Antex is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.antex_rate_limit_reached_type.map(|kind| {
        ...
        if self.antex_rate_limit_reached_type != rate_limit_reached_type {
            self.clear_backend_banner();
        }
        self.antex_rate_limit_reached_type = rate_limit_reached_type;
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        codex_error_info: Option<AppServerAntexErrorInfo>,
    ) {
        if codex_error_info == Some(AppServerAntexErrorInfo::MisalignmentPolicyViolation) {
    ...
    pub(super) fn on_plan_update(&mut self, update: UpdatePlanArgs) {
        ...
        self.transcript.latest_task_plan = Some(update.plan.clone());
    ...
    pub(super) fn interrupted_turn_message(&self, reason: TurnAbortReason) -> String {
        match reason {
            TurnAbortReason::Interrupted => "Interrupted.".to_string(),
