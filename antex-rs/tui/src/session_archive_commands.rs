//! Shared implementation for `antex archive`, `antex delete`, and `antex unarchive`.
//!
...
pub async fn run_session_archive_command(
    ...
    let codex_home = find_antex_home().wrap_err("failed to find Antex home")?;
    let mut app_server =
...
pub(super) async fn start_app_server_for_session_command(
    ...
    let workload_identity_selected = antex_login::is_workload_identity_selected();
