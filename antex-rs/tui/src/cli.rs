...
pub struct Cli {
    ...
    // Internal controls set by the top-level `antex resume` subcommand.
    ...
    /// top-level `antex resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `antex fork` subcommand.
    // These are not exposed as user flags on the base `codex` command.
    #[clap(skip)]
    ...
    /// top-level `antex fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
