...
impl EditorPaths {
    fn new() -> Self {
        ...
        let codex_home = canonical_root.join("antex-home");
        let cwd = canonical_root.join("workspace");
        fs::create_dir(&codex_home).expect("create Antex home");
...
#[test]
fn editor_directory_is_inside_isolated_antex_home() {
...
fn editor_directory_rejects_writable_home_editor_and_parent() {
    ...
    let parent = paths.codex_home.parent().expect("Antex home parent");
...
#[test]
#[cfg(unix)]
fn editor_directory_rejects_writable_antex_home_alias() {
    use std::os::unix::fs::symlink;

    let paths = EditorPaths::new();
    let aliased_home = paths.cwd.join("codex-home-link");
    symlink(&paths.codex_home, &aliased_home).expect("create Antex home symlink");
    let policy = workspace_write_policy(&[]);
...
#[test]
#[cfg(unix)]
fn editor_directory_rejects_writable_antex_home_alias_target() {
    use std::os::unix::fs::symlink;
...
fn editor_directory_uses_protected_workspace_fallback_with_default_temporary_grants() {
    let root = tempfile::tempdir().expect("create editor test root");
    let codex_home = root.path().join("antex-home");
    let cwd = root.path().join("workspace");
    fs::create_dir(&codex_home).expect("create Antex home");
    fs::create_dir(&cwd).expect("create workspace");
    let workspace_antex_home = cwd.join(".antex");
    ...
    assert!(!workspace_antex_home.exists());
    assert!(policy.can_write_local_path_with_cwd(&codex_home, &cwd));
    assert!(!policy.can_write_local_path_with_cwd(&workspace_antex_home, &cwd));
    assert!(!policy.can_write_local_path_with_cwd(&workspace_antex_home.join("editor"), &cwd));

    let directory = editor_directory(&[&codex_home, &workspace_antex_home], &policy, &cwd)
        .expect("use protected workspace metadata directory");

    assert_eq!(
        directory,
        dunce::canonicalize(&workspace_antex_home)
...
fn editor_directory_rejects_explicitly_writable_workspace_fallback() {
    ...
    fs::create_dir(&cwd).expect("create workspace");
    let workspace_antex_home = cwd.join(".antex");
    let writable_workspace_antex_home = AbsolutePathBuf::from_absolute_path(&workspace_antex_home)
        .expect("absolute workspace metadata directory");
    let policy = FileSystemSandboxPolicy::workspace_write(
        &[writable_workspace_antex_home],
        /*exclude_tmpdir_env_var*/ false,
        /*exclude_slash_tmp*/ false,
    );
    ...
    assert!(policy.can_write_local_path_with_cwd(&workspace_antex_home, &cwd));
    assert!(
        editor_directory(&[&codex_home, &workspace_antex_home], &policy, &cwd).is_err(),
...
fn editor_directory_rejects_workspace_fallback_symlink_to_writable_target() {
    ...
    let paths = EditorPaths::new();
    let workspace_antex_home = paths.cwd.join(".antex");
    symlink(&paths.codex_home, &workspace_antex_home).expect("create workspace metadata symlink");
    let policy = workspace_write_policy(&[&paths.codex_home]);

    assert!(!policy.can_write_local_path_with_cwd(&workspace_antex_home, &paths.cwd));
    assert!(policy.can_write_local_path_with_cwd(&paths.codex_home, &paths.cwd));
    assert!(
        editor_directory(
            &[&paths.codex_home, &workspace_antex_home],
            &policy,
            &paths.cwd,
        )
        .is_err()
...
fn editor_directory_uses_next_protected_candidate_after_creation_error() {
    ...
    fs::write(&unavailable_home, "not a directory").expect("create unavailable Antex home");
    let policy = workspace_write_policy(&[]);
...
#[tokio::test]
#[cfg(unix)]
async fn editor_process_receives_buffer_in_isolated_antex_home() {
    let paths = EditorPaths::new();
    let policy = workspace_write_policy(&[]);
...
async fn editor_process_uses_protected_workspace_fallback_with_default_temporary_grants() {
    ...
    fs::create_dir(&codex_home).expect("create Antex home");
    fs::create_dir(&cwd).expect("create workspace");
    let default_antex_home = dirs::home_dir().expect("home directory").join(".antex");
    let writable_default_antex_home = AbsolutePathBuf::from_absolute_path(&default_antex_home)
        .expect("absolute default Antex home");
    let policy = FileSystemSandboxPolicy::workspace_write(
        &[writable_default_antex_home],
        /*exclude_tmpdir_env_var*/ false,
        /*exclude_slash_tmp*/ false,
    );
    let workspace_antex_home = cwd.join(".antex");
    ...
    assert!(!workspace_antex_home.exists());
    assert!(policy.can_write_local_path_with_cwd(&codex_home, &cwd));
    assert!(policy.can_write_local_path_with_cwd(&default_antex_home, &cwd));
    assert!(!policy.can_write_local_path_with_cwd(&workspace_antex_home, &cwd));
