...
//! under `{ANTEX_HOME}/themes/`.  It provides:
//!
...
pub(crate) fn build_theme_picker_params(
    ...
    let antex_home_owned = codex_home.map(Path::to_path_buf);
    ...
    let preview_home = antex_home_owned.clone();
...
mod tests {
    ...
    #[test]
    fn subtitle_uses_tilde_path_when_antex_home_under_home_directory() {
        let home = dirs::home_dir().expect("home directory should be available");
        let codex_home = home.join(".antex");
    ...
    fn subtitle_falls_back_when_tilde_path_subtitle_is_too_wide() {
        ...
        let codex_home = home.join(long_segment).join(".antex");
