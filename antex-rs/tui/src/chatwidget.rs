//! The main Antex TUI chat surface.
//!
...
use crate::version::ANTEX_CLI_VERSION;
...
use antex_app_server_protocol::CodexErrorInfo as AppServerAntexErrorInfo;
...
use antex_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use antex_app_server_protocol::CreditsSnapshot;
...
use antex_app_server_protocol::ModelVerification as AppServerModelVerification;
use antex_app_server_protocol::RateLimitReachedType;
...
use antex_app_server_protocol::SkillMetadata;
use antex_app_server_protocol::SkillsListResponse;
use antex_app_server_protocol::ThreadGoal as AppThreadGoal;
use antex_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use antex_config::Constrained;
use antex_config::ConstraintResult;
...
#[cfg(test)]
use antex_git_utils::CommitLogEntry;
use antex_git_utils::current_branch_name;
use antex_git_utils::get_git_repo_root;
use antex_git_utils::local_git_branches;
use antex_git_utils::recent_commits;
use antex_otel::RuntimeMetricsSummary;
use antex_otel::SessionTelemetry;
use antex_plugin::PluginCapabilitySummary;
use antex_protocol::ThreadId;
use antex_protocol::account::PlanType;
use antex_protocol::approvals::GuardianAssessmentAction;
use antex_protocol::approvals::GuardianAssessmentDecisionSource;
use antex_protocol::approvals::GuardianAssessmentEvent;
use antex_protocol::approvals::GuardianAssessmentStatus;
...
use antex_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use antex_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
...
const PLAN_MODE_REASONING_SCOPE_ALL_MODES: &str = "Apply to conversation and Plan override";
...
use antex_file_search::FileMatch;
...
use antex_utils_approval_presets::ApprovalPreset;
use antex_utils_approval_presets::builtin_approval_presets;
use chrono::Local;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) has_antex_backend_auth: bool,
    pub(crate) model_catalog: Arc<ModelCatalog>,
    pub(crate) feedback: antex_feedback::AntexFeedback,
...
/// and by sending requests back to antex-core.
///
...
pub(crate) struct ChatWidget {
    ...
    antex_rate_limit_reached_type: Option<RateLimitReachedType>,
    antex_spend_control_reached: Option<bool>,
    ...
    tmux_command_log: Option<TmuxCommandLog>,
    ...
    pub(crate) pet_http_client: antex_http_client::RouteAwareClientPool,
    ...
    feedback: antex_feedback::AntexFeedback,
...
fn request_permissions_from_params(
    params: antex_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    pub(crate) fn open_feedback_consent(&mut self, category: crate::app_event::FeedbackCategory) {
        ...
        let include_windows_sandbox_log =
            antex_windows_sandbox::current_log_file_path_for_antex_home(&self.config.codex_home)
                .is_file();
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.antex_op_target {
            AntexOpTarget::Direct(antex_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = antex_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            AntexOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::AntexOp(op));
            }
        }
        true
    }
...
const PLACEHOLDER: &str = "Ask Antex to do anything";
