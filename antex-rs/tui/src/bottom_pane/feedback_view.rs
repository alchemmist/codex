...
use antex_feedback::DOCTOR_REPORT_ATTACHMENT_FILENAME;
use antex_feedback::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use antex_feedback::FeedbackDiagnostics;
...
const ANTEX_FEEDBACK_INTERNAL_URL: &str = "http://go/codex-feedback-internal";
...
pub(crate) fn feedback_success_cell(
    ...
    let mut lines = vec![Line::from(match issue_url.as_ref() {
        Some(_) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            format!("{prefix} You can share this in #antex-feedback:")
        }
...
fn slack_feedback_url(_thread_id: &str) -> String {
    ANTEX_FEEDBACK_INTERNAL_URL.to_string()
}
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec![
            "  • ".into(),
            format!("{ANTEX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME} (if available)").into(),
        ])
        .into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Antex session logs and diagnostics with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    use antex_feedback::FeedbackDiagnostic;
