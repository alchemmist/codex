...
mod tests {
    ...
    fn flush_buffered_typing_in_background_and_covering_editors() {
        ...
        let request = ToolRequestUserInputParams {
            ...
            questions: vec![antex_app_server_protocol::ToolRequestUserInputQuestion {
                id: "question".into(),
                header: String::new(),
    ...
    fn esc_with_skill_popup_does_not_interrupt_task() {
        ...
        while let Ok(ev) = rx.try_recv() {
            assert!(
                !matches!(ev, AppEvent::AntexOp(Op::Interrupt)),
    ...
    fn esc_interrupts_running_task_when_no_popup() {
        ...
        assert!(
            matches!(rx.try_recv(), Ok(AppEvent::AntexOp(Op::Interrupt))),
        ...
        assert!(matches!(
            rx.try_recv(),
            Ok(AppEvent::AntexOp(Op::Interrupt))
        ));
    }
