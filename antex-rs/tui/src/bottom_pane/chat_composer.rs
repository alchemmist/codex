...
//!
//! For the detailed burst state machine, see `antex-rs/tui/src/bottom_pane/paste_burst.rs`.
//!
...
use antex_message_history::HistoryBatchCursor;
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let display_name = antex_connectors::metadata::connector_display_label(connector);
...
mod tests {
    ...
    pub(super) fn new_test_composer() -> (ChatComposer, UnboundedReceiver<AppEvent>) {
        ...
        (
            ChatComposer::new(
                ...
                "Ask Antex to do anything".to_string(),
    ...
    fn vim_mode_resets_to_configured_insert_after_submission() {
        ...
        composer.set_vim_mode_start(antex_config::types::VimModeStart::Insert);
        composer.set_vim_enabled(/*enabled*/ true);
        composer.set_vim_mode_indicator_enabled(/*enabled*/ true);
    ...
    fn set_skill_mentions_refreshes_open_mention_popup() {
        ...
        composer.set_skill_mentions(Some(vec![SkillMetadata {
            ...
            description: "Primary personal Antex repo skill.".to_string(),
            short_description: None,
            interface: None,
            dependencies: None,
    ...
    fn default_unified_mention_popup_snapshot() {
        snapshot_composer_state(
            ...
            /*enhanced_keys_supported*/ false,
            |composer| {
                let features = antex_features::Features::with_defaults();
                composer
                    .set_mentions_v2_enabled(features.enabled(antex_features::Feature::MentionsV2));
                composer.set_skill_mentions(Some(vec![
                    SkillMetadata {
            ...
            snapshot_composer_state(
                ...
                |composer| {
                    ...
                    if selected {
                        ...
                        let mut buf = Buffer::empty(area);
                        composer.render(area, &mut buf);
                        assert_eq!(buf[(2, 1)].style().fg, Some(Color::Reset));
                    }
                },
            );
        }
    ...
    fn file_completion_preserves_large_paste_placeholder_elements() {
        ...
        composer.on_file_search_result(
            ...
            vec![FileMatch {
                ...
                match_type: antex_file_search::MatchType::File,
