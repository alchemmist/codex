...
const DISABLE_KEYBOARD_ENHANCEMENT_ENV_VAR: &str = "ANTEX_TUI_DISABLE_KEYBOARD_ENHANCEMENT";
...
fn read_windows_term_program() -> Option<String> {
    ...
    let executable = antex_utils_path::system_executable("cmd.exe")?;
...
fn read_tmux_extended_keys_format() -> Option<String> {
    let executable = antex_utils_path::system_executable("tmux")?;
    let path = antex_utils_path::system_path().ok()?;
    for args in [
