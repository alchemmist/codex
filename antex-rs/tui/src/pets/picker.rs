...
/// has a sensible starting point without implying that Antex is already the
...
fn custom_pet_entries(codex_home: &Path) -> Vec<PetPickerEntry> {
    ...
    for (directory_name, manifest_file) in [("avatars", "avatar.json"), ("pets", "pet.json")] {
        ...
        for child in children.flatten() {
            ...
            let Ok(pet) =
                Pet::load_with_antex_home(&selector, /*codex_home*/ Some(codex_home))
            else {
                continue;
            };
...
mod tests {
    ...
    #[test]
    fn picker_preselects_antex_without_marking_it_current_when_no_pet_is_configured() {
        let codex_home = tempfile::tempdir().unwrap();
        let params = build_pet_picker_params(
            /*current_pet*/ None,
            codex_home.path(),
        ...
        assert_eq!(params.items[1].name, "Antex");
