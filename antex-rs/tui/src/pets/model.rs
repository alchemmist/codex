...
//! in-memory `Pet`: built-in catalog ids, custom pet ids under ANTEX_HOME,
...
impl Pet {
    ...
    /// has already been materialized into ANTEX_HOME; if callers skip the
    /// asset-fetch step, they will get a missing-spritesheet error here on
    /// first use.
    pub(super) fn load_with_antex_home(value: &str, codex_home: Option<&Path>) -> Result<Self> {
        if path_like(value) {
...
fn load_builtin_pet(pet: catalog::BuiltinPet, codex_home: Option<&Path>) -> Result<Pet> {
    let codex_home = codex_home.context("ANTEX_HOME is not available")?;
...
mod tests {
    ...
    fn load_pet_from_dir(dir: &tempfile::TempDir) -> Pet {
        Pet::load_with_antex_home(dir.path().to_str().unwrap(), /*codex_home*/ None).unwrap()
    }

    fn load_pet_error_from_dir(dir: &tempfile::TempDir) -> anyhow::Error {
        Pet::load_with_antex_home(dir.path().to_str().unwrap(), /*codex_home*/ None).unwrap_err()
    }
    ...
    fn load_builtin_pet_uses_app_catalog_storage() {
        ...
        let pet =
            Pet::load_with_antex_home("dewey", /*codex_home*/ Some(codex_home.path())).unwrap();
    ...
    fn load_pet_json_path_uses_containing_directory() {
        ...
        let pet = Pet::load_with_antex_home(
    ...
    #[test]
    fn custom_pet_selector_loads_antex_home_pet_manifest() {
        let dir = write_minimal_pet();
        let codex_home = tempfile::tempdir().unwrap();
    ...
    fn custom_pet_rejects_spritesheet_path_escape() {
        ...
        let err = Pet::load_with_antex_home(
