...
const ANTEX_AUTH_SECRETS_FILENAME: &str = "antex_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// Antex authentication credentials used by the CLI, TUI, app server, and other clients.
    AntexAuth,
...
impl LocalSecretsBackend {
    ...
    fn secrets_path(&self) -> PathBuf {
        let filename = match self.namespace {
            ...
            LocalSecretsNamespace::AntexAuth => ANTEX_AUTH_SECRETS_FILENAME,
...
mod tests {
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        ...
        let codex_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let antex_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        antex_auth_backend.set(&scope, &name, "codex-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            antex_auth_backend.get(&scope, &name)?,
        ...
        assert!(
            MCP_OAUTH_CACHE
                ...
                .unwrap_or_else(PoisonError::into_inner)
                .as_ref()
                .is_none_or(|cached| cached.path != antex_auth_backend.secrets_path())
        );
        assert_eq!(
