...
use antex_utils_path_uri::PathUriParseError;
use anyhow::Context;
use anyhow::Result;
pub use parser::Hunk;
...
/// Special argv[1] flag used when the Antex executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `antex-apply-patch` (to avoid forcing
/// `antex-arg0` to depend on `antex-core`), it remains part of the "antex core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const ANTEX_CORE_APPLY_PATCH_ARG1: &str = "--antex-run-as-apply-patch";

/// Internal environment variable used to carry the selected update mode
/// through the arg0-dispatched standalone executable.
pub const ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR: &str =
    "ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS";
...
pub fn apply_patch_file_update_mode_from_env() -> ApplyPatchFileUpdateMode {
    match std::env::var(ANTEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR).as_deref() {
