...
def validate_plugin(plugin_root: Path) -> list[str]:
    errors: list[str] = []
    manifest_path = plugin_root / ".antex-plugin" / "plugin.json"
...
def load_json_object(path: Path, errors: list[str]) -> dict[str, Any] | None:
    if not path.is_file():
        errors.append("missing `.antex-plugin/plugin.json`")
        return None
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except OSError:
        errors.append("unable to read `.antex-plugin/plugin.json`")
        return None
    except json.JSONDecodeError:
        errors.append("`.antex-plugin/plugin.json` must be valid JSON")
        return None
    if not isinstance(payload, dict):
        errors.append("`.antex-plugin/plugin.json` must contain a JSON object")
        return None
    return payload
