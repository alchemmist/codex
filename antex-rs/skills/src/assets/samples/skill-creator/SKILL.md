...
Ask clarifying questions only when the missing information matters and cannot be reasonably inferred. Respect a user-specified location; otherwise create discoverable skills in `$ANTEX_HOME/skills`, or `~/.antex/skills` when `ANTEX_HOME` is unset.
...
```bash
scripts/init_skill.py my-skill --path "${ANTEX_HOME:-$HOME/.antex}/skills"
scripts/init_skill.py my-skill --path "${ANTEX_HOME:-$HOME/.antex}/skills" --resources references
```
