...
use antex_arg0::arg0_dispatch;
use tempfile::TempDir;

pub struct TestBinaryDispatchGuard {
    _antex_home: TempDir,
    arg0: Arg0PathEntryGuard,
    _previous_antex_home: Option<std::ffi::OsString>,
}
    ...
    F: FnOnce(&str, Option<&str>) -> TestBinaryDispatchMode,
{
    ...
    match classify(exe_name, argv1.as_deref().and_then(|arg| arg.to_str())) {
        ...
        TestBinaryDispatchMode::InstallAliases => {
            let codex_home = match tempfile::Builder::new().prefix(antex_home_prefix).tempdir() {
                Ok(codex_home) => codex_home,
                Err(error) => panic!("failed to create test ANTEX_HOME: {error}"),
            };
            let previous_antex_home = std::env::var_os("ANTEX_HOME");
            // Safety: this runs from a test ctor before test threads begin.
            unsafe {
                std::env::set_var("ANTEX_HOME", codex_home.path());
            }
            ...
            match previous_antex_home.as_ref() {
                Some(value) => unsafe {
                    std::env::set_var("ANTEX_HOME", value);
                },
                None => unsafe {
                    std::env::remove_var("ANTEX_HOME");
                },
            }
            ...
            Some(TestBinaryDispatchGuard {
                ...
                arg0,
                _previous_antex_home: previous_antex_home,
            })
        }
    }
}
