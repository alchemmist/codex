...
use antex_core_api::AbsolutePathBuf;
use antex_core_api::AltScreenMode;
use antex_core_api::AntexHomeUserInstructionsProvider;
use antex_core_api::AntexThread;
use antex_core_api::ApprovalsReviewer;
use antex_core_api::Arg0DispatchPaths;
use antex_core_api::AskForApproval;
use antex_core_api::AuthCredentialsStoreMode;
use antex_core_api::AuthManager;
use antex_core_api::AutoCompactTokenLimitScope;
use antex_core_api::CodexAppsToolsCache;
use antex_core_api::Config;
use antex_core_api::ConfigLayerStack;
use antex_core_api::Constrained;
use antex_core_api::EnvironmentManager;
use antex_core_api::EventMsg;
use antex_core_api::ExecServerRuntimePaths;
use antex_core_api::ExtensionRegistryBuilder;
use antex_core_api::Features;
use antex_core_api::GhostSnapshotConfig;
use antex_core_api::History;
use antex_core_api::MemoriesConfig;
use antex_core_api::ModelAvailabilityNuxConfig;
use antex_core_api::MultiAgentV2Config;
use antex_core_api::NewThread;
use antex_core_api::Notice;
use antex_core_api::OAuthCredentialsStoreMode;
use antex_core_api::OPENAI_PROVIDER_ID;
use antex_core_api::OtelConfig;
use antex_core_api::PermissionProfile;
use antex_core_api::Permissions;
use antex_core_api::ProjectConfig;
use antex_core_api::RealtimeAudioConfig;
use antex_core_api::RealtimeConfig;
use antex_core_api::SessionPickerViewMode;
use antex_core_api::SessionSource;
use antex_core_api::SqliteConfig;
use antex_core_api::StartIfIdleSubmission;
use antex_core_api::StartThreadOptions;
use antex_core_api::TerminalResizeReflowConfig;
use antex_core_api::ThreadManager;
use antex_core_api::ThreadStoreConfig;
use antex_core_api::ToolSuggestConfig;
use antex_core_api::TuiKeymap;
use antex_core_api::TuiNotificationSettings;
use antex_core_api::TuiPetAnchor;
use antex_core_api::TurnInputRequest;
use antex_core_api::UriBasedFileOpener;
use antex_core_api::UserInput;
use antex_core_api::WebSearchMode;
use antex_core_api::arg0_dispatch_or_else;
use antex_core_api::build_models_manager;
use antex_core_api::built_in_model_providers;
use antex_core_api::find_antex_home;
use antex_core_api::init_state_db;
use antex_core_api::install_image_generation_extension;
use antex_core_api::item_event_to_server_notification;
use antex_core_api::local_agent_graph_store_from_state_db;
use antex_core_api::passthrough_image_store;
use antex_core_api::resolve_installation_id;
use antex_core_api::set_default_originator;
use antex_core_api::thread_store_from_config;
use anyhow::Context;
use anyhow::bail;
use clap::Parser;
...
#[derive(Debug, Parser)]
#[command(
    name = "antex-thread-manager-sample",
...
async fn run_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("antex_thread_manager_sample".to_string()) {
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        config.antex_self_exe.clone(),
        config.antex_linux_sandbox_exe.clone(),
    )?;
...
fn new_config(model: Option<String>, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<Config> {
    let codex_home = find_antex_home().context("find Codex home")?;
    ...
    let mut config = Config {
        ...
        tui_startup_panel: Default::default(),
        tui_tmux_command_log: Default::default(),
        tui_pet: None,
        ...
        tui_vim_mode_start: Default::default(),
        tui_show_vim_mode_indicator: false,
...
async fn run_turn(thread: &AntexThread, thread_id: &str, prompt: String) -> anyhow::Result<()> {
