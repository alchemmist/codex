...
async fn ordinary_usage_permission_comes_from_backend_not_display_percent() {
    for (allowed, used_percent) in [(Some(true), 100), (Some(false), 0), (None, 0)] {
        ...
        Mock::given(method("GET"))
            ...
            .mount(&server)
            .await;
        let response = test_client(&server.uri(), PathStyle::AntexApi)
...
fn rate_limit_reset_contract_uses_expected_paths_and_payloads() {
    assert_eq!(
        test_client("https://example.test", PathStyle::AntexApi).rate_limit_status_url(),
        "https://example.test/api/codex/usage"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::AntexApi).rate_limit_reset_credits_url(),
        "https://example.test/api/codex/rate-limit-reset-credits"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::AntexApi)
