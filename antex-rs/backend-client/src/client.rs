...
use crate::types::AntexUserSettingsResponse;
use crate::types::AntexWorkspaceMessagesResponse;
...
use antex_http_client::RouteAwareRequestBuilder;
...
use antex_protocol::protocol::RateLimitReachedType;
use antex_protocol::protocol::RateLimitSnapshot;
use antex_protocol::protocol::RateLimitWindow;
use antex_protocol::protocol::SpendControlLimitSnapshot;
use anyhow::Result;
use http::Method;
use http::StatusCode;
...
impl Client {
    ...
    fn with_http(mut base_url: String, http: RouteAwareClientPool) -> Self {
        ...
        Self {
            base_url,
            http,
            auth_provider: antex_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
    ...
    pub fn from_auth(
        ...
        Self::new(base_url, http_client_factory)
            .with_user_agent(get_antex_user_agent())
            .with_auth_provider(antex_model_provider::auth_provider_from_auth(auth))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("antex-cli"));
        }
    ...
    pub async fn get_accounts_check(&self) -> Result<AccountsCheckResponse> {
        let url = match self.path_style {
            PathStyle::AntexApi => format!("{}/api/codex/accounts/check", self.base_url),
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::AntexApi => format!("{}/api/codex/profiles/me", self.base_url),
    ...
    fn list_tasks_url(
        ...
        let url = match self.path_style {
            PathStyle::AntexApi => format!("{}/api/codex/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::AntexApi => format!("{}/api/codex/tasks/{}", self.base_url, task_id),
    ...
    ///
    /// `GET /api/codex/config/bundle` (Antex API style) or
    ...
    pub async fn get_config_bundle(
        ...
        let url = match self.path_style {
            PathStyle::AntexApi => format!("{}/api/codex/config/bundle", self.base_url),
    ...
    /// Fetch authenticated Antex user settings from the active backend route.
    ///
    /// Uses `GET /api/codex/settings/user` for Antex API hosts and
    /// `GET /wham/settings/user` for ChatGPT `backend-api` hosts.
    pub async fn get_user_settings(
        &self,
    ) -> std::result::Result<AntexUserSettingsResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<AntexUserSettingsResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }

    pub async fn list_workspace_messages(
        &self,
    ) -> std::result::Result<AntexWorkspaceMessagesResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<AntexWorkspaceMessagesResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::AntexApi => format!("{}/api/codex/tasks", self.base_url),
    ...
    fn make_additional_rate_limit_snapshot(
        details: antex_backend_openapi_models::models::AdditionalRateLimitDetails,
    ...
    fn make_rate_limit_snapshot(
        ...
        spend_control: Option<antex_backend_openapi_models::models::SpendControlStatusDetails>,
        plan_type: Option<AccountPlanType>,
    ...
    fn workspace_messages_url(&self) -> String {
        match self.path_style {
            PathStyle::AntexApi => format!("{}/api/codex/workspace-messages", self.base_url),
    ...
    fn user_settings_url(&self) -> String {
        match self.path_style {
            PathStyle::AntexApi => format!("{}/api/codex/settings/user", self.base_url),
...
mod tests {
    use super::*;
    use antex_backend_openapi_models::models::AdditionalRateLimitDetails;
    use antex_backend_openapi_models::models::RateLimitReachedKind;
    use antex_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            spend_control: Some(Some(Box::new(
                antex_backend_openapi_models::models::SpendControlStatusDetails {
                    reached: false,
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let antex_client = test_client("https://example.test", PathStyle::AntexApi);
        assert_eq!(
            antex_client.send_add_credits_nudge_email_url(),
    ...
    fn token_usage_profile_uses_expected_paths() {
        let antex_client = test_client("https://example.test", PathStyle::AntexApi);
        assert_eq!(
            antex_client.token_usage_profile_url(),
    ...
    fn workspace_messages_uses_expected_paths() {
        let antex_client = test_client("https://example.test", PathStyle::AntexApi);
        assert_eq!(
            antex_client.workspace_messages_url(),
    ...
    async fn user_settings_request_uses_expected_paths_and_revalidates_cached_responses() {
        ...
        let antex_response = Client::new(
            server.uri(),
            HttpClientFactory::new(antex_http_client::OutboundProxyPolicy::ReqwestDefault),
        )
        .get_user_settings()
        .await
        .unwrap();
        ...
        assert_eq!(
            [antex_response, chatgpt_response],
            [
                AntexUserSettingsResponse {
    ...
    fn user_settings_missing_attribution_policy_defaults_to_disabled() {
        assert_eq!(
            serde_json::from_value::<AntexUserSettingsResponse>(serde_json::json!({})).unwrap(),
            AntexUserSettingsResponse {
                commit_attribution_enabled: false,
            }
        );
