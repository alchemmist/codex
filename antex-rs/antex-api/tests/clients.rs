...
use antex_api::ResponsesClient;
use antex_api::ResponsesEndpoint;
use antex_api::ResponsesOptions;
use antex_client::HttpTransport;
use antex_client::Request;
use antex_client::RequestBody;
use antex_client::Response;
use antex_client::StreamResponse;
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, antex_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, antex_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
async fn responses_client_stream_request_preserves_item_ids() -> Result<()> {
    ...
    let ticket = antex_protocol::guardian_ticket::GuardianTicket::from_server(&raw_ticket).unwrap();
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, antex_client::RequestCompression::None);
    Ok(())
}
