...
use antex_connectors::ConnectorMetadata;
use antex_connectors::ConnectorMetadataStore;
use antex_connectors::ConnectorToolSummary;
use antex_connectors::DirectoryListResponse;
use antex_connectors::merge::merge_connectors;
use antex_connectors::merge::merge_plugin_connectors;
use antex_core::config::Config;
pub use antex_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use antex_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use antex_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use antex_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use antex_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use antex_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
...
async fn connector_auth(config: &Config) -> anyhow::Result<AntexAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_antex_api_key_env*/ false).await?;
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = antex_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = antex_connectors::list_all_connectors_with_options(
        cache_context,
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        ...
        "ChatGPT account ID not available, please re-run antex login"
    );
...
mod tests {
    use super::*;
    use antex_connectors::metadata::connector_install_url;
    use antex_plugin::AppConnectorId;
