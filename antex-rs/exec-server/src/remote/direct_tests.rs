...
impl AuthProvider for StaticAuthProvider {
    ...
    fn apply_auth(
        &self,
        mut request: antex_http_client::Request,
    ) -> antex_api::AuthProviderFuture<'_> {
        Box::pin(async move {
    ...
    fn apply_auth(
        ...
        Box::pin(async move {
            let mut url = url::Url::parse(&request.url)
                .map_err(|error| antex_api::AuthError::Build(error.to_string()))?;
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                tracing_subscriber::filter::Targets::new()
                    .with_target("antex_http_client", tracing::Level::TRACE)
                    .with_target("antex_exec_server", tracing::Level::TRACE),
            ),
    );
    let _guard = tracing::subscriber::set_default(subscriber);
    tracing::debug!(target: "antex_exec_server", "direct registry log capture sentinel");
