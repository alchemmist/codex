            ...
            tokio::select! {
                maybe_message = outgoing_rx.recv(), if pending_outbound.is_none() && !force_incoming && !pong_deadline_expired => {
                    ...
                    let request_trace = match message {
                        antex_exec_server_protocol::JSONRPCMessage::Request(request) => request.trace,
                        antex_exec_server_protocol::JSONRPCMessage::Notification(_)
                        | antex_exec_server_protocol::JSONRPCMessage::Response(_)
                        | antex_exec_server_protocol::JSONRPCMessage::Error(_) => None,
                    };
