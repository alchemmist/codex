...
use antex_network_proxy::ManagedNetworkSandboxContext;
use antex_network_proxy::NetworkPolicyAuditObserver;
use antex_network_proxy::NetworkPolicyDecider;
use antex_network_proxy::NetworkProxy;
use antex_network_proxy::NetworkProxyHandle;
use antex_network_proxy::NetworkProxyState;
use antex_network_proxy::RemoteNetworkProxyLaunchConfig;
use antex_network_proxy::is_managed_mitm_ca_trust_bundle_path;
#[cfg(target_os = "windows")]
use antex_network_proxy::strip_managed_proxy_env;
...
use antex_sandboxing::WindowsSandboxProxySettingsMode;
use antex_sandboxing::WindowsSandboxSpawnRequest;
...
use antex_sandboxing::with_managed_mitm_ca_readable_root;
use antex_utils_absolute_path::AbsolutePathBuf;
use antex_utils_path_uri::PathUri;

#[cfg(unix)]
use crate::ANTEX_ARG0_EXEC_HELPER_ARG1;
...
pub(crate) async fn prepare_exec_request_with_telemetry(
    ...
    let sandbox_helper_paths = params
        .arg0
        .iter()
        .map(|_| runtime_paths.antex_self_exe.clone())
        .collect::<Vec<_>>();
    ...
    let sandbox_helper_paths = {
        ...
        if !sandbox_helper_paths.contains(&runtime_paths.antex_self_exe) {
            sandbox_helper_paths.push(runtime_paths.antex_self_exe.clone());
        }
        sandbox_helper_paths.extend(runtime_paths.antex_linux_sandbox_exe.iter().cloned());
    ...
    let sandbox_manager = sandbox_manager
        .with_allowed_symlinked_antex_home(runtime_paths.allowed_symlinked_antex_home.clone());
    ...
    let (program, args) = params.arg0.as_ref().map_or_else(
        ...
        |arg0| {
            ...
            helper_args.push(ANTEX_ARG0_EXEC_HELPER_ARG1.to_string());
    ...
    let transform_request = SandboxDirectSpawnTransformRequest {
        ...
        transform: SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd,
            antex_linux_sandbox_exe: runtime_paths.antex_linux_sandbox_exe.as_deref(),
...
> {
    ...
    state.set_process_log_metadata(antex_network_proxy::NetworkProxyProcessLogMetadata {
        thread_id: telemetry.thread_id.clone(),
        tool_call_id: telemetry.tool_call_id.clone(),
        executor_identity: telemetry
            .executor_registration
            .as_ref()
            .map(|registration| antex_network_proxy::ExecutorLogIdentity {
