...
pub(crate) fn environment_provider_from_antex_home(
    codex_home: &Path,
        ...
        let provider = TomlEnvironmentProvider::new(EnvironmentsToml {
            ...
            environments: vec![
                ...
                EnvironmentToml {
                    ...
                    args: Some(vec![
                        "dev".to_string(),
                        "antex exec-server --listen stdio".to_string(),
                    ]),
...
id = "ssh-dev"
program = "ssh"
args = ["dev", "antex exec-server --listen stdio"]
cwd = "/tmp"
[environments.env]
ANTEX_LOG = "debug"
"#,
...
"#,
    ...
    #[tokio::test]
    async fn environment_provider_from_antex_home_uses_present_environments_file() {
        let codex_home = tempdir().expect("tempdir");
        std::fs::write(
...
"#,
        ...
        let provider =
            environment_provider_from_antex_home(codex_home.path()).expect("environment provider");
    ...
    #[tokio::test]
    async fn environment_provider_from_antex_home_falls_back_when_file_is_missing() {
        let codex_home = tempdir().expect("tempdir");
