...
use crate::environment_toml::environment_provider_from_antex_home;
...
pub const ANTEX_EXEC_SERVER_URL_ENV_VAR: &str = "ANTEX_EXEC_SERVER_URL";
pub const ANTEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR: &str =
    "ANTEX_EXEC_SERVER_NOISE_REGISTRY_URL";
pub const ANTEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR: &str =
    "ANTEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID";
pub const ANTEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR: &str =
    "ANTEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID";
...
/// Owns the execution/filesystem environments available to the Antex runtime.
///
/// `EnvironmentManager` is a shared registry for concrete environments. Its
/// default constructor preserves the legacy `ANTEX_EXEC_SERVER_URL` behavior
/// while configured construction accepts a provider-supplied snapshot.
///
/// Setting `ANTEX_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    ///
    /// If `ANTEX_HOME/environments.toml` is present, it defines the configured
    /// environments. Otherwise this preserves the legacy
    /// `ANTEX_EXEC_SERVER_URL` behavior.
    pub async fn prepare_from_antex_home(
        ...
        let source = if let Some(config) = noise_environment_config_from_env()? {
            ...
            let provider = environment_provider_from_antex_home(codex_home.as_ref())?;
            PreparedEnvironmentSource::Snapshot(provider.snapshot().await?)
        };
        Ok(PreparedEnvironmentManager { source })
    }

    /// Builds a manager from `ANTEX_HOME` with an explicit outbound HTTP policy.
    pub async fn from_antex_home(
        codex_home: impl AsRef<std::path::Path>,
        local_runtime_paths: Option<ExecServerRuntimePaths>,
        http_client_factory: HttpClientFactory,
    ) -> Result<Self, ExecServerError> {
        Self::prepare_from_antex_home(codex_home)
            .await?
...
-> Result<Option<NoiseRendezvousEnvironmentConfig>, ExecServerError> {
    noise_environment_config_from_values(
        optional_environment_value(ANTEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR),
        optional_environment_value(ANTEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR),
        optional_environment_value(ANTEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR),
        optional_environment_value(ANTEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR),
    )
}
        ...
        match (registry_url, environment_id, auth_token) {
            ...
            _ => {
                return Err(ExecServerError::EnvironmentRegistryConfig(format!(
                    "Noise environment requires {ANTEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR}, \
{ANTEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR}, and \
{ANTEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR}"
                )));
            }
        };
...
impl Environment {
    ...
    /// Builds an environment from the raw `ANTEX_EXEC_SERVER_URL` value and
...
mod tests {
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = antex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = antex_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            antex_protocol::models::PermissionProfile::from_runtime_permissions(
                &antex_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
