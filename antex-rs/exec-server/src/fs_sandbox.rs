...
use antex_sandboxing::SandboxDirectSpawnTransformRequest;
use antex_sandboxing::SandboxExecRequest;
...
use crate::fs_helper::ANTEX_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.antex_self_exe;
        let sandbox_manager = SandboxManager::for_file_system_helpers();
        #[cfg(target_os = "macos")]
        let sandbox_manager = sandbox_manager.with_allowed_symlinked_antex_home(
            self.runtime_paths.allowed_symlinked_antex_home.clone(),
        );
        ...
        let command = SandboxCommand {
            ...
            args: vec![ANTEX_FS_HELPER_ARG1.to_string()],
            cwd: cwd.uri.clone(),
        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                transform: SandboxTransformRequest {
                    ...
                    antex_linux_sandbox_exe: self.runtime_paths.antex_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    let mut roots = vec![runtime_paths.antex_self_exe.clone()];
    if let Some(path) = &runtime_paths.antex_linux_sandbox_exe
        && !roots.contains(path)
    {
...
pub(crate) fn spawn_command(
    ...
    env.retain(|name, _| !antex_protocol::shell_environment::is_non_inheritable_env_var(name));
    command.env_clear();
    command.envs(env);
    command.stdin(stdin);
    ...
    unsafe {
        command.pre_exec(|| {
            antex_utils_pty::pty::close_inherited_fds_except(&[]);
            Ok(())
        });
    }
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let antex_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(antex_self_exe, /*antex_linux_sandbox_exe*/ None)
        ...
        let readable = runtime_paths.antex_self_exe.clone();
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let antex_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(antex_self_exe.clone(), Some(antex_self_exe))
                .expect("runtime paths");
        ...
        let sandbox_context = {
            ...
            crate::FileSystemSandboxContext {
                windows_sandbox_level:
                    antex_protocol::config_types::WindowsSandboxLevel::RestrictedToken,
                ..sandbox_context
            }
        };
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context = antex_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_only_the_helper_executable() {
        ...
        assert!(
            policy.can_read_local_path_with_cwd(
                runtime_paths.antex_self_exe.as_path(),
                cwd.as_path(),
            )
        );
    ...
    fn helper_permissions_include_only_linux_sandbox_alias_executable() {
        ...
        let antex_self_exe = root.path().join("bin").join("codex");
        let antex_linux_sandbox_exe = root.path().join("aliases").join("antex-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(antex_self_exe, Some(antex_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let antex_parent = runtime_paths.antex_self_exe.parent().expect("codex parent");
        ...
        assert!(!policy.can_read_local_path_with_cwd(antex_parent.as_path(), cwd.as_path()));
    ...
    fn sandbox_context_with_cwd(
        ...
        antex_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
            PermissionProfile::from_runtime_permissions(policy, NetworkSandboxPolicy::Restricted),
            cwd,
        )
    }
