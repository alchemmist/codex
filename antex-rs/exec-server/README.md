...
`antex-exec-server` is the library backing `codex exec-server`, a small
...
filesystem operations and `antex-linux-sandbox`.

## Transport

The server speaks the exec-specific `antex-exec-server-protocol` message
...
Agent Identity JWT in `ANTEX_ACCESS_TOKEN` can opt into that auth path with
...
Alternatively, API users can instead use `ANTEX_API_KEY`;
Codex sends it as a bearer token on the registration request. For example:

```sh
ANTEX_API_KEY="$OPENAI_API_KEY" \
codex exec-server \
  --remote ... \
...
- `arg0`: optional argv0 override forwarded to `antex-utils-pty`.
