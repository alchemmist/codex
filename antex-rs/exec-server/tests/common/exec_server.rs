...
pub(crate) struct TestAntexHelperPaths {
    pub(crate) antex_exe: PathBuf,
    pub(crate) antex_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_antex_helper_paths() -> anyhow::Result<TestAntexHelperPaths> {
    let (helper_binary, antex_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestAntexHelperPaths {
        antex_exe: helper_binary,
...
{
    let helper_paths = test_antex_helper_paths()?;
    let mut child = Command::new(&helper_paths.antex_exe);
...
impl ExecServerHarness {
    pub(crate) async fn start(mut command: Command) -> anyhow::Result<Self> {
        ...
        if !command
            .as_std()
            .get_envs()
            .any(|(key, value)| key == "ANTEX_HOME" && value.is_some())
        {
            command.env("ANTEX_HOME", codex_home.path());
        }
