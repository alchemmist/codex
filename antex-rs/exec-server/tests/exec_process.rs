...
use antex_exec_server::ExecBackend;
#[cfg(unix)]
use antex_exec_server::ExecEnvPolicy;
use antex_exec_server::ExecOutputStream;
use antex_exec_server::ExecParams;
use antex_exec_server::ExecProcess;
use antex_exec_server::ExecProcessEvent;
#[cfg(any(unix, windows))]
use antex_exec_server::FileSystemSandboxContext;
use antex_exec_server::ProcessId;
use antex_exec_server::ProcessSignal;
use antex_exec_server::ReadResponse;
#[cfg(unix)]
use antex_exec_server::ShellInfo;
#[cfg(unix)]
use antex_exec_server::ShellSnapshotRequest;
use antex_exec_server::StartedExecProcess;
use antex_exec_server::WriteStatus;
#[cfg(unix)]
use antex_network_proxy::NetworkProxyConfig;
#[cfg(unix)]
use antex_network_proxy::RemoteNetworkProxyConfig;
#[cfg(unix)]
...
async fn antex_home_symlink_opt_out_respects_host_config_and_scope() -> Result<()> {
    ...
    let other_alias = workspace.path().join(".antex/visualizations");
    std::fs::create_dir(workspace.path().join(".antex"))?;
    symlink(target.path(), &alias)?;
    symlink(target.path(), &other_alias)?;
    std::fs::write(
        workspace.path().join(".antex/config.toml"),
        "allow_symlinked_antex_home = true\n",
    )?;
    ...
    for enabled in [None, Some(false), Some(true)] {
        std::fs::write(
            ...
            enabled.map_or_else(String::new, |enabled| {
                format!("allow_symlinked_antex_home = {enabled}\n")
            }),
        )?;
        let mut server = exec_server_with_env([("ANTEX_HOME", home.path())], &[]).await?;
        let environment = Environment::create_for_tests(Some(server.websocket_url().to_string()))?;
...
async fn shell_snapshot_v2_filters_profile_exports_and_stays_in_memory(
    ...
    if use_sandbox
        && let Some(warning) =
            antex_sandboxing::system_bwrap_warning(&PermissionProfile::read_only())
    {
...
async fn shell_snapshot_v2_remote_managed_proxy_uses_prepared_execution_context() -> Result<()> {
    ...
    std::fs::write(
        ...
        "printf '%s\\n' \"$HTTP_PROXY\" >> \"$HOME/captures\"\ntest \"$ANTEX_NETWORK_PROXY_ACTIVE\" = 1 || exit 41\nexport PROFILE_ALLOWED=profile\nprofile_helper() { printf helper; }\n",
    )?;
    let policy = ExecEnvPolicy {
        inherit: ShellEnvironmentPolicyInherit::All,
        ignore_default_excludes: false,
        ...
        let started = context
            ...
            .start(ExecParams {
                ...
                argv: vec![
                    "/bin/bash".to_string(),
                    "-lc".to_string(),
                    "profile_helper; printf '|%s|%s|%s' \"$PROFILE_ALLOWED\" \"$ANTEX_NETWORK_PROXY_ACTIVE\" \"$HTTP_PROXY\"".to_string(),
                ],
                cwd: cwd.clone(),
                env_policy: Some(policy.clone()),
                shell_snapshot: Some(ShellSnapshotRequest {
...
async fn shell_snapshot_v2_capture_failure_falls_back_and_retries(
    ...
    if use_remote
        && let Some(warning) =
            antex_sandboxing::system_bwrap_warning(&PermissionProfile::workspace_write())
    {
        eprintln!("skipping sandbox test: {warning}");
        return Ok(());
    }
...
async fn remote_sandboxed_process_preserves_custom_arg0() -> Result<()> {
    if let Some(warning) = antex_sandboxing::system_bwrap_warning(&PermissionProfile::read_only()) {
    ...
    let session = context
        ...
        .start(ExecParams {
            ...
            argv: vec![
                "/bin/sh".to_string(),
                "-c".to_string(),
                "printf '%s' \"$0\"; if /bin/cat \"$ANTEX_TEST_DENIED_FILE\" >/dev/null 2>&1; then exit 42; fi"
                    .to_string(),
            ],
            cwd,
            shell_snapshot: None,
            ...
            env: HashMap::from([
                ...
                (
                    "ANTEX_TEST_DENIED_FILE".to_string(),
...
async fn remote_tty_process_uses_configured_sandbox_helper_with_hostile_path() -> Result<()> {
    ...
    let file = workspace.path().join("allowed.txt");
    std::fs::write(&file, b"allowed")?;
    let hostile_helper = workspace.path().join("antex-linux-sandbox");
