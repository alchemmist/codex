...
This repository uses Bazel to build the Rust workspace under `antex-rs`.
...
- `rules_rs` imports third-party crates from `antex-rs/Cargo.toml` and
  `antex-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `antex_rust_crate`, which wraps `rust_library`,
...
- Each crate in `antex-rs/*/BUILD.bazel` typically uses `antex_rust_crate` and
...
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/codex \
  ...
  build --config=ci-linux //antex-rs/cli:antex
```
...
2. Create a `BUILD.bazel` that calls `antex_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `antex_rust_crate` to configure it.
