...
- a `antex-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Codex CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `antex-exec` CLI can check if its arg0 is `antex-linux-sandbox` and, if so, execute as if it were `antex-linux-sandbox`
...
the helper falls back to the bundled `antex-resources/bwrap` binary shipped
with Codex.
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.antex`) are re-applied as read-only via `--ro-bind`.
