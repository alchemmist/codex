...
use antex_sandboxing::find_system_bwrap_in_path;
        ...
        BubblewrapLauncher::Unavailable => {
            panic!(
                "bubblewrap is unavailable: no system bwrap was found on PATH and no bundled \
                 antex-resources/bwrap binary was found next to the Antex executable"
            )
        }
    }
}
...
mod tests {
    ...
    fn translates_fd_mounts_for_legacy_system_bubblewrap() {
        let mut argv = vec![
            ...
            "--".to_string(),
            "/usr/bin/antex-linux-sandbox".to_string(),
