...
//! - sensitive subpaths such as `.git`, `.agents`, and `.antex` remain
...
use antex_protocol::permissions::is_protected_metadata_name;
...
use antex_protocol::protocol::FileSystemSpecialPath;
use antex_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.antex`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".antex")
                {
                    return None;
                }
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_antex = temp_dir.path().join("real-codex");
        let logical_antex = logical_home.join(".antex");
        let real_memories = real_antex.join("memories");
        let logical_memories = logical_antex.join("memories");
        ...
        std::os::unix::fs::symlink(&real_antex, &logical_antex)
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".antex"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_antex = path_to_string(&temp_dir.path().join(".antex"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_antex));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_antex)));
        assert_eq!(
