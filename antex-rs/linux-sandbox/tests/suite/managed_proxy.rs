...
use antex_network_proxy::PROXY_ATTRIBUTION_TOKEN_ENV_KEY;
use antex_network_proxy::write_attribution_frame;
...
async fn unsupported_system_bwrap_falls_back_to_bundled_bwrap() {
    if option_env!("ANTEX_BWRAP_SHA256")
    ...
    let Some(system_bwrap) = antex_sandboxing::find_system_bwrap_in_path() else {
    ...
    let sandbox_executable = tempdir.path().join("antex-linux-sandbox");
    ...
    let resources_dir = tempdir.path().join("antex-resources");
    ...
    let output = run_linux_sandbox_direct(
        &[
            ...
            concat!(
                ...
                "fi; ",
                "if [ -n \"${ANTEX_TEST_SHM_PATH:-}\" ]; then ",
                "test \"$(cat \"$ANTEX_TEST_SHM_PATH\")\" = host-before; ",
                "printf sandbox-after >\"$ANTEX_TEST_SHM_PATH\"; ",
                "fi; ",
                "stat -c '%d:%i' /dev",
            ),
        ],
...
async fn managed_proxy_mode_routes_through_bridge_and_blocks_direct_egress() {
    ...
    let sandbox_binary =
        antex_utils_cargo_bin::cargo_bin("antex-linux-sandbox").expect("sandbox binary");
    let sandbox_helper_dir = sandbox_binary
        .parent()
...
fn handoff_client() {
    let Ok(label) = std::env::var("ANTEX_TEST_HANDOFF_LABEL") else {
        return;
    };
    ...
    let shared = std::env::var("ANTEX_TEST_HANDOFF_SHARED").expect("shared directory");
    let shared = Path::new(&shared);
    let peer = std::env::var("ANTEX_TEST_HANDOFF_PEER").expect("peer label");
...
async fn handoff_isolates_concurrent_endpoints_and_closes_privileged_descriptors() {
    ...
    for (label, peer, profile) in [
        ...
        env.insert("ANTEX_TEST_HANDOFF_LABEL".to_string(), label.to_string());
        env.insert("ANTEX_TEST_HANDOFF_PEER".to_string(), peer.to_string());
        env.insert(
            "ANTEX_TEST_HANDOFF_SHARED".to_string(),
