...
fn antex_linux_sandbox_exe() -> PathBuf {
    let sandbox_program =
        antex_utils_cargo_bin::cargo_bin("antex-linux-sandbox").expect("sandbox binary");
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> antex_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<antex_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let antex_linux_sandbox_exe = Some(antex_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &antex_protocol::exec_output::ExecToolCallOutput) -> bool {
...
fn expect_denied(
    result: Result<antex_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> antex_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let antex_linux_sandbox_exe: Option<PathBuf> = Some(antex_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_antex_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_antex = tmpdir.path().join(".antex");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_antex).expect("create .codex");

    let git_target = dot_git.join("config");
    let antex_target = dot_antex.join("config.toml");
    ...
    let antex_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", antex_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".antex write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(antex_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_antex_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_antex = tmpdir.path().join(".antex");
    symlink(&decoy, &dot_antex).expect("create .codex symlink");
    ...
    let antex_output = expect_denied(
        ...
        .await,
        ".antex symlink replacement should be denied",
    );
    assert_ne!(antex_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_antex_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    .await
    {
        Err(err) => match err.details() {
            ...
            details => panic!(".antex symlink build failure should deny: {details:?}"),
        },
        Ok(output) => panic!(".antex symlink build failure should deny: {output:?}"),
    };
...
"#,
    ...
    let mkdir_antex_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".antex"],
            &subdir,
            std::slice::from_ref(&subdir),
    ...
    assert_ne!(mkdir_antex_output.exit_code, 0);
    assert!(!subdir.join(".antex").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = antex_linux_sandbox_exe()
        .parent()
...
async fn sandbox_starts_with_denied_tmp_without_exposing_registry() {
    ...
    let sandbox_helper = antex_linux_sandbox_exe();
