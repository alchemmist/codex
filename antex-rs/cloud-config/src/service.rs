...
fn auth_identity(auth: &AntexAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_config_eligible_auth(auth: &AntexAuth) -> bool {
    ...
    C: BundleClient + 'static,
{
    ...
    pub(crate) async fn load_startup_bundle_with_timeout(
        ...
        let _timer =
            antex_otel::start_global_timer("codex.cloud_config_bundle.fetch.duration_ms", &[]);
