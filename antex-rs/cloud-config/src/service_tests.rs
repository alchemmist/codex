...
use antex_backend_client::ConfigBundleResponse;
use antex_backend_client::DeliveredTomlFragment;
use antex_config::AbsolutePathBuf;
use antex_config::CloudConfigFragment;
use antex_config::CloudConfigTomlBundle;
use antex_config::CloudRequirementsFragment;
use antex_config::CloudRequirementsTomlBundle;
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(AntexAuth::AgentIdentity(
...
impl BundleClient for StaticBundleClient {
    async fn get_bundle(&self, _auth: &AntexAuth) -> Result<CloudConfigBundle, BundleRequestError> {
...
impl BundleClient for TokenBundleClient {
    async fn get_bundle(&self, auth: &AntexAuth) -> Result<CloudConfigBundle, BundleRequestError> {
        self.request_count.fetch_add(1, Ordering::SeqCst);
        if matches!(
...
struct TestExternalChatgptAuth {
    current: RwLock<AntexAuth>,
    refreshed: AntexAuth,
    refresh_count: AtomicUsize,
}

impl ExternalAuth for TestExternalChatgptAuth {
    fn resolve(&self) -> antex_login::ExternalAuthFuture<'_, AntexAuth> {
        Box::pin(async {
            self.current
                .read()
                .map(|auth| auth.clone())
    ...
    ) -> antex_login::ExternalAuthFuture<'_, AntexAuth> {
        Box::pin(async {
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = AntexAuth::from_external_chatgpt_tokens(
    ...
    let refreshed_auth = AntexAuth::from_external_chatgpt_tokens(
        &refreshed_token,
        "account-12345",
        Some("enterprise"),
    )
...
async fn production_loader_recovers_startup_timeouts_in_background() {
    ...
    assert_eq!(
        ...
        antex_protocol::protocol::AskForApproval::Never
    );
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(antex_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            antex_backend_client::DeliveredRequirementsToml {
