...
`antex-voice-host` establishes the inherited-pipe lifecycle for the proposed
...
`antex-resources/voice/bin/antex-voice-host[.exe]`, filters the child environment,
and owns process cleanup through `antex-utils-pty`. Its runtime must remain alive
...
the host platform. From `antex-rs`, run
`ANTEX_TEST_VOICE_RUNTIME=/absolute/prepared/runtime just test -p antex-voice-host --test packaged_runtime --run-ignored all`.
