...
#[tokio::test]
#[ignore = "requires ANTEX_TEST_VOICE_RUNTIME containing real prepared native libraries"]
async fn relocated_runtime_initializes_closes_and_rejects_duplicate_initialization() -> Result<()> {
    let source = std::env::var_os("ANTEX_TEST_VOICE_RUNTIME")
        .context("set ANTEX_TEST_VOICE_RUNTIME to a matching prepared runtime")?;
    ...
    let runtime = root.join("antex-resources/voice");
    ...
    let helper_source = cargo_bin("antex-voice-host")?;
    ...
    let app_name = if cfg!(windows) { "antex.exe" } else { "antex" };
