...
use antex_realtime_webrtc::Message;
use antex_realtime_webrtc::SessionDescription;
use antex_realtime_webrtc::VoiceHost;
use antex_realtime_webrtc::decode_frame;
use antex_realtime_webrtc::encode_frame;
...
fn spawn() -> Result<Child> {
    Ok(Command::new(cargo_bin("antex-voice-host")?)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .kill_on_drop(true)
...
async fn build_commit() -> Result<String> {
    let output = timeout(
        DEADLINE,
        Command::new(cargo_bin("antex-voice-host")?)
            .arg("--build-commit")
            .kill_on_drop(true)
            .output(),
    )
...
fn install_helper(root: &Path) -> Result<AntexPackageLayout> {
    let bin = root.join("bin");
    let helper_dir = root.join("antex-resources/voice/bin");
    fs::create_dir_all(&bin)?;
    fs::create_dir_all(&helper_dir)?;
    fs::write(root.join("antex-package.json"), "{}")?;
    let app = bin.join(if cfg!(windows) { "antex.exe" } else { "antex" });
    fs::write(&app, [])?;
    let source = cargo_bin("antex-voice-host")?;
...
async fn installed_client_rejects_mixed_builds_and_missing_helper() -> Result<()> {
    ...
    let helper = directory
        .path()
        .join("antex-resources/voice/bin")
