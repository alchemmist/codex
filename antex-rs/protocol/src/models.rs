...
use antex_utils_image::ImageProcessingError;
...
pub enum SandboxEnforcement {
    /// Antex owns sandbox construction for this profile.
    #[default]
    Managed,
...
/// Filesystem permissions for profiles where Antex owns sandbox construction.
...
fn convert_mcp_content_to_items(
    ...
    const ANTEX_ENCRYPTED_CONTENT_META_KEY: &str = "codex/encryptedContent";
    const ANTEX_IMAGE_DETAIL_META_KEY: &str = "codex/imageDetail";
    ...
    for content in contents {
        let item = match serde_json::from_value::<McpContent>(content.clone()) {
            Ok(McpContent::Text { text, meta }) => {
                if meta
                    .as_ref()
                    .and_then(|meta| meta.get(ANTEX_ENCRYPTED_CONTENT_META_KEY))
            ...
            Ok(McpContent::Image {
                ...
                FunctionCallOutputContentItem::InputImage {
                    ...
                    detail: meta
                        ...
                        .and_then(|meta| meta.get(ANTEX_IMAGE_DETAIL_META_KEY))
...
// (Moved event mapping logic into antex-core to avoid coupling protocol to UI-facing events.)
        ...
        for i in 0..200 {
            exec_policy
                .add_prefix_rule(
                    ...
                    antex_execpolicy::Decision::Allow,
                )
                .expect("add rule");
        }
