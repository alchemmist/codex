...
use antex_utils_absolute_path::canonicalize_preserving_symlinks;
...
const PROTECTED_METADATA_ANTEX_PATH_NAME: &str = ".antex";
const PROTECTED_METADATA_LEGACY_PATH_NAME: &str = ".codex";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_ANTEX_PATH_NAME,
    PROTECTED_METADATA_LEGACY_PATH_NAME,
];
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Antex degrades to warn-and-ignore
    /// instead of failing to load. Antex 0.112.0 rejected unknown values here,
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".antex");
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".codex");
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_antex*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_antex*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_antex*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    fn get_writable_roots_with_cwd_impl(
        ...
        .map(|root| {
            ...
            let protect_missing_dot_antex = effective_cwd.as_ref() == Some(&root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_antex)
                    .into_iter()
            ...
            // Example: if `<root>/.antex -> <root>/decoy`, bwrap must still see
            // `<root>/.antex`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.antex` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_antex: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    let top_level_antex = writable_root.join(PROTECTED_METADATA_ANTEX_PATH_NAME);
    if protect_missing_dot_antex || top_level_antex.as_path().is_dir() {
        subpaths.push(top_level_antex);
    }
    let legacy_metadata = writable_root.join(PROTECTED_METADATA_LEGACY_PATH_NAME);
    if legacy_metadata.as_path().is_dir() {
        subpaths.push(legacy_metadata);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "ANTEX_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn uri_deny_and_metadata_matcher_use_executor_paths() {
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry::new(
                path("file:///C:/workspace/.antex").into(),
                FileSystemAccessMode::Read,
            ),
        ...
        assert!(!policy.can_write_path(&path("file:///C:/workspace/.antex/config"), &context));
        assert_eq!(
            policy.metadata_write_denial(&path("file:///C:/workspace/.antex/config"), &context),
            Some(".antex"),
        );
    }
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".antex".into())),
                    },
                    FileSystemAccessMode::Read,
                ),
                FileSystemSandboxEntry::skip_missing_path(
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_antex_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_antex = expected_root.join(".antex");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_antex.clone().into(),
                },
                access: FileSystemAccessMode::Write,
                missing_path_behavior: None,
            },
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_antex_config = cwd.path().join(".antex").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_local_path_with_cwd(&dot_antex_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_antex_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_antex*/ true,
            )
            .into_iter()
            .map(|path| {
        ...
        assert!(
            !file_system_policy
                .can_write_local_path_with_cwd(Path::new(".antex/config.toml"), relative_cwd,)
        );
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let antex_dir = real_root.join(".antex");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&antex_dir).expect("create .codex");
        ...
        let expected_antex = link_root.join(".antex");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_antex = expected_root.join(".antex");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let dot_antex = root.join(".antex");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_antex).expect("create .codex symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_antex = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let antex_dir = real_tmpdir.join(".antex");
