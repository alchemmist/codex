...
use antex_http_client::HttpError;
use antex_utils_string::truncate_middle_chars;
...
#[strum_discriminants(name(AntexErrKind))]
...
pub enum CodexErrorDetails {
    ...
    #[error("antex-linux-sandbox was required but not provided")]
...
impl From<&CodexErr> for AntexErrKind {
...
macro_rules! antex_err_unit_constructors {
    ($($variant:ident),* $(,)?) => {
        $(
            #[doc(hidden)]
...
// TODO(anp): Remove this compatibility macro once callers construct
// `CodexErrorDetails` directly.
macro_rules! antex_err_tuple_constructors {
...
impl CodexErr {
    ...
    pub fn to_antex_protocol_error(&self) -> CodexErrorInfo {
        match &self.details {
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            codex_error_info: Some(self.to_antex_protocol_error()),
