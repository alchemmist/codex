//! Defines the protocol for a Antex session between a client and an agent.
//!
...
use crate::error::Result as AntexResult;
...
use antex_extension_items::image_generation::ImageGenerationFailure;
...
pub struct ConversationStartParams {
    /// Whether Antex response handoffs are managed through explicit client append calls.
    ...
    /// Whether to route any remaining transcript tail through Antex when the session ends.
    ...
    /// Sends automatic Antex responses as realtime conversation items instead of handoff appends.
    pub codex_responses_as_items: bool,
    /// Optional prefix added to automatic Antex response items when `codex_responses_as_items` is set.
    pub codex_response_item_prefix: Option<String>,
    /// Selects how automatic Antex handoffs are routed in Frameless Bidi sessions.
    ...
    /// Whether to append Antex's startup context to the realtime backend prompt.
    ...
    /// Developer instructions given to Antex when this realtime session starts.
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to Antex when this realtime session ends.
...
pub enum Op {
    ...
    TurnInput {
        ...
        mode: TurnInputMode,
        reply: oneshot::Sender<AntexResult<TurnInputSubmission>>,
    },
    ...
    SuspendTurnAndShutdown {
        reply: oneshot::Sender<AntexResult<SuspendTurnOutcome>>,
    },
    ...
    /// Request Antex to drop the last N user turns from in-memory context.
    ///
...
/// running the command proposed by Antex.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.antex`, `.git`, notably `.git/hooks`) under
...
/// Antex errors that we expose to clients.
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Antex
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(antex_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                antex_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_antex() {
        assert_eq!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = antex_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            antex_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".antex")).expect("create .codex");
        let canonical_cwd = antex_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_antex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".antex"))
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_antex.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_antex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".antex"))
            .expect("canonical .codex");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_antex.to_path_buf(),
