...
//! `schemars`) so they can be embedded in Antex's own protocol structures.
...
/// MCP extensions supplied by the client that created a Antex session.
...
// === Adapter helpers ===
//
// These types and conversions intentionally live in `antex-protocol` so other crates can convert
