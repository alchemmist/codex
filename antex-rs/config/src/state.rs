...
pub struct LoaderOverrides {
    /// Optional configuration file supplied with the installed Antex package.
...
impl ConfigLayerEntry {
    ...
    // Get the `.antex/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project { dot_antex_folder } => Some(dot_antex_folder.clone()),
    ...
    /// Returns the `.antex/` folder that should be used for hook declarations.
    ///
...
impl ConfigLayerStack {
    ...
    /// the base `$ANTEX_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<()> {
    ...
    // user config.
    let mut previous_project_dot_antex_folder: Option<&AbsolutePathBuf> = None;
    for layer in layers {
        if let ConfigLayerSource::Project {
            dot_antex_folder: current_project_dot_antex_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_antex_folder {
                ...
                if previous == current_project_dot_antex_folder
                    || !current_project_dot_antex_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_antex_folder = Some(current_project_dot_antex_folder);
        }
    }
