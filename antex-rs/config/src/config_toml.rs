//! Schema-heavy configuration TOML types used by Antex.
...
use antex_model_provider_info::AMAZON_BEDROCK_RUNTIME_PROVIDER_ID;
...
use antex_utils_path::normalize_for_path_comparison;
...
/// Base config deserialized from ~/.antex/config.toml.
...
pub struct ConfigToml {
    ...
    /// Allow macOS sandbox writable roots at or beneath ANTEX_HOME to traverse
    ...
    /// they change between commands or lie outside ANTEX_HOME.
    /// This setting has no effect on Linux or Windows.
    pub allow_symlinked_antex_home: Option<bool>,
    ...
    /// sanctioned by Antex will likely degrade model performance.
    ...
    /// file: Use a file in the Antex home directory.
    ...
    /// Directory where Antex stores the SQLite state DB.
    /// Defaults to `$ANTEX_SQLITE_HOME` when set. Otherwise uses `$ANTEX_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Antex writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$ANTEX_HOME/log`.
    ...
    /// directories for `.antex` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub use antex_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use antex_protocol::protocol::RealtimeVoice;
