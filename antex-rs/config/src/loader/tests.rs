...
use antex_file_system::CopyOptions;
use antex_file_system::CreateDirectoryOptions;
use antex_file_system::ExecutorFileSystemFuture;
use antex_file_system::FileMetadata;
use antex_file_system::FileSystemReadStream;
use antex_file_system::FileSystemSandboxContext;
use antex_file_system::GetMetadataOptions;
use antex_file_system::ReadDirectoryEntry;
use antex_file_system::ReadFileOptions;
use antex_file_system::RemoveOptions;
use antex_file_system::WalkOptions;
use antex_file_system::WalkOutcome;
use antex_file_system::WriteFileOptions;
...
fn project_environment_filters_preserve_only_trusted_credential_host_bindings() {
    ...
    for project_policy in [
        ...
        let actual = antex_protocol::shell_environment::populate_env(
            [
                ("GH_HOST", host),
...
"#,
    ...
    let expected_warnings = vec![format!(
        "Ignoring deprecated managed config file at {}; ANTEX_HOME/managed_config.toml is no longer supported on Windows. Use %ProgramData%\\OpenAI\\Codex\\requirements.toml for enforced settings or config.toml for defaults.",
...
async fn local_layers_keep_raw_paths_order_and_legacy_requirements() {
    ...
    let dot_antex = project.join(".antex");
    let system_dir = tmp.path().join("system");
    let managed_dir = tmp.path().join("managed");
    for dir in [&codex_home, &dot_antex, &system_dir, &managed_dir] {
