...
use antex_file_system::ExecutorFileSystem;
use antex_git_utils::resolve_root_git_project_for_trust;
use antex_network_proxy::credential_broker_provider_context_env_keys;
use antex_network_proxy::is_credential_broker_provider_env_key;
...
///   `%ProgramData%\OpenAI\Antex\requirements.toml` (Windows)
...
///
/// - package:  optional default configuration supplied with the Antex package
/// - admin:    managed preferences (*)
/// - system    `/etc/codex/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Antex\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${ANTEX_HOME}/config.toml`
/// - profile   `${ANTEX_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.antex/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.antex/config.toml` (loaded but disabled when untrusted)
...
#[cfg(windows)]
fn windows_antex_system_dir() -> PathBuf {
...
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_antex_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_antex_system_dir().join("config.toml");
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for Antex to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".antex"))
    }
}

fn project_layer_entry(
    dot_antex_folder: &AbsolutePathBuf,
    config: TomlValue,
    disabled_reason: Option<String>,
    hooks_config_folder_override: Option<AbsolutePathBuf>,
) -> ConfigLayerEntry {
    let source = ConfigLayerSource::Project {
        dot_antex_folder: dot_antex_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_antex_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    for layer in discovered.layers {
        let config =
            resolve_relative_paths_in_config_toml(layer.config, layer.dot_antex_folder.as_path())?;
...
async fn discover_project_layers(
    ...
    let antex_home_abs = AbsolutePathBuf::from_absolute_path(codex_home)?;
    let antex_home_normalized =
        normalize_path(antex_home_abs.as_path()).unwrap_or_else(|_| antex_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_antex_abs = dir.join(".antex");
        let dot_antex_uri = PathUri::from_abs_path(&dot_antex_abs);
        if !fs
            .get_metadata(&dot_antex_uri, Default::default(), /*sandbox*/ None)
            .await
        ...
        let dot_antex_normalized =
            normalize_path(dot_antex_abs.as_path()).unwrap_or_else(|_| dot_antex_abs.to_path_buf());
        if dot_antex_abs == antex_home_abs || dot_antex_normalized == antex_home_normalized {
            continue;
        }
        let config_file = dot_antex_abs.join(CONFIG_TOML_FILE);
                ...
                let config: TomlValue = match toml::from_str(&contents) {
                    ...
                    Err(e) => {
                        ...
                        layers.push(DiscoveredProjectLayer {
                            dot_antex_folder: dot_antex_abs,
