...
mod antex_tool_config;
mod antex_tool_runner;
mod exec_approval;
...
pub use crate::antex_tool_config::AntexToolCallParam;
pub use crate::antex_tool_config::AntexToolCallReplyParam;
...
const OTEL_SERVICE_NAME: &str = "antex_mcp_server";
...
pub async fn run_main(
    ...
    reject_workload_identity(antex_login::is_workload_identity_selected())?;
...
fn reject_workload_identity(workload_identity_selected: bool) -> IoResult<()> {
    if workload_identity_selected {
        return Err(std::io::Error::new(
            ...
            "workload identity is not supported by `antex mcp-server`",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    async fn mcp_server_builds_otel_provider_with_logs_traces_and_metrics() -> anyhow::Result<()> {
        ...
        let provider = antex_core::otel_init::build_provider(
            &config,
            "0.0.0-test",
            Some(OTEL_SERVICE_NAME),
            DEFAULT_ANALYTICS_ENABLED,
