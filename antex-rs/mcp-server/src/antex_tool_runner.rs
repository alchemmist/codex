//! Asynchronous worker that executes a **Antex** tool-call inside a spawned
...
use antex_core::config::Config as AntexConfig;
...
/// To adhere to MCP `tools/call` response format, include the Antex
...
/// Run a complete Antex session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_antex_tool_session(
    id: RequestId,
    ...
    run_antex_tool_session_inner(thread_id, thread, outgoing, id, active_turns).await;
}

pub async fn run_antex_tool_session_reply(
    thread_id: ThreadId,
    ...
    run_antex_tool_session_inner(thread_id, thread, outgoing, request_id, active_turns).await;
}

async fn run_antex_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<AntexThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
