...
pub struct ExecApprovalElicitRequestParams {
    ...
    pub antex_command: Vec<String>,
    #[serde(rename = "codex_cwd")]
    pub antex_cwd: PathBuf,
    #[serde(rename = "codex_parsed_cmd")]
    pub antex_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    antex_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_exec_approval_response(approval_id, event_id, on_response, antex).await;
        });
    }
}
