...
use antex_login::default_client::USER_AGENT_SUFFIX;
...
use crate::antex_tool_config::AntexToolCallParam;
use crate::antex_tool_config::AntexToolCallReplyParam;
use crate::antex_tool_config::create_tool_for_antex_tool_call_param;
use crate::antex_tool_config::create_tool_for_antex_tool_call_reply_param;
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        antex_image_generation_extension::install(
            &mut extensions,
            auth_manager.clone(),
            |config: &Config| Some(config.codex_home.clone()),
        );
        let skill_providers = antex_skills_extension::SkillProviders::new()
            .with_host_provider(Arc::new(antex_skills_extension::HostSkillProvider::new()));
        antex_skills_extension::install_with_providers_and_metrics(
            &mut extensions,
            skill_providers,
            antex_otel::global(),
            |config: &Config| antex_skills_extension::SkillsExtensionConfig {
        ...
        let thread_manager = Arc::new(ThreadManager::new(
            ...
            antex_core::build_models_manager(config.as_ref(), auth_manager),
            ...
            antex_core::thread_store_from_config(config.as_ref(), state_db.clone()),
    ...
    async fn handle_initialize(
        ...
        let server_info =
            Implementation::new("antex-mcp-server", env!("CARGO_PKG_VERSION")).with_title("Antex");

        // Preserve Antex's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_antex_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult::with_all_items(vec![
            create_tool_for_antex_tool_call_param(),
            create_tool_for_antex_tool_call_reply_param(),
        ]);
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "codex" => self.handle_tool_call_antex(id, arguments).await,
            "codex-reply" => {
                self.handle_tool_call_antex_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_antex(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<AntexToolCallParam>(json_val) {
        ...
        // Spawn an async task to handle the Antex session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Antex session and stream events back to the client.
            crate::antex_tool_runner::run_antex_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
    ...
    async fn handle_tool_call_antex_session_reply(
        &self,
        ...
        // parse arguments
        let antex_tool_call_reply_param: AntexToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<AntexToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
        ...
        let thread_id = match antex_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let antex = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::antex_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = antex_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let active_turns = Arc::clone(&active_turns);

            async move {
                crate::antex_tool_runner::run_antex_tool_session_reply(
                    thread_id,
                    antex,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        // Obtain the Antex thread from the server.
        let antex_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Antex.
        if let Err(e) = antex_arc
            .submit(antex_protocol::protocol::Op::Interrupt)
            .await
        {
