...
use antex_mcp_server::AntexToolCallParam;
use antex_mcp_server::ExecApprovalElicitRequestParams;
use antex_mcp_server::ExecApprovalResponse;
use antex_mcp_server::PatchApprovalElicitRequestParams;
use antex_mcp_server::PatchApprovalResponse;
use antex_protocol::protocol::FileChange;
use antex_protocol::protocol::ReviewDecision;
use antex_shell_command::parse_command;
...
async fn exec_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // as an elicitation.
    let antex_request_id = mcp_process
        .send_antex_tool_call(AntexToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            antex_request_id.to_string(),
            params.antex_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    let antex_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(antex_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(antex_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    antex_mcp_tool_call_id: String,
    antex_event_id: String,
    ...
    let antex_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        antex_elicitation: "exec-approval".to_string(),
        antex_mcp_tool_call_id,
        antex_event_id,
        antex_command: command,
        antex_cwd: workdir.to_path_buf(),
        antex_call_id: "call1234".to_string(),
        antex_parsed_cmd,
    })?;
    Ok(params_json)
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_antex_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    antex_tool_passes_base_instructions()
        .await
        .expect("codex tool should pass base instructions");
}

async fn antex_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `antex mcp` with a specific config.toml.
    ...
    let mut mcp_process = McpProcess::new_with_env(
        codex_home.path(),
        &[("OPENAI_API_KEY", None), ("ANTEX_ACCESS_TOKEN", None)],
    )
    .await?;
    ...
    .await??;
    assert_eq!(antex_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(antex_response.id, RequestId::Number(antex_request_id));
    assert_eq!(
        antex_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_antex_tool_forwards_skills_extension_warnings() {
    skip_if_no_network!();

    antex_tool_forwards_skills_extension_warnings()
        .await
        .expect("codex tool should forward skills extension warnings");
}

async fn antex_tool_forwards_skills_extension_warnings() -> anyhow::Result<()> {
    let server =
        create_mock_responses_server(vec![create_final_assistant_message_sse_response("Enjoy!")?])
            .await;
    let codex_home = TempDir::new()?;
    ...
    let warning = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_antex_event_matching("warning", |params| {
            params["msg"]["message"]
                .as_str()
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        ...
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        antex_elicitation: "patch-approval".to_string(),
        antex_mcp_tool_call_id,
        antex_event_id,
        antex_reason: reason,
        antex_grant_root: grant_root,
        antex_changes: changes,
        antex_call_id: "call1234".to_string(),
    })?;
...
/// Create a Antex config that uses the mock server as the model provider.
