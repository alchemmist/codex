...
impl McpProcess {
    ...
    pub async fn new_with_env(
        ...
        let program = antex_utils_cargo_bin::cargo_bin("antex-mcp-server")
            .context("should find binary for antex-mcp-server")?;
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("antex-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = antex_login::default_client::originator().value;
    ...
    pub async fn send_antex_tool_call(
        &mut self,
        params: AntexToolCallParam,
    ) -> anyhow::Result<i64> {
        let antex_tool_call_params = CallToolRequestParams::new("codex").with_arguments(
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(antex_tool_call_params)?),
        )
        .await
    }
    ...
    pub async fn read_stream_until_legacy_task_complete_notification(
        ...
        self.read_stream_until_antex_event("task_complete").await
    }

    pub async fn read_stream_until_antex_event(
        &mut self,
        event_type: &str,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        self.read_stream_until_antex_event_matching(event_type, |_| true)
            .await
    }

    pub async fn read_stream_until_antex_event_matching(
        &mut self,
        event_type: &str,
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `antex-mcp-server` child process.
        //
