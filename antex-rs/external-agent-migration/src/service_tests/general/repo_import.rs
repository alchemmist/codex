...
#[tokio::test]
async fn import_repo_hooks_preserves_disabled_antex_hooks_feature() {
    let root = TempDir::new().expect("create tempdir");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create external agent dir");
    fs::create_dir_all(repo_root.join(".antex")).expect("create codex dir");
    fs::write(
    ...
    assert_eq!(
        fs::read_to_string(repo_root.join(".antex").join("config.toml")).expect("read config"),
        "[features]\ncodex_hooks = false\n"
    );
    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".antex").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
    assert_eq!(
        hooks,
...
async fn repo_hooks_migration_skips_symlink_targets() {
    ...
    for (repo_name, initial_target_contents) in [
        ...
        let repo_root = root.path().join(repo_name);
        let linked_target = root.path().join(format!("{repo_name}-target"));
        let hooks_json = repo_root.join(".antex").join("hooks.json");
        fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
        fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create external agent dir");
        fs::create_dir_all(repo_root.join(".antex")).expect("create codex dir");
        fs::write(
...
async fn import_repo_mcp_uses_home_settings_toggles_when_repo_settings_missing() {
    ...
    let outcome = service_for_paths(external_agent_home, root.path().join(".antex"))
        .import(vec![ExternalAgentConfigMigrationItem {
...
async fn import_repo_mcp_uses_local_settings_toggles_over_project_settings() {
    ...
    service_for_paths(external_agent_home, root.path().join(".antex"))
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::McpServerConfig,
            description: String::new(),
            cwd: Some(repo_root.clone()),
