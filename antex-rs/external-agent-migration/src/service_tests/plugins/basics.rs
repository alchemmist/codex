...
async fn authenticated_plugin_migration_uses_chatgpt_curated_marketplace() {
    ...
    fs::create_dir_all(plugin_root.join(".antex-plugin")).expect("create curated plugin directory");
    fs::write(
...
fn marketplace_import_sources_prefers_scoped_source_over_registry_name_collision() {
    let (root, external_agent_home, _antex_home) = fixture_paths();
...
fn marketplace_import_sources_prefers_supported_declaration_over_materialization() {
    let (_root, external_agent_home, _antex_home) = fixture_paths();
...
async fn detect_repo_skips_plugins_from_remote_marketplace() {
    ...
    let codex_home = root.path().join(".antex");
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_disabled_in_antex() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let codex_home = root.path().join(".antex");
    let repo_root = root.path().join("repo");
...
#[tokio::test]
async fn detect_repo_skips_plugins_without_explicit_enabled_in_antex() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let codex_home = root.path().join(".antex");
    let repo_root = root.path().join("repo");
...
#[tokio::test]
async fn detect_repo_skips_plugins_only_configured_in_project_antex() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let codex_home = root.path().join(".antex");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".antex")).expect("create repo codex dir");
    fs::create_dir_all(&codex_home).expect("create codex home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
    ...
    .expect("write repo settings");
    fs::write(
        repo_root.join(".antex").join("config.toml"),
        r#"
[plugins."formatter@acme-tools"]
enabled = true
"#,
