...
async fn import_plugins_treats_empty_cwd_as_home_scope() {
    ...
    fs::create_dir_all(plugin_root.join(".antex-plugin")).expect("create plugin manifest dir");
...
async fn import_plugins_reuses_configured_marketplace_with_different_source() {
    ...
    fs::create_dir_all(configured_plugin_root.join(".antex-plugin"))
    ...
    fs::create_dir_all(source_plugin_root.join(".antex-plugin"))
...
"#
    ...
    fs::write(
        source_plugin_root.join(".antex-plugin/plugin.json"),
