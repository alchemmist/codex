...
use antex_app_server_protocol::ServerNotificationEnvelope;
use antex_config::types::AuthCredentialsStoreMode;
use antex_core::test_support::auth_manager_from_auth;
use antex_core::test_support::auth_manager_from_auth_with_home;
use antex_login::AntexAuth;
use antex_login::AuthDotJson;
use antex_login::AuthKeyringBackendKind;
use antex_login::AuthManager;
use antex_login::save_auth;
use antex_login::token_data::TokenData;
use antex_login::token_data::parse_chatgpt_jwt_claims;
...
fn remote_control_auth_manager() -> Arc<AuthManager> {
    auth_manager_from_auth(AntexAuth::create_dummy_chatgpt_auth_for_testing())
}
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    antex_app_server_protocol::JSONRPCNotification {
    ...
    let initialize_message = JSONRPCMessage::Request(antex_app_server_protocol::JSONRPCRequest {
        id: antex_app_server_protocol::RequestId::Integer(1),
    ...
    let followup_message =
        JSONRPCMessage::Notification(antex_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(antex_app_server_protocol::JSONRPCRequest {
                    id: antex_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(antex_app_server_protocol::JSONRPCRequest {
                id: antex_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
    ...
    writer
        .send(QueuedOutgoingMessage::new(OutgoingMessage::Response(
            crate::outgoing_message::OutgoingResponse {
                id: antex_app_server_protocol::RequestId::Integer(11),
                result: Box::new(
                    antex_app_server_protocol::ClientResponsePayload::Initialize(
                        antex_app_server_protocol::InitializeResponse {
