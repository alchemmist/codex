...
async fn hosted_apps_mcp_requires_chatgpt_auth() -> TestResult {
    ...
    let config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    let auth = AntexAuth::from_api_key("test");
    let manager = installed_manager(&config, Some(auth.clone()));
...
fn installed_manager(config: &Config, auth: Option<AntexAuth>) -> McpManager {
    let mut builder = ExtensionRegistryBuilder::new();
    antex_mcp_extension::install(&mut builder);
...
impl McpServerContributor<Config> for RemoveCodexApps {
    ...
    ) -> antex_extension_api::ExtensionFuture<'a, Vec<McpServerContribution>> {
        Box::pin(async move {
