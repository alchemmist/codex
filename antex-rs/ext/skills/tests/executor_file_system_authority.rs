...
use antex_exec_server::CapabilityTextFile;
use antex_exec_server::CopyOptions;
use antex_exec_server::CreateDirectoryOptions;
use antex_exec_server::DiscoveredSkillFiles;
...
use antex_exec_server::ExecutorFileSystemFuture;
use antex_exec_server::FileMetadata;
use antex_exec_server::FileSystemReadStream;
use antex_exec_server::FileSystemSandboxContext;
use antex_exec_server::GetMetadataOptions;
use antex_exec_server::ReadDirectoryEntry;
use antex_exec_server::ReadFileOptions;
use antex_exec_server::RemoveOptions;
use antex_exec_server::WalkEntry;
use antex_exec_server::WalkEntryKind;
use antex_exec_server::WalkOptions;
use antex_exec_server::WalkOutcome;
use antex_exec_server::WriteFileOptions;
...
async fn skill_loading_and_reads_use_the_supplied_executor_file_system() {
    ...
    let project_folder = AbsolutePathBuf::from_absolute_path_checked(test_root.join(".antex"))
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_antex_folder: project_folder,
            },
...
async fn executor_discovery_routes_produce_equivalent_catalog_metadata() {
    ...
    let plugin_manifest = test_root.join(".antex-plugin/plugin.json");
    ...
    let comparable_entries = |catalog: &antex_skills_extension::catalog::SkillCatalog| {
        catalog
            .entries
            .iter()
            .map(|entry| {
...
async fn pre_discovered_executor_catalog_snapshot() {
    ...
    let manifest_dir = test_root.join(".antex-plugin");
...
async fn direct_executor_discovery_preserves_hidden_nested_and_probed_metadata() {
    ...
    let outer_manifest = test_root.join(".antex-plugin/plugin.json");
    ...
    let inner_manifest = test_root.join("nested/.antex-plugin/plugin.json");
