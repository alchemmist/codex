...
use antex_exec_server::CapabilityRootDiscovery;
use antex_exec_server::ExecutorCapabilityDiscoverySnapshot;
use antex_exec_server::LOCAL_FS;
use antex_extension_api::ConversationHistory;
...
use antex_extension_api::FunctionCallError;
use antex_extension_api::NoopTurnItemEmitter;
use antex_extension_api::PreviousWorldStateSection;
use antex_extension_api::RenderedWorldStateFragment;
use antex_extension_api::SkillInvocationInput;
use antex_extension_api::SkillInvocationKind;
use antex_extension_api::ThreadStartInput;
use antex_extension_api::ToolCall;
use antex_extension_api::ToolCallSource;
use antex_extension_api::ToolPayload;
...
use antex_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use antex_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use antex_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use antex_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use antex_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use antex_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
use antex_protocol::protocol::SessionSource;
use antex_protocol::protocol::SkillScope;
use antex_protocol::protocol::TruncationPolicy;
use antex_protocol::protocol::TurnEnvironmentSelection;
use antex_protocol::user_input::UserInput;
use antex_skills::SkillMetadata;
use antex_skills_extension::HostSkillProvider;
use antex_skills_extension::HostSkillsLoadInput;
use antex_skills_extension::HostSkillsService;
use antex_skills_extension::HostSkillsSnapshot;
use antex_skills_extension::InjectedHostSkillPrompts;
use antex_skills_extension::SkillLoadOutcome;
use antex_skills_extension::SkillProviders;
use antex_skills_extension::SkillsExtensionConfig;
use antex_skills_extension::catalog::SkillAuthority;
use antex_skills_extension::catalog::SkillCatalog;
use antex_skills_extension::catalog::SkillCatalogEntry;
use antex_skills_extension::catalog::SkillPackageId;
use antex_skills_extension::catalog::SkillProviderError;
use antex_skills_extension::catalog::SkillReadResult;
use antex_skills_extension::catalog::SkillResourceId;
use antex_skills_extension::catalog::SkillSearchResult;
use antex_skills_extension::catalog::SkillSourceKind;
use antex_skills_extension::install;
use antex_skills_extension::install_with_providers;
use antex_skills_extension::install_with_providers_and_metrics;
use antex_skills_extension::provider::SkillListQuery;
use antex_skills_extension::provider::SkillProvider;
use antex_skills_extension::provider::SkillProviderFuture;
use antex_skills_extension::provider::SkillReadRequest;
use antex_skills_extension::provider::SkillSearchRequest;
...
static NEXT_ANTEX_HOME_ID: AtomicUsize = AtomicUsize::new(0);
...
async fn skill_world_state_fragments(
    registry: &antex_extension_api::ExtensionRegistry<TestConfig>,
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    let codex_home = test_antex_home();
...
async fn host_world_state_records_catalog_metrics_on_publish_and_change() -> TestResult {
    ...
    let skill_path = AbsolutePathBuf::try_from(test_antex_home().join("skills/demo/SKILL.md"))?;
    let mut outcome = SkillLoadOutcome::default();
    outcome.skills.push(SkillMetadata {
        name: "demo".to_string(),
        description: "Demo skill.".to_string(),
    ...
    let second_skill_path =
        AbsolutePathBuf::try_from(test_antex_home().join("skills/other/SKILL.md"))?;
    outcome.skills.push(SkillMetadata {
...
fn test_antex_home() -> PathBuf {
    let id = NEXT_ANTEX_HOME_ID.fetch_add(1, Ordering::Relaxed);
