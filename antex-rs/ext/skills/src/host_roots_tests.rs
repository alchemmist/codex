...
fn project_layer(dot_antex_folder: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
...
fn layer_roots_preserve_scope_precedence_and_disabled_projects() {
    ...
    let project_folder = absolute(temp_dir.path().join("repo/.antex"));
    let nested_project_folder = absolute(temp_dir.path().join("repo/nested/.antex"));
    let config_stack = stack(vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_antex_folder: project_folder.clone(),
            },
            empty_config(),
            "untrusted project",
        ),
...
async fn resolved_project_layer_loads_skill_without_git_marker() {
    ...
    let dot_antex = workspace.join(".antex");
    let skill_root = dot_antex.join("skills");
    fs::create_dir_all(&workspace).expect("create workspace");
    let skill_path = write_skill(&skill_root, "local", "local-skill");
    let config_stack = stack(vec![project_layer(&dot_antex)]);
...
async fn resolved_project_layer_loads_skill_when_cwd_is_file() {
    ...
    let dot_antex = repository.join(".antex");
    let skill_root = dot_antex.join("skills");
...
async fn resolved_config_and_repo_roots_preserve_order_and_dedupe_paths_not_names() {
    ...
    let project_dot_antex = repository.join(".antex");
    let nested_project_dot_antex = repository.join("nested/.antex");
    let user_skills = codex_home.join("skills");
    let root_project_skill = write_skill(
        &project_dot_antex.join("skills"),
        "root-duplicate",
        "duplicate-skill",
    );
    let nested_project_skill = write_skill(
        &nested_project_dot_antex.join("skills"),
        "nested-duplicate",
        "duplicate-skill",
    );
    ...
    let config_stack = stack(vec![
        ...
        project_layer(&project_dot_antex),
        project_layer(&nested_project_dot_antex),
    ]);
