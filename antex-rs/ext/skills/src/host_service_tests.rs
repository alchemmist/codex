...
use antex_skills::ImplicitSkillLookup;
use antex_skills::LoadedSkillRoot;
use antex_skills::SkillRootSnapshotCache;
...
async fn snapshot_for_config_preserves_host_precedence_for_symlinked_plugin_root() {
    ...
    assert_eq!(
        outcome.skills,
        vec![antex_skills::SkillMetadata {
...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    let codex_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let repo_dot_antex = cwd.path().join(".antex");
    fs::create_dir_all(&repo_dot_antex).expect("create repo config dir");

    write_user_skill(&codex_home, "user", "user-skill", "from local user root");
    let repo_skill_dir = repo_dot_antex.join("skills/repo");
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_antex_folder: repo_dot_antex.abs(),
                },
    ...
    let other_file_system: Arc<dyn antex_exec_server::ExecutorFileSystem> =
        Arc::new(antex_exec_server::LocalFileSystem::unsandboxed());
