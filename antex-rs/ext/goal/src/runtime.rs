...
struct GoalRuntimeInner {
    thread_id: ThreadId,
    state_dbs: Arc<antex_state::StateRuntime>,
...
pub struct PreviousGoalSnapshot {
    pub goal_id: String,
    pub status: antex_state::ThreadGoalStatus,
    pub objective: String,
}

impl From<&antex_state::ThreadGoal> for PreviousGoalSnapshot {
    fn from(goal: &antex_state::ThreadGoal) -> Self {
        Self {
...
impl GoalRuntimeHandle {
    ...
    pub async fn apply_external_goal_set(
        ...
        match goal.status {
            ...
            antex_state::ThreadGoalStatus::Paused
            | antex_state::ThreadGoalStatus::Blocked
            | antex_state::ThreadGoalStatus::UsageLimited
            | antex_state::ThreadGoalStatus::Complete => {
                self.inner.accounting_state.clear_active_goal();
            }
        }
        Ok(())
    ...
    pub(crate) async fn stop_active_goal_for_turn(
        ...
        let (event_name, status, expected_goal_id) = match reason {
            ActiveGoalStopReason::TurnError => {
                ("turn-error", antex_state::ThreadGoalStatus::Blocked, None)
            }
        ...
        let can_stop = active_goal.status == antex_state::ThreadGoalStatus::Active
            || (active_goal.status == antex_state::ThreadGoalStatus::BudgetLimited
                && status == antex_state::ThreadGoalStatus::UsageLimited);
        if !can_stop {
            self.inner.accounting_state.clear_active_goal();
            return Ok(());
        }
    ...
    pub async fn restore_after_resume(&self) -> Result<(), String> {
        ...
        let goal = self
            ...
            .map_err(|err| err.to_string())?;
        match goal {
            Some(goal) if goal.status == antex_state::ThreadGoalStatus::Active => {
                self.inner
                    .accounting_state
    ...
    pub(crate) async fn continue_if_idle(&self) -> Result<(), String> {
        ...
        if goal.status != antex_state::ThreadGoalStatus::Active {
            self.inner.accounting_state.clear_active_goal();
            return Ok(());
        }
    ...
    async fn account_idle_goal_progress(
        ...
        Ok(match outcome {
            ...
            antex_state::GoalAccountingOutcome::Unchanged(_) => {
    ...
    ) -> Result<Option<antex_state::ThreadGoalStatus>, String> {
        let goal = self
            .inner
            .state_dbs
            .thread_goals()
