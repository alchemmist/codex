...
use antex_extension_api::JsonToolOutput;
use antex_extension_api::ToolCall;
use antex_extension_api::ToolExecutor;
use antex_extension_api::ToolName;
use antex_extension_api::ToolOutput;
use antex_extension_api::ToolSpec;
use antex_protocol::ThreadId;
use antex_protocol::protocol::ThreadGoal;
use antex_protocol::protocol::ThreadGoalStatus;
use antex_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<antex_state::StateRuntime>,
...
impl<'call> ToolExecutor<ToolCall<'call>> for GoalToolExecutor {
    ...
    ) -> antex_extension_api::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
        Box::pin(async move {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => antex_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => antex_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: antex_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            antex_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            antex_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<antex_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &antex_state::StateRuntime,
    thread_id: ThreadId,
    goal: &antex_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: antex_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: antex_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> antex_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => antex_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => antex_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => antex_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => antex_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => antex_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => antex_state::ThreadGoalStatus::Complete,
    }
}
