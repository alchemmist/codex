...
use antex_extension_api::TurnAbortInput;
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                antex_protocol::config_types::ModeKind::Plan
            ) {
    ...
    C: Send + Sync + 'static,
{
    fn tools(
        ...
        Arc<dyn for<'call> antex_extension_api::ToolExecutor<antex_extension_api::ToolCall<'call>>>,
    > {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };
