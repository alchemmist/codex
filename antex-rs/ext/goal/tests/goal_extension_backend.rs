...
use antex_extension_api::ThreadResumeInput;
use antex_extension_api::ThreadStartInput;
use antex_extension_api::ThreadStopInput;
...
use antex_extension_api::TurnErrorInput;
use antex_extension_api::TurnStartInput;
use antex_extension_api::TurnStopInput;
use antex_goal_extension::GoalExtensionConfig;
use antex_goal_extension::GoalObjectiveUpdate;
use antex_goal_extension::GoalRuntimeHandle;
use antex_goal_extension::GoalService;
use antex_goal_extension::GoalSetRequest;
use antex_goal_extension::GoalTokenBudgetUpdate;
use antex_goal_extension::install_with_backend;
...
use antex_protocol::protocol::ThreadGoalStatus;
...
async fn spawned_descendant_usage_exhausts_root_goal_budget_once() -> anyhow::Result<()> {
    ...
    assert_eq!(antex_state::ThreadGoalStatus::BudgetLimited, goal.status);
    Ok(())
}
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(antex_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(antex_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
        ...
        [(None, None, 3), (Some(2), None, 5), (None, Some(2), 4)]
    {
        ...
        for turn in 1..=blocking_turn {
            ...
            let expected = if turn == blocking_turn {
                antex_state::ThreadGoalStatus::Blocked
            } else {
                antex_state::ThreadGoalStatus::Active
            };
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(antex_state::ThreadGoalStatus::Active, goal.status);
...
async fn installed_tools(
    runtime: Arc<antex_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: Arc<antex_extension_api::ExtensionRegistry<()>>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish_with_outcome(
        ...
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = antex_extension_api::ToolName::plain(tool_name);
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall<'static> {
    ToolCall {
        ...
        tool_name: antex_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        antex_turn_metadata: None,
        truncation_policy: TruncationPolicy::Bytes(1024),
        source: ToolCallSource::Direct,
        conversation_history: antex_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<antex_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    antex_state::StateRuntime::init(
        antex_state::SqliteConfig::new_for_testing(tempdir.keep().as_path().abs()),
...
async fn seed_thread_metadata(
    runtime: &antex_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = antex_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .sqlite()
            .home()
...
fn protocol_status(status: antex_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        antex_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        antex_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        antex_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        antex_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        antex_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        antex_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
