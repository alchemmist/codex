...
use antex_analytics::ThreadHintStatus;
use antex_analytics::ThreadHintStatusEvent;
...
impl HistoryNotesExtension {
    fn update_config(&self, thread_store: &ExtensionData, config: &Config) {
        if config
            ...
            && self.auth_manager.current_auth_uses_antex_backend()
        {
...
/// Installs the standalone history and notes tools backed by the Antex backend.
