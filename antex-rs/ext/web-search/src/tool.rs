...
use antex_api::SearchClient;
use antex_api::SearchCommands;
use antex_api::SearchQuery;
use antex_api::SearchRequest;
use antex_api::SearchSettings;
use antex_core::X_ANTEX_TURN_METADATA_HEADER;
use antex_core::web_search_action_detail;
use antex_extension_api::ExtensionTurnItem;
use antex_extension_api::FunctionCallError;
use antex_extension_api::ResponsesApiTool;
...
use antex_extension_api::parse_tool_input_schema_without_compaction;
use antex_extension_items::ExtensionItem;
use antex_extension_items::web_search::WebSearchAction;
...
impl<'call> ToolExecutor<ToolCall<'call>> for WebSearchTool {
    ...
    fn handle<'a>(&'a self, call: ToolCall<'call>) -> antex_extension_api::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
...
impl WebSearchTool {
    async fn handle_call(
        ...
        let extra_headers = search_request_headers(
            ...
            call.antex_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = antex_otel::global()
...
fn search_request_headers(originator: Option<&str>, turn_metadata: Option<&str>) -> HeaderMap {
    ...
    {
        headers.insert(X_ANTEX_TURN_METADATA_HEADER, header_value);
    }
