...
use antex_features::GuardianV2ConfigToml;
use antex_features::GuardianV2TranscriptConfigToml;
...
impl GuardianV2Config {
    pub(crate) fn resolve(config: &Config) -> Result<Self, String> {
        ...
        resolved.force_synchronous_review = config
            ...
            .requirements()
            .approvals_reviewer
            .can_set(&antex_protocol::config_types::ApprovalsReviewer::User)
            .is_err();
        resolved.scoring_disabled = !config.features.enabled(antex_features::Feature::GuardianV2);
        Ok(resolved)
    }

    pub(super) fn policy_for_model(
        &self,
        model: Option<&antex_protocol::openai_models::ModelInfo>,
    ) -> GuardianPolicy {
