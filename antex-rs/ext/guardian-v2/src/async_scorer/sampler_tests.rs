...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn classifier_uses_free_endpoint_only_with_antex_backend_auth() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn preconnected_sampler_reuses_authenticated_websocket_for_classifications() -> Result<()> {
    ...
    .await?;
    let manager = AuthManager::from_auth_for_testing(AntexAuth::from_api_key("test-api-key"));
    manager
...
async fn guardian_ticket_survives_classifier_transport_retry() -> Result<()> {
    ...
    for free_guardian in [false, true] {
        ...
        request.guardian_ticket =
            antex_protocol::guardian_ticket::GuardianTicket::from_server(&raw_ticket);
