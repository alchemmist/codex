...
use antex_extension_api::ToolName;
...
use antex_login::ExternalAuth;
use antex_login::ExternalAuthFuture;
use antex_login::ExternalAuthRefreshContext;
...
use antex_protocol::openai_models::GuardianScope;
use antex_protocol::openai_models::GuardianV2ModelConfig;
use antex_protocol::openai_models::GuardianV2TranscriptModelConfig;
...
use antex_protocol::security_risk::SecurityRiskScore;
use anyhow::Result;
...
use antex_features::GuardianV2ReviewScopeConfigToml;
...
impl ExternalAuth for RefreshableAuth {
    fn resolve(&self) -> ExternalAuthFuture<'_, AntexAuth> {
        Box::pin(async { Ok(AntexAuth::from_api_key(*self.0.lock().expect("auth"))) })
    }

    fn refresh(&self, _: ExternalAuthRefreshContext) -> ExternalAuthFuture<'_, AntexAuth> {
...
async fn installed_extension_warms_connections_without_blocking_thread_start() -> Result<()> {
    ...
    let thread_store = test.antex.thread_extension_data();
...
async fn installed_extension_reconnects_after_auth_refresh() -> Result<()> {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(AntexAuth::from_api_key("original"));
    auth_manager
...
fn user_instruction(text: &str) -> ResponseItem {
    ResponseItem::Message {
        ...
        internal_chat_message_metadata_passthrough: Some(InternalChatMessageMetadataPassthrough {
            content_item_kinds: Some(vec![antex_protocol::models::ContentItemKind(
                "user.text".to_owned(),
            )]),
            ..Default::default()
        }),
...
async fn sandboxed_shell_classification_respects_review_scope() -> Result<()> {
    ...
    let thread_store = fixture.test.antex.thread_extension_data();
...
async fn computer_use_only_scores_cannot_approve_other_actions() -> Result<()> {
    ...
    let authorization =
        super::super::authorization::ScoreAuthorization::current(&fixture.test.antex).await;
    *progress.authorization.lock().unwrap() = Some(authorization);
    ...
    let mut changed_model = fixture
        ...
        .await;
    changed_model.guardian = Some(antex_protocol::openai_models::GuardianModelPolicy {
        computer_use: Some(antex_protocol::openai_models::GuardianReviewMode::Adaptive),
        ..Default::default()
    });
...
) -> Result<(serde_json::Value, TestAntex, ExtensionRegistry<Config>)> {
...
async fn sample_configured_conversation_history_with_source(
    ...
    let conversation_history = test.antex.conversation_history_snapshot().await;
...
async fn disabled_code_mode_invalidates_cached_scores() -> Result<()> {
    ...
    let mut model = thread_store
        ...
        .as_ref()
        .clone();
    model.guardian = Some(antex_protocol::openai_models::GuardianModelPolicy {
        computer_use: Some(antex_protocol::openai_models::GuardianReviewMode::Adaptive),
        ..Default::default()
    });
    thread_store.insert(model);
...
impl GuardianFailureFixture {
    ...
    async fn score_tool(&self, tool_name: ToolName) {
        let thread_store = self.test.antex.thread_extension_data();
        thread_store.insert(SecurityRiskScore {
            scores: BTreeMap::from([("action_risk".to_owned(), 0.25)]),
            call_id: None,
            action: None,
...
async fn contributor_fails_closed_when_luna_classification_fails() -> Result<()> {
    ...
    fixture.registry.thread_lifecycle_contributors()[0]
        .on_thread_start(ThreadStartInput {
            ...
            extension_metrics: None,
            session_store: &fixture.session_store,
            thread_store: fixture.test.antex.thread_extension_data(),
        })
        .await;
    fixture
        .test
...
async fn contributor_skips_required_models_in_standard_scope() -> Result<()> {
    ...
    let thread_server = responses::start_mock_server().await;
    let initial = test_antex().build_with_auto_env(&thread_server).await?;
    std::fs::write(
    ...
    let authorization = super::ScoreAuthorization::current(&test.antex).await;
    let progress = thread_store
        .get::<GuardianV2ScoreProgress>()
        .expect("Guardian v2 should track score progress per thread");
    *progress.authorization.lock().unwrap() = Some(authorization);
...
async fn assert_compaction_approval_policy(thread_context_enabled: bool) -> Result<()> {
    ...
    let authorization = fixture.test.antex.guardian_authorization_version().await;
    fixture
        .test
        .antex
    ...
    thread_store.insert(model);
    assert_eq!(
        fixture.test.antex.guardian_authorization_version().await,
        authorization
    );
    let score_authorization = super::ScoreAuthorization::current(&fixture.test.antex).await;
    *thread_store
        .get::<GuardianV2ScoreProgress>()
        .expect("score progress")
        .authorization
