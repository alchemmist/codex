...
use antex_extension_api::McpToolInfo;
...
async fn trusts_only_tools_configured_in_antex_home() -> Result<()> {
...
#[cfg(unix)]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn rejects_paths_that_escape_antex_home_through_symlinks() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    std::fs::create_dir_all(plugin_root.join(".antex-plugin"))?;
    std::fs::write(
        plugin_root.join(".antex-plugin").join("plugin.json"),
        r#"{"name":"trusted"}"#,
    )?;
