...
use antex_config::McpServerTransportConfig;
...
const ANTEX_CONNECTORS_TOKEN_ENV_VAR: &str = "CODEX_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `antex_core::config::Config`.
///
...
pub struct McpConfig {
    ...
    /// Antex home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `antex-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn host_owned_codex_apps_enabled(config: &McpConfig, auth: Option<&AntexAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(AntexAuth::uses_antex_backend)
}
...
fn codex_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(ANTEX_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(ANTEX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(ANTEX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}
