...
use antex_async_utils::CancelErr;
...
use antex_protocol::protocol::McpStartupUpdateEvent;
use antex_rmcp_client::ExecutorStdioServerLauncher;
use antex_rmcp_client::LocalStdioServerLauncher;
use antex_rmcp_client::McpOAuthRefreshMode;
use antex_rmcp_client::McpProtocolMode;
use antex_rmcp_client::RmcpClient;
use antex_rmcp_client::StdioServerLauncher;
use antex_rmcp_client::StreamableHttpBearerToken;
use antex_rmcp_client::StreamableHttpRedirectMode;
use antex_rmcp_client::ToolWithConnectorId;
use antex_rmcp_client::is_authentication_required_error;
use anyhow::Result;
use anyhow::anyhow;
...
/// MCP server capability indicating that Antex should include [`SandboxState`]
...
fn record_protocol_discovery_metrics(
    ...
    let Some(metrics) = antex_otel::global() else {
        return;
    };
...
pub(crate) async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(antex_utils_path_uri::LegacyAppPathString::into_string);
                ...
                && let Some(env_var) = bearer_token_env_var.as_ref()
            {
                (
                    Arc::new(ExecutorEnvironmentHttpClient {
                        ...
                        http_client,
                    }) as Arc<dyn antex_exec_server::HttpClient>,
