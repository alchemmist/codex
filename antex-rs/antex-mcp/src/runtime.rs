...
use antex_rmcp_client::with_http_headers_helper;
...
impl McpRuntime {
    ...
    pub fn current_auth_matches(&self, auth: Option<&AntexAuth>) -> bool {
...
pub struct SandboxState {
    ...
    #[serde(rename = "codexLinuxSandboxExe")]
...
pub fn apply_http_headers_helper(
    ...
    config: &antex_config::McpServerConfig,
    local_process_cwd: PathBuf,
    ...
    let antex_config::McpServerTransportConfig::StreamableHttp {
        url,
    ...
    pub(crate) fn resolve_server_environment(
        ...
        if config.is_local_environment() {
            return match config.transport {
                antex_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                antex_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }
