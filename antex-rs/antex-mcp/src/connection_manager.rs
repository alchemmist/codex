//! Aggregates MCP server connections for Antex.
//!
...
use antex_protocol::protocol::McpStartupCompleteEvent;
use antex_protocol::protocol::McpStartupFailure;
...
use antex_rmcp_client::determine_streamable_http_auth_status_from_credentials;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
use anyhow::bail;
...
const WORKFLOW_FORBID_QUALITY_GRAPH_IGNORE_ENV: &str = "ANTEX_WORKFLOW_FORBID_QUALITY_GRAPH_IGNORE";
...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let static_chatgpt_auth_provider = auth
            .filter(|auth| auth.uses_antex_backend())
            .map(antex_model_provider::auth_provider_from_auth);
        let codex_apps_auth_provider = auth_manager.as_ref().and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_antex_backend()).map(|auth| {
                antex_model_provider::auth_provider_from_auth_manager(
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            // not also attach the ambient AntexAuth provider.
            ...
            let expected_protocol_mode = match &configured_config.transport {
                ...
                McpServerTransportConfig::Stdio { env, .. } => match env
                    .as_ref()
                    .and_then(|variables| variables.get("ANTEX_MCP_PROTOCOL_VERSION"))
                {
