...
use antex_config::AppToolApproval;
use antex_config::Constrained;
use antex_config::McpServerAuth;
use antex_config::McpServerConfig;
use antex_config::McpServerEnvVar;
use antex_config::McpServerToolConfig;
use antex_config::types::AuthKeyringBackendKind;
use antex_config::types::OAuthCredentialsStoreMode;
use antex_connectors::ConnectorRuntimeContext;
use antex_connectors::ConnectorRuntimeContextKey;
use antex_connectors::ConnectorRuntimeFetchSource;
use antex_connectors::ConnectorRuntimeManager;
use antex_exec_server::ExecServerError;
use antex_exec_server::HttpClient;
use antex_exec_server::HttpRequestParams;
use antex_exec_server::HttpRequestResponse;
use antex_exec_server::HttpResponseBodyStream;
use antex_exec_server_test_support::environment_manager_without_environments;
use antex_login::AntexAuth;
use antex_login::AuthHeaders;
use antex_login::AuthManager;
use antex_protocol::ToolName;
...
use antex_rmcp_client::InProcessTransportFactory;
use antex_rmcp_client::McpAuthState;
use antex_rmcp_client::McpLoginRequirement;
use antex_rmcp_client::McpOAuthRefreshMode;
use antex_rmcp_client::RmcpClient;
...
async fn connection_statuses_observe_clients_without_starting_them() {
    use antex_protocol::mcp::McpServerConnectionStatus as Status;
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        antex_rmcp_client::Elicitation::Mcp(ElicitRequestParams::FormElicitationParams {
            meta: None,
...
async fn assert_elicitation_declined_with_reviewer_calls(
    ...
    let response = tokio::select! {
        ...
        response = sender(
            NumberOrString::Number(1),
            antex_rmcp_client::Elicitation::Mcp(elicitation),
...
async fn assert_requested_user_input_is_declined(
    ...
    let response = tokio::select! {
        ...
        response = sender(
            NumberOrString::Number(1),
            antex_rmcp_client::Elicitation::Mcp(
...
#[tokio::test]
async fn disabled_permissions_surface_requested_user_input_with_non_antex_approval_metadata() {
...
async fn assert_disabled_permissions_surface_requested_user_input(
    ...
    let antex_protocol::mcp::RequestId::String(request_id) = request.id else {
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let (
        antex_protocol::mcp::RequestId::String(request_a_id),
        antex_protocol::mcp::RequestId::String(request_b_id),
...
async fn cancelled_elicitation_is_removed_without_affecting_other_pending_requests() {
    ...
    let (
        antex_protocol::mcp::RequestId::String(cancelled_id),
        antex_protocol::mcp::RequestId::String(pending_id),
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<antex_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
fn hosted_actor_credentials_are_only_available_to_host_owned_mcp_servers() {
    let bootstrap_auth = AntexAuth::create_dummy_chatgpt_auth_for_testing();
    let mut actor_headers = Default::default();
    antex_model_provider::auth_provider_from_auth(&bootstrap_auth)
    ...
    let hosted_auth = AntexAuth::Headers(AuthHeaders::new(actor_headers));
    let provider = antex_model_provider::auth_provider_from_auth(&hosted_auth);
...
async fn executor_owned_chatgpt_mcp_accepts_only_safe_explicit_authorization() -> anyhow::Result<()>
{
    ...
    let cases = [
        ...
        (
            ...
            None,
            Some(("aUtHoRiZaTiOn", "ANTEX_TEST_HOSTED_SECRET")),
            false,
        ),
        (
            "environment bearer",
            Some(("Authorization", "Bearer executor-secret")),
            Some("ANTEX_TEST_HOSTED_SECRET"),
            None,
            false,
        ),
        (
    ...
    for (case, static_header, bearer_env_var, env_header, allows_executor_auth) in cases {
        ...
        assert!(
            chatgpt_auth_provider_for_server(
                remote_server,
                Some(antex_model_provider::auth_provider_from_auth(&hosted_auth)),
            )
            .is_none(),
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = ANTEX_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `antex mcp login {server_name}`."
    );
...
fn mcp_init_error_display_identifies_oauth_reauthentication() {
    ...
    for (config, recovery_hint) in [
        (None, "Run `antex mcp login example`."),
        (
            Some(&executor_config),
...
fn reusable_server_config(url: &str) -> McpServerConfig {
    McpServerConfig {
        ...
        transport: McpServerTransportConfig::StreamableHttp {
            url: url.to_string(),
            bearer_token_env_var: Some("ANTEX_MCP_REUSE_TEST_TOKEN".to_string()),
            http_headers: None,
            env_http_headers: None,
            http_headers_helper: None,
        },
...
fn connection_identity_uses_effective_authorization_headers() {
    let runtime_context = reusable_server_runtime_context();
    let missing_env_var = format!("ANTEX_TEST_UNSET_MCP_AUTHORIZATION_{}", std::process::id());
...
async fn reconciliation_replaces_connection_when_protocol_mode_changes() {
    ...
    mcp_config.protocol_mode = antex_rmcp_client::McpProtocolMode::V20260728;
...
async fn connection_identity_distinguishes_accounts_with_the_same_token() -> anyhow::Result<()> {
    ...
    let previous_auth = AntexAuth::from_external_chatgpt_tokens(
        access_token,
        "account-a",
        /*chatgpt_plan_type*/ None,
    )?;
    let changed_auth = AntexAuth::from_external_chatgpt_tokens(
        access_token,
        "account-b",
        /*chatgpt_plan_type*/ None,
    )?;
    let connection_identity = |auth: &AntexAuth| {
        let provider = antex_model_provider::auth_provider_from_auth(auth);
        McpServerConnectionIdentity::new(
            "docs",
            &server,
            /*host_plugin_root*/ None,
...
async fn connection_identity_distinguishes_agent_account_runtime_and_task() -> anyhow::Result<()> {
    ...
    let config = reusable_server_config("http://127.0.0.1:1");
    let server = EffectiveMcpServer::configured(config);
    let record = antex_login::auth::AgentIdentityAuthRecord {
    ...
    let auth_route_config = antex_login::test_support::transport_default_auth_route_config();
    let previous_auth = AntexAuth::AgentIdentity(
        antex_login::auth::AgentIdentityAuth::from_record(
            record.clone(),
    ...
    for changed_record in [
        antex_login::auth::AgentIdentityAuthRecord {
        ...
        let changed_auth = AntexAuth::AgentIdentity(
            antex_login::auth::AgentIdentityAuth::from_record(
                changed_record,
                "https://auth.openai.com/api/accounts",
                &auth_route_config,
