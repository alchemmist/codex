...
impl McpEventStreamOpener {
    ...
    fn matches_auth(&self, current: Option<&AntexAuth>) -> bool {
        match (self.connection.auth.as_ref(), current) {
            (Some(AntexAuth::AgentIdentity(expected)), Some(AntexAuth::AgentIdentity(current))) => {
                expected.record() == current.record()
            }
            (Some(AntexAuth::AgentIdentity(_)), _) | (_, Some(AntexAuth::AgentIdentity(_))) => {
                false
            }
