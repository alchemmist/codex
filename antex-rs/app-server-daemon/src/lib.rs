...
use antex_app_server_protocol::RemoteControlConnectionStatus;
use antex_app_server_protocol::RemoteControlPairingStartResponse;
use antex_app_server_transport::app_server_control_socket_path;
...
use managed_install::managed_antex_bin;
#[cfg(any(unix, windows))]
use managed_install::managed_antex_version;
use serde::Serialize;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    #[serde(rename = "managedCodexPath")]
    pub managed_antex_path: PathBuf,
    #[serde(rename = "managedCodexVersion")]
    pub managed_antex_version: Option<String>,
...
pub async fn start_with_antex_bin(antex_bin: &Path) -> Result<LifecycleOutput> {
    ensure_supported_platform()?;
    let mut daemon = Daemon::from_environment()?;
    daemon.managed_antex_bin = antex_bin.to_path_buf();
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "antex app-server daemon lifecycle is only supported on Unix and Windows platforms"
    ))
}
...
impl Daemon {
    fn from_environment() -> Result<Self> {
        ...
        Ok(Self {
            ...
            managed_antex_bin: managed_antex_bin(codex_home.as_path()),
        })
    }
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by antex app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_antex_version(managed_antex_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    #[cfg(windows)]
                    backend::windows::ensure_detached_launch(managed_antex_bin)?;
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_antex_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_antex_bin)?;
        }
        #[cfg(windows)]
        if should_reexec_updater(updater_refresh_mode, outcome) {
            backend::pid_update_loop_backend(
                self.backend_paths_with_bin(&settings, managed_antex_bin),
            )
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_antex_version = self
            .managed_antex_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_antex_version}",
            self.managed_antex_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let info = self.wait_until_ready().await?;
        let managed_antex_version = self.managed_antex_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled: true,
            remote_control_enabled: settings.remote_control_enabled,
            managed_antex_path: self.managed_antex_bin.clone(),
            managed_antex_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_antex_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_antex_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_antex_bin(&self) -> Result<()> {
        if self.managed_antex_bin.is_file() {
            #[cfg(windows)]
            backend::windows::ensure_detached_launch(&self.managed_antex_bin)?;
            return Ok(());
        }

        let managed_antex_path = self.managed_antex_bin.display();
        let install_command = if cfg!(windows) {
            "irm https://raw.githubusercontent.com/alchemmist/antex/main/scripts/install/install.ps1 | iex"
        } else {
            "curl -fsSL https://raw.githubusercontent.com/alchemmist/antex/main/scripts/install/install.sh | sh"
        };
        Err(anyhow!(
            "managed standalone Codex install not found at {managed_antex_path}\n\n\
    ...
    #[cfg(any(unix, windows))]
    async fn managed_antex_version_best_effort(&self) -> Option<String> {
        managed_antex_version(&self.managed_antex_bin).await.ok()
    }
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_antex_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            antex_bin: managed_antex_bin.to_path_buf(),
    ...
    async fn open_operation_lock_file(&self) -> Result<tokio::fs::File> {
        if let Some(parent) = self.operation_lock_file.parent() {
            ...
            antex_uds::prepare_private_socket_directory(parent)
                .await
                .with_context(|| {
                    format!(
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_antex_path: "codex".into(),
            managed_antex_version: Some("1.2.3".to_string()),
    ...
    async fn stop_creates_missing_home_parent() {
        ...
        let daemon = Daemon {
            ...
            managed_antex_bin: state.join("missing-codex"),
        };
        assert_eq!(
            daemon
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_antex_bin: temp_dir.path().join("missing-codex"),
        };
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_antex_bin.display(),
                stderr_log.display()
            )
        );
    }
