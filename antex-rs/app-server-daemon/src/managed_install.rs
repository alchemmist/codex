...
pub(crate) fn managed_antex_bin(codex_home: &Path) -> PathBuf {
    let current = codex_home
        .join("packages")
        .join("standalone")
        .join("current");
    let packaged = current.join("bin").join(managed_antex_file_name());
    let legacy = current.join(managed_antex_file_name());
...
pub(crate) async fn resolved_managed_antex_bin(antex_bin: &Path) -> Result<PathBuf> {
    fs::canonicalize(antex_bin).await.with_context(|| {
        format!(
...
pub(crate) async fn managed_antex_version(antex_bin: &Path) -> Result<String> {
    let mut command = Command::new(antex_bin);
    #[cfg(windows)]
...
fn managed_antex_file_name() -> &'static str {
    if cfg!(windows) { "antex.exe" } else { "antex" }
}

fn parse_antex_version(output: &str) -> Result<String> {
    let version = output
        .split_whitespace()
        .nth(1)
