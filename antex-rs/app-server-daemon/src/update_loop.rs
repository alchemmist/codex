...
use crate::managed_install::resolved_managed_antex_bin;
...
async fn update_once(
    ...
    let daemon = Daemon::from_environment()?;
    let managed_antex_bin = resolved_managed_antex_bin(&daemon.managed_antex_bin).await?;
    let managed_identity = executable_identity(&managed_antex_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_antex_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_antex_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_antex_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Codex binary {}",
            managed_antex_bin.display()
        )
    })
}
...
impl Signal {
    async fn recv(&mut self) -> Option<()> {
        ...
        let _ = antex_app_server_transport::daemon_shutdown_signal().await;
        Some(())
    }
}
