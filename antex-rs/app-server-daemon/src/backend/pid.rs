...
use antex_app_server_transport::REMOTE_CONTROL_DISABLED_ENV_VAR;
use anyhow::Context;
use anyhow::Result;
use anyhow::bail;
...
impl PidBackend {
    pub(crate) fn new(antex_bin: PathBuf, pid_file: PathBuf, remote_control_enabled: bool) -> Self {
    ...
    pub(crate) fn new_update_loop(antex_bin: PathBuf, pid_file: PathBuf) -> Self {
        let lock_file = pid_file.with_extension("pid.lock");
        Self {
            antex_bin,
            pid_file,
