...
impl PidBackend {
    pub(super) async fn start_inner(&self, replacement: Option<PidRecord>) -> Result<Option<u32>> {
        ...
        #[cfg(windows)]
        let antex_bin = fs::canonicalize(&self.antex_bin)
            .await
            .unwrap_or_else(|_| self.antex_bin.clone());
        #[cfg(not(windows))]
        let antex_bin = &self.antex_bin;
        ...
        #[cfg(windows)]
        {
            ...
            for name in [
                ...
                if matches!(name, "ANTEX_SQLITE_HOME" | "npm_config_cafile") {
                ...
                let expands_home = if name == "npm_config_cafile" {
                    ...
                    matches!(
                        name,
                        "ANTEX_SQLITE_HOME" | "AWS_CONFIG_FILE" | "AWS_SHARED_CREDENTIALS_FILE"
