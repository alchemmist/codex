# antex-app-server-daemon

> `antex-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`antex-app-server-daemon` backs the machine-readable `codex app-server`
...
shorter `ANTEX_HOME` to share the daemon; discovery does not trust a mutable alias.
...
An invocation that sets `ANTEX_EXEC_SERVER_URL` skips implicit daemon attachment
...
$HOME/.antex/packages/standalone/current/codex app-server daemon bootstrap --remote-control
```
...
$codexHome = if ($env:ANTEX_HOME) { $env:ANTEX_HOME } else { Join-Path $HOME '.antex' }
...
settings under `ANTEX_HOME/app-server-daemon/`, starts app-server as a
...
on Windows) and its managed binary under `ANTEX_HOME/packages/standalone/current`:
...
All mutating lifecycle commands are serialized per `ANTEX_HOME`, so a concurrent
...
The daemon stores its local state under `ANTEX_HOME/app-server-daemon/`:
