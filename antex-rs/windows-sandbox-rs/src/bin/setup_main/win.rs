...
use antex_otel::StatsigMetricsSettings;
use antex_windows_sandbox::DirectoryOpenDisposition;
use antex_windows_sandbox::SETUP_VERSION;
use antex_windows_sandbox::SetupErrorCode;
use antex_windows_sandbox::SetupErrorReport;
use antex_windows_sandbox::SetupFailure;
use antex_windows_sandbox::acquire_sandbox_setup_lock;
use antex_windows_sandbox::add_deny_write_ace;
use antex_windows_sandbox::convert_string_sid_to_sid;
use antex_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use antex_windows_sandbox::ensure_allow_write_aces;
use antex_windows_sandbox::extract_setup_failure;
use antex_windows_sandbox::hide_newly_created_users;
use antex_windows_sandbox::install_wfp_filters;
use antex_windows_sandbox::local_user_flags;
use antex_windows_sandbox::log_note;
use antex_windows_sandbox::log_writer;
use antex_windows_sandbox::open_directory_no_reparse;
use antex_windows_sandbox::path_mask_allows;
use antex_windows_sandbox::path_write_aces_need_refresh;
use antex_windows_sandbox::resolve_sid;
use antex_windows_sandbox::sandbox_bin_dir;
use antex_windows_sandbox::sandbox_dir;
use antex_windows_sandbox::sandbox_secrets_dir;
use antex_windows_sandbox::set_local_user_flags;
use antex_windows_sandbox::setup_error_path;
use antex_windows_sandbox::setup_log_writer;
use antex_windows_sandbox::string_from_sid_bytes;
use antex_windows_sandbox::sync_persistent_deny_read_acls;
use antex_windows_sandbox::to_wide;
use antex_windows_sandbox::workspace_write_cap_sid_for_root;
use antex_windows_sandbox::workspace_write_root_overlaps_path;
use antex_windows_sandbox::write_file_atomically;
use antex_windows_sandbox::write_setup_error_report;
use anyhow::Context;
use anyhow::Result;
use base64::Engine;
...
fn lock_sandbox_dir(
    ...
    // ProvisionOnly accepts another user's ANTEX_HOME; keep its ACL mutation
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(codex_home) = std::env::var("ANTEX_HOME") {
...
fn run_setup_full(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if refresh_only {
        setup_runtime_bin::ensure_antex_app_runtime_paths_readable(
            sandbox_group_psid,
            &mut refresh_errors,
            log,
        )?;
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.antex`, and `.agents`.
        //
...
mod tests {
    ...
    use antex_otel::StatsigMetricsSettings;
    use antex_windows_sandbox::ensure_allow_mask_aces;
    use antex_windows_sandbox::ensure_allow_write_aces;
    use antex_windows_sandbox::load_or_create_cap_sids;
    use antex_windows_sandbox::path_mask_allows;
    use antex_windows_sandbox::path_write_aces_need_refresh;
    use antex_windows_sandbox::workspace_write_cap_sid_for_root;
    ...
    fn write_root_refresh_replaces_stale_delete_child_grant() {
        ...
        let codex_home = temp.path().join("antex-home");
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let codex_home = temp.path().join("antex-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".antex");
