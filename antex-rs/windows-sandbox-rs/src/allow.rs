...
mod tests {
    ...
    #[test]
    fn denies_antex_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let antex_dir = command_cwd.join(".antex");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&antex_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&antex_dir).unwrap(),
