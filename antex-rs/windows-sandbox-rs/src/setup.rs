...
const SETUP_EXE_FILENAME: &str = "antex-windows-sandbox-setup.exe";
...
struct ElevationPayload {
    ...
    otel: Option<antex_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "ANTEX_NETWORK_ALLOW_LOCAL_BINDING";
...
fn elevated_provisioning_payload(
    ...
    ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: antex_otel::global_statsig_metrics_settings(),
...
pub fn run_elevated_provisioning_setup_with_retained_handles(
        ...
        )
    {
        return Err(failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!(
                "sandbox provisioning ANTEX_HOME must be an absolute local disk path: {}",
                codex_home.display()
            ),
        ));
    }
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, codex_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to ANTEX_HOME or anything under ANTEX_HOME/.sandbox,
    // ANTEX_HOME/.sandbox-bin, or ANTEX_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let antex_home_key = canonical_path_key(codex_home);
...
mod tests {
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("antex.exe");
        let setup_exe = resources_dir.join("antex-windows-sandbox-setup.exe");
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "ANTEX_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_antex_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let forbidden_antex_home = dunce::canonicalize(&codex_home).expect("canonical codex home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_antex_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_antex).expect("canonical extra .codex"),
                explicit_deny,
            ]
            .into_iter()
