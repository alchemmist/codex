...
use super::ANTEX_HOME_FLAG;
use super::ANTEX_WINDOWS_SANDBOX_ARG1;
...
fn windows_wrapper_args_round_trip() {
    ...
    let args = create_windows_sandbox_command_args_for_permission_profile(
        vec![
            "antex.exe".to_string(),
            "--antex-run-as-fs-helper".to_string(),
        ],
        &command_cwd,
        ...
        Path::new(r"C:\Users\me\.antex"),
    );

    assert_eq!(args[0], ANTEX_WINDOWS_SANDBOX_ARG1);
    assert!(args.contains(&ANTEX_HOME_FLAG.to_string()));
    ...
    assert_eq!(
        parsed.command,
        vec!["antex.exe", "--antex-run-as-fs-helper"]
    );
