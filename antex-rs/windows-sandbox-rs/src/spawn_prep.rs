...
use crate::sandbox_utils::ensure_antex_home_exists;
...
use crate::workspace_acl::protect_workspace_antex_dir;
...
fn prepare_spawn_context_common(
    ...
    ensure_antex_home_exists(codex_home)?;
    ...
    unsafe {
                ...
                matching_root_capability(current_dir, acl_sids.write_root_sids)
        {
            ...
            if is_command_cwd_root(&workspace_sid.root, &canonical_cwd) {
                let _ = protect_workspace_antex_dir(current_dir, workspace_sid.sid.as_ptr());
...
pub(crate) fn prepare_elevated_spawn_context_for_permissions(
    ...
    let sandbox_base = codex_home.join(".sandbox");
    ensure_antex_home_exists(&sandbox_base)?;
