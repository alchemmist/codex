...
def _resolve_antex_cmd() -> List[str]:
    ...
    raise FileNotFoundError(
        ...
        "  cargo build -p antex-cli --release\n"
        "or for debug:\n"
        "  cargo build -p antex-cli\n"
    )


ANTEX_CMD = _resolve_antex_cmd()
print(ANTEX_CMD)
TIMEOUT_SEC = 20
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"ANTEX_HOME": str(proxy_home)},
            )
            add(
    ...
    codex_home = Path(os.environ["USERPROFILE"]) / ".antex"
