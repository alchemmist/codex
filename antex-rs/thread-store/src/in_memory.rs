...
use antex_rollout::persisted_rollout_items;
use chrono::DateTime;
use chrono::Utc;
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        section: state
            ...
            .get(&thread_id)
            .cloned()
            .map(|id| antex_state::ThreadSection {
                name: if id == antex_state::PINNED_THREAD_SECTION_ID {
                    antex_state::PINNED_THREAD_SECTION_NAME.to_string()
                } else {
                    id.clone()
                },
                id,
...
fn git_info_from_patch(patch: &ThreadMetadataPatch) -> Option<antex_protocol::protocol::GitInfo> {
    ...
    Some(antex_protocol::protocol::GitInfo {
