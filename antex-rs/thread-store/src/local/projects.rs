...
fn state(store: &LocalThreadStore) -> ThreadStoreResult<&antex_rollout::StateDbHandle> {
    store
        .state_db
        .as_ref()
...
fn state_root(root: StoredProjectRoot) -> antex_state::ProjectRoot {
    antex_state::ProjectRoot { path: root.path }
}

fn stored_project(project: antex_state::Project) -> StoredProject {
    StoredProject {
        id: project.id,
        name: project.name,
        roots: project
