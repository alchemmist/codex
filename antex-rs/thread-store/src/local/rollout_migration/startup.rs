...
use antex_state::RolloutMigrationCursor;
use antex_state::RolloutMigrationSkippedRollout;
...
async fn run_startup_migration(
    ...
    loop {
        let Some(maintenance_guard) =
            antex_rollout::try_acquire_rollout_maintenance_lock(&store.config.codex_home)
                .map_err(migration_error)?
        else {
...
fn plain_rollout_file_name(path: &Path) -> Option<OsString> {
    antex_rollout::plain_rollout_path(path)
        .file_name()
