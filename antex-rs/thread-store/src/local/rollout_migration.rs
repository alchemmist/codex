...
use antex_app_server_protocol::project_rollout_line;
...
impl LocalThreadStore {
    ...
    async fn migrate_rollouts_with_progress_inner(
        ...
        let _maintenance_guard = match options.mode {
            ...
            RolloutMigrationMode::Apply => Some(
                antex_rollout::try_acquire_rollout_maintenance_lock(&self.config.codex_home)
        ...
        let legacy_names = if options.mode == RolloutMigrationMode::Apply {
            let thread_ids = paths
                ...
                .collect();
            antex_rollout::find_thread_names_by_ids(&self.config.codex_home, &thread_ids)
                .await
                .unwrap_or_default()
        } else {
            HashMap::new()
    ...
    async fn migrate_rollout_path(
        ...
        let metadata = loop {
            let error = match antex_rollout::read_session_meta_line(&path).await {
                Ok(metadata) => break metadata,
                Err(error) if !retried_moved_path && error.kind() == io::ErrorKind::NotFound => {
                    // A different Antex process can archive or compress this rollout after path
                ...
                && let Some(thread_id) = thread_id
            {
                ...
                match antex_rollout::read_session_meta_line(&path).await {
                    Ok(metadata) => break metadata,
                    Err(error) => error,
                }
            } else {
    ...
    async fn write_canonical_rollout(
        ...
        while let Some(record) = read_rollout_record(&mut source, &mut bytes).await? {
            ...
            if plan.is_none()
                && matches!(
                    &line.item,
                    RolloutItem::EventMsg(antex_protocol::protocol::EventMsg::ThreadRolledBack(_))
                )
            {
    ...
    async fn cleanup_failed_unpublished_migration(
        ...
        let Ok(metadata) = antex_rollout::read_session_meta_line(rollout_path).await else {
            return Ok(());
        };
        if metadata.meta.history_mode == ThreadHistoryMode::Paginated {
            return Ok(());
    ...
    async fn project_rollout_in_batches(
        ...
        let subagent_history_start_ordinal = antex_rollout::read_session_meta_line(rollout_path)
            .await
            .map_err(migration_error)?
            .meta
        ...
        while let Some(entry) = entries.next_entry().await.map_err(migration_error)? {
                ...
                && (name.ends_with(".jsonl") || name.ends_with(".jsonl.zst"))
            {
                ...
                if name.ends_with(".jsonl.zst")
                    && tokio::fs::try_exists(antex_rollout::plain_rollout_path(&path))
                        .await
                        .map_err(migration_error)?
                {
                    continue;
...
async fn find_all_rollout_paths(codex_home: &Path) -> ThreadStoreResult<Vec<PathBuf>> {
    let mut paths = find_rollout_paths(&codex_home.join(antex_rollout::SESSIONS_SUBDIR)).await?;
    paths.extend(
        find_rollout_paths(&codex_home.join(antex_rollout::ARCHIVED_SESSIONS_SUBDIR)).await?,
    );
    Ok(paths)
}
...
async fn find_current_rollout_path(
    ...
    let plain_path = antex_rollout::plain_rollout_path(stale_path);
    ...
    Ok(find_all_rollout_paths(codex_home)
        ...
        .into_iter()
        .find(|candidate| {
            antex_rollout::plain_rollout_path(candidate).file_name() == Some(file_name)
        }))
}
