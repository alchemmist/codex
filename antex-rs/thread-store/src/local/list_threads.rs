...
use antex_rollout::parse_cursor;
use antex_state::ThreadFilterOptions;
use chrono::DateTime;
use chrono::Utc;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => antex_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => antex_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => antex_rollout::ThreadSortKey::RecencyAt,
    ...
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => antex_rollout::SortDirection::Asc,
        SortDirection::Desc => antex_rollout::SortDirection::Desc,
    };
...
async fn list_section_threads(
    ...
    let anchor = params
        .cursor
        .as_deref()
        .map(|cursor| -> ThreadStoreResult<antex_state::Anchor> {
            ...
            let thread_id = antex_protocol::ThreadId::from_string(thread_id).map_err(|_| {
                ThreadStoreError::InvalidRequest {
                    message: format!("invalid cursor: {cursor}"),
                }
            })?;
            Ok(antex_state::Anchor {
                ts: timestamp,
                id: Some(thread_id),
            })
        })
    ...
    let normalized_cwd_filters = params.cwd_filters.as_ref().map(|filters| {
        filters
            .iter()
            .map(|cwd| antex_rollout::state_db::normalize_cwd_for_state_db(cwd))
            .collect::<Vec<_>>()
    });
    let filters = ThreadFilterOptions {
        ...
        sort_key: antex_state::SortKey::SectionPosition,
        sort_direction: match params.sort_direction {
            SortDirection::Asc => antex_state::SortDirection::Asc,
            SortDirection::Desc => antex_state::SortDirection::Desc,
        },
    ...
    let page = match params.relation_filter {
        Some(ThreadRelationFilter::DirectChildrenOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    antex_state::ThreadRelationFilter::DirectChildrenOf(thread_id),
                    filters,
                )
                .await
        }
        Some(ThreadRelationFilter::DescendantsOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    antex_state::ThreadRelationFilter::DescendantsOf(thread_id),
                    filters,
                )
                .await
        }
...
pub(super) async fn list_rollout_threads(
    state_db: Option<antex_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&antex_rollout::Cursor>,
    sort_key: antex_rollout::ThreadSortKey,
    sort_direction: antex_rollout::SortDirection,
) -> ThreadStoreResult<antex_rollout::ThreadsPage> {
        ...
        let relation_filter = params
            ...
            .map(|relation_filter| match relation_filter {
                ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                    antex_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
                }
                ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                    antex_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
                }
            });
        let page = antex_rollout::state_db::list_threads_db(
            state_db.as_deref(),
            &config.sqlite,
            params.page_size,
            cursor,
...
mod tests {
    ...
    use antex_state::PINNED_THREAD_SECTION_ID;
    ...
    async fn list_paginated_threads_uses_sqlite_name_over_legacy_compatibility() {
        ...
        runtime
            ...
            .await
            .expect("state db upsert should succeed");
        antex_rollout::append_thread_name(home.path(), thread_id, "stale index name")
            .await
    ...
    async fn section_listing_uses_sqlite_metadata_without_reading_rollouts() {
        let home = TempDir::new().expect("temp dir");
        let config = test_config(home.path());
        let state = antex_state::StateRuntime::init(
            config.sqlite.clone(),
            config.default_model_provider_id.clone(),
        )
        .await
        ...
        for index in 0..3 {
            ...
            antex_rollout::state_db::reconcile_rollout(
                Some(state.as_ref()),
