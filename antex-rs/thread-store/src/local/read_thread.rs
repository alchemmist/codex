...
use antex_rollout::find_thread_name_by_id;
use antex_rollout::read_session_meta_line;
use antex_rollout::read_thread_item_from_rollout;
...
async fn sqlite_rollout_path_can_load_history_for_thread(
    ...
    if antex_rollout::existing_rollout_path(path).await.is_none() {
        return false;
    }
...
async fn resolve_requested_rollout_path(
    ...
    let Some(path) = antex_rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
async fn read_thread_from_rollout_path(
    ...
    thread.rollout_path = Some(antex_rollout::plain_rollout_path(path.as_path()));
...
pub(super) async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<antex_rollout::RolloutItem>> {
...
pub(super) async fn stored_thread_from_sqlite_metadata(
    ...
    let session_meta = match read_required_session_meta_line(metadata.rollout_path.as_path()).await
    {
        ...
        Err(_)
            if antex_rollout::existing_rollout_path(metadata.rollout_path.as_path())
                .await
                .is_none() =>
        {
            None
...
pub(super) fn stored_thread_from_state_metadata(
    store: &LocalThreadStore,
    metadata: ThreadMetadata,
    parent_thread_id: Option<antex_protocol::ThreadId>,
) -> StoredThread {
    ...
    let rollout_path = antex_rollout::plain_rollout_path(metadata.rollout_path.as_path());
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = antex_rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
mod tests {
    ...
    use antex_state::ThreadMetadataBuilder;
    use chrono::Utc;
    ...
    async fn read_thread_by_rollout_path_preserves_legacy_name_and_sqlite_git_info() {
        ...
        metadata.section = Some(antex_state::ThreadSection {
            id: antex_state::PINNED_THREAD_SECTION_ID.to_string(),
            name: antex_state::PINNED_THREAD_SECTION_NAME.to_string(),
            appearance: None,
        });
        ...
        antex_rollout::append_thread_name(home.path(), thread_id, "Latest index name")
            .await
        ...
        assert_eq!(
            ...
            Some(antex_state::PINNED_THREAD_SECTION_ID)
        );
    ...
    async fn read_paginated_thread_prefers_sqlite_display_metadata() {
        ...
        .expect("session file");
        antex_rollout::append_rollout_item_to_path(
            &rollout_path,
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        antex_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
            .expect("append legacy thread name");
