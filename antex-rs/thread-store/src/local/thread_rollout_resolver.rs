...
async fn resolve(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && antex_rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
            ...
            Ok(Some(metadata)) => {
                    ...
                    antex_rollout::existing_rollout_path(metadata.rollout_path.as_path()).await
                {
                    let belongs_to_thread =
                        match antex_rollout::read_session_meta_line(path.as_path()).await {
...
async fn resolve_path(
    ...
    let rollout_id = match antex_rollout::rollout_id_from_path(path.as_path()) {
        Some(rollout_id) => rollout_id,
        None => {
            let history_mode = antex_rollout::read_session_meta_line(path.as_path())
                .await
...
pub(super) fn rollout_id_from_path_or_legacy_thread_id(
    ...
    Ok(match antex_rollout::rollout_id_from_path(path) {
        Some(rollout_id) => rollout_id,
        None => {
