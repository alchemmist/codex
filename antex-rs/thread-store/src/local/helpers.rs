...
use antex_rollout::ThreadItem;
use antex_rollout::find_thread_names_by_ids;
...
pub(super) fn validated_rollout_file_name(
    ...
    if antex_rollout::rollout_id_from_path(rollout_path).is_some() {
        Ok(file_name)
    } else {
        Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let preview = item
        ...
        .unwrap_or_default();
    let rollout_path = antex_rollout::plain_rollout_path(item.path.as_path());
