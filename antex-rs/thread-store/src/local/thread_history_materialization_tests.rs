...
use antex_protocol::realtime::BemItemPresentation;
use antex_protocol::realtime::RealtimeItem;
use antex_protocol::realtime::RealtimeItemContent;
use antex_protocol::realtime::RealtimeSessionOutcome;
use antex_protocol::realtime::RealtimeTranscriptRole;
use antex_rollout::RolloutConfig;
use antex_rollout::RolloutItem;
use antex_rollout::RolloutLine;
use antex_rollout::RolloutRecorder;
use antex_rollout::RolloutRecorderParams;
...
/// Separate Antex and SQLite homes must work together across startup backfill,
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let runtime = antex_rollout::state_db::try_init(&rollout_config)
        .await
...
async fn paginated_live_append_materializes_turn_items_and_state() {
    ...
    let pool = antex_state::open_thread_history_db(&antex_state::SqliteConfig::new_for_testing(
        home.path().abs(),
    ))
    .await
...
async fn summary_items_use_final_answers_and_ignore_commentary() {
    ...
    let config = test_config(home.path());
    let thread_id = ThreadId::default();
    let runtime = antex_state::StateRuntime::init(
        config.sqlite.clone(),
        config.default_model_provider_id.clone(),
    )
    .await
    .expect("state runtime");
    let mut builder = antex_state::ThreadMetadataBuilder::new(
        thread_id,
...
async fn catch_up_skips_invalid_complete_suffixes_and_projects_later_history() {
    ...
    for (name, suffix, expected_next_ordinal, expected_turn_ids) in cases {
        ...
        let pool = antex_state::open_thread_history_db(
...
fn rollout_line_byte_offsets(path: &std::path::Path, ordinal: u64) -> (i64, i64) {
    ...
    for line in bytes.split_inclusive(|byte| *byte == b'\n') {
        ...
        if antex_rollout::parse_rollout_line_bytes(line)
            .ok()
