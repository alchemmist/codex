...
use antex_rollout::ARCHIVED_SESSIONS_SUBDIR;
use antex_rollout::RolloutReferenceIndex;
use antex_rollout::SESSIONS_SUBDIR;
use antex_rollout::find_archived_thread_path_by_id_str;
use antex_rollout::find_thread_path_by_id_str;
use antex_rollout::remove_thread_name_entries;
...
struct ThreadRollouts {
    ...
    rollout_ids: HashSet<antex_protocol::ThreadId>,
    paths: Vec<PathBuf>,
}
...
impl ThreadRollouts {
    ...
    fn add_path(&mut self, path: PathBuf) {
        if let Some(rollout_id) = antex_rollout::rollout_id_from_path(path.as_path()) {
...
fn referenced_thread_error(thread_id: antex_protocol::ThreadId) -> ThreadStoreError {
...
fn delete_rollout_file(store: &LocalThreadStore, rollout_path: &Path) -> ThreadStoreResult<bool> {
    let plain_path = antex_rollout::plain_rollout_path(rollout_path);
...
mod tests {
    ...
    async fn delete_thread_without_state_db_preserves_materialized_thread_history() {
        ...
        .expect("session file");
        let pool = antex_state::open_thread_history_db(&config.sqlite)
            .await
