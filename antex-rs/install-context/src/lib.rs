...
const CODE_MODE_HOST_EXECUTABLE_NAME: &str = if cfg!(windows) {
    "antex-code-mode-host.exe"
} else {
    "antex-code-mode-host"
};
const PACKAGE_METADATA_FILENAME: &str = "antex-package.json";
const PATH_DIRNAME: &str = "antex-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "antex-resources";
...
/// Version metadata recorded in a bundled Antex runtime package.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq)]
pub struct AntexPackageManifest {
    pub version: Version,
}
...
pub struct InstallContext {
    ...
    pub package_layout: Option<AntexPackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.antex/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `antex-resources/`, and `antex-path/`.
    ...
    /// A Antex binary launched through the npm-managed `antex.js` shim.
    Npm,
    /// A Antex binary launched through the bun-managed `antex.js` shim.
    Bun,
    /// A Antex binary launched through the pnpm-managed `antex.js` shim.
    Pnpm,
    /// A Antex binary launched through the Vite+-managed `antex.js` shim.
    VitePlus,
    /// A Antex binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Antex binaries, custom
    /// internal launchers, and tests that execute Antex from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let codex_home = antex_utils_home_dir::find_antex_home().ok();
        Self::from_exe_with_antex_home(
            is_macos,
            current_exe,
            method_override,
            codex_home.as_deref(),
    ...
    fn from_exe_with_antex_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        method_override: Option<InstallMethod>,
        codex_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(AntexPackageLayout::from_exe);
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("ANTEX_MANAGED_BY_VITE_PLUS").is_some() {
                Some(InstallMethod::VitePlus)
            } else if std::env::var_os("ANTEX_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("ANTEX_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("ANTEX_MANAGED_BY_BUN").is_some() {
    ...
    pub fn package_manifest(&self) -> Option<AntexPackageManifest> {
    ...
    pub fn code_mode_host_program(&self) -> PathBuf {
        // prefer the one packed under antex-resources
...
fn standalone_install_method(
    ...
    let canonical_antex_home = canonical_absolute_path(codex_home?)?;
    ...
    let releases_root = canonical_antex_home
        .join("packages")
...
mod tests {
    ...
    fn code_mode_host_program_prefers_package_resource_over_legacy_binary() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "antex.exe" } else { "antex" });
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let resources_dir = release_dir.join(RESOURCES_DIRNAME);
        fs::create_dir_all(&resources_dir)?;
        let exe_path = release_dir.join(if cfg!(windows) { "antex.exe" } else { "antex" });
        fs::write(&exe_path, "")?;
        ...
        let context = InstallContext::from_exe_with_antex_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
...
"#,
        ...
        let package_layout = AntexPackageLayout {
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_antex_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
