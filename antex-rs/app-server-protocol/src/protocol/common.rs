...
pub enum AuthMode {
    /// OpenAI API key provided by the caller and stored by Antex.
    ApiKey,
    /// ChatGPT OAuth managed by Antex (tokens persisted and refreshed by Antex).
    Chatgpt,
    ...
    Headers,
    /// Programmatic Antex auth backed by a registered Agent Identity.
    #[serde(rename = "agentIdentity")]
    #[ts(rename = "agentIdentity")]
    #[strum(serialize = "agentIdentity")]
    AgentIdentity,
    /// Programmatic Antex auth backed by a personal access token.
    #[serde(rename = "personalAccessToken")]
    #[ts(rename = "personalAccessToken")]
    #[strum(serialize = "personalAccessToken")]
    PersonalAccessToken,
    /// Amazon Bedrock bearer token managed by Antex.
    #[serde(rename = "bedrockApiKey")]
    #[ts(rename = "bedrockApiKey")]
    #[strum(serialize = "bedrockApiKey")]
    BedrockApiKey,
    /// Amazon Bedrock AWS access keys managed by Antex.
...
impl AuthMode {
    ...
    /// Returns whether this mode is backed by Antex services rather than a direct model API.
...
client_request_definitions! {
    ...
    #[experimental("thread/context/read")]
    ...
    /// Spawn a standalone process (argv vector) without a Antex sandbox.
...
/// Superset of [`antex_file_search::FileMatch`]
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
...
mod tests {
    ...
    fn jsonrpc_request_conversion_preserves_serde_enum_decoding() {
        let requests = [
            JSONRPCRequest {
                ...
                trace: Some(antex_protocol::protocol::W3cTraceContext {
        ...
        let response = ClientResponse::ThreadStart {
            ...
            response: v2::ThreadStartResponse {
                ...
                instruction_sources: vec![
                    antex_utils_path_uri::LegacyAppPathString::from_abs_path(&absolute_path(
                        "/tmp/AGENTS.md",
                    )),
                ],
    ...
    fn account_serializes_fields_in_camel_case() -> Result<()> {
        ...
        let antex_managed_bedrock = v2::Account::AmazonBedrock {
            uses_antex_managed_credentials: true,
        };
        assert_eq!(
            json!({
                "type": "amazonBedrock",
            ...
            serde_json::to_value(&antex_managed_bedrock)?,
        );
        ...
        let notification =
            ServerNotification::ThreadSettingsUpdated(v2::ThreadSettingsUpdatedNotification {
                ...
                thread_settings: v2::ThreadSettings {
                    ...
                    effort: None,
                    summary: None,
                    collaboration_mode: antex_protocol::config_types::CollaborationMode {
