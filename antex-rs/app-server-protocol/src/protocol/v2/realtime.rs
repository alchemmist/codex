...
use antex_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
use antex_protocol::realtime::BemItemPresentation as CoreBemItemPresentation;
use antex_protocol::realtime::RealtimeItem as CoreRealtimeItem;
use antex_protocol::realtime::RealtimeItemContent as CoreRealtimeItemContent;
use antex_protocol::realtime::RealtimeSessionOutcome as CoreRealtimeSessionOutcome;
use antex_protocol::realtime::RealtimeTranscriptRole as CoreRealtimeTranscriptRole;
...
pub struct ThreadRealtimeStartParams {
    ...
    /// Leaves Antex response handoffs to the client's explicit append calls instead of forwarding
    ...
    /// Routes any transcript tail remaining at session end through Antex. Defaults to false.
    ...
    /// Optional prefix added to automatic Antex response items when `codexResponsesAsItems` is true.
    #[ts(optional = nullable)]
    pub codex_response_item_prefix: Option<String>,
    /// Selects how automatic Antex responses are routed in Frameless Bidi sessions. Omitted values
    ...
    /// Set to false to start without Antex's startup context. Omitted or null includes it.
    #[ts(optional = nullable)]
    ...
    /// Developer instructions given to the backing Antex model when this realtime session starts.
    #[ts(optional = nullable)]
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to the backing Antex model when this realtime session ends.
    #[ts(optional = nullable)]
