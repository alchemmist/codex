...
use antex_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use antex_protocol::approvals::GuardianAssessmentAction as CoreGuardianAssessmentAction;
use antex_protocol::config_types::MultiAgentMode;
use antex_protocol::items::AgentMessageContent;
use antex_protocol::items::AgentMessageItem;
use antex_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use antex_protocol::items::CollabAgentToolCallItem;
use antex_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use antex_protocol::items::CommandExecutionItem;
...
use antex_protocol::items::FileChangeItem;
use antex_protocol::items::ImageViewItem;
use antex_protocol::items::McpToolCallItem;
use antex_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use antex_protocol::items::ReasoningItem;
use antex_protocol::items::SubAgentActivityItem;
use antex_protocol::items::TurnItem;
use antex_protocol::items::UserMessageItem;
use antex_protocol::items::WebSearchItem as CoreWebSearchItem;
use antex_protocol::mcp::CallToolResult;
use antex_protocol::mcp::McpServerInfo;
use antex_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use antex_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use antex_protocol::models::MessagePhase;
...
use antex_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use antex_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use antex_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use antex_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use antex_protocol::protocol::AgentStatus as CoreAgentStatus;
use antex_protocol::protocol::AskForApproval as CoreAskForApproval;
use antex_protocol::protocol::CodexErrorInfo as CoreAntexErrorInfo;
use antex_protocol::protocol::ConversationTextRole;
use antex_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use antex_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use antex_protocol::protocol::NetworkAccess as CoreNetworkAccess;
...
use antex_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: antex_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        antex_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        antex_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn config_requirements_read_accepts_foreign_path_uris() {
    let response: ConfigRequirementsReadResponse = serde_json::from_value(json!({
        "requirements": {
            "sqliteHome": "file:///C:/Users/alice/.antex/state",
            "logDir": "file:///C:/Users/alice/.antex/logs",
            "modelCatalogJson": "file:///C:/Users/alice/.antex/models.json"
        }
    }))
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///C:/Users/alice/.antex/state").expect("valid URI"))
    );
    assert_eq!(
        requirements.log_dir,
        Some(PathUri::parse("file:///C:/Users/alice/.antex/logs").expect("valid URI"))
    );
    assert_eq!(
        requirements.model_catalog_json,
        Some(PathUri::parse("file:///C:/Users/alice/.antex/models.json").expect("valid URI"))
    );
}
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        antex_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.antex/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![antex_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            antex_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
    ...
    let sender_thread_id = antex_protocol::ThreadId::default();
    let receiver_thread_id = antex_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: antex_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            antex_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(antex_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
#[test]
fn antex_error_info_serializes_http_status_code_in_camel_case() {
...
fn core_error_info_converts_to_camel_case() {
    for (core, expected) in [
        (CoreAntexErrorInfo::CyberPolicy, json!("cyberPolicy")),
        (
            CoreAntexErrorInfo::RateLimitExceeded,
...
#[test]
fn antex_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
...
fn turn_start_cyber_access_program_uses_separate_wire_formats() {
    for (app_server_value, core_value) in [
        ...
        .expect("params should deserialize");
        let core_program: antex_protocol::turn_input::CyberAccessProgram = params
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(antex_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn turn_start_params_round_trip_subagent_spawn_policy() {
    ...
    assert_eq!(
        ...
        Some(antex_protocol::config_types::SubagentSpawnPolicy::Disallow)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        thread_id: "thread_123".to_string(),
        collaboration_mode: Some(antex_protocol::config_types::CollaborationMode {
            mode: antex_protocol::config_types::ModeKind::Plan,
            settings: antex_protocol::config_types::Settings {
