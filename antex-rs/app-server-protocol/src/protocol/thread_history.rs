...
use antex_extension_items::image_generation::ImageGenerationItem;
use antex_protocol::items::parse_hook_prompt_message;
use antex_protocol::protocol::AgentMessageEvent;
use antex_protocol::protocol::AgentReasoningEvent;
use antex_protocol::protocol::AgentReasoningRawContentEvent;
use antex_protocol::protocol::AgentStatus;
use antex_protocol::protocol::ApplyPatchApprovalRequestEvent;
use antex_protocol::protocol::ContextCompactedEvent;
use antex_protocol::protocol::DynamicToolCallResponseEvent;
use antex_protocol::protocol::ErrorEvent;
use antex_protocol::protocol::EventMsg;
use antex_protocol::protocol::ExecCommandBeginEvent;
use antex_protocol::protocol::ExecCommandEndEvent;
use antex_protocol::protocol::GuardianAssessmentEvent;
use antex_protocol::protocol::GuardianAssessmentStatus;
use antex_protocol::protocol::ImageGenerationBeginEvent;
use antex_protocol::protocol::ImageGenerationEndEvent;
use antex_protocol::protocol::ItemCompletedEvent;
use antex_protocol::protocol::ItemStartedEvent;
use antex_protocol::protocol::McpToolCallBeginEvent;
use antex_protocol::protocol::McpToolCallEndEvent;
use antex_protocol::protocol::PatchApplyBeginEvent;
use antex_protocol::protocol::PatchApplyEndEvent;
...
use antex_protocol::protocol::ViewImageToolCallEvent;
use antex_protocol::protocol::WebSearchBeginEvent;
use antex_protocol::protocol::WebSearchEndEvent;
#[cfg(test)]
use antex_protocol::review_format::REVIEW_FALLBACK_MESSAGE;
use antex_rollout::CompactedItem;
...
#[cfg(test)]
use antex_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use antex_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `antex-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &antex_protocol::models::ResponseItem) {
        let antex_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &antex_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            antex_protocol::items::TurnItem::EnteredReviewMode(_)
                | antex_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            antex_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            antex_protocol::items::TurnItem::HookPrompt(_)
            | antex_protocol::items::TurnItem::FunctionCallOutput(_)
            | antex_protocol::items::TurnItem::CommandExecution(_)
            | antex_protocol::items::TurnItem::DynamicToolCall(_)
            | antex_protocol::items::TurnItem::CollabAgentToolCall(_)
            | antex_protocol::items::TurnItem::SubAgentActivity(_)
            | antex_protocol::items::TurnItem::Extension(_)
            | antex_protocol::items::TurnItem::EnteredReviewMode(_)
            | antex_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            antex_protocol::items::TurnItem::UserMessage(_)
            | antex_protocol::items::TurnItem::AgentMessage(_)
            | antex_protocol::items::TurnItem::Reasoning(_)
            | antex_protocol::items::TurnItem::WebSearch(_)
            | antex_protocol::items::TurnItem::ImageView(_)
            | antex_protocol::items::TurnItem::ImageGeneration(_)
            | antex_protocol::items::TurnItem::FileChange(_)
            | antex_protocol::items::TurnItem::McpToolCall(_)
            | antex_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &antex_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &antex_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &antex_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &antex_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &antex_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &antex_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &antex_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &antex_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &antex_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &antex_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &antex_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &antex_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &antex_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &antex_protocol::protocol::ExitedReviewModeEvent,
    ) {
...
fn convert_dynamic_tool_content_items(
    items: &[antex_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            antex_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            antex_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
            } => DynamicToolCallOutputContentItem::InputImage { image_url },
            antex_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputAudio {
                audio_url,
...
mod tests {
    ...
    use antex_extension_items::ExtensionItem as CoreExtensionItem;
    use antex_extension_items::sleep::SleepItem as CoreSleepItem;
    use antex_protocol::ThreadId;
    use antex_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use antex_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use antex_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use antex_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use antex_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use antex_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use antex_protocol::items::SubAgentActivityItem as CoreSubAgentActivityItem;
    use antex_protocol::items::TurnItem as CoreTurnItem;
    use antex_protocol::items::UserMessageItem as CoreUserMessageItem;
    use antex_protocol::items::build_hook_prompt_message;
    use antex_protocol::mcp::CallToolResult;
    use antex_protocol::models::ImageDetail;
    use antex_protocol::models::MessagePhase as CoreMessagePhase;
    use antex_protocol::models::WebSearchAction as CoreWebSearchAction;
    use antex_protocol::parse_command::ParsedCommand;
    use antex_protocol::protocol::AgentReasoningEvent;
    use antex_protocol::protocol::AgentReasoningRawContentEvent;
    use antex_protocol::protocol::ApplyPatchApprovalRequestEvent;
    use antex_protocol::protocol::CodexErrorInfo;
    use antex_protocol::protocol::DynamicToolCallResponseEvent;
    use antex_protocol::protocol::EnteredReviewModeEvent;
    use antex_protocol::protocol::ExecCommandBeginEvent;
    use antex_protocol::protocol::ExecCommandEndEvent;
    use antex_protocol::protocol::ExecCommandSource;
    use antex_protocol::protocol::ExitedReviewModeEvent;
    use antex_protocol::protocol::ItemStartedEvent;
    use antex_protocol::protocol::McpInvocation;
    use antex_protocol::protocol::McpToolCallEndEvent;
    use antex_protocol::protocol::PatchApplyBeginEvent;
    use antex_protocol::protocol::ReviewTarget;
    use antex_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
    use antex_protocol::protocol::ThreadRolledBackEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![antex_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn replays_image_generation_end_events_into_turn_history() {
        let items = vec![
            ...
            RolloutItem::EventMsg(EventMsg::ImageGenerationEnd(ImageGenerationEndEvent {
                ...
                transparent_background: Some(true),
                failure: Some(
                    antex_extension_items::image_generation::ImageGenerationFailure::UsageLimitExceeded {
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                antex_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    antex_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(antex_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(antex_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: antex_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(antex_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                antex_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                antex_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt = CoreTurnItem::HookPrompt(antex_protocol::items::HookPromptItem {
    ...
    fn completed_sub_agent_activity_updates_completed_parent_turn() {
        ...
        let child_path = antex_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path");
        let items = vec![
            RolloutItem::EventMsg(EventMsg::TurnStarted(TurnStartedEvent {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(
                antex_protocol::models::ResponseItem::Message {
                    id: Some(antex_protocol::ResponseItemId::with_suffix("msg", "1")),
                    role: "user".into(),
                    content: vec![antex_protocol::models::ContentItem::InputText {
