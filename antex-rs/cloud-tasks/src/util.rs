...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = antex_login::default_client::USER_AGENT_SUFFIX.lock() {
...
pub(crate) fn validate_chatgpt_base_url(input: &str) -> anyhow::Result<String> {
    let invalid_url = || {
        anyhow::anyhow!(
            "ANTEX_CLOUD_TASKS_BASE_URL must use a trusted HTTPS origin on port 443, without user information, a query, or a fragment; custom backends cannot use saved ChatGPT credentials"
        )
    };
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    let ua = antex_login::default_client::get_antex_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("antex-cli")),
    );
    if let Some(am) = load_auth_manager(/*chatgpt_base_url*/ None).await.0
        && let Some(auth) = am.auth().await
        && auth.uses_antex_backend()
    {
        headers.extend(antex_model_provider::auth_provider_from_auth(&auth).to_auth_headers());
    }
    headers
}
