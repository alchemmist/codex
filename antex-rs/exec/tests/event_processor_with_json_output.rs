use antex_app_server_protocol::CollabAgentState as ApiCollabAgentState;
use antex_app_server_protocol::CollabAgentStatus as ApiCollabAgentStatus;
use antex_app_server_protocol::CollabAgentTool;
use antex_app_server_protocol::CollabAgentToolCallStatus as ApiCollabAgentToolCallStatus;
use antex_app_server_protocol::CommandAction;
use antex_app_server_protocol::CommandExecutionSource;
use antex_app_server_protocol::CommandExecutionStatus as ApiCommandExecutionStatus;
use antex_app_server_protocol::ErrorNotification;
use antex_app_server_protocol::FileUpdateChange as ApiFileUpdateChange;
use antex_app_server_protocol::ItemCompletedNotification;
use antex_app_server_protocol::ItemStartedNotification;
use antex_app_server_protocol::McpToolCallError;
use antex_app_server_protocol::McpToolCallResult;
use antex_app_server_protocol::McpToolCallStatus as ApiMcpToolCallStatus;
use antex_app_server_protocol::PatchApplyStatus as ApiPatchApplyStatus;
use antex_app_server_protocol::PatchChangeKind as ApiPatchChangeKind;
...
use antex_app_server_protocol::WebSearchAction as ApiWebSearchAction;
use antex_app_server_protocol::WebSearchItem as ApiWebSearchItem;
...
use antex_exec::AgentMessageItem;
use antex_exec::AntexStatus;
use antex_exec::CollabAgentState;
use antex_exec::CollabAgentStatus;
use antex_exec::CollabTool;
use antex_exec::CollabToolCallItem;
use antex_exec::CollabToolCallStatus;
use antex_exec::CollectedThreadEvents;
use antex_exec::CommandExecutionItem;
use antex_exec::CommandExecutionStatus;
use antex_exec::ErrorItem;
use antex_exec::EventProcessorWithJsonOutput;
use antex_exec::ExecThreadItem;
use antex_exec::FileChangeItem;
use antex_exec::FileUpdateChange as ExecFileUpdateChange;
use antex_exec::ItemCompletedEvent;
use antex_exec::ItemStartedEvent;
use antex_exec::ItemUpdatedEvent;
use antex_exec::McpToolCallItem;
use antex_exec::McpToolCallItemError;
use antex_exec::McpToolCallItemResult;
use antex_exec::McpToolCallStatus;
use antex_exec::PatchApplyStatus;
use antex_exec::PatchChangeKind;
use antex_exec::ReasoningItem;
use antex_exec::ThreadErrorEvent;
use antex_exec::ThreadEvent;
use antex_exec::ThreadItemDetails;
use antex_exec::ThreadStartedEvent;
use antex_exec::TodoItem;
use antex_exec::TodoListItem;
use antex_exec::TurnCompletedEvent;
use antex_exec::TurnFailedEvent;
use antex_exec::TurnStartedEvent;
use antex_exec::Usage;
use antex_exec::WebSearchItem;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_configured = SessionConfiguredEvent {
        ...
        approvals_reviewer: antex_protocol::config_types::ApprovalsReviewer::User,
...
fn plan_update_emits_started_then_updated_then_completed() {
    ...
    assert_eq!(
        ...
        CollectedThreadEvents {
            ...
            status: AntexStatus::InitiateShutdown,
        }
    );
}
...
fn token_usage_update_is_emitted_on_turn_completion() {
    ...
    let usage_update =
        processor.collect_thread_events(ServerNotification::ThreadTokenUsageUpdated(
            antex_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
...
fn failed_turn_clears_stale_final_message() {
    ...
    assert_eq!(collected.status, AntexStatus::Running);
    ...
    assert_eq!(collected.status, AntexStatus::InitiateShutdown);
...
fn model_reroute_surfaces_as_error_item() {
    ...
    let collected = processor.collect_thread_events(ServerNotification::ModelRerouted(
        antex_app_server_protocol::ModelReroutedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            from_model: "gpt-5".to_string(),
            to_model: "gpt-5-mini".to_string(),
            reason: antex_app_server_protocol::ModelRerouteReason::HighRiskCyberActivity,
        },
    ));
