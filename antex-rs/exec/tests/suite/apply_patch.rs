...
/// at some point, we must ensure that the smaller `antex-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(antex_utils_cargo_bin::cargo_bin("antex-exec")?)
        .arg(ANTEX_CORE_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
...
async fn shutdown_flushes_completed_turn_and_file_diff() -> anyhow::Result<()> {
    ...
    test.cmd_with_server(&server)
        ...
        .env_remove("ANTEX_ANALYTICS_EVENTS_CAPTURE_FILE")
        .arg("--skip-git-repo-check")
        .arg("-s")
        .arg("danger-full-access")
        .arg("-c")
