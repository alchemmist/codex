...
async fn antex_home_symlink_opt_out_uses_loaded_user_config() -> anyhow::Result<()> {
    ...
    for (home_env, root_alias, enabled, ignore_user_config) in [
        ...
        ("home-link/../.antex", Some(".antex"), false, false),
        ("home-link/../.antex", None, true, false),
        ("user-home", Some("user-home"), true, true),
    ] {
        let test = test_antex_exec();
        let home = test.home_path().join(".antex");
        let project_home = test.cwd_path().join(".antex");
