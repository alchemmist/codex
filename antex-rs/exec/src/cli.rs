...
#[command(
    version,
    override_usage = "antex exec [OPTIONS] [PROMPT]\n       antex exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Antex outside a Git repository.
    ...
    /// Do not load `$ANTEX_HOME/config.toml`; auth still uses `ANTEX_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "antex resume --last <prompt>" treats the positional as a prompt, not a session id.
