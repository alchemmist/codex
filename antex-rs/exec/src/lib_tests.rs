...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = antex_protocol::protocol::W3cTraceContext {
...
fn runtime_warnings_are_filtered_to_the_primary_thread() {
    ...
    let turn_id = "turn-1";
    let outcomes = [
        antex_app_server_protocol::WarningNotification {
            thread_id: None,
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        originator: None,
        source: antex_app_server_protocol::SessionSource::Exec,
        ...
        name: None,
        turns: vec![
            antex_app_server_protocol::Turn {
                id: "turn-1".to_string(),
    ...
    let notification =
        ServerNotification::TurnCompleted(antex_app_server_protocol::TurnCompletedNotification {
            ...
            turn: antex_app_server_protocol::Turn {
                id: "turn-1".to_string(),
                items_view: antex_app_server_protocol::TurnItemsView::Summary,
                items: Vec::new(),
                status: antex_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(antex_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
async fn thread_start_params_match_history_to_persistence() {
    ...
    assert_eq!(
        ...
        Some(antex_app_server_protocol::ThreadSource::User)
    );
...
async fn session_configured_from_thread_response_preserves_thread_source() {
    ...
    assert_eq!(
        event.thread_source,
        Some(antex_protocol::protocol::ThreadSource::User)
    );
}
...
fn sample_thread_start_response() -> ThreadStartResponse {
    ThreadStartResponse {
        ...
        approval_policy: antex_app_server_protocol::AskForApproval::OnRequest,
        approvals_reviewer: antex_app_server_protocol::ApprovalsReviewer::AutoReview,
        sandbox: antex_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
