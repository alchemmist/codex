...
use antex_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use antex_app_server_client::EnvironmentManager;
use antex_app_server_client::ExecServerRuntimePaths;
use antex_app_server_client::InProcessAppServerClient;
use antex_app_server_client::InProcessClientStartArgs;
use antex_app_server_client::InProcessServerEvent;
...
use antex_app_server_protocol::Thread as AppServerThread;
...
use antex_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use antex_config::format_config_error_with_source;
use antex_core::StateDbHandle;
use antex_core::check_execpolicy_for_warnings;
use antex_core::config::Config;
use antex_core::config::ConfigBuilder;
use antex_core::config::ConfigOverrides;
use antex_core::config::ConfigTomlLoadResult;
...
use antex_core::config::resolve_oss_provider;
use antex_core::config::resolve_profile_v2_config_path;
use antex_core::find_thread_meta_by_name_str;
use antex_core::format_exec_policy_error_with_source;
use antex_core::path_utils;
use antex_core::read_session_meta_line;
...
use antex_login::default_client::set_default_client_residency_requirement;
use antex_login::default_client::set_default_originator;
use antex_login::enforce_login_restrictions;
...
use antex_otel::set_parent_from_context;
use antex_otel::traceparent_context_from_env;
...
use antex_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use antex_utils_cli::SharedCliOptions;
use antex_utils_oss::ensure_oss_provider_ready;
use antex_utils_oss::get_default_model_for_oss_provider;
use antex_worktree::CreateWorktree;
use antex_worktree::WorktreeManager;
use antex_worktree::WorktreeSettings;
pub use cli::Cli;
pub use cli::Command;
pub use cli::ReviewArgs;
use event_processor_with_human_output::EventProcessorWithHumanOutput;
pub use event_processor_with_jsonl_output::AntexStatus;
...
enum StdinPromptBehavior {
    ...
    /// `antex exec` behavior for `antex exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `antex exec -`
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<antex_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("antex_exec".to_string()) {
        tracing::warn!(?err, "Failed to set antex exec originator override {err:?}");
    }
    ...
    if worktree {
        ...
        match command.as_ref() {
            Some(ExecCommand::Resume(_)) => {
                anyhow::bail!("--worktree is not supported with `antex exec resume`");
            }
            Some(ExecCommand::Review(_)) => {
                anyhow::bail!("--worktree is not supported with `antex exec review`");
            }
    ...
    #[allow(clippy::print_stderr)]
    let codex_home = match find_antex_home() {
    ...
    if worktree
        && EnvironmentManager::prepare_from_antex_home(&codex_home)
            .await?
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        antex_core::otel_init::build_provider(
            &config,
    ...
    antex_core::otel_init::record_process_start(otel.as_ref(), "antex_exec");
    antex_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "antex_exec");
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.antex_self_exe.clone(),
        arg0_paths.antex_linux_sandbox_exe.clone(),
    )?;
    #[cfg(target_os = "macos")]
    let local_runtime_paths = local_runtime_paths.with_allowed_symlinked_antex_home(
        antex_config::allowed_symlinked_antex_home(&config.config_layer_stack, &config.codex_home),
    );
    let state_db = antex_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_antex_home(
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "antex_exec".to_string(),
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, antex_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = antex_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Antex
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            antex_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(notification) => {
                ...
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        antex_app_server_protocol::TurnStatus::Failed
                            | antex_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<antex_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<antex_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<antex_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
fn should_backfill_turn_completed_items(
    ...
    !thread_ephemeral && payload.turn.items_view != antex_app_server_protocol::TurnItemsView::Full
}
...
async fn parse_latest_turn_context_cwd(path: &Path) -> Option<PathBuf> {
    ...
    tokio::task::spawn_blocking(move || {
        let reader = antex_rollout::open_rollout_seekable_reader(&path).ok()?;
        let mut scanner = antex_rollout::ReverseJsonlScanner::new(reader).ok()?;
        while let Some(outcome) = scanner.scan_next_rollout_line().ok()? {
            if let antex_rollout::ScanOutcome::Parsed(RolloutLine {
