//! Registry of model providers supported by Antex.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Antex works out-of-the-box.
//!   2. User-defined entries inside `~/.antex/config.toml` under the `model_providers`
//!      key. These override or extend the defaults at runtime.

use antex_api::Provider as ApiProvider;
use antex_api::RetryConfig as ApiRetryConfig;
use antex_protocol::auth::AuthMode;
use antex_protocol::config_types::ModelProviderAuthInfo;
use antex_protocol::error::CodexErr;
use antex_protocol::error::EnvVarError;
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> AntexResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> AntexResult<ApiProvider> {
    ...
    pub fn api_key(&self) -> AntexResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
    ...
    pub fn supports_antex_backend_routes(&self) -> bool {
        self.is_openai()
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with Antex CLI, so we only include the OpenAI and
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These ANTEX_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_antex_oss_base_url = format!(
        "http://localhost:{antex_oss_port}/v1",
        antex_oss_port = std::env::var("ANTEX_OSS_PORT")
            .ok()
    ...
    let antex_oss_base_url = std::env::var("ANTEX_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_antex_oss_base_url);
    create_oss_provider_with_base_url(&antex_oss_base_url, wire_api)
}
