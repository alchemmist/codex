...
fn queue_rejects_empty_message() -> Result<()> {
    let output = std::process::Command::new(antex_utils_cargo_bin::cargo_bin("antex")?)
...
async fn run_remote_queue_command(response: QueueResponse) -> Result<(Output, Value)> {
    ...
    let output = tokio::process::Command::new(antex_utils_cargo_bin::cargo_bin("antex")?)
        .env("ANTEX_HOME", codex_home.path())
        .env("ANTEX_REMOTE_TOKEN", "test-token")
        .args(remote_args)
...
async fn queue_rejects_local_daemon_that_does_not_support_queueing() -> Result<()> {
    ...
    let socket_path = antex_app_server::app_server_control_socket_path(codex_home.path())?;
