...
impl Fixture {
    fn new() -> Result<Self> {
        ...
        let source = antex_utils_cargo_bin::cargo_bin("antex")?;
...
"#,
        ...
        for name in [
            ...
            std::fs::write(
                ...
                "@echo helper ran>>\"%ANTEX_TEST_HELPER_MARKER%\"\r\n@exit /b 0\r\n",
            )?;
        }
    ...
    fn command(&self) -> Result<assert_cmd::Command> {
        ...
        command
            ...
            .env("ANTEX_HOME", &self.home)
            .env("HOME", self.root.path())
            .env("PATH", &self.path)
            .env("ANTEX_TEST_HELPER_MARKER", &self.marker)
            .env("ANTEX_MANAGED_BY_NPM", "1")
            .env(
                "ANTEX_MANAGED_PACKAGE_ROOT",
                self.workspace.join("node_modules/@alchemmist/antex"),
            )
            .env(
...
async fn interactive_tmux_startup_does_not_execute_workspace_helpers() -> Result<()> {
    ...
    env.insert(
        "ANTEX_TUI_DISABLE_KEYBOARD_ENHANCEMENT".to_string(),
        "0".to_string(),
    );
    env.insert(
    ...
    let spawned = antex_utils_pty::spawn_pty_process(
        fixture.program.to_str().unwrap(),
        &[],
        &fixture.workspace,
        &env,
        /*arg0*/ &None,
        antex_utils_pty::TerminalSize {
            rows: 40,
            cols: 120,
        },
        &[],
...
async fn feedback_with_logs_does_not_execute_path_helpers() -> Result<()> {
    ...
    let package_root = fixture.workspace.join("node_modules/@alchemmist/antex");
    let package_root = package_root.to_string_lossy();
    let mut app_server = TestAppServer::builder()
        .with_program(&fixture.program)
        .with_antex_home(&fixture.home)
        ...
        .with_env_overrides(&[
            ...
            ("ANTEX_TEST_HELPER_MARKER", Some(marker.as_ref())),
            ("ANTEX_MANAGED_BY_NPM", Some("1")),
            ("ANTEX_MANAGED_PACKAGE_ROOT", Some(package_root.as_ref())),
            ("ZELLIJ", Some("0")),
            ("ZELLIJ_VERSION", None),
            ("TMUX", None),
            ("TMUX_PANE", None),
