...
async fn cloud_list_only_allows_trusted_credential_destinations() -> Result<()> {
    ...
    let command = || -> Result<assert_cmd::Command> {
        let mut command = assert_cmd::Command::new(antex_utils_cargo_bin::cargo_bin("antex")?);
        command
            .current_dir(codex_home.path())
            .env("ANTEX_HOME", codex_home.path())
            .env_remove("ANTEX_ACCESS_TOKEN")
            .env_remove("OPENAI_API_KEY")
            .env_remove("ANTEX_CLOUD_TASKS_MODE")
    ...
    let output = command()?
        ...
        .stderr
        .clone();
    insta::assert_snapshot!(String::from_utf8(output)?, @"Error: ANTEX_CLOUD_TASKS_BASE_URL must use a trusted HTTPS origin on port 443, without user information, a query, or a fragment; custom backends cannot use saved ChatGPT credentials");
    ...
    command()?
        ...
        .env("ANTEX_ACCESS_TOKEN", "at-synthetic-cloud")
        .env("ANTEX_AUTHAPI_BASE_URL", auth_server.uri())
        .args(["cloud", "list", "--limit", "1", "--json"])
        .assert()
        .failure()
        .stderr(contains("Not signed in. Please run 'antex login'"));
