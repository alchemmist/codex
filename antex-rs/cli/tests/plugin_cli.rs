...
use antex_config::MarketplaceConfigUpdate;
use antex_config::record_user_marketplace;
...
fn antex_command_in(codex_home: &Path, current_dir: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = antex_command(codex_home)?;
...
fn write_marketplace_source_with_manifest(source: &Path, marketplace_manifest: &str) -> Result<()> {
    ...
    std::fs::create_dir_all(source.join("plugins").join("sample").join(".antex-plugin"))?;
    std::fs::write(
        source
            .join(".agents")
            .join("plugins")
...
async fn marketplace_list_json_prints_configured_marketplaces() -> Result<()> {
    ...
    let assert = antex_command(codex_home.path())?
...
async fn plugin_list_excludes_unconfigured_repo_local_marketplaces() -> Result<()> {
    ...
    antex_command_in(codex_home.path(), source.path())?
...
async fn plugin_add_reinstalls_from_configured_marketplace_snapshot() -> Result<()> {
    ...
    assert!(
        codex_home
            .path()
            .join("plugins/cache/debug/sample/1.2.3/.antex-plugin/plugin.json")
            .is_file()
    );
...
impl RemoteMarketplaceFixture {
    ...
    async fn run(&self, args: &[&str]) -> Result<Output> {
        Ok(Command::new(antex_utils_cargo_bin::cargo_bin("antex")?)
            .current_dir(self.home.path())
            .env("ANTEX_HOME", self.home.path())
            .env("HOME", self.home.path())
            .env_remove("OPENAI_API_KEY")
            .env_remove("ANTEX_API_KEY")
            .env_remove("ANTEX_ACCESS_TOKEN")
            .env("ANTEX_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS", "1")
            .args(args)
            .output()
            .await?)
    }
...
async fn remote_plugin_add_list_and_remove() -> Result<()> {
    ...
    let installed_root = canonicalize_existing_preserving_symlinks(fixture.home.path())?
        ...
        .join("sample")
        .join("1.2.3");
    let manifest = installed_root.join(".antex-plugin/plugin.json");
