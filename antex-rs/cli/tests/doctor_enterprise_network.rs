...
"#,
        ...
        let output = Command::new(antex_utils_cargo_bin::cargo_bin("antex")?)
...
async fn invalid_custom_ca_falls_back_to_system_roots() -> Result<()> {
    ...
    for sandbox in [None, Some("seatbelt")] {
        let mut command = Command::new(antex_utils_cargo_bin::cargo_bin("antex")?);
        command
            .args(["doctor", "--json"])
            .env("ANTEX_HOME", codex_home.path())
            .env("ANTEX_CA_CERTIFICATE", &certificate)
            .stdin(Stdio::null());
