...
use antex_exec_server::ExecServerClient;
use antex_exec_server::NoiseChannelIdentity;
use antex_exec_server::NoiseChannelPublicKey;
use antex_exec_server::NoiseRendezvousConnectArgs;
...
"#,
    ...
    let mut cmd = antex_command(codex_home.path())?;
    cmd.args([
        "exec-server",
        "--strict-config",
        "--listen",
...
fn local_exec_server_allows_disabled_parent_lifetime_environment_variable() -> Result<()> {
    ...
    let mut cmd = antex_command(codex_home.path())?;
    cmd.env(
        antex_exec_server::ANTEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
        "false",
    )
...
"#
    ...
    std::fs::copy(antex_utils_cargo_bin::cargo_bin("antex")?, &executable)?;
    let manifest = package.path().join("antex-package.json");
    std::fs::write(&manifest, r#"{"version":"1.2.3-alpha.4"}"#)?;

    let mut command = tokio::process::Command::new(executable);
    command
        .env("ANTEX_HOME", codex_home.path())
        .env("ANTEX_API_KEY", "test-api-key")
        .env(
            antex_exec_server::ANTEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
            "true",
        )
    ...
    let argv = vec![
        "cmd.exe",
        "/C",
        "if defined ANTEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE (exit /b 1) else ping -n 61 127.0.0.1",
    ];
    #[cfg(not(windows))]
    let argv = vec![
        "/bin/sh",
        "-c",
        "[ -z \"${ANTEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE+present}\" ] && exec /bin/sleep 60",
    ];
    ...
    client
        .exec(ExecParams {
            ...
            shell_snapshot: None,
            env_policy: Some(antex_exec_server::ExecEnvPolicy {
                inherit: antex_protocol::config_types::ShellEnvironmentPolicyInherit::All,
...
"#
    ...
    let antex_bin = antex_utils_cargo_bin::cargo_bin("antex")?;
    let codex_home = codex_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(antex_bin);
        command
            .env("ANTEX_HOME", codex_home)
            .env("NO_PROXY", "127.0.0.1,localhost")
            .env("no_proxy", "127.0.0.1,localhost")
            .args(["exec-server", "--listen", "stdio"])
            .stdin(Stdio::piped())
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let codex_home = TempDir::new()?;
    let mut child = std::process::Command::new(antex_utils_cargo_bin::cargo_bin("antex")?)
        .env("ANTEX_HOME", codex_home.path())
