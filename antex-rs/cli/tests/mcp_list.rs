...
"#,
  ...
  "mcpServers": {
    ...
    let mut list_cmd = antex_command(codex_home.path())?;
    list_cmd
        .env(ANTEX_API_KEY_ENV_VAR, "sk-test")
        .args(["mcp", "list", "--json"])
        .assert()
        .success()
        .stdout(contains(r#""name": "api-docs""#));

    let mut get_cmd = antex_command(codex_home.path())?;
    get_cmd
        .env(ANTEX_API_KEY_ENV_VAR, "sk-test")
...
async fn list_discovers_local_oauth_server_through_environment_proxy() -> Result<()> {
    ...
    let mut command = antex_command(codex_home.path())?;
    command
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = antex_command(codex_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut list_json_cmd = antex_command(codex_home.path())?;
    ...
    assert!(stdout.contains("remove: antex mcp remove docs"));

    let mut get_json_cmd = antex_command(codex_home.path())?;
    get_json_cmd
...
fn list_and_get_redact_http_headers_helper() -> Result<()> {
    ...
    let list_output = antex_command(codex_home.path())?
        .args(["mcp", "list", "--json"])
        .output()?;
    assert!(list_output.status.success());
    let stdout = String::from_utf8(list_output.stdout)?;
    ...
    for args in [
        ...
        let output = antex_command(codex_home.path())?.args(args).output()?;
        assert!(output.status.success());
        let stdout = String::from_utf8(output.stdout)?;
        assert!(stdout.contains("http_headers_helper"));
        assert!(stdout.contains("<redacted>"));
