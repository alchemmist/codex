...
#[derive(Debug, Parser)]
#[command(name = "antex-state-logs")]
#[command(about = "Tail Antex logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to ANTEX_HOME. Defaults to $ANTEX_HOME or ~/.antex.
    #[arg(long = "antex-home", env = "ANTEX_HOME")]
    codex_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --antex-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["antex-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["antex-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["antex-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["antex-state-logs", "--level", "WARN"])
            .expect("parse uppercase log level");

        assert_eq!(args.level, Some(LogLevelThreshold::Warn));
    }

    /// Explicit database selection must not parse an overridden Antex home.
    #[tokio::test]
    async fn direct_db_skips_antex_home_config() {
        let codex_home = tempfile::tempdir().expect("create Antex home");
        ...
        let args = Args::try_parse_from([
            OsString::from("antex-state-logs"),
            OsString::from("--antex-home"),
