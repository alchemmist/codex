...
use antex_thread_store::RolloutMigrationMode;
use antex_thread_store::RolloutMigrationOptions;
use antex_thread_store::RolloutMigrationProgress;
use antex_thread_store::RolloutMigrationReport;
use antex_thread_store::RolloutMigrationStatus;
...
pub(crate) async fn run(
    ...
    antex_core::otel_init::record_process_start(otel.as_ref(), "codex_migrate_rollouts");
    ...
    let state_db = if mode == RolloutMigrationMode::Apply {
        Some(
            antex_rollout::state_db::try_init(&config)
                .await
...
fn print_human_report(
    ...
    if mode == RolloutMigrationMode::DryRun && counts.eligible > 0 {
        println!("Run `antex migrate-rollouts --apply` to migrate eligible sessions.");
    }
...
fn print_outcome(outcome: &antex_thread_store::RolloutMigrationOutcome) {
...
async fn thread_storage_bytes(codex_home: &Path, thread_history_db_path: &Path) -> io::Result<u64> {
    ...
    let mut directories = vec![
        codex_home.join(antex_rollout::SESSIONS_SUBDIR),
        codex_home.join(antex_rollout::ARCHIVED_SESSIONS_SUBDIR),
    ];
