//! Implements the `antex doctor` diagnostic report.
//!
...
use antex_api::ResponsesWebsocketClient;
use antex_api::is_azure_responses_provider;
...
use antex_install_context::AntexPackageLayout;
use antex_install_context::InstallContext;
use antex_install_context::InstallMethod;
use antex_install_context::StandalonePlatform;
use antex_login::ANTEX_ACCESS_TOKEN_ENV_VAR;
use antex_login::ANTEX_API_KEY_ENV_VAR;
use antex_login::AntexAuth;
use antex_login::AuthDotJson;
use antex_login::AuthManager;
use antex_login::OPENAI_API_KEY_ENV_VAR;
use antex_login::default_client::create_client_without_request_logging;
use antex_login::default_client::default_headers;
use antex_login::load_auth_dot_json;
use antex_model_provider::create_model_provider;
use antex_protocol::auth::AuthMode;
use antex_protocol::protocol::AskForApproval;
use antex_terminal_detection::Multiplexer;
use antex_terminal_detection::TerminalInfo;
use antex_terminal_detection::TerminalName;
use antex_terminal_detection::terminal_info;
...
/// Options for building a local Antex diagnostic report.
///
...
async fn build_report(
    ...
    DoctorReport {
        ...
        overall_status,
        antex_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    .build()
    .await
    .context("failed to load Antex config")
}
...
/// JSON support report emitted by `antex doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        antex_version: report.antex_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("ANTEX_MANAGED_BY_BUN").is_some()
    ));
    details.push(format!(
        "managed by Vite+: {}",
        env::var_os("ANTEX_MANAGED_BY_VITE_PLUS").is_some()
    ));
    details.push(format!(
        "managed by pnpm: {}",
        env::var_os("ANTEX_MANAGED_BY_PNPM").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "ANTEX_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = antex_path_entries();
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        ...
        if env::var_os("ANTEX_MANAGED_PACKAGE_ROOT").is_none() {
            status = status.max(CheckStatus::Warning);
            summary = "npm-managed launch is missing package-root provenance".to_string();
            remediation = Some(
                "Reinstall or update Antex so the JS shim provides ANTEX_MANAGED_PACKAGE_ROOT."
                    .to_string(),
            );
        }
    }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("ANTEX_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("ANTEX_MANAGED_BY_NPM").is_none()
        && env::var_os("ANTEX_MANAGED_BY_BUN").is_none()
        && env::var_os("ANTEX_MANAGED_BY_VITE_PLUS").is_none()
        && env::var_os("ANTEX_MANAGED_BY_PNPM").is_none()
    {
        return false;
    }
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&AntexPackageLayout>,
) -> String {
...
fn antex_path_entries() -> Vec<String> {
...
fn config_check(config: &Config) -> DoctorCheck {
    ...
    details.push(format!("ANTEX_HOME: {}", config.codex_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.codex_home.join(antex_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    ...
    let env_auth_vars = [
        ...
        ANTEX_API_KEY_ENV_VAR,
        ANTEX_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run antex login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no Antex credentials were found",
        )
        .details(details)
        .remediation("Run antex login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run antex login again."),
    }
}
...
fn stored_auth_mode(auth: &antex_login::AuthDotJson) -> &'static str {
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        AuthMode::ApiKey => {
            ...
            let env_key_present =
                env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(ANTEX_API_KEY_ENV_VAR);
...
async fn state_check(config: &Config, command: &DoctorCommand) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "ANTEX_HOME", &config.codex_home);
...
async fn sqlite_integrity_detail(
    sqlite: &antex_state::SqliteConfig,
    details: &mut Vec<String>,
    label: &str,
    path: &Path,
    ...
    let (rows, timed_out) = match antex_state::sqlite_integrity_check(sqlite, path, deadline).await
    {
        Ok(antex_state::SqliteIntegrityCheck::Complete(rows)) => (rows, false),
        Ok(antex_state::SqliteIntegrityCheck::TimedOut(rows)) => (rows, true),
        Err(err) => {
...
fn auth_mode_name(auth: &AntexAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match codex_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "ANTEX_HOME was resolved without config",
        )
        .detail(format!("ANTEX_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "ANTEX_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if provider_base_url.is_some_and(|url| !url.trim().is_empty())
        ...
        || env_var_present(ANTEX_API_KEY_ENV_VAR)
    {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(ANTEX_ACCESS_TOKEN_ENV_VAR) {
...
mod tests {
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].antex_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            antex_self_exe: Some(PathBuf::from("/bin/codex")),
            antex_linux_sandbox_exe: Some(PathBuf::from("/bin/antex-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/antex-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.antex_self_exe, arg0_paths.antex_self_exe);
        assert_eq!(
            overrides.antex_linux_sandbox_exe,
            arg0_paths.antex_linux_sandbox_exe
        );
        assert_eq!(
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "ANTEX_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("ANTEX_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running Antex."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Antex.".to_string())
        );
    }
    ...
    fn provider_reachability_mode_uses_api_key_auth() {
        ...
        assert_eq!(
            provider_auth_reachability_mode_from_auth(
                ...
                /*provider_env_key*/ None,
                /*provider_base_url*/ None,
                |name| name == ANTEX_API_KEY_ENV_VAR,
                /*stored_auth*/ None,
            ),
            ProviderAuthReachabilityMode::ApiKey
        );
