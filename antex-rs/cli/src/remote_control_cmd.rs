...
use antex_app_server_daemon::LifecycleOutput as AppServerLifecycleOutput;
use antex_app_server_daemon::LifecycleStatus as AppServerLifecycleStatus;
use antex_app_server_daemon::RemoteControlReadyOutput as AppServerRemoteControlReadyOutput;
use antex_app_server_daemon::RemoteControlReadyStatus as AppServerRemoteControlReadyStatus;
use antex_app_server_daemon::RemoteControlStartOutput as AppServerRemoteControlStartOutput;
...
pub(crate) async fn run(
    ...
    match command.subcommand {
        ...
        Some(RemoteControlSubcommand::Start) => {
            ...
            let output = antex_app_server_daemon::ensure_remote_control_ready().await?;
        ...
        Some(RemoteControlSubcommand::Stop) => {
            ...
            let output = antex_app_server_daemon::run(AppServerLifecycleCommand::Stop).await?;
            print_remote_control_stop_output(&output, command.json)?;
        }
        Some(RemoteControlSubcommand::Pair) => {
            let output = antex_app_server_daemon::start_remote_control_pairing().await?;
...
async fn run_foreground_remote_control(
    ...
    let runtime_options = AppServerRuntimeOptions {
        remote_control_startup_mode: antex_app_server::RemoteControlStartupMode::EnabledEphemeral,
        install_shutdown_signal_handler: false,
        ..Default::default()
    };
    let (stop_rx, stop_signal_task) = foreground_stop_signal();
    let mut app_server_task = tokio::spawn(antex_app_server::run_main_with_transport_options(
        arg0_paths,
...
async fn wait_for_foreground_remote_control_ready(
    ...
    antex_app_server_daemon::enable_remote_control_on_socket(
        socket_path.as_path(),
...
fn daemon_app_server_human_lines(output: &AppServerRemoteControlStartOutput) -> Vec<String> {
    let (managed_antex_path, managed_antex_version) = daemon_app_server_identity(output);
    vec![
        "Daemon used app-server:".to_string(),
        format!("  path: {}", managed_antex_path.display()),
        format!("  version: {}", managed_antex_version.unwrap_or("unknown")),
    ]
}
...
fn daemon_app_server_identity(
    ...
    match output {
        AppServerRemoteControlStartOutput::Bootstrap(output) => (
            &output.managed_antex_path,
            output.managed_antex_version.as_deref(),
        ),
    ...
    fn daemon_ready_output(
        ...
        AppServerRemoteControlReadyOutput {
            daemon: AppServerRemoteControlStartOutput::Start(AppServerLifecycleOutput {
                ...
                backend: None,
                pid: Some(42),
                managed_antex_path: PathBuf::from("/opt/codex/bin/codex"),
                managed_antex_version: Some("1.0.0".to_string()),
