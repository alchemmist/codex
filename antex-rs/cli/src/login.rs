...
//! interactive-session layers. Direct `antex login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `antex-login.log` artifact that
...
use antex_login::CLIENT_ID;
use antex_login::ServerOptions;
use antex_login::is_workload_identity_selected;
use antex_login::login_with_access_token;
use antex_login::login_with_api_key;
use antex_login::logout_with_revoke;
use antex_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `antex login` flows.
///
...
/// command produce a durable `antex-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match antex_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("antex-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("antex_cli=info,antex_core=info,antex_login=info"));
    ...
    // Direct `antex login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `antex login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | antex login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv ANTEX_ACCESS_TOKEN | antex login --with-access-token`.",
...
/// `antex login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    if is_workload_identity_selected() {
        match AuthManager::shared_from_config(&config, /*enable_antex_api_key_env*/ false).await {
            Ok(_) => {
    ...
    match auth_config
        .load_auth(/*enable_antex_api_key_env*/ false)
        .await
    {
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let codex_home = tempdir().expect("create temporary Antex home");
        login_with_api_key(
            codex_home.path(),
            "sk-existing",
