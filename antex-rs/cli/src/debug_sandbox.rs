...
#[cfg(target_os = "macos")]
use antex_core::spawn::CODEX_SANDBOX_ENV_VAR;
use antex_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
use antex_sandboxing::landlock::allow_network_for_proxy;
use antex_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use antex_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use antex_sandboxing::seatbelt::create_seatbelt_command_args;
...
pub async fn run_command_under_seatbelt(
    ...
    _antex_linux_sandbox_exe: Option<PathBuf>,
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<antex_mcp::SandboxState>)
        .transpose()
    ...
    let antex_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .antex_linux_sandbox_exe
                .clone()
                .or(antex_linux_sandbox_exe)
        }
        None => antex_linux_sandbox_exe,
    };
    ...
    let network = network_proxy
        .as_ref()
        .map(antex_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            // for the unsandboxed shell that resumes when Antex exits.
        ...
        SandboxType::Landlock => {
            ...
            let antex_linux_sandbox_exe = config
                .antex_linux_sandbox_exe
                .expect("antex-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use antex_windows_sandbox::WindowsSandboxProxySettingsMode;
    use antex_windows_sandbox::WindowsSandboxSessionRequest;
    use antex_windows_sandbox::resolve_windows_deny_read_paths;
    use antex_windows_sandbox::spawn_windows_sandbox_session_for_level;
    ...
    let exit_code = antex_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_antex_home(
        cli_overrides,
        antex_linux_sandbox_exe,
        options,
        /*codex_home*/ None,
...
async fn load_debug_sandbox_config_with_antex_home(
    cli_overrides: Vec<(String, TomlValue)>,
    antex_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    // For legacy configs, `antex sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            antex_linux_sandbox_exe: antex_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        codex_home.clone(),
        managed_requirements_mode,
...
"#;
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let antex_home_path = codex_home.path().to_path_buf();

        let profile_config = build_debug_sandbox_config(
            Vec::new(),
            ConfigOverrides::default(),
            Some(antex_home_path.clone()),
        ...
        let config = load_debug_sandbox_config_with_antex_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = antex_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
