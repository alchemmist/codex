...
use antex_core_plugins::ConfiguredMarketplace;
use antex_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use antex_core_plugins::PluginInstallOutcome;
...
use antex_core_plugins::allowed_configured_marketplace_names;
use antex_core_plugins::installed_marketplaces::marketplace_install_root;
use antex_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use antex_core_plugins::marketplace::MarketplaceListError;
use antex_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use antex_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use antex_core_plugins::marketplace::MarketplacePluginSource;
use antex_core_plugins::marketplace::find_marketplace_manifest_path;
use antex_core_plugins::remote;
...
use antex_core_plugins::remote::RemotePluginSummary;
use antex_login::AntexAuth;
use antex_login::AuthManager;
use antex_plugin::PluginId;
use antex_plugin::validate_plugin_segment;
...
pub(crate) async fn load_cli_auth_manager(config: &Config) -> Result<Arc<AuthManager>> {
    Ok(AuthManager::shared_from_config(config, /*enable_antex_api_key_env*/ true).await?)
}
...
async fn fetch_remote_marketplaces(
    ...
    let auth = context.auth.as_ref();
    if !auth.is_some_and(AntexAuth::uses_antex_backend) {
        ensure!(
