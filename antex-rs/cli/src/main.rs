use antex_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use antex_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use antex_app_server_daemon::RemoteControlMode as AppServerRemoteControlMode;
use antex_arg0::Arg0DispatchPaths;
use antex_arg0::arg0_dispatch_or_else;
use antex_chatgpt::apply_command::ApplyCommand;
use antex_chatgpt::apply_command::run_apply_command;
use antex_cli::read_access_token_from_stdin;
use antex_cli::read_api_key_from_stdin;
use antex_cli::run_login_status;
use antex_cli::run_login_with_access_token;
use antex_cli::run_login_with_api_key;
use antex_cli::run_login_with_chatgpt;
use antex_cli::run_login_with_device_code;
use antex_cli::run_logout;
use antex_cloud_config::cloud_config_bundle_loader_for_storage;
use antex_cloud_tasks::Cli as CloudTasksCli;
use antex_exec::Cli as ExecCli;
use antex_exec::Command as ExecCommand;
use antex_exec::ReviewArgs;
use antex_exec_server::ExecServerRuntimePaths;
use antex_execpolicy::ExecPolicyCheckCommand;
...
use antex_rollout_trace::REDUCED_STATE_FILE_NAME;
use antex_rollout_trace::replay_bundle;
use antex_state::StateRuntime;
use antex_tui::AppExitInfo;
use antex_tui::Cli as TuiCli;
use antex_tui::ExitReason;
use antex_tui::UpdateAction;
use antex_utils_absolute_path::AbsolutePathBuf;
use antex_utils_cli::CliConfigOverrides;
use antex_utils_cli::ProfileV2Name;
...
use antex_core::build_models_manager;
use antex_core::config::Config;
use antex_core::config::ConfigBuilder;
use antex_core::config::ConfigLoadOptions;
...
use antex_core::config::resolve_profile_v2_config_path;
use antex_features::FEATURES;
use antex_features::Stage;
use antex_features::is_known_feature_key;
use antex_home::AntexHomeUserInstructionsProvider;
use antex_login::AntexAuth;
use antex_login::AuthManager;
use antex_login::is_workload_identity_selected;
use antex_login::read_antex_access_token_from_env;
...
use antex_terminal_detection::TerminalName;
...
#[clap(
    ...
    // `antex-x86_64-unknown-linux-gnu`, but the help output should always use
    // the generic `antex` command name that users run.
    bin_name = "antex",
...
enum Subcommand {
    #[command(about = "Inspect or migrate existing Codex data into an independent Antex home")]
    Migrate(home_migration::MigrationCommand),
    ...
    /// Manage external MCP servers for Antex.
    Mcp(McpCli),
    ...
    /// Update Antex to the latest version.
    Update,

    /// Diagnose local Antex installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Antex-provided sandbox.
    ...
    /// Apply the latest diff produced by Antex agent as a `git apply` to your local working tree.
...
#[cfg(target_os = "macos")]
type HostSandboxArgs = antex_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type HostSandboxArgs = antex_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type HostSandboxArgs = antex_cli::WindowsCommand;
...
struct UnsupportedSandboxArgs {
    /// Layer $ANTEX_HOME/<name>.config.toml on top of the base user config.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | antex login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv ANTEX_ACCESS_TOKEN | antex login --with-access-token`)"
    )]
...
struct AppServerCommand {
    ...
    #[command(flatten)]
    code_mode_host: antex_app_server::AppServerCodeModeHostArgs,
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = antex_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: antex_app_server::AppServerTransport,
    ...
    #[command(flatten)]
    auth: antex_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    request_dispatch_mode: antex_exec_server::RequestDispatchMode,
    ...
    /// Use Agent Identity auth from ANTEX_ACCESS_TOKEN for remote registration.
    #[arg(
    ...
    #[arg(
        ...
        env = antex_exec_server::ANTEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Antex tooling.
    #[clap(hide = true)]
...
struct AppServerDaemonStartCommand {
    /// Antex executable used to launch app-server instead of the managed standalone install.
    #[arg(long, value_name = "PATH", value_parser = parse_socket_path)]
    antex_bin: Option<AbsolutePathBuf>,
}
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("\n🎉 Update ran successfully! Please restart Antex.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`antex update` is not available in debug builds. Install a release build of Antex to use this command."
        );
    }
...
async fn run_session_archive_cli_command(
    action: antex_tui::SessionArchiveAction,
    ...
    antex_tui::run_session_archive_command(
        action,
        target,
        antex_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<antex_tui::SessionArchiveAction> {
    if force && antex_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => antex_tui::DeleteConfirmation::Skip,
        false => antex_tui::DeleteConfirmation::Prompt,
    };
    Ok(antex_tui::SessionArchiveAction::Delete(confirmation))
}

async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let antex_bin = std::env::current_exe()?;
            antex_app_server_test_client::send_message_v2(&antex_bin, &[], cmd.user_message, &None)
                .await
        }
    }
}
...
fn main() -> anyhow::Result<()> {
    if home_migration::dispatch()?.is_some() {
        return Ok(());
    }
    home_migration::guard_initial_start()?;
    antex_build_info::initialize!();
    let remote_control_disabled = antex_app_server::take_remote_control_disabled_env();
...
async fn cli_main(
    ...
    {
        anyhow::bail!("`antex agents` received conflicting remote server endpoints");
    }
    ...
    match subcommand {
        Some(Subcommand::Migrate(_)) => {
            anyhow::bail!("migration must run before runtime initialization")
        }
        None | Some(Subcommand::Agents(_)) => {
            ...
            if open_agents_overview {
                if interactive.prompt.is_some() || !interactive.images.is_empty() {
                    anyhow::bail!("`antex agents` does not accept an initial prompt or images");
                }
                ...
                {
                    anyhow::bail!(
                        "`antex agents` cannot apply local provider or additional-directory overrides to a remote server"
                    );
                }
                if is_workload_identity_selected() {
                    anyhow::bail!(
                        "`antex agents` is unavailable while workload identity is active"
                    );
                }
                if root_remote.is_none() {
                    ...
                    anyhow::bail!("`antex agents` requires `--remote` on this platform");
                }
        ...
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["antex", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
            ...
            match subcommand {
                None => {
                    ...
                    let runtime_options = antex_app_server::AppServerRuntimeOptions {
                        ...
                        {
                            (true, _) => {
                                antex_app_server::RemoteControlStartupMode::EnabledEphemeral
                            }
                            (false, true) => {
                                antex_app_server::RemoteControlStartupMode::DisabledEphemeral
                            }
                            (false, false) => {
                                antex_app_server::RemoteControlStartupMode::ResolvePersisted
                            }
                        },
                        ..Default::default()
                    };
                    antex_app_server::run_main_with_transport_options(
                        ...
                        analytics_default_enabled,
                        transport,
                        antex_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    AppServerDaemonSubcommand::Start(start_cli) => {
                        if let Some(antex_bin) = start_cli.antex_bin {
                            let output =
                                antex_app_server_daemon::start_with_antex_bin(antex_bin.as_path())
                                    .await?;
                    ...
                    AppServerDaemonSubcommand::Bootstrap(bootstrap_cli) => {
                        let output =
                            antex_app_server_daemon::bootstrap(AppServerBootstrapOptions {
                ...
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            let codex_home = find_antex_home()?;
                            antex_app_server::app_server_control_socket_path(&codex_home)?
                        }
                    };
                    antex_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = antex_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    antex_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    antex_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    antex_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                antex_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                antex_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | antex login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            antex_cloud_tasks::run_main(cloud_cli, arg0_paths.antex_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            antex_cli::run_command_under_seatbelt(
                sandbox_cli,
                arg0_paths.antex_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "linux")]
            antex_cli::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.antex_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "windows")]
            antex_cli::run_command_under_windows_sandbox(
                sandbox_cli,
                arg0_paths.antex_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`antex sandbox` is not supported on this operating system");
            }
        }
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || antex_responses_api_proxy::run_main(args))
                .await??;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `antex mcp`: `antex`, `antex exec`, `antex review`, `antex resume`, `antex queue`, `antex archive`, `antex delete`, `antex unarchive`, `antex fork`, `antex mcp`, `antex sandbox`, and `antex debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let antex_self_exe = arg0_paths
        .antex_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Antex executable path is not configured"))?;
    let runtime_paths =
        ExecServerRuntimePaths::new(antex_self_exe, arg0_paths.antex_linux_sandbox_exe.clone())?;
    if let Some(base_url) = cmd.remote.take() {
        ...
        let auth_provider = if cmd.aws_sigv4 {
            exec_server_auth::aws_sigv4_auth_provider(antex_aws_auth::AwsAuthConfig {
        ...
        let mut remote_config = antex_exec_server::RemoteEnvironmentConfig::new_with_transport(
            base_url,
            environment_id,
            if direct_transport {
                antex_exec_server::RemoteEnvironmentTransport::Direct
            } else {
                antex_exec_server::RemoteEnvironmentTransport::Noise
            },
            auth_provider,
        ...
        #[cfg(target_os = "macos")]
        let runtime_paths = runtime_paths.with_allowed_symlinked_antex_home(
            antex_config::allowed_symlinked_antex_home(
            ...
            async move {
                ...
                match cmd.command {
                    Some(ExecServerSubcommand::Forward { connect }) => {
                        antex_exec_server::run_remote_environment_forward_until_shutdown(
                            remote_config,
                            connect,
                            shutdown,
                        )
                    ...
                    None => {
                        antex_exec_server::run_remote_environment_until_shutdown(
                            remote_config,
                            runtime_paths,
                            shutdown,
                        )
        ...
        #[cfg(target_os = "macos")]
        let runtime_paths =
            runtime_paths.with_allowed_symlinked_antex_home(config.as_ref().and_then(|config| {
                antex_config::allowed_symlinked_antex_home(
                    &config.config_layer_stack,
                    &config.codex_home,
                )
        ...
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| antex_exec_server::DEFAULT_LISTEN_URL.to_string());
...
async fn load_exec_server_remote_auth_provider(
    config: &antex_core::config::Config,
    base_url: &str,
    use_agent_identity_auth: bool,
) -> anyhow::Result<antex_api::SharedAuthProvider> {
    if use_agent_identity_auth {
        read_antex_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!("ANTEX_ACCESS_TOKEN is required when --use-agent-identity-auth is set")
        })?;
        let auth = AuthManager::shared_from_config(config, /*enable_antex_api_key_env*/ false)
            .await?
            .auth()
            .await
            .ok_or_else(|| anyhow::anyhow!("Agent Identity authentication is unavailable"))?;
        if !matches!(auth, AntexAuth::AgentIdentity(_)) {
            anyhow::bail!(
                "ANTEX_ACCESS_TOKEN did not provide permitted Agent Identity authentication"
            );
        }
        return Ok(antex_model_provider::auth_provider_from_auth(&auth));
    }

    let (auth_manager, auth) = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `antex login` or set ANTEX_API_KEY",
    )
    .await?;
    ...
    if auth_manager.is_workload_identity_selected() {
        Ok(antex_model_provider::auth_provider_from_auth_manager(
            auth_manager,
            &auth,
        ))
    } else {
        Ok(antex_model_provider::auth_provider_from_auth(&auth))
    }
}

fn is_supported_exec_server_remote_auth(auth: &AntexAuth) -> bool {
...
) -> anyhow::Result<antex_core::config::Config> {
...
) -> anyhow::Result<(Arc<AuthManager>, antex_login::AntexAuth)> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_antex_api_key_env*/ true).await?;
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let codex_home = find_antex_home()?;
            Ok(loader_overrides_for_profile_at_antex_home(
                Some(profile_v2),
                &codex_home,
            ))
        }
...
fn loader_overrides_for_profile_at_antex_home(
    profile_v2: Option<&ProfileV2Name>,
...
fn maybe_print_under_development_feature_warning(codex_home: &std::path::Path, feature: &str) {
    ...
    let config_path = codex_home.join(antex_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(antex_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        antex_self_exe: arg0_paths.antex_self_exe,
        antex_linux_sandbox_exe: arg0_paths.antex_linux_sandbox_exe,
    ...
    let user_instructions_provider = Arc::new(AntexHomeUserInstructionsProvider::new(
        config.codex_home.clone(),
    ));
    let auth_manager =
        AuthManager::shared_from_config(&config, /*enable_antex_api_key_env*/ false).await?;
    let mut extensions = antex_extension_api::ExtensionRegistryBuilder::new();
    antex_git_attribution::install(
        &mut extensions,
        auth_manager,
        config.chatgpt_base_url.clone(),
        config.http_client_factory(),
    );
    antex_skills_extension::install(&mut extensions, |config: &Config| {
        antex_skills_extension::SkillsExtensionConfig {
            ...
            shadow_selection_enabled: config
                .features
                .enabled(antex_features::Feature::SkillSearch),
        }
    });
    let prompt_input = antex_core::build_prompt_input(
        config,
        input,
        /*state_db*/ None,
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `antex {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `antex {subcommand}`"
        );
    }
    Ok(())
}
    ...
    match subcommand {
        Some(Subcommand::Exec(command)) => match &command.command {
            ...
            Some(ExecCommand::Resume(_)) => anyhow::bail!(
                "`--worktree` cannot resume an existing session; use `antex exec fork --worktree`"
            ),
...
/// `antex --strict-config` continue to work for the TUI and for wrappers that
...
fn unsupported_subcommand_name_for_strict_config(
    ...
    match subcommand {
        ...
        Some(Subcommand::Migrate(_)) => Some("migrate"),
        #[cfg(any(target_os = "macos", target_os = "windows"))]
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `antex {subcommand}`");
    }
    Ok(())
}
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = antex_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}

fn updater_http_client_factory(
    config: anyhow::Result<antex_core::config::Config>,
) -> antex_http_client::HttpClientFactory {
    match config {
        Ok(config) => config.http_client_factory(),
        Err(error) => {
            eprintln!("warning: failed to load updater network configuration: {error}");
            antex_http_client::HttpClientFactory::new(
                antex_http_client::OutboundProxyPolicy::ReqwestDefault,
            )
        }
    }
}
...
async fn print_app_server_remote_control_output(
    ...
    let output = antex_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = antex_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Antex's interactive TUI may not work in this terminal."
        );
    ...
    if interactive.agents_overview && remote.is_none() {
        ...
        antex_app_server_daemon::run(AppServerLifecycleCommand::Start)
            .await
            .map_err(std::io::Error::other)?;
    }
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Antex local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<antex_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(antex_tui::resolve_remote_addr)
        .transpose()
        .map_err(std::io::Error::other)?;
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !antex_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let antex_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
/// Build the final `TuiCli` for a `antex resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `antex` without additional flags.
...
/// Build the final `TuiCli` for a `antex fork` invocation.
...
mod tests {
    ...
    async fn updater_http_client_factory_honors_respect_system_proxy() {
        let codex_home = tempfile::tempdir().expect("temporary Antex home");
        let config = ConfigBuilder::default()
        ...
        assert_eq!(
            ...
            antex_http_client::OutboundProxyPolicy::RespectSystemProxy
        );
    }
    ...
    fn exec_server_remote_auth_accepts_api_key_auth() {
        let auth = AntexAuth::from_api_key("sk-test");
    ...
    #[test]
    fn profile_loader_overrides_use_explicit_antex_home() -> anyhow::Result<()> {
        let codex_home = tempfile::tempdir()?;
        let profile: ProfileV2Name = "work".parse()?;

        let overrides =
            loader_overrides_for_profile_at_antex_home(Some(&profile), codex_home.path());
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        ...
        let Some(antex_exec::Command::Resume(args)) = exec.command else {
    ...
    fn later_exec_sandbox_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            exec.sandbox_mode,
            Some(antex_utils_cli::SandboxModeCliArg::ReadOnly)
        );
        assert_eq!(
            exec.config_overrides.raw_overrides,
            vec![
    ...
    fn later_resume_approval_policy_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            ...
            Some(antex_utils_cli::ApprovalModeCliArg::Never)
        );
        assert_eq!(
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let codex_home = find_antex_home().expect("antex home");
        antex_app_server::app_server_control_socket_path(&codex_home)
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        ...
        for (subcommand, usage) in [
            ("add", "Usage: antex plugin marketplace add"),
            ("list", "Usage: antex plugin marketplace list"),
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: thread_id.map(|thread_id| antex_tui::ResumableThread {
                thread_id,
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "  antex resume 123e4567-e89b-12d3-a456-426614174000".to_string(),
            ]
        );
    }
        ...
        insta::allow_duplicates! {
            for thread_name in [None, Some("")] {
                ...
                To continue this session, run:
                  antex resume 123e4567-e89b-12d3-a456-426614174000
                ");
            }
        }
    }
    ...
    fn format_exit_messages_applies_color_when_enabled() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2",
                "To continue this session, run:",
                "  \u{1b}[36mantex resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        To continue this session, run:
          antex resume 123e4567-e89b-12d3-a456-426614174000
        Or run antex resume and select my-thread.
        ");
    }
    ...
    fn format_exit_messages_colors_commands_and_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run:",
                "  \u{1b}[36mantex resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
                "Or run \u{1b}[36mantex resume\u{1b}[39m and select \u{1b}[36mmy-thread\u{1b}[39m.",
            ]
        );
    }
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(antex_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(antex_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `antex remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `antex app-server proxy`"
        );
    }
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("ANTEX_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("ANTEX_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_listen_websocket_url_parses() {
        ...
        assert_eq!(
            app_server.listen,
            antex_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_daemon_subcommands_parse() {
        ...
        let local_antex = AbsolutePathBuf::from_absolute_path("/tmp/local-codex").unwrap();
        assert!(matches!(
            app_server_from_args(
                [
                    "codex",
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(antex_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(antex_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
