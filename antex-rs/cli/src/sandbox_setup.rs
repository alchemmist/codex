...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run Antex after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
    ...
    /// Use the current Windows user as the Antex user.
    #[arg(
        long = "current-user",
    ...
    /// ANTEX_HOME for the Antex user. Required with --user.
    #[arg(long = "antex-home", value_name = "DIR")]
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`antex sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        ...
        .loader_overrides(super::loader_overrides_for_profile_at_antex_home(
        ...
        .build()
        .await
        .context("failed to load target user's Antex config for sandbox provisioning")?;

    antex_core::windows_sandbox::run_elevated_provisioning_setup(
...
fn resolve_sandbox_setup_identity(
    ...
    if cmd.current_user {
        ...
        let codex_home = match cmd.codex_home.clone() {
            ...
            None => find_antex_home()?.to_path_buf(),
        };
    ...
    let codex_home = cmd
        .codex_home
        .clone()
        .ok_or_else(|| anyhow::anyhow!("--antex-home is required with --user"))?;
...
mod tests {
    ...
    fn parses_managed_user_identity() {
        ...
        assert_eq!(
            ...
            Some(std::path::Path::new(r"C:\Users\alice\.antex"))
        );
    }
    ...
    #[test]
    fn requires_antex_home_for_managed_user() {
        let err =
    ...
    fn parses_setup_from_sandbox_command_args() {
        let command = parse_setup_command(&[
            ...
            "--antex-home".to_string(),
            r"C:\Users\alice\.antex".to_string(),
        ])
        .expect("parse")
