...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.antex_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.antex_version);
...
Notes
   ...
   ✗ auth         token expired - Run `antex login`.
─────────────────────────────────────────────────────────────
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `antex login`.
-------------------------------------------------------------
...
",
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: OPENAI_API_KEY, ANTEX_API_KEY"),
            "auth env vars present: OPENAI_API_KEY, ANTEX_API_KEY"
        );
    }
