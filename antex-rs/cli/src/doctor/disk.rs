...
pub(super) fn check(config: Option<&Config>, cwd: &Path) -> DoctorCheck {
    let home = config
        ...
        .or_else(|| find_antex_home().ok().map(Into::into))
        .or_else(|| std::env::var_os("ANTEX_HOME").map(Into::into));
...
fn check_with_paths(
    ...
    for (label, path) in [("ANTEX_HOME", home.as_deref()), ("worktree", Some(cwd))] {
