...
fn disk_capacity_is_reported_without_loaded_configuration() {
    ...
    for label in ["ANTEX_HOME available: ", "worktree available: "] {
        assert!(
            check
                .details
                .iter()
...
fn disk_capacity_thresholds_produce_complete_diagnostics() {
    for (available, status, measured, summary) in [
        ...
        let issues = ["ANTEX_HOME", "worktree"]
            .into_iter()
            .filter(|_| status != "ok")
            .map(|label| {
                json!({
        ...
        assert_eq!(
            ...
            json!({
                ...
                "details": [
                    ...
                    format!("ANTEX_HOME available: {measured}"),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "details": [
                "warning threshold: 5.0 GiB",
                "failure threshold: 1.0 GiB",
                "ANTEX_HOME available: unavailable (PermissionDenied)",
...
#[test]
fn nonexistent_relative_antex_home_uses_current_directory_capacity() {
