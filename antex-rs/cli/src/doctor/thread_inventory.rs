...
use antex_state::ThreadStateAuditRow;
...
async fn thread_inventory_check_for_roots(
    ...
    let rows = match antex_state::read_thread_state_audit_rows(sqlite).await {
        Ok(rows) => rows,
        Err(err) => {
...
fn missing_state_db_check(scan: RolloutScan, details: Vec<String>) -> DoctorCheck {
    ...
    if !scan.files.is_empty() {
        check = check
            .issue(
                ...
                .remedy("Start Antex with no state DB present so startup backfill can create it from rollout files."),
        )
            .remediation(
                "Start Antex with no state DB present so startup backfill can create it from rollout files.",
            );
    }
    if !scan.scan_errors.is_empty() || !scan.malformed_names.is_empty() || scan.reached_scan_cap {
        check = check.issue(
            ...
            .remedy("Check file permissions and unexpected files under ANTEX_HOME sessions."),
        );
    }
    check
}
...
async fn scan_rollout_root(root: &Path, archived: bool, scan: &mut RolloutScan) {
    ...
    while let Some(dir) = dirs.pop() {
        ...
        for entry in entries {
            ...
            let logical_path = antex_rollout::plain_rollout_path(&path);
...
async fn thread_id_from_rollout(path: &Path) -> RolloutThreadId {
    let mut lines = match antex_rollout::open_rollout_line_reader(path).await {
        Ok(lines) => lines,
    ...
    for _ in 0..MAX_ROLLOUT_HEADER_LINES {
        ...
        if item_type == "session_meta" {
            return match antex_rollout::parse_rollout_line(line.trim()) {
        ...
        if !has_legacy_item {
            has_legacy_item = antex_rollout::parse_rollout_line(line.trim()).is_ok();
        }
    }
    ...
    let logical_path = antex_rollout::plain_rollout_path(path);
    antex_rollout::builder_from_items(&[], &logical_path)
...
fn rollout_path_key(path: &Path) -> PathBuf {
    path_key(&antex_rollout::plain_rollout_path(path))
}
...
mod tests {
    ...
    async fn thread_inventory_check_warns_for_missing_stale_and_mismatched_rows() {
        ...
        assert!(check.issues.iter().all(|issue| {
            !issue
                .remedy
                .as_deref()
                .is_some_and(|remedy| remedy.starts_with("Restart Antex"))
        }));
    ...
    async fn thread_inventory_check_uses_metadata_id_when_filename_and_db_disagree() {
        ...
        let mut rollout_line =
            antex_rollout::parse_rollout_line(contents.trim()).expect("rollout line");
    ...
    impl Fixture {
        async fn new() -> Self {
            let codex_home = TempDir::new().expect("antex home");
            let sqlite_home = TempDir::new().expect("sqlite home");
            let _runtime = antex_state::StateRuntime::init(
        ...
        fn sqlite(&self) -> antex_state::SqliteConfig {
            antex_state::SqliteConfig::new_for_testing(self.sqlite_home.path().abs())
        }
        ...
        fn write_rollout(&self, archived: bool, timestamp: &str, thread_id: &str) -> PathBuf {
            ...
            let rollout_line = RolloutLine {
                ...
                ordinal: None,
                item: RolloutItem::SessionMeta(antex_protocol::protocol::SessionMetaLine {
