...
pub(super) fn endpoint_check(inspection: EndpointInspection) -> DoctorCheck {
    ...
    let targets = if cfg!(target_os = "windows") {
        "signed Codex app; codex.exe; antex-windows-sandbox-setup.exe; antex-command-runner.exe; antex-code-mode-host.exe"
    ...
    let mut check = DoctorCheck::new(
        ...
        "security",
        CheckStatus::Warning,
        "endpoint protection detected; Antex exclusions are unverified",
    )
    .detail(format!("endpoint products: {}", products.join(", ")))
    .detail(format!("exclusion targets: {targets}"))
    .detail("Antex exclusions: not verified")
    .remediation(
        "ask your security administrator to verify Antex exclusions and required helper allowances",
    );
    ...
    for product in products {
        let remedy = match product {
            "CrowdStrike Falcon" => {
                "CrowdStrike Falcon: Add a certificate or IOA exclusion for Antex. If sensor overhead continues, exclude the Antex agent from sensor visibility. Keep monitoring descendant processes."
            }
            "BeyondTrust Privilege Management" => {
                "BeyondTrust: Remove Antex from application blocking rules. Add allow rules for Antex helper executables. Do not grant administrator privileges."
            }
            "Microsoft Defender" => {
                "Microsoft Defender: Add a certificate or executable-path exclusion for Antex and its helpers. If Attack Surface Reduction blocks Antex, add a rule exclusion. If Controlled Folder Access blocks Antex, allow the app."
            }
            "SentinelOne" => {
                "SentinelOne: Add a signer, file-hash, or executable-path exclusion for Antex and its helpers."
            }
            "Jamf Protect" => {
            ...
            _ => {
                "Add an exclusion for Antex and its helpers. Use the endpoint product instructions."
            }
        };
        check = check.issue(
            DoctorIssue::new(
                CheckStatus::Warning,
                format!("{product} can interfere with Antex. Verify Antex exclusions."),
            )
            .measured("not verified")
            .expected("Codex application and helper exclusions")
            .remedy(remedy)
            .field("Antex exclusions"),
        );
    }
