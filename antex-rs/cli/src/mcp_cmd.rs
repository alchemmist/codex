...
use antex_core::config::load_global_mcp_servers;
use antex_core::plugins_manager_for_config;
use antex_exec_server::EnvironmentManager;
use antex_exec_server::HttpClient;
use antex_exec_server::RouteAwareHttpClient;
use antex_login::AuthManager;
use antex_mcp::McpOAuthLoginSupport;
use antex_mcp::McpRuntimeContext;
use antex_mcp::ResolvedMcpOAuthScopes;
use antex_mcp::apply_http_headers_helper;
use antex_mcp::compute_auth_statuses;
...
use antex_rmcp_client::McpOAuthCallbackMode;
...
use antex_rmcp_client::delete_oauth_tokens;
use antex_rmcp_client::perform_oauth_login;
use antex_rmcp_client::resolve_mcp_oauth_callback_url;
use antex_utils_cli::CliConfigOverrides;
use antex_utils_cli::format_env_display;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
use anyhow::bail;
...
/// - `add`    — add a server launcher entry to `~/.antex/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "antex mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: antex_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: antex_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    match login_support {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `antex mcp login {name}` to login."
        ),
    }
...
async fn run_list(config: &Config, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `antex mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config: &Config, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: antex mcp remove {}", get_args.name);
