...
const ANTEX_BUNDLE_IDENTIFIER: &str = "com.openai.codex";
const OPENAI_APPLE_TEAM_IDENTIFIER: &str = "2DC432GLL2";
const ANTEX_DMG_URL_ARM64: &str = "https://persistent.oaistatic.com/codex-app-prod/Codex.dmg";
const ANTEX_DMG_URL_X64: &str =
...
pub async fn run_mac_app_open_or_install(
    ...
    if let Some(app_path) = find_existing_antex_app_path(&antex_app_search_dirs()) {
        eprintln!(
            "Opening Desktop app at {app_path}...",
            app_path = app_path.display()
        );
        open_antex_app(&app_path, &workspace).await?;
        return Ok(());
    }
    ...
    let installed_app = download_and_install_antex_to_user_applications(&download_url)
        .await
    ...
    open_antex_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_antex_app_path(applications_dirs: &[PathBuf]) -> Option<PathBuf> {
    applications_dirs
        .iter()
        .flat_map(|dir| ["ChatGPT.app", "Codex.app"].map(|app_name| dir.join(app_name)))
        .find(|candidate| is_antex_app_bundle(candidate))
}

fn antex_app_search_dirs() -> Vec<PathBuf> {
...
fn is_antex_app_bundle(app_path: &Path) -> bool {
    if !app_path.is_dir() {
        return false;
    }
    ...
    std::process::Command::new("/usr/bin/plutil")
        ...
        .is_ok_and(|output| {
            output.status.success()
                && String::from_utf8_lossy(&output.stdout).trim() == ANTEX_BUNDLE_IDENTIFIER
        })
}

async fn open_antex_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    verify_antex_app_bundle(app_path).await?;
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = antex_new_thread_url(workspace);
...
async fn verify_antex_app_bundle(app_path: &Path) -> anyhow::Result<()> {
    let requirement = format!(
        "identifier \"{ANTEX_BUNDLE_IDENTIFIER}\" and anchor apple generic and certificate leaf[subject.OU] = \"{OPENAI_APPLE_TEAM_IDENTIFIER}\""
    );
    ...
    anyhow::bail!(
        "Desktop app at {} failed OpenAI signature verification (team {OPENAI_APPLE_TEAM_IDENTIFIER}, bundle {ANTEX_BUNDLE_IDENTIFIER}): {}",
        app_path.display(),
        String::from_utf8_lossy(&output.stderr).trim()
    );
}

fn antex_new_thread_url(workspace: &Path) -> String {
...
async fn download_and_install_antex_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    ...
    let result = async {
        let app_in_volume = find_antex_app_in_mount(&mount_point)
            .context("failed to locate Codex.app in mounted dmg")?;
        verify_antex_app_bundle(&app_in_volume)
            .await
            .context("refusing to install an unverified Desktop app")?;
        install_antex_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_antex_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
...
fn find_antex_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
...
#[cfg(test)]
mod tests {
    use super::antex_new_thread_url;
    use super::find_existing_antex_app_path;
    use super::open_antex_app;
    use super::parse_hdiutil_attach_mount_point;
    use super::verify_antex_app_bundle;
    ...
    fn finds_chatgpt_app_with_codex_bundle_identifier() {
        ...
        assert_eq!(
            find_existing_antex_app_path(&[temp_dir.path().to_path_buf()]),
            Some(app_path)
        );
    }
    ...
    fn ignores_classic_chatgpt_app() {
        ...
        let antex_app_path = temp_dir.path().join("Codex.app");
        write_app_bundle(&antex_app_path, "com.openai.codex");
    ...
    #[tokio::test]
    async fn rejects_unsigned_app_with_antex_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("Codex.app");
        write_app_bundle(&app_path, "com.openai.codex");

        let err = verify_antex_app_bundle(&app_path)
            .await
    ...
    #[tokio::test]
    async fn refuses_to_launch_unsigned_existing_antex_app() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("Codex.app");
        write_app_bundle(&app_path, "com.openai.codex");

        let err = open_antex_app(&app_path, temp_dir.path())
            .await
    ...
    #[test]
    fn antex_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&antex_new_thread_url(Path::new("/tmp/codex workspace/#1")))
