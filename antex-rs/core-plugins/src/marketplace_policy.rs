...
use antex_config::MarketplaceAllowedSourceKind;
use antex_config::MarketplaceAllowedSourceToml;
use antex_config::RequirementSource;
use antex_config::types::MarketplaceConfig;
use antex_config::types::MarketplaceSourceType;
use antex_config::types::PluginConfig;
use antex_plugin::PluginId;
use antex_utils_absolute_path::AbsolutePathBuf;
use antex_utils_path::paths_match_after_normalization;
use regex::Regex;
...
fn is_expected_managed_path(path: &Path, expected: &Path) -> bool {
    ...
    #[cfg(windows)]
    {
        ...
        let path = antex_utils_absolute_path::normalize_windows_device_path(&path)
            .unwrap_or_else(|| path.into_owned());
        let expected = antex_utils_absolute_path::normalize_windows_device_path(&expected)
