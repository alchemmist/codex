...
fn catalog_cache_paths_are_isolated_by_scope_and_collection() {
    let codex_home = Path::new("/tmp/antex-home");
...
fn global_catalog_cache_reuses_legacy_cache_file() {
    ...
    let auth = AntexAuth::Headers(AuthHeaders::new(http::HeaderMap::new()));
    ...
    antex_utils_path::write_atomically(&legacy_cache_path, &contents).expect("write legacy cache");
...
fn header_auth_does_not_cache_private_catalogs_without_a_stable_identity() {
    ...
    for scope in [RemotePluginScope::User, RemotePluginScope::Workspace] {
        ...
        antex_utils_path::write_atomically(&insecure_cache_path, &contents)
