...
fn archive_plugin_for_upload_places_manifest_at_archive_root() {
    ...
    assert_eq!(
        ...
        vec![
            ".antex-plugin/plugin.json".to_string(),
...
fn archive_plugin_for_upload_round_trips_through_plugin_bundle_archive_with_long_paths() {
    ...
    assert_eq!(
        fs::read_to_string(destination.path().join(".antex-plugin/plugin.json")).unwrap(),
        r#"{"name":"demo-plugin"}"#
    );
    assert_eq!(
