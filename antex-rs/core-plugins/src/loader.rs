...
use antex_config::types::PluginMcpServerConfig;
use antex_connectors::parse_plugin_app_config;
use antex_connectors::parse_plugin_app_config_value;
use antex_mcp::parse_agent_plugin_mcp_config;
use antex_mcp::parse_plugin_mcp_config;
use antex_plugin::AppDeclaration;
use antex_plugin::LoadedPlugin;
...
fn configured_plugins_from_antex_home(
    codex_home: &Path,
...
pub fn configured_curated_plugin_ids_from_antex_home(codex_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_antex_home(
        codex_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".antex-plugin/plugin.json"));
...
pub(crate) async fn load_plugin_mcp_servers_from_manifest_with_format(
    ...
    if manifest_format == PluginManifestFormat::AgentPlugin {
        agent_plugin_mcp_overlay::apply_antex_env_overlay(plugin_root, &mut mcp_servers).await;
    }
