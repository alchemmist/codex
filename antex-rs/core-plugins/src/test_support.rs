...
use antex_login::auth::BedrockAccessKeysAuth;
...
use antex_skills::LoadedSkillRoot;
use antex_skills::LoadedSkills;
use antex_skills::SkillError;
use antex_skills::SkillLoadFuture;
use antex_skills::SkillMetadata;
use antex_skills::SkillRootLoadRequest;
use antex_skills::SkillRootLoader;
use antex_skills::parse_skill_frontmatter_metadata;
...
use antex_utils_plugins::migrated_command_skills_root;
use http::Method;
use toml::Value;
...
pub(crate) fn test_auth_manager(auth_mode: Option<AuthMode>) -> Arc<AuthManager> {
    auth_manager_from_optional_auth(test_antex_auth(auth_mode))
}

pub(crate) async fn set_test_auth_mode(auth_manager: &AuthManager, auth_mode: Option<AuthMode>) {
    set_test_auth(auth_manager, test_antex_auth(auth_mode)).await;
}

pub(crate) async fn set_test_auth(auth_manager: &AuthManager, auth: Option<AntexAuth>) {
...
fn test_antex_auth(auth_mode: Option<AuthMode>) -> Option<AntexAuth> {
    auth_mode.map(|auth_mode| match auth_mode {
        AuthMode::ApiKey => AntexAuth::from_api_key("test-api-key"),
        AuthMode::Chatgpt => AntexAuth::create_dummy_chatgpt_auth_for_testing(),
        AuthMode::ChatgptAuthTokens => AntexAuth::from_external_chatgpt_tokens(
            "header.e30.test",
            "test-account",
            /*chatgpt_plan_type*/ None,
        )
        .expect("test ChatGPT tokens should parse"),
        AuthMode::Headers => AntexAuth::Headers(AuthHeaders::new(http::HeaderMap::new())),
        AuthMode::BedrockApiKey => AntexAuth::BedrockApiKey(BedrockApiKeyAuth {
            api_key: "test-api-key".to_string(),
            region: "us-east-1".to_string(),
        }),
        AuthMode::BedrockAccessKeys => AntexAuth::BedrockAccessKeys(BedrockAccessKeysAuth {
