...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".antex-plugin")).unwrap();
...
fn remote_installed_plugin_in_marketplace(
    ...
    RemoteInstalledPlugin {
        ...
        installed_at: None,
        enabled: true,
        install_policy: antex_app_server_protocol::PluginInstallPolicy::Available,
        install_policy_source: None,
        must_show_installation_interstitial: None,
        auth_policy: antex_app_server_protocol::PluginAuthPolicy::OnUse,
        availability: antex_app_server_protocol::PluginAvailability::Available,
...
async fn build_remote_installed_plugin_marketplaces_from_cache_uses_remote_metadata() {
    ...
    plugin.install_policy = antex_app_server_protocol::PluginInstallPolicy::InstalledByDefault;
    plugin.auth_policy = antex_app_server_protocol::PluginAuthPolicy::OnInstall;
    plugin.interface = Some(antex_app_server_protocol::PluginInterface {
    ...
    assert_eq!(
        ...
        antex_app_server_protocol::PluginInstallPolicy::InstalledByDefault
    );
    assert_eq!(
        plugin.auth_policy,
        antex_app_server_protocol::PluginAuthPolicy::OnInstall
    );
...
async fn install_plugin_materializes_default_command_skills() {
    ...
    write_file(
        &source_root.join(".antex-plugin/plugin.json"),
        r#"{
  "name": "sample",
  "skills": "./custom-skills/"
    ...
    write_file(
        &source_root.join(".antex-plugin/migrated-command-skills/undeclared-command/SKILL.md"),
    ...
    let migrated_skill = result
        .installed_path
        .join(".antex-plugin/migrated-command-skills/source-command-pr-review/SKILL.md");
    ...
    assert!(
        !result
            .installed_path
            .join(".antex-plugin/migrated-command-skills/undeclared-command")
            .exists()
    );
    assert!(
        !result
            .installed_path
            .join(".antex-plugin/migrated-command-skills/source-command-oversized")
            .exists()
    );
        ...
        vec![
            ...
            AbsolutePathBuf::from_absolute_path_checked(
                fs::canonicalize(result.installed_path.join(
                    ".antex-plugin/migrated-command-skills/source-command-summarize/SKILL.md"
                ))
                .unwrap()
            )
            .unwrap()
...
fn install_plugin_ignores_invalid_commands_manifest_field() {
    ...
    assert!(
        !result
            .installed_path
            .join(".antex-plugin/migrated-command-skills")
            .exists()
    );
}
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    ...
    write_file(
        &plugin_a_root.join(".antex-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".antex-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
...
async fn plugins_for_config_discards_in_flight_load_after_account_change() {
    ...
    let account_b = AntexAuth::from_external_chatgpt_tokens(
        "header.e30.other",
        "other-account",
...
async fn install_plugin_writes_marketplace_manifest_fallback_when_missing_plugin_json() {
  ...
  "plugins": [
    ...
    assert!(!plugin_root.join(".antex-plugin/plugin.json").exists());
    assert!(
        !tmp.path()
    ...
    let fallback_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(installed_path.join(".antex-plugin/plugin.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        fs::read_to_string(
            installed_path
                .join(".antex-plugin/migrated-command-skills/source-command-review/SKILL.md")
        )
        .unwrap(),
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".antex-plugin/plugin.json").is_file());
}
...
#[tokio::test]
async fn read_plugin_for_config_filters_mcp_servers_for_antex_backend_auth() {
    let tmp = tempfile::tempdir().unwrap();
    let repo_root = tmp.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).unwrap();
    fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
  ...
  "plugins": [
    ...
    write_file(
        &repo_root.join("sample-plugin/.antex-plugin/plugin.json"),
...
async fn read_plugin_for_config_does_not_fallback_from_invalid_plugin_manifest() {
  ...
  "plugins": [
    ...
    write_file(&plugin_root.join(".antex-plugin/plugin.json"), "{");
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
"#,
    ...
    write_file(
        &repo_root.join(".antex/config.toml"),
        r#"[[skills.config]]
...
async fn read_plugin_for_config_installed_git_source_reads_from_cache_without_cloning() {
  ...
  "plugins": [
    ...
    write_file(
        &cached_plugin_root.join(".antex-plugin/plugin.json"),
        r#"{
  "name": "toolkit",
...
async fn configured_marketplace_upgrade_invalidates_cached_tool_suggest_metadata() {
  ...
  "plugins": [
    ...
    write_file(
        &remote_repo.join("plugins/sample/.antex-plugin/plugin.json"),
...
"#,
    ...
    let featured_plugin_ids = manager
        .featured_plugin_ids_for_config(
            &config,
            Some(&AntexAuth::create_dummy_chatgpt_auth_for_testing()),
        )
        .await
        .unwrap();
...
#[tokio::test]
async fn featured_plugin_ids_for_config_defaults_query_param_to_antex() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["antex-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["antex-plugin".to_string()]);
}
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_antex_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_antex_home(tmp.path())
            .into_iter()
...
async fn load_plugins_uses_project_config_files() {
    ...
    write_file(
        &project_root.join(".antex/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_antex_folder: AbsolutePathBuf::try_from(project_root.join(".antex")).unwrap(),
            },
