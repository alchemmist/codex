...
#[tokio::test]
async fn returns_fallback_plugins_when_remote_disabled_for_antex_auth() {
...
async fn returns_api_curated_fallback_plugins_for_direct_provider_auth() {
    ...
    let auth = AntexAuth::from_api_key("test-api-key");
    let discoverable_plugins = list_discoverable_plugins(
        &plugins_manager,
        discovery_input(plugins, &[], &[], &[]),
        Some(&auth),
...
#[tokio::test]
async fn omits_openai_curated_but_keeps_configured_marketplaces_for_remote_antex_auth() {
    let codex_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(codex_home.path());
...
async fn clear_cache_invalidates_cached_tool_suggest_metadata() {
    ...
    let curated_root = curated_plugins_repo_path(codex_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    let plugin_manifest = curated_root.join("plugins/slack/.antex-plugin/plugin.json");
    write_file(
        &plugin_manifest,
        r#"{
  "name": "slack",
...
async fn normalizes_description() {
    ...
    write_openai_curated_marketplace(&curated_root, &["installed", "slack"]);
    write_file(
        &curated_root.join("plugins/slack/.antex-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["gmail", "openai-developers"] {
        write_file(
            &curated_root.join(format!("plugins/{plugin_name}/.antex-plugin/plugin.json")),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(logs.matches("gmail/.antex-plugin/plugin.json").count(), 4);
    assert_eq!(
        logs.matches("openai-developers/.antex-plugin/plugin.json")
            .count(),
        4
    );
}
