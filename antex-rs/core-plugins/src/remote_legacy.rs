...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    if let Some(auth) = auth.filter(|auth| auth.uses_antex_backend()) {
        request =
            request.headers(antex_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
fn ensure_antex_backend_auth(
    auth: Option<&AntexAuth>,
) -> Result<&AntexAuth, RemotePluginMutationError> {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_antex_backend_auth(auth)?;
