...
use antex_plugin::PluginLoadOutcome;
...
fn resolves_primary_runtime_scripts_from_the_installed_plugin_cache() {
    ...
    fs::create_dir_all(source_root.join(".antex-plugin")).expect("create manifest directory");
    fs::write(
...
fn recognizes_windows_executor_plugin_cache_root() {
    ...
    let script = PathUri::parse(
        "file:///C:/Users/user/.antex/plugins/cache/openai-primary-runtime/presentations/0.1.29/skills/presentations/container_tools/mark_artifact_operation_started.mjs",
    )
...
fn only_emits_safe_normalized_relative_script_paths() {
    ...
    assert_eq!(
        normalized_relative_script_path(Path::new(
            "/home/user/.antex/plugins/cache/openai-curated/sample/scripts/run.py"
        )),
        None
    );
}
