...
use antex_exec_server::DiscoveredPluginFiles;
...
fn expected_source(index: usize) -> ExecutorPluginHookSource {
    ExecutorPluginHookSource {
        ...
        manifest_path: PathUri::parse("file:///plugins/computer-use/.antex-plugin/plugin.json")
            .expect("manifest path"),
        source_relative_path: format!(".antex-plugin/plugin.json#hooks[{index}]"),
...
fn discovers_allowlisted_executor_plugin_hook_sources() {
    ...
    let snapshot = snapshot_for_manifest(
        ...
        "executor-a",
        "file:///plugins/computer-use/.antex-plugin/plugin.json",
        manifest,
    );
