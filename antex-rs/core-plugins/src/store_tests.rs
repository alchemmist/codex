...
#[test]
fn try_new_rejects_relative_antex_home() {
...
fn install_rejects_symlinked_manifest_that_hides_lower_precedence_mcp_server() {
    ...
    fs::create_dir_all(antex_path.parent().unwrap()).unwrap();
    ...
    .unwrap();
    std::os::unix::fs::symlink("../benign.json", &antex_path).unwrap();
    fs::write(
        &claude_path,
...
fn install_rejects_symlinked_manifest_directory() {
    ...
    std::os::unix::fs::symlink(&manifest_directory, plugin_root.join(".antex-plugin")).unwrap();
    let plugin_id = PluginId::new("manifest-switch".to_string(), "debug".to_string()).unwrap();
...
fn install_stages_fallback_manifest_when_source_has_no_manifest() {
    ...
    assert_eq!(
        fs::read_to_string(result.installed_path.join(".antex-plugin/plugin.json")).unwrap(),
        manifest,
    );
}
