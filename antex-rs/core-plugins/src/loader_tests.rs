...
use antex_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
...
#[tokio::test]
async fn agent_plugin_antex_mcp_overlay_only_forwards_matching_stdio_server_env_vars() {
    let temp_dir = TempDir::new().expect("tempdir");
    let plugin_root = temp_dir.path().join("plugin");
    write_file(
        &plugin_root.join("plugin.json"),
...
#[tokio::test]
async fn agent_plugin_antex_mcp_overlay_supports_inline_legacy_servers_without_portable_env() {
    let temp_dir = TempDir::new().expect("tempdir");
    let plugin_root = temp_dir.path().join("plugin");
    write_file(
        &plugin_root.join("plugin.json"),
...
async fn legacy_manifest_can_point_at_root_mcp_json() {
    let temp_dir = TempDir::new().expect("tempdir");
    let plugin_root = temp_dir.path().join("plugin");
    fs::create_dir_all(plugin_root.join(".antex-plugin")).expect("create manifest directory");
    fs::write(
        plugin_root.join(".antex-plugin/plugin.json"),
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_antex_folder: user_config_path(&temp_dir, "project/.antex"),
                },
                toml::from_str(
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_antex_folder: user_config_path(&temp_dir, "project/untrusted/.antex"),
                },
...
async fn hooks_only_scope_shares_plugin_resolution_without_loading_other_capabilities() {
  ...
  "hooks": {
    ...
    write_file(
        &disabled_root.join(".antex-plugin/plugin.json"),
        r#"{"name":"disabled"}"#,
    );
    write_file(
    ...
    write_file(
        &malformed_root.join(".antex-plugin/plugin.json"),
        "not valid json",
    );

    let warning_root = temp_dir.path().join("plugins/cache/test/warning/local");
    write_file(
        &warning_root.join(".antex-plugin/plugin.json"),
        r#"{"name":"warning"}"#,
    );
...
fn write_manifest(plugin_root: &AbsolutePathBuf, manifest: &str) {
    fs::write(plugin_root.join(".antex-plugin/plugin.json"), manifest).expect("write manifest");
}
...
#[test]
fn external_codex_plugin_manifest_remains_loadable() {
    let temp = TempDir::new().expect("plugin directory");
    let root = temp.path();
    let manifest =
        r#"{"name":"external-plugin","version":"1.0.0","description":"External plugin"}"#;
    write_file(&root.join(".codex-plugin/plugin.json"), manifest);
    let external = load_plugin_manifest(root).expect("external manifest");
    write_file(&root.join(".antex-plugin/plugin.json"), manifest);
    let native = load_plugin_manifest(root).expect("native manifest");
    assert_eq!(external, native);
}
