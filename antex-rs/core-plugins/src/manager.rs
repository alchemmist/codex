...
use crate::loader::configured_curated_plugin_ids_from_antex_home;
...
use antex_config::clear_user_plugin;
use antex_config::set_user_plugin_enabled;
use antex_config::skill_config_rules_from_stack;
use antex_config::types::PluginConfig;
use antex_config::types::ToolSuggestDisabledTool;
use antex_config::types::ToolSuggestDiscoverableType;
use antex_hooks::plugin_hook_declarations;
...
use antex_plugin::PluginIdError;
use antex_plugin::PluginTelemetryMetadata;
use antex_plugin::app_connector_ids_from_declarations;
use antex_plugin::prompt_safe_plugin_description;
...
use antex_skills::SkillRootLoader;
use antex_skills::SkillRootSnapshots;
...
use antex_utils_plugins::PluginIdentity;
use antex_utils_plugins::PluginSkillRoot;
...
impl RemoteInstalledPluginsAuthIdentity {
    fn from_auth(auth: Option<&AntexAuth>) -> Self {
        Self {
            auth_mode: auth.map(AntexAuth::api_auth_mode),
            account_id: auth.and_then(AntexAuth::get_account_id),
            chatgpt_user_id: auth.and_then(AntexAuth::get_chatgpt_user_id),
            is_workspace_account: auth.map(AntexAuth::is_workspace_account),
        }
    }
}
...
impl RemoteCatalogCacheRefreshRequest {
    fn has_same_cache_identity(&self, other: &Self) -> bool {
        self.service_config == other.service_config
            && self.auth.as_ref().and_then(AntexAuth::get_account_id)
                == other.auth.as_ref().and_then(AntexAuth::get_account_id)
            && self.auth.as_ref().and_then(AntexAuth::get_chatgpt_user_id)
                == other.auth.as_ref().and_then(AntexAuth::get_chatgpt_user_id)
            && self.auth.as_ref().map(AntexAuth::is_workspace_account)
                == other.auth.as_ref().map(AntexAuth::is_workspace_account)
    }
}
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        ...
        account_id: auth.and_then(AntexAuth::get_account_id),
        chatgpt_user_id: auth.and_then(AntexAuth::get_chatgpt_user_id),
        is_workspace_account: auth.is_some_and(AntexAuth::is_workspace_account),
    }
}
...
fn target_curated_marketplace(auth_mode: Option<AuthMode>) -> TargetCuratedMarketplace {
    if auth_mode.is_some_and(AuthMode::uses_antex_backend) {
...
impl PluginsManager {
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given ANTEX_HOME:
        ...
        // contents of that ANTEX_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single ANTEX_HOME is only used by one product.
    ...
    fn remote_global_catalog_active(&self, config: &PluginsConfigInput) -> bool {
        config.remote_plugin_enabled && self.auth_mode().is_some_and(AuthMode::uses_antex_backend)
    }
    ...
    pub fn cached_global_remote_discoverable_plugins_for_config(
        ...
        let Some(auth) = auth.filter(|auth| auth.uses_antex_backend()) else {
            return Vec::new();
        };
    ...
    fn begin_remote_installed_plugins_reconcile(&self, auth: Option<&AntexAuth>) -> Option<u64> {
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let loaded_manifest =
            if antex_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
                ...
                match sync_openai_plugins_repo(codex_home.as_path(), http_client_factory) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_antex_home(codex_home.as_path());
