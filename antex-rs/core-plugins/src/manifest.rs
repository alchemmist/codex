use antex_config::HooksFile;
use antex_utils_absolute_path::AbsolutePathBuf;
use antex_utils_path_uri::PathConvention;
use antex_utils_path_uri::PathUri;
use antex_utils_plugins::AGENT_PLUGIN_MANIFEST_RELATIVE_PATH;
use antex_utils_plugins::find_plugin_manifest_path;
...
pub type PluginManifest = antex_plugin::manifest::PluginManifest<AbsolutePathBuf>;
pub type PluginManifestHooks = antex_plugin::manifest::PluginManifestHooks<AbsolutePathBuf>;
pub type PluginManifestInterface = antex_plugin::manifest::PluginManifestInterface<AbsolutePathBuf>;
pub type PluginManifestMcpServers =
    antex_plugin::manifest::PluginManifestMcpServers<AbsolutePathBuf>;
pub type PluginManifestPaths = antex_plugin::manifest::PluginManifestPaths<AbsolutePathBuf>;

pub type UriPluginManifest = antex_plugin::manifest::PluginManifest<PathUri>;
...
pub(crate) fn load_plugin_manifest_with_format(plugin_root: &Path) -> Option<LoadedPluginManifest> {
    ...
    let overlay = if is_agent_plugin {
        let native_overlay = plugin_root.join(".antex-plugin/plugin.json");
        let overlay_path = if native_overlay.exists() {
            native_overlay
        } else {
...
fn resolve_raw_plugin_manifest(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = antex_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(antex_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: antex_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<antex_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| antex_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        RawPluginManifestHooks::Paths(paths) => {
            let hooks = paths
                ...
                .collect::<Vec<_>>();
            (!hooks.is_empty()).then_some(antex_plugin::manifest::PluginManifestHooks::Paths(hooks))
        }
        RawPluginManifestHooks::Inline(hooks) => {
            Some(antex_plugin::manifest::PluginManifestHooks::Inline(vec![
                *hooks,
            ]))
        }
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty())
            .then_some(antex_plugin::manifest::PluginManifestHooks::Inline(hooks)),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<antex_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(antex_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => Some(antex_plugin::manifest::PluginManifestMcpServers::Object(
                servers,
            )),
            Err(err) => {
...
mod tests {
    ...
    use antex_plugin::PluginProvider;
    use antex_plugin::ResolvedPlugin;
    use antex_plugin::manifest::PluginManifest as GenericPluginManifest;
    use antex_plugin::manifest::PluginManifestHooks;
    use antex_plugin::manifest::PluginManifestInterface;
    use antex_plugin::manifest::PluginManifestMcpServers;
    use antex_plugin::manifest::PluginManifestPaths;
    ...
    fn write_manifest(plugin_root: &Path, version: Option<&str>, interface: &str) {
        fs::create_dir_all(plugin_root.join(".antex-plugin")).expect("create manifest dir");
        let version = version
  ...
  "interface": {{
    ...
    "composerIcon": "./assets/icon.svg"
        ...
        let manifest_contents =
            fs::read_to_string(plugin_root.join(".antex-plugin/plugin.json")).expect("manifest");
