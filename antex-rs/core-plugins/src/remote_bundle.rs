...
use antex_http_client::HttpResponse;
...
use antex_utils_plugins::AgentPluginSchemaStatus;
use antex_utils_plugins::agent_plugin_schema_status;
...
pub enum RemotePluginBundleInstallError {
    ...
    DownloadBody {
        url: String,
        #[source]
        source: antex_http_client::HttpError,
    },
...
mod tests {
    ...
    fn find_extracted_plugin_root_uses_local_manifest_discovery() {
        ...
        std::fs::create_dir_all(extraction_root.path().join(".antex-plugin"))
            .expect("create manifest dir");
        std::fs::write(
            extraction_root.path().join(".antex-plugin/plugin.json"),
            r#"{"name":"linear"}"#,
        )
    ...
    fn find_extracted_plugin_root_rejects_nested_plugin_root() {
        ...
        std::fs::create_dir_all(plugin_root.join(".antex-plugin")).expect("create manifest dir");
        std::fs::write(
