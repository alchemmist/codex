...
targeted package runs such as `just argument-comment-lint -p antex-core`.
...
```bash
./tools/argument-comment-lint/run.py -p antex-core
```

Run the lint against `antex-rs` from the repo root:

```bash
just argument-comment-lint
bazel build --config=argument-comment-lint -- \
  $(./tools/argument-comment-lint/list-bazel-targets.sh)
./tools/argument-comment-lint/run-prebuilt-linter.py -p antex-core
just argument-comment-lint -p antex-core
```

If no package selection is provided, `just argument-comment-lint` now defaults
to the Bazel aspect path over `//antex-rs/...`. The Python wrappers remain the
...
```bash
./tools/argument-comment-lint/run-prebuilt-linter.py -p antex-core -- --all-targets
```
