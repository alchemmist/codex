...
Use `TestAntexBuilder::build_with_auto_env()` to opt-in to remote execution in core integration
...
`$ANTEX_HOME/environments.toml` or will define custom environments at runtime.
...
bash -c '
  ...
  cd antex-rs
  just test -p antex-core --test all
'
```
...
bash -c '
  ...
  cd antex-rs
  just test -p antex-app-server --test all
'
```
...
```sh
bazel test //antex-rs/app-server:app-server-all-wine-exec-test
```
