...
import { antexExecPath } from "./testAntex";
...
function conformanceDirectory(): string {
  ...
  for (const candidate of candidates) {
    if (existsSync(path.join(candidate, "run_antex_compliance.py"))) {
      return candidate;
    }
  }
...
describe("MCP client conformance", () => {
  it(
    ...
    () => {
      ...
      const antexBinary = requireFile(antexExecPath, "built Antex binary");
      ...
      const reportPath = path.join(reportDirectory, `antex-mcp-conformance-${process.pid}.json`);
      const result = spawnSync(
        "python3",
        [
          path.join(directory, "run_antex_compliance.py"),
          antexBinary,
          "--mode",
          "all",
          "--transport",
  ...
  it(
    ...
    () => {
      ...
      const reportDirectory = process.env.RUNNER_TEMP ?? os.tmpdir();
      const reportPath = path.join(reportDirectory, `antex-mcp-review-${process.pid}.json`);
      const result = spawnSync(
        "python3",
        [
          reviewer,
