...
import { Antex } from "../src/antex";
import type { AntexConfigObject } from "../src/antexOptions";

export const antexExecPath =
  process.env.ANTEX_EXEC_PATH ??
  path.join(process.cwd(), "..", "..", "antex-rs", "target", "debug", "antex");
...
export function createTestClient(options: CreateTestClientOptions = {}): TestClient {
  ...
  return {
    cleanup: () => {},
    client: new Antex({
      antexPathOverride: antexExecPath,
...
function mergeTestConfig(
  ...
  config: AntexConfigObject | undefined,
): AntexConfigObject | undefined {
  const mergedConfig: AntexConfigObject | undefined =
  ...
  return {
    ...
    // sync does not race temp ANTEX_HOME cleanup.
    features:
...
function hasExplicitProviderConfig(config: AntexConfigObject | undefined): boolean {
...
function getCurrentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "ANTEX_INTERNAL_ORIGINATOR_OVERRIDE") {
      continue;
    }
    if (value !== undefined) {
      env[key] = value;
