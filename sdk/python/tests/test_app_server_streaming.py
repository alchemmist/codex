...
from antex_sdk.generated.v2_all import (
...
def test_low_level_sync_stream_text_uses_real_turn_routing(tmp_path) -> None:
    """AntexClient.stream_text should stream through a real app-server turn."""
    with AppServerHarness(tmp_path) as harness:
        harness.responses.enqueue_sse(
        ...
        with Antex(config=harness.app_server_config()) as antex:
            thread = antex.thread_start()
            chunks = list(antex._client.stream_text(thread.id, "low-level sync"))  # noqa: SLF001
...
def test_low_level_async_stream_text_allows_parallel_model_list(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncAntex(config=harness.app_server_config()) as antex:
                thread = await antex.thread_start()
                stream = antex._client.stream_text(  # noqa: SLF001
                    thread.id,
                    "low-level async",
                )
                first = await anext(stream)
                models_task = asyncio.create_task(antex.models())
...
def test_interleaved_sync_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Antex(config=harness.app_server_config()) as antex:
            first_thread = antex.thread_start()
            second_thread = antex.thread_start()
...
def test_interleaved_async_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncAntex(config=harness.app_server_config()) as antex:
                first_thread = await antex.thread_start()
                second_thread = await antex.thread_start()
