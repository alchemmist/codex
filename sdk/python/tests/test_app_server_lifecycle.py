...
from antex_sdk import Antex, AsyncAntex
...
def test_sync_and_async_initialization_round_trip_metadata(tmp_path) -> None:
    ...
    async def async_scenario(harness: AppServerHarness) -> dict[str, object]:
        async with AsyncAntex(config=harness.app_server_config()) as antex:
            ...
            server = antex.metadata.serverInfo
            return {
                "thread_id": thread.id,
                "user_agent": antex.metadata.userAgent,
...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Antex(config=harness.app_server_config()) as antex:
            active_thread = antex.thread_start()
            archived_thread = antex.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            antex.thread_archive(archived_thread.id)
            active_list = antex.thread_list(archived=False)
            archived_list = antex.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncAntex(config=harness.app_server_config()) as antex:
                ...
                resumed = await antex.thread_resume(thread.id)
                forked = await antex.thread_fork(thread.id)
                archive_response = await antex.thread_archive(thread.id)
                unarchived = await antex.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Antex(config=harness.app_server_config()) as antex:
            ...
            forked = antex.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Antex(config=harness.app_server_config()) as antex:
            ...
            archived = antex.thread_archive(thread.id)
            unarchived = antex.thread_unarchive(thread.id)
...
def test_models_rpc(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        with Antex(config=harness.app_server_config()) as antex:
            models = antex.models(include_hidden=True)
