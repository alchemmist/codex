...
from antex_sdk import Antex, ApprovalMode, AsyncAntex
from antex_sdk.generated.v2_all import AskForApprovalValue, ThreadResumeParams
...
def test_thread_resume_inherits_deny_all_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Antex(config=harness.app_server_config()) as antex:
            source = antex.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            resumed = antex.thread_resume(source.id)
            resumed_state = antex._client.thread_resume(  # noqa: SLF001
                resumed.id,
...
def test_thread_fork_inherits_deny_all_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Antex(config=harness.app_server_config()) as antex:
            source = antex.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            forked = antex.thread_fork(source.id)
            forked_state = antex._client.thread_resume(  # noqa: SLF001
                forked.id,
...
def test_thread_run_approval_mode_persists_until_explicit_override(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Antex(config=harness.app_server_config()) as antex:
            thread = antex.thread_start(approval_mode=ApprovalMode.deny_all)

            first_result = thread.run("keep approvals denied")
            after_default_run = antex._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
            ...
            after_override_run = antex._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
...
def test_async_thread_run_approval_mode_persists_until_explicit_override(
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncAntex(config=harness.app_server_config()) as antex:
                thread = await antex.thread_start(approval_mode=ApprovalMode.deny_all)
                first_result = await thread.run("keep async approvals denied")
                after_default_run = await antex._client.thread_resume(  # noqa: SLF001
                    thread.id,
                    ThreadResumeParams(thread_id=thread.id),
                )
                ...
                after_override_run = await antex._client.thread_resume(  # noqa: SLF001
                    thread.id,
                    ThreadResumeParams(thread_id=thread.id),
                )
