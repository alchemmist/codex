...
import antex_sdk
import antex_sdk.types as public_types
...
from antex_sdk._initialize_metadata import validate_initialize_metadata
from antex_sdk.types import InitializeResponse
...
def test_root_exports_antex_config() -> None:
    """The root package should expose the process configuration object."""
    assert AntexConfig.__name__ == "AntexConfig"
...
def test_package_and_default_client_versions_follow_project_version() -> None:
    ...
    assert antex_sdk.__version__ == pyproject["project"]["version"]
    assert AntexConfig().client_version == antex_sdk.__version__
...
def test_curated_public_api_has_builtin_help_documentation() -> None:
    ...
    documented = {
        "module": antex_sdk,
        "Antex": Antex,
        "AsyncAntex": AsyncAntex,
        "AntexConfig": AntexConfig,
        "Thread": Thread,
        ...
        "Sandbox": Sandbox,
        "thread_start": Antex.thread_start,
        "thread_resume": Antex.thread_resume,
...
def test_package_includes_py_typed_marker() -> None:
    ...
    marker = resources.files("antex_sdk").joinpath("py.typed")
...
def test_package_root_exports_only_public_api() -> None:
    ...
    assert antex_sdk.__all__ == EXPECTED_ROOT_EXPORTS
    assert {name: hasattr(antex_sdk, name) for name in EXPECTED_ROOT_EXPORTS} == dict.fromkeys(
        EXPECTED_ROOT_EXPORTS, True
    )
    assert {
        "AntexClient": hasattr(antex_sdk, "AntexClient"),
        "AsyncAntexClient": hasattr(antex_sdk, "AsyncAntexClient"),
        "InitializeResponse": hasattr(antex_sdk, "InitializeResponse"),
        "ThreadStartParams": hasattr(antex_sdk, "ThreadStartParams"),
        "TurnStartParams": hasattr(antex_sdk, "TurnStartParams"),
        "TurnCompletedNotification": hasattr(antex_sdk, "TurnCompletedNotification"),
        "TurnStatus": hasattr(antex_sdk, "TurnStatus"),
    } == {
        "AntexClient": False,
        "AsyncAntexClient": False,
...
def test_package_star_import_matches_public_api() -> None:
    ...
    exec("from antex_sdk import *", namespace)
...
def test_types_module_exports_curated_public_types() -> None:
    """The public type module should expose Antex protocol models."""
...
def test_types_star_import_matches_public_types() -> None:
    ...
    namespace: dict[str, object] = {}
    exec("from antex_sdk.types import *", namespace)
...
def test_lifecycle_methods_are_antex_scoped() -> None:
    """Lifecycle operations should hang off the client rather than thread objects."""
    assert hasattr(Antex, "thread_resume")
    assert hasattr(Antex, "thread_fork")
    assert hasattr(Antex, "thread_archive")
    assert hasattr(Antex, "thread_unarchive")
    assert hasattr(AsyncAntex, "thread_resume")
    assert hasattr(AsyncAntex, "thread_fork")
    assert hasattr(AsyncAntex, "thread_archive")
    assert hasattr(AsyncAntex, "thread_unarchive")
    assert not hasattr(Antex, "thread")
    assert not hasattr(AsyncAntex, "thread")
    ...
    for fn in (
        Antex.thread_archive,
        Antex.thread_unarchive,
        AsyncAntex.thread_archive,
        AsyncAntex.thread_unarchive,
    ):
        _assert_no_any_annotations(fn)
...
def test_initialize_metadata_parses_user_agent_shape() -> None:
    ...
    payload = InitializeResponse.model_validate({"userAgent": "antex-cli/1.2.3"})
    parsed = validate_initialize_metadata(payload)
    assert parsed is payload
    assert parsed.userAgent == "antex-cli/1.2.3"
    assert parsed.serverInfo is not None
    assert parsed.serverInfo.name == "antex-cli"
