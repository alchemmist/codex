...
from antex_sdk._goal import _GoalNotificationStream
from antex_sdk._run import _collect_turn_result
from antex_sdk.generated.notification_registry import notification_turn_id
from antex_sdk.generated.v2_all import TurnStatus
...
def test_private_goal_operation_coalesces_runtime_continuations(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Antex(config=harness.app_server_config()) as antex:
            ...
            state, turn_id = antex._client.start_goal_operation(  # noqa: SLF001
                thread.id,
            ...
            stream = _GoalNotificationStream(
                ...
                lambda: antex._client.unregister_goal_operation(state),  # noqa: SLF001
                lambda: antex._client.cancel_goal_operation(state),  # noqa: SLF001
            )
            events = list(stream)
            result = _collect_turn_result(iter(events), turn_id=turn_id)
            routes = antex._client._router._goal_operations.copy()  # noqa: SLF001
