...
class AsyncAntexClient:
    """Async wrapper around AntexClient using thread offloading."""

    def __init__(self, config: AntexConfig | None = None) -> None:
        """Create the wrapped sync client that owns the transport process."""
        self._sync = AntexClient(config=config)

    async def __aenter__(self) -> "AsyncAntexClient":
        """Start the Antex process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the Antex process when leaving an async context."""
        await self.close()
