...
from .errors import AntexError, InvalidRequestError, TransportClosedError
...
RUNTIME_PKG_NAME = "antex-cli-bin"
...
def _installed_antex_path() -> Path:
    try:
        from antex_cli_bin import bundled_antex_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned Antex runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set AntexConfig.antex_bin "
            "explicitly."
        ) from exc

    return bundled_antex_path()


def _installed_antex_path_dirs() -> tuple[Path, ...]:
    try:
        from antex_cli_bin import bundled_path_dir
...
class AntexBinResolverOps:
    installed_antex_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_antex_bin_resolver_ops() -> AntexBinResolverOps:
    return AntexBinResolverOps(
        installed_antex_path=_installed_antex_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_antex_bin(config: "AntexConfig", ops: AntexBinResolverOps) -> Path:
    if config.antex_bin is not None:
        antex_bin = Path(config.antex_bin)
        if not ops.path_exists(antex_bin):
            raise FileNotFoundError(
                f"Antex binary not found at {antex_bin}. Set AntexConfig.antex_bin "
                "to a valid binary path."
            )
        return antex_bin

    return ops.installed_antex_path()


def _resolve_antex_bin(config: "AntexConfig") -> Path:
    return resolve_antex_bin(config, _default_antex_bin_resolver_ops())


@dataclass(slots=True)
class AntexConfig:
    """Configuration for launching and identifying the local Antex runtime.

    Most callers can use ``Antex()`` without configuration. Set ``antex_bin``
    only when intentionally using a specific local Antex executable.
    """

    antex_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "codex_python_sdk"
    client_title: str = "Antex Python SDK"
...
class AntexClient:
    ...
    def __init__(
        self,
        config: AntexConfig | None = None,
        approval_handler: ApprovalHandler | None = None,
    ) -> None:
        self.config = config or AntexConfig()
    ...
    def __enter__(self) -> "AntexClient":
        self.start()
        return self
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            antex_bin = _resolve_antex_bin(self.config)
            if self.config.antex_bin is None:
                path_dirs = _installed_antex_path_dirs()
            args = [str(antex_bin)]
    ...
    def request(
        ...
        *,
        ...
        if not isinstance(result, dict):
            raise AntexError(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("Antex process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"Antex process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise AntexError(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise AntexError(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_antex_home() -> str:
    return str(Path.home() / ".antex")
