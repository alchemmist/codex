...
from .client import AntexClient
...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncAntex needed by async login handles."""

    _client: AsyncAntexClient

    async def _ensure_initialized(self) -> None:
        """Ensure the owning SDK client has a live Antex connection."""
        ...


def start_chatgpt_login(client: AntexClient) -> ChatgptLoginHandle:
...
def start_device_code_login(client: AntexClient) -> DeviceCodeLoginHandle:
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `Antex.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `Antex.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncAntex.login_chatgpt()`."""

    _antex: _AsyncLoginOwner
    login_id: str
    auth_url: str
    ...
    async def wait(self) -> AccountLoginCompletedNotification:
        ...
        return await self._antex._client.wait_for_login_completed(self.login_id)

    async def cancel(self) -> CancelLoginAccountResponse:
        """Cancel this browser login attempt."""
        await self._antex._ensure_initialized()
        return await self._antex._client.account_login_cancel(self.login_id)


@dataclass(slots=True)
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncAntex.login_chatgpt_device_code()`."""
