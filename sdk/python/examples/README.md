...
- `sync.py` (public sync surface: `Antex`)
- `async.py` (public async surface: `AsyncAntex`)

All examples intentionally use only public SDK exports from `antex_sdk`
and `antex_sdk.types`.
...
```bash
python -m pip install antex-sdk
```

The SDK installs its pinned `antex-cli-bin` runtime dependency.
