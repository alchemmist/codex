...
with Antex(config=runtime_config()) as antex:
    ...
    reopened = antex.thread_resume(thread.id)
    listing_active = antex.thread_list(limit=20, archived=False)
    reading = reopened.read(include_turns=True)

    _ = reopened.set_name("sdk-lifecycle-demo")
    _ = antex.thread_archive(reopened.id)
    listing_archived = antex.thread_list(limit=20, archived=True)
    unarchived = antex.thread_unarchive(reopened.id)

    resumed = antex.thread_resume(
        unarchived.id,
        model="gpt-5.4",
    ...
    forked = antex.thread_fork(unarchived.id, model="gpt-5.4")
