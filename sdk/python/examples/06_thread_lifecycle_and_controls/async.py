...
from antex_sdk import AsyncAntex
...
async def main() -> None:
    async with AsyncAntex(config=runtime_config()) as antex:
        ...
        reopened = await antex.thread_resume(thread.id)
        listing_active = await antex.thread_list(limit=20, archived=False)
        reading = await reopened.read(include_turns=True)

        _ = await reopened.set_name("sdk-lifecycle-demo")
        _ = await antex.thread_archive(reopened.id)
        listing_archived = await antex.thread_list(limit=20, archived=True)
        unarchived = await antex.thread_unarchive(reopened.id)

        resumed = await antex.thread_resume(
            unarchived.id,
            model="gpt-5.4",
        ...
        forked = await antex.thread_fork(unarchived.id, model="gpt-5.4")
        forked_result = await (
