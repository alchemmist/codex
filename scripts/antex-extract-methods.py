...
def main():
    ...
    for implementation in tree.root_node.children:
        if implementation.type != "impl_item":
            continue
        body = implementation.child_by_field_name("body")
        methods = [node for node in body.named_children if node.type == "function_item"]
        names = [
            source[
        ...
        if args.first not in names or args.last not in names:
            continue
        selected = methods[names.index(args.first) : names.index(args.last) + 1]
        if not selected:
        ...
        return
    raise SystemExit("method range not found")
