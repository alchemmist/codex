#!/usr/bin/env python3
"""Black-box MCP compliance runner for a supplied Antex binary."""
...
class AppServerClient:
    """Minimal JSONL client for the Antex app-server protocol."""
    ...
    def _send(self, message: Mapping[str, object]) -> None:
        if self.process.poll() is not None:
            raise AppServerError(
                f"Antex app-server exited with code {self.process.returncode}"
            )
    ...
    def request(
        ...
        while True:
            ...
            if message is None:
                raise AppServerError(
                    f"Antex app-server closed stdout while handling {method}"
                )
    ...
    def wait_for_notification(
        ...
        *,
        ...
        while True:
            ...
            if message is None:
                raise AppServerError(
                    f"Antex app-server closed stdout while waiting for {method}"
                )
            if "id" in message and isinstance(message.get("method"), str):
                self._handle_server_request(message)
                continue
...
def _isolated_environment(codex_home: Path) -> dict[str, str]:
    env = dict(os.environ)
    env["ANTEX_HOME"] = str(codex_home)
    # Direct app-server MCP calls do not need a model or credentials.
    for name in ("ANTEX_API_KEY", "ANTEX_ACCESS_TOKEN", "OPENAI_API_KEY"):
        env.pop(name, None)
    return env
    ...
    *,
    ...
    if transport == "stdio":
        if mode == MODERN_VERSION:
            command.extend(["--env", f"ANTEX_MCP_PROTOCOL_VERSION={MODERN_VERSION}"])
        return [
            *command,
            "--",
            sys.executable,
...
def _validate_registration(
    ...
    *,
    ...
    if transport == "stdio":
        ...
        modern_opt_in = (
            isinstance(env, dict)
            and env.get("ANTEX_MCP_PROTOCOL_VERSION") == MODERN_VERSION
        )
    ...
    *,
    ...
    try:
        ...
        if registered:
            ...
            listed = _run_command(
                [str(antex_binary), "mcp", "list", "--json"],
                env=env,
                cwd=workspace,
                timeout_seconds=timeout_seconds,
            )
...
def _run_official_case(
    ...
    *,
    ...
    case_home.mkdir(parents=True)
    env = _isolated_environment(case_home)
    env["ANTEX_CONFORMANCE_TIMEOUT"] = str(timeout_seconds)
    env["ANTEX_CONFORMANCE_ENABLE_MODERN_FEATURE"] = (
        "1" if enable_modern_feature else "0"
    )
    env["ANTEX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH"] = (
        "1" if require_automatic_auth else "0"
    )
    results = run_official_mode(
        conformance_command=conformance_command,
        adapter_script=adapter_script,
        antex_binary=antex_binary,
        mode=mode,
        scenarios=scenarios,
...
def _cimd_registration_checks(
    ...
    *,
    ...
    for check_name, client_registration in (
        ...
        env["ANTEX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH"] = "1"
        if client_registration is not None:
            env["ANTEX_CONFORMANCE_CLIENT_REGISTRATION"] = client_registration
        results = run_official_mode(
            conformance_command=conformance_command,
            adapter_script=adapter_script,
            antex_binary=antex_binary,
...
def _required_regression_checks(
    ...
    *,
    ...
    if not isinstance(version_check, dict) or version_check.get("success") is not True:
        errors.append(f"{label} does not contain a successful Antex version check")
...
def _print_human_report(report: Mapping[str, object]) -> None:
    ...
    print(f"Antex MCP compliance: {'PASS' if success else 'FAIL'}")
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "supplied Antex binary, plus the supplemental local stdio suite."
        )
    )
    ...
    parser.add_argument(
        "--adapter-script",
        type=Path,
        default=Path(__file__).resolve().with_name("antex_conformance_adapter.py"),
        help="Path to the Antex adapter used by the official client suite.",
    )
    parser.add_argument(
        "--conformance-cli",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Require Antex to recover from OAuth scope escalation and "
    ...
    parser.add_argument(
        "--artifact-parent",
        type=Path,
        help="Parent directory for isolated temporary Antex homes.",
    )
    parser.add_argument(
        "--keep-artifacts",
        action="store_true",
        help="Keep isolated Antex homes and include their path in the report.",
    )
    parser.add_argument(
