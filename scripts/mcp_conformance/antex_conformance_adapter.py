#!/usr/bin/env python3
"""Adapt one official MCP client scenario to Antex app-server requests."""
...
CLIENT_REGISTRATION_OVERRIDE_ENV_VAR = "ANTEX_CONFORMANCE_CLIENT_REGISTRATION"
...
def _elicitation_content(
    ...
    if scenario == "elicitation-sep1034-client-defaults":
        # The official scenario deliberately supplies no values. Antex, as the
...
def _validated_callback_url(authorization_url: str, location: str) -> str:
    ...
    if (
        ...
        raise AdapterFailure(
            "authorization server redirect did not target Antex's exact loopback callback"
        )
...
def _drive_headless_authorization(
    ...
    *,
    ...
    if not 200 <= callback_status < 500:
        raise AdapterFailure("Antex OAuth callback endpoint returned an invalid status")
    ...
    *,
    ...
    if scenario == "auth/scope-step-up":
        ...
        if require_automatic_auth:
            _auth_tool_call(client, workspace)
            return "Antex automatically recovered from the challenged OAuth scope"
        try:
            _auth_tool_call(client, workspace)
    ...
    if scenario == "auth/scope-retry-limit":
        if require_automatic_auth:
            ...
            try:
                ...
                return "observed Antex's production OAuth retry-limit behavior"
            raise AdapterFailure(
    ...
    if scenario == "auth/authorization-server-migration":
        ...
        if require_automatic_auth:
            _auth_tool_call(client, workspace)
            return (
                "Antex automatically registered with the migrated authorization server"
            )
        try:
            _auth_tool_call(client, workspace)
        except AdapterFailure:
...
def run_adapter(server_url: str) -> dict[str, object]:
    antex_binary = _required_path("ANTEX_CONFORMANCE_BINARY")
    codex_home = _required_path("ANTEX_CONFORMANCE_HOME")
    scenario = _required_env("MCP_CONFORMANCE_SCENARIO")
    protocol_version = _required_env("MCP_CONFORMANCE_PROTOCOL_VERSION")
    timeout_seconds = float(os.environ.get("ANTEX_CONFORMANCE_TIMEOUT", "30"))
    enable_modern_feature = (
        os.environ.get("ANTEX_CONFORMANCE_ENABLE_MODERN_FEATURE", "1") != "0"
    )
    require_automatic_auth = (
        os.environ.get("ANTEX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH", "0") == "1"
    )
    context = _conformance_context()
    ...
    # consumes it directly and does not expose the full blob to Antex.
    ...
    try:
        ...
        if not registration_ok:
            raise AdapterFailure("Antex registration did not preserve the scenario URL")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report_path_value = os.environ.get("ANTEX_CONFORMANCE_ADAPTER_REPORT")
