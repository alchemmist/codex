...
import pytest
from run_antex_compliance import (
...
def test_registration_commands_select_stdio_then_http_shapes() -> None:
    ...
    assert stdio == [
        ...
        TEST_SERVER_NAME,
        "--env",
        f"ANTEX_MCP_PROTOCOL_VERSION={MODERN_VERSION}",
        "--",
        sys.executable,
        "/src/server.py",
        "--mode",
    ...
    valid, _ = _validate_registration(
        {
            ...
            "transport": {
                ...
                "env": {"ANTEX_MCP_PROTOCOL_VERSION": MODERN_VERSION},
            },
        },
        transport="stdio",
        mode=MODERN_VERSION,
...
def test_isolated_environment_drops_model_credentials(
    ...
    monkeypatch.setenv("ANTEX_API_KEY", "secret")
    monkeypatch.setenv("ANTEX_ACCESS_TOKEN", "secret")
    monkeypatch.setenv("OPENAI_API_KEY", "secret")

    env = _isolated_environment(tmp_path)

    assert env["ANTEX_HOME"] == str(tmp_path)
    assert "ANTEX_API_KEY" not in env
    assert "ANTEX_ACCESS_TOKEN" not in env
...
def test_official_case_preserves_adapter_check_identity_on_failure_and_success(
    ...
    monkeypatch.setattr(
        "run_antex_compliance.run_official_mode",
        lambda **_kwargs: [result],
    )

    case = _run_official_case(
        Path("/opt/codex"),
        Path("/src/antex_conformance_adapter.py"),
...
def test_official_case_keeps_cimd_registration_checks_in_regression_gated_http_case(
    ...
    assert auto_env.get("ANTEX_CONFORMANCE_CLIENT_REGISTRATION") is None
    assert forced_env["ANTEX_CONFORMANCE_CLIENT_REGISTRATION"] == "cimd"
...
def test_regression_cli_extracts_compact_baseline_without_starting_conformance(
    ...
    antex = tmp_path / "codex"
    antex.write_text("#!/bin/sh\nexit 99\n", encoding="utf-8")
    antex.chmod(0o755)
...
def test_regression_cli_rejects_extraction_without_a_source_report(
    ...
    assert main([str(antex), "--extract-baseline", str(tmp_path / "compact.json")]) == 2
...
def test_regression_cli_rejects_malformed_baselines(
    ...
    assert main([str(antex), "--baseline-report", str(baseline)]) == 2
...
def test_regression_cli_rejects_missing_baseline_before_starting_conformance(
    ...
    assert main([str(antex), "--baseline-report", str(tmp_path / "missing.json")]) == 2
