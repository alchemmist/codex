"""Command-line interface for building Antex package directories."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a canonical Antex package directory and optional archive.",
    ...
    parser.add_argument(
        ...
        help="Semantic version to record in antex-package.json.",
    )
    parser.add_argument(
        "--package-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional prebuilt antex-code-mode-host executable. If omitted, "
    ...
    zsh_source.add_argument(
        ...
        help=(
            ...
            "scripts/antex_package/antex-zsh."
        ),
    )
    zsh_source.add_argument(
        "--zsh-bin",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional prebuilt Windows antex-command-runner.exe executable. "
            "If omitted for Windows targets, antex-command-runner is built "
            "with Cargo."
        ),
    )
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional prebuilt Windows antex-windows-sandbox-setup.exe "
            "executable. If omitted for Windows targets, "
            "antex-windows-sandbox-setup is built with Cargo."
        ),
    )
    parser.add_argument(
        "--rg-bin",
...
def main() -> int:
    ...
    package_dir = (
        ...
        else Path(tempfile.mkdtemp(prefix="antex-package-")).resolve()
    )
    ...
    source_outputs = build_source_binaries(
        ...
        antex_command_runner_bin=resolve_optional_input_path(
            args.antex_command_runner_bin,
            "prebuilt Windows antex-command-runner.exe executable",
            "--antex-command-runner-bin",
        ),
        antex_windows_sandbox_setup_bin=resolve_optional_input_path(
            args.antex_windows_sandbox_setup_bin,
            "prebuilt Windows antex-windows-sandbox-setup.exe executable",
            "--antex-windows-sandbox-setup-bin",
        ),
    )
    inputs = PackageInputs(
        ...
        antex_command_runner_bin=source_outputs.antex_command_runner_bin,
        antex_windows_sandbox_setup_bin=source_outputs.antex_windows_sandbox_setup_bin,
    )
    ...
    for archive_output in args.archive_output:
        ...
        print(f"Built Antex package archive at {archive_path}")

    print(f"Built Antex package directory at {package_dir}")
    return 0
