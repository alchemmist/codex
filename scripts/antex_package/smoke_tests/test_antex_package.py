...
├── bin
│   ├── antex[.exe]                       # CLI package only
│   ├── antex-app-server[.exe]            # app-server package only
...
Each package contains one entrypoint, not both antex and antex-app-server.
"""
...
from antex_sdk import ApprovalMode, Antex, AntexConfig, Sandbox
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(("completion", "bash"), "antex", id="completion"),
    ],
)
...
@pytest.mark.parametrize("entrypoint", ["antex", "antex-app-server"])
...
def _code_mode_host_symbol_address(
    ...
    *arguments: str,
    ...
    return next(
        ...
        and parts[1].lower() == "t"
        and "antex_code_mode_host" in parts[2]
    )
...
def test_linux_debug_symbols_resolve_packaged_code(
    ...
    assert "antex_code_mode_host" in resolved, resolved
...
def test_windows_debug_symbols_resolve_packaged_code(
    ...
    symbols = code_mode_host_debug_symbols.rename(
        code_mode_host_debug_symbols.with_name("antex_code_mode_host.pdb")
    )
    host = symbols.with_suffix(".exe")
    shutil.copy2(package.cli.with_name("antex-code-mode-host.exe"), host)
