...
class SmokePackage:
    ...
    def from_archives(
        ...
        config_dir = directory / "antex-config"
        config_dir.mkdir()
        ...
        # Isolate package configuration and state from the user's Antex setup.
        environment["ANTEX_HOME"] = str(config_dir)
        ...
        for index, archive_path in enumerate((cli_archive, app_server_archive)):
            ...
            manifest = json.loads((extracted / "antex-package.json").read_text())
...
def package(
    ...
    directory = tmp_path_factory.mktemp(f"antex-package-smoke-{compression}")
...
def responses_server(package: SmokePackage) -> Iterator[MockResponsesServer]:
    config_path = Path(package.environment["ANTEX_HOME"]) / "config.toml"
    original_config = config_path.read_text()
    with MockResponsesServer() as server:
        # Direct CLI commands and SDK-launched servers must share a provider;
        # AntexConfig only controls how the SDK launches its server process.
...
def code_mode_host_debug_symbols(
    ...
    binaries = {"antex", "antex-app-server", "antex-code-mode-host"}
    if "windows" in target:
        binaries.update({"antex-command-runner", "antex-windows-sandbox-setup"})
    ...
    destination = tmp_path_factory.mktemp("antex-debug-symbols")
    found: set[str] = set()
    symbol_path = None
        ...
        for member in archive:
            ...
            for binary, marker in markers.items():
                ...
                if not matches:
                    continue
                found.add(binary)
                if binary == "antex-code-mode-host":
