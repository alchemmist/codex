"""Canonical Antex package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "antex-resources"
    path_dir = package_dir / "antex-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    copy_executable(
        ...
        bin_dir / f"antex-code-mode-host{spec.exe_suffix}",
        is_windows=spec.is_windows,
    )
    ...
    if inputs.antex_command_runner_bin is not None:
        copy_executable(
            inputs.antex_command_runner_bin,
            resources_dir / "antex-command-runner.exe",
            is_windows=True,
        )

    if inputs.antex_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.antex_windows_sandbox_setup_bin,
            resources_dir / "antex-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    write_json(package_dir / "antex-package.json", metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    metadata_path = package_dir / "antex-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: antex-package.json")
    ...
    required_files = [
        ...
        Path("bin") / f"antex-code-mode-host{spec.exe_suffix}",
        Path("antex-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path("antex-resources") / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path("antex-resources") / "bwrap")
        executable_files.append(Path("antex-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("antex-resources") / "antex-command-runner.exe",
                Path("antex-resources") / "antex-windows-sandbox-setup.exe",
            ]
        )
