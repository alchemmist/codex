# Antex package builder

This package contains the implementation behind `scripts/build_antex_package.py`.
...
```bash
just assemble-antex-package --help
just assemble-antex-package --variant antex-app-server
just assemble-antex-package --target x86_64-unknown-linux-gnu
```

The builder creates a canonical Antex package directory:
...
│   ├── antex-command-runner.exe          # Windows only
│   └── antex-windows-sandbox-setup.exe   # Windows only
└── antex-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match Antex release
...
`antex` and `antex-app-server`. The `--package-version` flag sets the version in
`antex-package.json`; it defaults to `[workspace.package].version` in
`antex-rs/Cargo.toml`.
...
- all targets: `antex-code-mode-host`, unless `--code-mode-host-bin` is provided
- Linux targets: `bwrap`, unless `--bwrap-bin` is provided
- Windows targets: `antex-command-runner` and `antex-windows-sandbox-setup`,
...
`--antex-command-runner-bin` plus `--antex-windows-sandbox-setup-bin` for
...
downloads and verifies the matching Antex-built V8 release pair before invoking
...
DotSlash manifest at `scripts/antex_package/rg`. Downloaded archives are cached
under `$TMPDIR/antex-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/antex_package/antex-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/antex-package/<target>-zsh` and installed at
`antex-resources/zsh/bin/zsh`. Pass `--zsh-bin` to package a prebuilt, signed
