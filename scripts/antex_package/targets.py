...
_repo_root = os.environ.get("ANTEX_REPO_ROOT")
if _repo_root is None:
    raise RuntimeError(
        "ANTEX_REPO_ROOT must point to the repository root; "
        "run `just assemble-antex-package` to set it automatically"
    )
...
PACKAGE_VARIANTS: dict[str, PackageVariant] = {
    "antex": PackageVariant(
        name="antex",
        cargo_bin="antex",
        executable_stem="antex",
    ),
    "antex-app-server": PackageVariant(
        name="antex-app-server",
        cargo_bin="antex-app-server",
        executable_stem="antex-app-server",
    ),
}
